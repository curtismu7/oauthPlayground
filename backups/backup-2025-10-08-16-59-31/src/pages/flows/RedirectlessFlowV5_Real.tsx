...
const RedirectlessFlowV5Real: React.FC = () => {
	...
	useEffect(() => {
		const stored = sessionStorage.getItem('oauth-redirectless-flow-v5-app-config');
		if (stored) {
			try {
	...
	useEffect(() => {
		...
		const storedStep = sessionStorage.getItem('oauth-redirectless-flow-v5-current-step');
		...
		if (error) {
			...
			setCurrentStep(0);
			sessionStorage.setItem('oauth-redirectless-flow-v5-current-step', '0');
			return;
		}
		...
		if (finalAuthCode) {
			...
			setCurrentStep(4);
			sessionStorage.setItem('oauth-redirectless-flow-v5-current-step', '4');
		...
		if (urlStep) {
			...
			if (!Number.isNaN(stepIndex) && stepIndex >= 0 && stepIndex < STEP_METADATA.length) {
				...
				sessionStorage.setItem('oauth-redirectless-flow-v5-current-step', stepIndex.toString());
				return;
			}
		}
	...
	useEffect(() => {
		sessionStorage.setItem('oauth-redirectless-flow-v5-current-step', currentStep.toString());
	}, [currentStep]);
	...
	const handleClearConfiguration = useCallback(() => {
		...
		sessionStorage.removeItem('oauth-redirectless-flow-v5-app-config');
	...
	const savePingOneConfig = useCallback(
		(config: PingOneApplicationState) => {
			setPingOneConfig(config);
			sessionStorage.setItem('oauth-redirectless-flow-v5-app-config', JSON.stringify(config));
	...
	const navigateToTokenManagement = useCallback(() => {
		...
		sessionStorage.setItem('flow_source', 'oauth-redirectless-flow-v5');

		// Store comprehensive flow context for Token Management page
		const flowContext = {
			flow: 'oauth-redirectless-flow-v5',
			step: currentStep,
		...
		if (controller.tokens?.access_token) {
			...
			localStorage.setItem('flow_source', 'oauth-redirectless-flow-v5');
			console.log(
		...
		storeFlowNavigationState('oauth-redirectless-flow-v5', currentStep, 'oauth');
	...
	const renderStepContent = useMemo(() => {
		...
		switch (currentStep) {
			case 0:
				return (
					<>
						<FlowConfigurationRequirements flowType="redirectless" variant="oauth" />
						<CollapsibleSection>
				...
				return (
					<>
						...
						<CollapsibleSection>
							...
							>
								<CollapsibleTitle>
									<FiSettings /> PingOne Flow Object (pi.flow)
								</CollapsibleTitle>
							...
							{!collapsedSections.responseMode && (
								<CollapsibleContent>
									<ResponseModeSelector
										flowKey="redirectless"
										responseType="code"
										redirectUri={`${window.location.origin}/redirectless-callback`}
										clientId={controller.credentials.clientId}
										scope={controller.credentials.scope || 'openid'}
			...
			<LoginSuccessModal
				...
				onClose={() => {
					console.log('🔴 [AuthorizationCodeFlowV5] Closing LoginSuccessModal', {
						...
						storedStep: sessionStorage.getItem('oauth-redirectless-flow-v5-current-step'),
					});
