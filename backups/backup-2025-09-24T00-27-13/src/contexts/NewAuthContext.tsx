	...
	const login = useCallback(
		async (
			...
			try {
				...
				console.log(
					'🚀 [NewAuthContext] Authorization URL prepared, returning for modal display...'
				);
		...
		async (url: string): Promise<LoginResult> => {
			try {
				...
				if (!clientId || !environmentId) {
					console.log(
						'⚠️ [NewAuthContext] Config not loaded, trying credential manager fallback...'
					);
					try {
				...
				try {
					...
					if (flowContextRaw) {
						...
						console.log(
							'🚨 [NewAuthContext] ENHANCED FLOW DETECTED?',
							isEnhancedV2 || isEnhancedV3
						);
				...
				if (flowContext) {
					...
					console.log(
						'🧹 [NewAuthContext] Cleaning up flow context after determining redirect URL'
					);
