...
class OIDCDiscoveryService {
  ...
  async discover(config: DiscoveryConfig): Promise<DiscoveryResult> {
    try {
      const { issuerUrl, timeout = this.DEFAULT_TIMEOUT, cacheTimeout = this.DEFAULT_CACHE_TIMEOUT } = config;
      ...
      const wellKnownUrl = this.constructWellKnownUrl(normalizedIssuer);
      console.log('[OIDC Discovery] Fetching discovery document from:', wellKnownUrl);
      ...
      const document: OIDCDiscoveryDocument = await response.json();
  ...
  private constructWellKnownUrl(issuerUrl: string): string {
    ...
    return `${base}/.well-known/openid_configuration`;
  }
  ...
  private validateDiscoveryDocument(document: OIDCDiscoveryDocument, expectedIssuer: string): void {
    ...
    if (document.issuer.toLowerCase() !== expectedIssuer.toLowerCase()) {
      console.warn('[OIDC Discovery] Issuer mismatch:', document.issuer, 'expected:', expectedIssuer);
    }
  }
...
export type { OIDCDiscoveryDocument, DiscoveryResult, DiscoveryConfig };
