...
import { FiCheckCircle, FiCopy, FiKey, FiRotateCcw, FiShield, FiUser } from 'react-icons/fi';
...
import EnhancedErrorRecovery, { type RecoveryAction } from '../../utils/errorRecovery';
...
import { generateComplianceReport, validateOIDCCompliance } from '../../utils/oidcCompliance';
...
const EnhancedAuthorizationCodeFlowV3: React.FC = () => {
	...
	const _performanceMonitor = usePerformanceMonitor('EnhancedAuthorizationCodeFlowV3');
	...
	const hasStepResult = useCallback((stepId: string) => stepId in stepResultsRef.current, []);
		...
		try {
			...
			if (flowConfig.acrValues && flowConfig.acrValues.length > 0) {
				...
				if (validAcrValues.length > 0) {
					...
					console.warn(' [OIDC-V3] No valid ACR values found, skipping acr_values parameter');
				}
			}
	...
	const handleOAuthError = useCallback(
		async (error: unknown, context?: string, retryFunction?: () => Promise<void>) => {
			...
			showGlobalError(`${interpretedError?.title}: ${interpretedError?.message}`);
		},
	...
	const exchangeTokens = useCallback(async () => {
		...
		try {
			...
			if (tokenData.id_token) {
				...
				console.log(' [OIDC-V3] ID Token validation result:', validationResult);
			...
			const messageHandler = (event: MessageEvent) => {
				...
				if (event.data.type === 'oauth-callback') {
					...
					if (error) {
						...
						showGlobalError(` Authorization failed: ${error_description || error}`);
						setIsAuthorizing(false);
			...
			const checkClosed = setInterval(() => {
				if (popup.closed) {
					...
					if (!authorizationSuccessful) {
						...
						showGlobalError(' Authorization was cancelled or popup was closed before completion');
					}
				}
			}, 1000);
		} else {
			console.error(' [OIDC-V3] Failed to open popup window');
			showGlobalError(' Failed to open popup window. Please check your browser settings.');
			setIsAuthorizing(false);
		}
								...
								<div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
									<button
										...
										style={{
											...
											background: tokens.access_token ? '#f8fafc' : '#9ca3af',
											color: 'white',
											border: 'none',
											borderRadius: '0.5rem',
											fontSize: '0.875rem',
											...
											boxShadow: tokens.access_token ? '0 2px 4px rgba(59, 130, 246, 0.3)' : 'none',
						...
						return (
							...
							>
								...
								{isLowSecurity && (
									...
									>
										<strong>Security Recommendation:</strong> Consider using 'client_secret_jwt' or
										'private_key_jwt' for production environments.
									</div>
								)}
							</div>
						);
				...
				{(() => {
					...
					return (
						...
						>
							...
							>
								<span style={{ fontSize: '1.2em' }}>{complianceResult.isCompliant ? '' : ''}</span>
				...
				>
					...
					>
						...
						<button
							...
							style={{
								padding: '0.75rem 1.25rem',
								backgroundColor: isResettingFlow ? '#9ca3af' : resetHover ? '#b91c1c' : '#ef4444',
								color: 'white',
								border: 'none',
								borderRadius: '0.375rem',
								fontSize: '0.875rem',
