...
import { FiCopy, FiExternalLink, FiRefreshCw, FiXCircle, FiCheckCircle, FiAlertTriangle, FiPrinter, FiFileText, FiPaperclip, FiMove, FiMaximize2, FiArrowLeft, FiTrash2, FiArrowRight } from 'react-icons/fi';
...
const ToolbarButton = styled.button<{ $variant: 'move' | 'expand' | 'back' | 'delete' | 'forward' }>`
  width: 32px;
  height: 32px;
  border-radius: 6px;
  border: none;
  ...
  background: ${({ $variant }) => {
    switch ($variant) {
      ...
      case 'expand': return '#3b82f6';
      case 'back': return '#6b7280';
...
const PrinterControlButton = styled.button<{ $variant: 'primary' | 'secondary' | 'success' | 'danger' }>`
  background: ${props => {
    switch (props.$variant) {
      case 'primary': return '#22d3ee';
      case 'secondary': return '#374151';
      case 'success': return '#f97316';
...
const StatusDisplay = styled.div<{ $status: string }>`
  background: ${props => {
    switch (props.$status) {
      ...
      case 'authorized': return 'linear-gradient(135deg, #f97316 0%, #ea580c 100%)';
    ...
    <SmartPrinterContainer $authorized={isAuthorized}>
      ...
      <PrinterDisplayScreen>
        ...
        <div style={{ padding: '1.5rem' }}>
          ...
          {/* User Code Display */}
          <ScreenLabel style={{ color: '#64748b', fontSize: '0.875rem', fontWeight: 600, marginBottom: '0.5rem', marginTop: '1rem' }}>
