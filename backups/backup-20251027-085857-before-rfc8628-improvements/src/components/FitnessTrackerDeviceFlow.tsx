...
const StatusRing = styled.div<{ $status: string }>`
  ...
  box-shadow: ${(props) => {
		switch (props.$status) {
			case 'pending':
				return '0 0 15px #ffa500';
			case 'authorized':
				return '0 0 15px #00ff96';
			case 'denied':
				return '0 0 15px #ff0000';
			case 'expired':
				return '0 0 15px #666666';
			default:
				return '0 0 15px #666666';
		}
	}};
...
const StatusDisplay = styled.div<{ $status: string }>`
  background: ${(props) => {
		switch (props.$status) {
			...
			case 'authorized':
				return 'linear-gradient(135deg, #00ff96 0%, #00cc77 100%)';
			case 'denied':
		...
		<FitnessTrackerContainer>
			...
			<PhoneScreen>
				...
				<StravaContent>
					...
					{/* Activity Title */}
					<div
						style={{ fontSize: '1.5rem', fontWeight: 700, color: '#1f2937', marginBottom: '1rem' }}
					>
						Evening Run
					</div>
					...
					<AuthCodeSection>
						...
						<AuthCodeValue>{deviceFlowService.formatUserCode(state.userCode)}</AuthCodeValue>
					</AuthCodeSection>
