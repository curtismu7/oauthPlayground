...
import { FiCopy, FiExternalLink, FiRefreshCw, FiXCircle, FiCheckCircle, FiAlertTriangle, FiVolume2, FiMusic, FiMic, FiPause, FiHome, FiSettings } from 'react-icons/fi';
...
// Amazon Echo/Alexa Smart Speaker Main Container - Authentic Speaker Design
const SmartSpeakerContainer = styled.div`
  ...
  /* Simulated speaker grille texture */
  &::before {
    content: '';
    position: absolute;
    top: 0;
...
// Alexa Display Screen
const SpeakerDisplayScreen = styled.div`
  background: linear-gradient(135deg, #000000 0%, #0f1419 100%);
...
const SpeakerControlButton = styled.button<{ $variant: 'primary' | 'secondary' | 'success' | 'danger' }>`
  background: ${props => {
    ...
    <SmartSpeakerContainer>
      ...
      <SpeakerDisplayScreen>
        <SpeakerTitle>
          ...
          Alexa Device Authorization
        </SpeakerTitle>
        <SpeakerSubtitle>Voice-Controlled Authorization System</SpeakerSubtitle>
        
        {/* Audio Wave Indicators */}
        <div style={{ margin: '1.5rem 0', display: 'flex', justifyContent: 'center', gap: '0.5rem', alignItems: 'end' }}>
          <AudioWave $active={state.status === 'pending'} $color="#ff9900" $height={20} />
          <AudioWave $active={state.status === 'authorized'} $color="#ff9900" $height={30} />
          <AudioWave $active={state.status === 'denied'} $color="#ef4444" $height={15} />
          <AudioWave $active={state.status === 'expired'} $color="#6b7280" $height={10} />
          <AudioWave $active={state.status === 'pending'} $color="#ff9900" $height={25} />
          <AudioWave $active={state.status === 'authorized'} $color="#ff9900" $height={35} />
          <AudioWave $active={state.status === 'denied'} $color="#ef4444" $height={18} />
          <AudioWave $active={state.status === 'expired'} $color="#6b7280" $height={12} />
          <AudioWave $active={state.status === 'pending'} $color="#ff9900" $height={22} />
          <AudioWave $active={state.status === 'authorized'} $color="#ff9900" $height={28} />
        </div>
        ...
        <div style={{ 
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: '1.125rem', fontWeight: '600', color: 'white', marginBottom: '0.25rem' }}>
                  Now Playing
                </div>
                <div style={{ fontSize: '0.875rem', color: '#94a3b8' }}>
