...
export const FlowConfiguration: React.FC<FlowConfigurationProps> = ({
	...
	const updateConfig = useCallback(
		(updates: Partial<FlowConfig>) => {
			onConfigChange({
				...config,
				...updates,
				scopes: ensureMandatoryScopes(updates.scopes ?? config.scopes),
			});
		},
	...
	const toggleScope = useCallback(
		(scope: string) => {
			...
			const hasScope = normalizedConfig.scopes.includes(scope);
	...
	}, [normalizedConfig.customParams, updateConfig]);
	...
	}, [normalizedConfig.customClaims, updateConfig]);
	...
	const updateCustomClaimKey = useCallback(
	...
	const removeCustomClaim = useCallback(
		(key: string) => {
			const newClaims = { ...normalizedConfig.customClaims };
	...
	}, [generateRandomString, updateConfig]);
	...
	const handleSave = useCallback(async () => {
		...
		try {
			await new Promise((resolve) => {
				saveTimeoutRef.current = window.setTimeout(resolve, SAVE_FEEDBACK_TIMEOUT_MS);
			});
		...
		<ConfigContainer>
			<Card>
				...
				<CardBody>
					<CollapsibleContent $isExpanded={isExpanded}>
							...
							<ConfigGrid>
								...
								<ConfigField>
									...
									>
										...
										<option value="urn:ietf:params:oauth:grant-type:device_code">
											device_code
										</option>
									</select>
							...
							<ConfigSection>
								...
								<ConfigGrid>
									<ToggleContainer>
										...
										>
											<div
												className={`toggle-slider ${normalizedConfig.enablePKCE ? 'active' : ''}`}
											/>
										</div>
									...
									<ConfigField>
										...
										<select
											...
											onChange={(e) => {
												...
												const validAcrValues = selected.filter(
													(acr) =>
														acr &&
														acr.trim() !== '' &&
						...
						<ConfigSection>
							...
							<ConfigField>
								...
								<select
									...
									onChange={(e) =>
										updateConfig({
											clientAuthMethod: e.target.value as FlowConfig['clientAuthMethod'],
										})
									}
								>
									<option value="client_secret_post">
										client_secret_post (Secret in POST body) - Current
									</option>
									<option value="client_secret_basic">
										client_secret_basic (HTTP Basic Auth) - Secure
									</option>
									<option value="client_secret_jwt">
										client_secret_jwt (JWT with shared secret) - More Secure
									</option>
									<option value="private_key_jwt">
										private_key_jwt (JWT with private key) - Most Secure
									</option>
									<option value="none">
								...
								<div style={{ fontSize: '0.75rem', color: '#6b7280', marginTop: '0.5rem' }}>
									...
									{normalizedConfig.clientAuthMethod === 'client_secret_basic' &&
										' Client secret sent via HTTP Basic Authentication header'}
									{normalizedConfig.clientAuthMethod === 'client_secret_jwt' &&
										' Client creates JWT signed with client secret (HS256)'}
									{normalizedConfig.clientAuthMethod === 'private_key_jwt' &&
										' Client creates JWT signed with private key (RS256) - Enterprise grade'}
									{normalizedConfig.clientAuthMethod === 'none' &&
										' No client authentication - PKCE required for security'}
								</div>
							</ConfigField>
						</ConfigSection>
						...
						<ConfigSection>
							...
							>
								...
								{normalizedConfig.enableOIDC && (
									<div style={{ marginBottom: '0.5rem' }}>
										<strong>OIDC:</strong> Nonce:{' '}
										{normalizedConfig.nonce
											? `${normalizedConfig.nonce.substring(0, 10)}...`
											: 'none'}
										, <strong>State:</strong>{' '}
										{normalizedConfig.state
											? `${normalizedConfig.state.substring(0, 10)}...`
											: 'none'}
									</div>
								)}
								...
								{normalizedConfig.clientAuthMethod &&
									normalizedConfig.clientAuthMethod !== 'client_secret_post' && (
										<div style={{ marginBottom: '0.5rem' }}>
								...
								{Object.keys(normalizedConfig.customParams).length > 0 && (
									<div style={{ marginBottom: '0.5rem' }}>
										...
										{Object.entries(normalizedConfig.customParams)
											.map(([k, v]) => `${k}=${v}`)
											.join(', ')}
									</div>
								)}
								...
								{normalizedConfig.enableOIDC &&
									Object.keys(normalizedConfig.customClaims).length > 0 && (
										<div style={{ marginBottom: '0.5rem' }}>
											<strong>Custom Claims:</strong>{' '}
											{Object.entries(normalizedConfig.customClaims)
												.map(([k, v]) => `${k}=${v}`)
												.join(', ')}
										</div>
									)}
								...
								>
									...
									{!normalizedConfig.showSuccessModal &&
										!normalizedConfig.showAuthCodeInModal &&
										!normalizedConfig.showCredentialsModal &&
										!normalizedConfig.enableErrorRecovery &&
										!normalizedConfig.showAuthRequestModal &&
										' None enabled'}
								</div>
							</div>
						</ConfigSection>
					...
					{/* Success Message */}
					{showSuccessMessage && (
						<StandardMessage type="success" message="Configuration saved successfully!" />
					)}
				</CardBody>
			</Card>
		</ConfigContainer>
