...
const UnifiedAuthorizationCodeFlowV3: React.FC<UnifiedFlowProps> = ({ flowType }) => {
	...
	const [credentials, setCredentials] = useState<StepCredentials>(() => {
					...
					if (!allCredentials.environmentId || !allCredentials.clientId) {
						...
						if (loginCreds) {
							try {
								...
								console.warn(
									`⚠️ [${flowType.toUpperCase()}-V3] Failed to parse login_credentials:`,
									e
								);
							}
						}
		...
		const redirectUri =
			urlRedirect || allCredentials?.redirectUri || `${window.location.origin}/authz-callback`;
	...
	const [flowConfig, setFlowConfig] = useState<FlowConfig>(() => {
		...
		try {
			...
			console.warn(
				`⚠️ [${flowType.toUpperCase()}-V3] Failed to load saved flow configuration:`,
				error
			);
		}
		...
		const reloadCredentials = () => {
			...
			if (foundCredentials && foundCredentials.environmentId && foundCredentials.clientId) {
				...
				setCredentials((prev) => ({
					...
					scope: Array.isArray(foundCredentials.scopes)
						? foundCredentials.scopes.join(' ')
						: foundCredentials.scopes || prev.scope,
					scopes: Array.isArray(foundCredentials.scopes)
						? foundCredentials.scopes
						: foundCredentials.scopes
							? foundCredentials.scopes.split(' ')
							: prev.scopes,
				}));
			} else {
				console.warn(
					`⚠️ [${flowType.toUpperCase()}-V3] No valid credentials found in any storage after reload`
				);
			}
		};
		...
		try {
			...
			if (response.ok && result.access_token) {
				...
				if (refreshSuccess) {
					...
					console.warn(
						`⚠️ [${flowType.toUpperCase()}-V3] Failed to store new tokens for Token Management page`
					);
				}
	...
	useEffect(() => {
		const loadCredentialsOnMount = () => {
			...
			if (allCredentials && allCredentials.environmentId && allCredentials.clientId) {
				...
				console.warn(
					`⚠️ [${flowType.toUpperCase()}-V3] No valid credentials found in any storage mechanism on mount`
				);
				console.log(
			...
			if (flowType === 'oidc') {
				...
				if (flowConfig.acrValues && flowConfig.acrValues.length > 0) {
					...
					if (validAcrValues.length > 0) {
						...
						console.warn(
							`⚠️ [${flowType.toUpperCase()}-V3] No valid ACR values found, skipping acr_values parameter`
						);
					}
				}
	...
	const handlePopupAuthorizationWithModal = useCallback(() => {
		...
		console.log(
			`🔍 [${flowType.toUpperCase()}-V3] Modal disabled, proceeding directly with authorization`
		);
	...
	const handleFullRedirectAuthorizationWithModal = useCallback(() => {
		...
		if (flowConfig.showAuthRequestModal && !skipModal) {
			console.log(
				`🔍 [${flowType.toUpperCase()}-V3] Showing OAuth Authorization Request Modal for redirect`
			);
			setShowAuthRequestModal(true);
			return;
		}
		...
		console.log(
			`🔍 [${flowType.toUpperCase()}-V3] Modal disabled, proceeding directly with full redirect`
		);
		// Otherwise proceed directly
...
Original Error: ${errorData.error_description || errorData.error}
          `.trim();
			...
			if (success) {
				...
				console.warn(
					`⚠️ [${flowType.toUpperCase()}-V3] Failed to store tokens for Token Management page`
				);
			}
			...
			if (flowType === 'oidc' && tokenData.id_token) {
				try {
					...
					if (!idTokenValidation.isValid) {
						console.warn(
							`⚠️ [${flowType.toUpperCase()}-V3] ID token validation failed:`,
