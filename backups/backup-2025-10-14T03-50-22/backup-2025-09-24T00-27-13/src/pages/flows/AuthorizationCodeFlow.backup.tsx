...
const AuthorizationCodeFlow: React.FC = () => {
	...
	const generateAuthUrl = useCallback(
		(pkceChallenge?: string) => {
			...
			console.log('🔧 [AuthorizationCodeFlow] Generating authorization URL...', {
				credentials,
				pkceCodes,
				challengeToUse,
			});
	...
	const steps: EnhancedFlowStep[] = [
		{
			...
			content: (
				<div>
					...
					<InfoBox type="info">
						<FiInfo />
						<div>
							<strong>Security Note:</strong> Your credentials will be saved locally in your browser
							and are not transmitted to any external servers.
						</div>
					</InfoBox>
					...
					{credentialsSaved && (
						<InfoBox type="success">
							...
							<div>
								...
								<br />
								Your OAuth credentials have been saved and are ready to use. You can now proceed to
								the next step.
							</div>
						</InfoBox>
					)}
			...
			canExecute: Boolean(
				credentials.clientId && credentials.environmentId && credentials.clientSecret
			),
		},
		{
			id: 'generate-pkce',
			...
			canExecute: Boolean(
				credentials.clientId && credentials.environmentId && credentials.authorizationEndpoint
			),
		},
		{
			id: 'build-auth-url',
		...
		},
		{
			...
			execute: async () => {
				if (!authCode) {
					return {
						success: false,
						error: 'Authorization code is required. Please complete the redirect step first.',
					};
				}
		...
		},
		{
			...
			execute: async () => {
				if (!tokens?.access_token) {
					return {
						success: false,
						error: 'Access token is required. Please complete the token exchange step first.',
					};
				}
				await getUserInfo();
				return { success: true };
