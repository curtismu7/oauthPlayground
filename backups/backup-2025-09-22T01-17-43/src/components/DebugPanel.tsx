...
const DebugPanel: React.FC = () => {
				...
				const shouldLog =
					...
					(message.includes('❌') &&
						(message.includes('OAuth') ||
							...
							message.includes('PingOne'))) || // OAuth/OIDC errors only
					(message.includes('✅') &&
						(message.includes('Token') ||
							message.includes('Auth') ||
							message.includes('PKCE') ||
							message.includes('Exchange') ||
							message.includes('User'))) || // Success indicators for flows only
					(message.includes('⚠️') &&
						(message.includes('OAuth') || message.includes('OIDC') || message.includes('Token'))) || // OAuth/OIDC warnings only
					...
					(message.includes('Failed to') &&
						(message.includes('token') || message.includes('auth') || message.includes('OAuth'))); // Specific failures

				if (shouldLog) {
					const filteredMessage = args
						.map((arg) => {
	...
	const checkForRecentErrors = (logEntries: LogEntry[]) => {
		...
		const recentErrors = logEntries.filter(
			(log) => log.level === 'error' && new Date(log.timestamp).getTime() > fiveMinutesAgo
		);
	...
	const getErrorType = (): 'configuration' | 'network' | 'oauth' | 'general' => {
		const recentLogs = logs.filter(
			(log) =>
				log.level === 'error' && new Date(log.timestamp).getTime() > Date.now() - 5 * 60 * 1000
		);
				...
				<DebugContent>
					{hasRecentErrors && !errorHelpDismissed && (
						<>
							...
							/>
							{getErrorType() === 'oauth' && <TokenExchangeDebugger />}
						</>
					)}
					...
					{logs.length === 0 ? (
						...
						logs.map((log, index) => (
							<LogEntryContainer key={index} $level={log.level}>
								...
								{log.data ? <LogData>{formatData(log.data)}</LogData> : null}
								{log.error && <LogData>Error: {log.error.message}</LogData>}
							</LogEntryContainer>
						))
					)}
				</DebugContent>
