...
if [ -f ".env" ]; then
    # Check for required environment variables (more flexible)
    run_test "PINGONE_ENVIRONMENT_ID present" "grep -q '^PINGONE_ENVIRONMENT_ID=' .env" 0
    run_test "PINGONE_CLIENT_ID present" "grep -q '^PINGONE_CLIENT_ID=' .env" 0
    run_test "PINGONE_CLIENT_SECRET present" "grep -q '^PINGONE_CLIENT_SECRET=' .env" 0
    run_test "PINGONE_REDIRECT_URI present" "grep -q '^PINGONE_REDIRECT_URI=' .env" 0
    run_test "PINGONE_API_URL present" "grep -q '^PINGONE_API_URL=' .env" 0
else
    echo "   • Skipping PingOne tests (.env file missing)"
    # This is now OK since credentials are handled by UI
    run_warning_test ".env.example exists as template" "[ -f '.env.example' ]" 0
fi
...
if check_port $PORT; then
    echo -e "   • Port $PORT is in use... ${YELLOW}⚠️  WARNING${NC} (will try to use anyway)"
    TEST_RESULTS+=("Port $PORT availability: WARN (in use but will continue)")
    ((TESTS_WARNING++))
else
    echo -e "   • Port $PORT is available... ${GREEN}✅ PASS${NC}"
    TEST_RESULTS+=("Port $PORT availability: PASS")
    ((TESTS_PASSED++))
fi
