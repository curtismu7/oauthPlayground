...
if (command !== 'info' && command !== 'clear' && !workerToken) {
	console.error(
		'Worker token is required for this command. Set PINGONE_WORKER_TOKEN environment variable or use --worker-token option'
	);
	process.exit(1);
}
...
class MockUserServiceV8 {
	static async fetchUsersPaginated(params) {
		...
		console.log(
			`🌐 Fetching users page ${page} (size: ${pageSize}) for environment ${environmentId}`
		);
	...
	static async fetchUsersPaginated(params) {
		...
		try {
			...
			const hasMore =
				...
				(totalCount === 0 || offset + pageSize < totalCount);
...
async function main() {
	try {
		switch (command) {
			case 'sync': {
				console.log(
					`${incremental ? '🔄 Starting incremental sync...' : '🔄 Starting full sync...'}`
				);

				// Worker token is required for sync
				if (!workerToken) {
					console.error(
						'❌ Worker token is required for sync. Set PINGONE_WORKER_TOKEN or use --worker-token'
					);
					process.exit(1);
				}
				...
				console.log(
					`📊 ${syncResult.totalUsers} users ${syncResult.incremental ? 'updated' : 'synced'}`
				);
				break;
			}
			...
			case 'info': {
				...
				const statusResponse = await fetch(
					`http://localhost:3001/api/users/sync-status/${environmentId}`
				);
				...
				if (statusData.last_sync_completed) {
					console.log(
						`   Last Synced: ${new Date(statusData.last_sync_completed).toLocaleString()}`
					);
				}
				if (statusData.last_sync_started && statusData.isSyncing) {
					console.log(
						`   Sync Started: ${new Date(statusData.last_sync_started).toLocaleString()}`
					);
				}
