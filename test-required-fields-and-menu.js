...
async function testRequiredFieldsAndMenu() {
    ...
    try {
        ...
        page.on('console', msg => {
            ...
            } else if (text.includes('Configuration cleared') || 
        ...
        const redBorderCheck = await page.evaluate(() => {
            ...
            requiredFields.forEach(fieldName => {
                const input = document.querySelector(`input[placeholder*="Required: Enter your ${fieldName.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())}"]`);
                if (input) {
                    const computedStyle = window.getComputedStyle(input);
                    results[fieldName] = {
                        hasRedBorder: computedStyle.borderColor.includes('239, 68, 68') || computedStyle.borderColor.includes('rgb(239, 68, 68)'),
        ...
        await page.evaluate(() => {
            const envInput = document.querySelector('input[placeholder*="Required: Enter your Environment ID"]');
            if (envInput) {
                envInput.focus();
        ...
        console.log('✅ Environment ID after typing:', afterTyping.notFound ? 'Not found' : `Red border: ${afterTyping.hasRedBorder}, Value: "${afterTyping.value}"`);
