...
const FlowButton = styled.button<{ $selected: boolean }>`
  ...
  color: ${({ $selected, theme }) => ($selected ? theme.colors.primary : theme.colors.gray700)};
  font-weight: 500;
  cursor: pointer;
  transition: all 0.2s;
...
const InteractiveFlowDiagram: React.FC = () => {
	...
	const getActorDescription = (actor: string) => {
		switch (actor) {
			...
			case 'auth-server':
				return 'PingOne authorization server';
			case 'server':
		...
		<DiagramContainer>
			...
			<FlowSelector>
				{flowDiagrams.map((flow) => (
					...
					>
						{getActorIcon(
							flow.id === 'authorization-code' ? 'client' : flow.id === 'pkce' ? 'client' : 'client'
						)}
						{flow.title}
					</FlowButton>
				))}
			...
			<DiagramArea>
				...
				{steps.map((step, index) => {
					...
					return (
						<Arrow key={`arrow-${step.id}`} $status={step.status} $delay={index * 200}>
							<FiArrowRight />
						</Arrow>
					);
				})}
