...
const DiscoveryPanel: React.FC<DiscoveryPanelProps> = ({ onConfigurationDiscovered, onClose }) => {
  ...
  const handleDiscover = async () => {
    if (!environmentId.trim()) {
      ...
      showFlowError('❌ Environment ID Required', 'Please enter your PingOne Environment ID to discover configuration');
      return;
    }
    ...
    if (!discoveryService.validateEnvironmentId(environmentId)) {
      ...
      showFlowError('❌ Invalid Environment ID', 'Please enter a valid UUID format for your PingOne Environment ID');
      return;
    }
    ...
    try {
      ...
      if (result.success && result.configuration) {
        ...
        showFlowSuccess('🔍 Configuration Discovered Successfully!', `Found ${Object.keys(result.configuration).length} endpoints for environment ${environmentId}`);
        ...
        showFlowError('❌ Discovery Failed', result.error || 'Failed to discover configuration. Please check your Environment ID and try again.');
      }
    } catch (error) {
      setStatus({ 
        type: 'error', 
      ...
      showFlowError('❌ Discovery Error', error instanceof Error ? error.message : 'Discovery failed. Please check your connection and try again.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleApplyConfiguration = () => {
    if (discoveredConfig) {
      try {
        ...
        showFlowSuccess('✅ Configuration Applied Successfully!', 'Your PingOne endpoints have been automatically configured');
        onClose();
      } catch (error) {
        showFlowError('❌ Failed to Apply Configuration', 'There was an error applying the discovered configuration. Please try again.');
        logger.error('DiscoveryPanel', 'Failed to apply configuration', error);
      }
    } else {
      showFlowError('❌ No Configuration to Apply', 'Please discover configuration first before applying it.');
    }
  };
  ...
  const handleCopyToClipboard = async (text: string, field: string) => {
    try {
      ...
      showFlowSuccess(`📋 ${field} Copied to Clipboard`, `The ${field} endpoint has been copied successfully`);
            ...
            <ConfigurationDisplay>
              ...
              <div style={{ marginTop: '1rem', display: 'flex', gap: '1rem' }}>
                ...
                <Button
                  ...
                  onClick={() => {
                    setDiscoveredConfig(null);
                    showFlowSuccess('🧹 Configuration Cleared', 'The discovered configuration has been cleared');
                  }}
                >
                  Clear
                </Button>
                <Button
                  variant="secondary"
                  onClick={() => {
                    showFlowSuccess('👋 Discovery Panel Closed', 'You can reopen it anytime from the Configuration page');
                    onClose();
                  }}
                >
                  <FiX />
