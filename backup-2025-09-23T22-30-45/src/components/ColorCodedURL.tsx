...
const getParameterDescription = (paramName: string): string => {
	const descriptions: Record<string, string> = {
		client_id:
			'The unique identifier for your OAuth application. This is provided by the authorization server when you register your application.',
		redirect_uri:
			'The URL where the authorization server will redirect the user after they grant or deny permission. Must match the URI registered with your application.',
		response_type:
			'Specifies the type of response you want to receive. Common values are "code" (authorization code flow) or "token" (implicit flow).',
		scope:
			'A space-separated list of permissions your application is requesting. Common scopes include "openid", "profile", "email", etc.',
		state:
			'A random string used to prevent CSRF attacks. The authorization server will return this exact value, allowing you to verify the request.',
		nonce:
			'A random string used to prevent replay attacks, especially important for OpenID Connect flows.',
		...
		grant_type:
			'Specifies the OAuth grant type being used. For authorization code flow, this should be "authorization_code".',
		client_secret:
			'A secret key that authenticates your application to the authorization server. Keep this secure and never expose it in client-side code.',
		code_challenge:
			'Used in PKCE (Proof Key for Code Exchange) flow. A challenge derived from a code verifier.',
		code_challenge_method:
			'The method used to generate the code challenge. Usually "S256" for SHA256.',
		code_verifier: 'A random string used in PKCE flow to generate the code challenge.',
		access_token:
			'A token that allows your application to access protected resources on behalf of the user.',
		token_type: 'The type of token returned. Usually "Bearer" for OAuth 2.0.',
		expires_in: 'The number of seconds until the access token expires.',
		refresh_token:
			'A token that can be used to obtain new access tokens without requiring user interaction.',
...
export const ColorCodedURL: React.FC<ColorCodedURLProps> = ({
	url,
	className,
	showInfoButton = true,
}) => {
		...
		if (part === '?') {
			...
			if (i === 0) {
				...
				urlParts.push({
					...
					content: part,
					description:
						'The base URL of the authorization server endpoint. This is where the OAuth request is sent.',
				});
			} else {
				...
				if (part.includes('=')) {
					...
					const paramType =
						paramIndex % 3 === 0 ? 'param' : paramIndex % 3 === 1 ? 'param2' : 'param3';
