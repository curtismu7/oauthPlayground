...
import { FiSettings, FiToggleLeft, FiToggleRight, FiAlertTriangle, FiCheckCircle, FiX } from 'react-icons/fi';
    ...
    const baseSettings = [
      {
        ...
        description: 'Enhances security for public clients by preventing authorization code interception',
  ...
  return (
    <SettingsPanel className={isExpanded ? 'expanded' : ''}>
      ...
      <SettingsContent className={isExpanded ? 'expanded' : ''}>
        <div style={{ marginBottom: '1rem', padding: '1rem', backgroundColor: '#eff6ff', borderRadius: '0.5rem', border: '1px solid #dbeafe' }}>
          ...
          <p style={{ margin: 0, fontSize: '0.875rem', color: '#3730a3' }}>
            Toggle settings on/off to see how they affect the OAuth flow. Some combinations may cause errors or change behavior - this is intentional for learning!
          </p>
        </div>
        ...
        <SettingGroup>
          ...
          {getSettingConfig().map((setting) => (
            <SettingItem key={setting.key} className={!settings[setting.key] && setting.required ? 'disabled' : ''}>
              <SettingInfo>
                <div className="label">
                  {setting.label}
                  {setting.required && <span style={{ color: '#ef4444', marginLeft: '0.25rem' }}>*</span>}
                </div>
