...
import { FiGitCompare, FiTrendingUp, FiShield, FiClock, FiUsers, FiKey, FiEye, FiDownload, FiShare2 } from 'react-icons/fi';
...
const FlowChip = styled.button<{ $selected: boolean; $flowType: string }>`
  ...
  border: 2px solid ${({ $selected, $flowType, theme }) => {
    if ($selected) {
      switch ($flowType) {
        case 'authorization-code': return theme.colors.primary;
        case 'implicit': return theme.colors.warning;
        case 'client-credentials': return theme.colors.success;
        case 'device-code': return theme.colors.info;
        case 'password': return theme.colors.danger;
  ...
  background: ${({ $selected, $flowType, theme }) => {
    if ($selected) {
      switch ($flowType) {
        case 'authorization-code': return theme.colors.primaryLight;
        case 'implicit': return theme.colors.warningLight;
        case 'client-credentials': return theme.colors.successLight;
        case 'device-code': return theme.colors.infoLight;
        case 'password': return theme.colors.dangerLight;
...
const MetricIcon = styled.div<{ $metric: string }>`
  color: ${({ $metric, theme }) => {
    switch ($metric) {
      case 'security': return theme.colors.success;
      case 'complexity': return theme.colors.warning;
      case 'performance': return theme.colors.info;
      case 'usability': return theme.colors.primary;
      default: return theme.colors.gray500;
    }
  }};
  font-size: 1.25rem;
  ...
  const toggleFlow = (flowType: string) => {
    setSelectedFlows(prev => {
      if (prev.includes(flowType)) {
        ...
        announceToScreenReader(`${flowData[flowType as keyof typeof flowData]?.name} removed from comparison`);
        return newFlows;
      } else if (prev.length < 4) {
        const newFlows = [...prev, flowType];
        announceToScreenReader(`${flowData[flowType as keyof typeof flowData]?.name} added to comparison`);
        return newFlows;
      } else {
                        ...
                        <TableCell>
                          <ul style={{ margin: 0, paddingLeft: '1rem' }}>
                            {flow.useCases.map((useCase, index) => (
                              <li key={index} style={{ fontSize: '0.875rem' }}>{useCase}</li>
                            ))}
                          </ul>
                        </TableCell>
                      </TableRow>
