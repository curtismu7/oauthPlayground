...
export async function requestClientCredentialsToken(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Token request failed: ${response.status} ${response.statusText}. ${errorData.error_description || errorData.error || 'Please check your credentials and scopes.'}`);
    }
...
export async function introspectToken(
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(`Token introspection failed: ${response.status} ${response.statusText}. ${errorData.error_description || errorData.error || 'Unable to introspect token.'}`);
    }
 ...
 */
export function validateWorkerCredentials(clientId: string, clientSecret: string): {
  isValid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
 ...
 */
export function shouldRefreshToken(token: WorkerTokenResponse, issuedAt: number, ttlPercent: number = 0.8): boolean {
  const now = Date.now();
