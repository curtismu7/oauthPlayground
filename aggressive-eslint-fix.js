...
function removeUnusedImports() {
  ...
  for (const filePath of files) {
    try {
      ...
      const unusedIcons = [
        'FiSettings', 'FiUser', 'FiPlay', 'FiEye', 'FiEyeOff', 'FiGlobe', 
        'FiArrowRight', 'FiAlertTriangle', 'FiClock', 'FiLoader'
      ];
      ...
      for (const icon of unusedIcons) {
        ...
        const patterns = [
          new RegExp(`import\\s*\\{\\s*${icon}\\s*\\}\\s*from\\s*['"]react-icons\\/fi['"];?\\s*\\n`, 'g'),
          new RegExp(`import\\s*\\{([^}]*),\\s*${icon}\\s*([^}]*)\\}\\s*from\\s*['"]react-icons\\/fi['"];`, 'g'),
          new RegExp(`import\\s*\\{\\s*${icon}\\s*,([^}]*)\\}\\s*from\\s*['"]react-icons\\/fi['"];`, 'g'),
      ...
      const unusedStyledComponents = [
        'CardHeader', 'CardFooter', 'FormGroup', 'TextArea', 'Select', 
        'InfoContainer', 'ErrorIcon', 'BackLink', 'StatusIndicator',
        'TokenDisplay', 'ResponseBox', 'DemoControls', 'DemoButton'
      ];
  ...
  for (const filePath of files) {
    try {
      ...
      const unusedVariables = [
        'isLoading', 'showModal', 'isVisible', 'error', 'result', 
        'data', 'response', 'config', 'credentials', 'tokens',
        'fileFixCount', 'unusedPatterns', 'disablePatterns'
      ];
      ...
      content = content.replace(/const\s*\{\s*([^}]*)\s*\}\s*=\s*[^;]+;\s*\/\/.*unused/gmi, '');
