  ...
  for (const filePath of files) {
    try {
      ...
      const unusedPatterns = [
        ...
        { from: /^\s*const\s+(\w+)\s*=\s*[^;]+;\s*$/gm, to: '  // const $1 = ...; // Unused variable removed' },
        
        // Unused destructuring assignments
        { from: /^\s*const\s*\{\s*([^}]+)\s*\}\s*=\s*[^;]+;\s*$/gm, to: '  // const { $1 } = ...; // Unused destructuring removed' },
      ...
      const safePatterns = [
        ...
        { from: /const\s+(\w+)\s*=\s*([^;]+);\s*\/\/.*unused/gmi, to: 'const _$1 = $2; // Unused variable' },
...
function addESLintDisableComments() {
  ...
  for (const filePath of files) {
    try {
      ...
      if (unusedVarCount > 5 && !content.includes('eslint-disable @typescript-eslint/no-unused-vars')) {
