...
export const RESPONSE_MODE_CONFIG: Record<ResponseMode, ResponseModeInfo> = {
	query: {
		mode: 'query',
		name: 'Query String',
		description: 'Authorization response parameters are encoded in the query string added to the redirect_uri when redirecting back to the application.',
	...
	fragment: {
		mode: 'fragment',
		name: 'URL Fragment',
		description: 'Authorization response parameters are encoded in the fragment added to the redirect_uri when redirecting back to the application.',
		...
		implementation: {
			...
			responseFormat: 'https://yourapp.com/callback#access_token=abc123&token_type=Bearer&expires_in=3600',
	...
	'form_post': {
		mode: 'form_post',
		name: 'Form POST',
		description: 'Authorization response parameters are encoded as HTML form values that are auto-submitted in the browser, transmitted through HTTP POST to the application.',
	...
	'pi.flow': {
		...
		description: 'PingOne proprietary redirectless flow that returns a flow object instead of redirecting. Enables embedded authentication without browser redirects.',
...
export class ResponseModeService {
	 ...
	 */
	static buildAuthUrl(baseUrl: string, responseMode: ResponseMode, additionalParams: Record<string, string> = {}): string {
	...
	static validateCompatibility(
		...
		if (!modeInfo.compatibility.responseTypes.includes(responseType)) {
			issues.push(`Response mode '${responseMode}' is not compatible with response type '${responseType}'`);
		}

		// Check client type compatibility
		if (!modeInfo.compatibility.clientTypes.includes(clientType)) {
			issues.push(`Response mode '${responseMode}' is not compatible with client type '${clientType}'`);
		}
