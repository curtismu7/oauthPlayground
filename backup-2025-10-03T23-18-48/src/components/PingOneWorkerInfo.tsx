...
export const PingOneWorkerData = {
  overview: {
    ...
    description: "A Worker application is an admin-level, non-interactive application connection to PingOne that acts on behalf of administrators to interact with PingOne APIs programmatically.",
    definition: "Think of it as a service account or machine identity that performs administrative tasks without requiring a human user to be present."
  },
  keyFunctions: [
    {
      name: "API Management",
      description: "Create, read, update, and delete PingOne resources (users, groups, applications, environments)"
    },
    {
      name: "Automation",
      description: "Enable infrastructure-as-code tools like Terraform to manage PingOne configurations"
    },
    {
      name: "Integration",
      description: "Connect external systems to PingOne for provisioning, authentication flows, and administrative operations"
    },
    {
      name: "Administrative Tasks",
      description: "Perform bulk operations, reporting, and monitoring without manual console access"
    }
  ],
  tokenUsage: {
    description: "Worker apps obtain access tokens via OAuth 2.0 Client Credentials Flow. Access tokens are JSON Web Tokens (JWTs) that authorize calls to PingOne Platform APIs.",
  ...
  roles: {
    description: "Worker applications have no roles by default. Roles must be assigned after creation to grant specific permissions.",
    commonRoles: [
      {
  ...
  security: {
    ...
    criticalNote: "Worker app credentials apply only to the Worker app itself, not to PingOne admin users. Admin users need separate role assignments to perform the same actions."
  }
};
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-800">
              {PingOneWorkerData.overview.title}
            </h1>
            <p className="text-gray-600 mt-2">
              {PingOneWorkerData.overview.description}
            </p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {PingOneWorkerData.keyFunctions.map((func, idx) => (
            <div key={idx} className="border rounded-lg p-4 bg-gradient-to-br from-blue-50 to-white hover:shadow-md transition">
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
        ...
        <div className="grid md:grid-cols-2 gap-3 mb-6">
          {PingOneWorkerData.security.bestPractices.map((practice, idx) => (
            <div key={idx} className="flex items-start gap-3 bg-red-50 rounded-lg p-3 border border-red-200">
