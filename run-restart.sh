...
# Check SQLite database schema (non-interactive version)
check_sqlite_database() {
    ...
    if [ -z "$schema_check" ]; then
        print_warning "Could not read database schema - deleting and recreating"
    ...
    if [ "$has_user_type" -eq 0 ] || [ "$has_updated_at" -eq 0 ]; then
        print_warning "Database schema is outdated - deleting and recreating"
        rm -f "$db_file" "${db_file}-shm" "${db_file}-wal"
...
main() {
    ...
    # Step 9: Final success message and server status summary
    show_final_summary
    
    print_success "🎉 OAuth Playground servers restarted successfully!"
    print_info "🌐 Ready to use at: https://localhost:3000"
    print_info "🔧 Backend HTTP API available at: http://localhost:3001"
    print_info "🔐 Backend HTTPS API available at: https://localhost:3002"
