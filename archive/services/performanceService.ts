...
class PerformanceService {
	...
	private initializePerformanceObservers(): void {
		...
		this.observePerformanceEntry('layout-shift', (entries) => {
			...
			entries.forEach((entry) => {
				if (
					!(entry as PerformanceEntry & { hadRecentInput: boolean; value: number }).hadRecentInput
				) {
					clsValue += (entry as PerformanceEntry & { hadRecentInput: boolean; value: number })
						.value;
				}
			});
	...
	private trackChunkLoading(): void {
		...
		type DynWindow = Window & {
			import?: (...a: unknown[]) => Promise<unknown>;
			__dynamicImport__?: (...a: unknown[]) => Promise<unknown>;
		};
	...
	public getSlowestChunks(limit: number = 5): ChunkLoadInfo[] {
		return [...this.chunkLoads].sort((a, b) => b.loadTime - a.loadTime).slice(0, limit);
	}
	...
	public logPerformanceSummary(): void {
		...
		if (slowChunks.length > 0) {
			...
			slowChunks.forEach((chunk, index) => {
				console.log(
					`  ${index + 1}. ${chunk.name}: ${chunk.loadTime.toFixed(2)}ms (${(chunk.size / 1024).toFixed(1)}KB)`
				);
			});
		}
