...
export class MFAServiceV8 {
	...
	static async getAllDevices(
		...
		try {
			...
			let devicesEndpoint = `https://api.pingone.com/v1/environments/${params.environmentId}/users/${user.id}/devices`;
			
			// Add filter to endpoint URL for API call tracking
			if (filter) {
				devicesEndpoint += `?filter=${encodeURIComponent(filter)}`;
			}
			devicesEndpoint += `&_t=${Date.now()}`;
