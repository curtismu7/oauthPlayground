...
import { callGroq, callGroqStream, isGroqAvailable, type GroqMessage } from '../services/groqService';
...
const AIAssistant: React.FC<AIAssistantProps> = ({ fullPage = false, onStartOAuthFlow, onClose }) => {
	...
	const [includeApiDocs, setIncludeApiDocs] = useState<boolean>(() => {
		try { return JSON.parse(localStorage.getItem(LS_TOGGLES_KEY) || '{}').includeApiDocs ?? false; } catch { return false; }
	});
	const [includeSpecs, setIncludeSpecs] = useState<boolean>(() => {
		try { return JSON.parse(localStorage.getItem(LS_TOGGLES_KEY) || '{}').includeSpecs ?? false; } catch { return false; }
	});
	const [includeWorkflows, setIncludeWorkflows] = useState<boolean>(() => {
		try { return JSON.parse(localStorage.getItem(LS_TOGGLES_KEY) || '{}').includeWorkflows ?? false; } catch { return false; }
	});
	const [includeUserGuide, setIncludeUserGuide] = useState<boolean>(() => {
		try { return JSON.parse(localStorage.getItem(LS_TOGGLES_KEY) || '{}').includeUserGuide ?? false; } catch { return false; }
	});
	...
	const [messages, setMessages] = useState<Message[]>(() => {
		try {
			...
			if (raw) {
				...
				if (parsed.ts && Date.now() - parsed.ts < MESSAGE_TTL_MS && Array.isArray(parsed.messages)) {
	...
	useEffect(() => {
		try {
			localStorage.setItem(LS_TOGGLES_KEY, JSON.stringify({
	...
	const handleExportConversation = useCallback(() => {
		const lines: string[] = ['# AI Assistant Conversation', `*Exported: ${new Date().toLocaleString()}*`, ''];
		messages.forEach((m) => {
	...
	const handleSend = async () => {
		...
		try {
			await callGroqStream(
				...
				(err) => {
					...
					usedFallback = true;
					const { answer } = aiAgentService.getAnswer(query, { includeApiDocs, includeSpecs, includeWorkflows, includeUserGuide });
		...
		const localResult = (!groqUsed || usedFallback)
			? aiAgentService.getAnswer(query, { includeApiDocs, includeSpecs, includeWorkflows, includeUserGuide })
			: null;
		const links: Array<{ title: string; path: string; type: string; external?: boolean }> =
			(localResult?.relatedLinks ?? []).map((link) => ({
				title: link.title,
				path: link.path,
				type: link.type,
	...
	const promptCategories = [
		...
		},
		{
			...
			prompts: [
				'List MFA policies',
				'List MFA devices for user <user-uuid>',
			],
		},
		{
			label: '📡 Webhooks',
	...
	return (
		<>
			...
			{isOpen && (
				...
				{showPromptsGuide && (
					<PromptsGuidePanel $expanded={!fullPage && isExpanded} $fullPage={fullPage}>
						...
						<PromptsGuideSubtitle>
							Click any prompt to use it. Start with <strong>&quot;Get worker token&quot;</strong> for live data.
						</PromptsGuideSubtitle>
						<PromptsGuideScroll>
				...
				<ChatWindow $expanded={!fullPage && isExpanded} $collapsed={!fullPage && isCollapsed} $fullPage={fullPage}>
					<ChatHeader>
						<HeaderContent>
							...
							<HeaderText>
								...
								<StatusRow>
									<StatusDot
										...
										title={groqAvailable ? 'Groq LLM connected (Llama 3.3 70B)' : groqAvailable === false ? 'Groq not configured — click to add key' : 'Checking Groq…'}
										onClick={groqAvailable === false ? () => { navigate('/configuration'); setIsOpen(false); } : undefined}
									...
									<StatusDot
										...
										title={braveAvailable ? 'Brave Search connected' : braveAvailable === false ? 'Brave Search not configured — click to add key' : 'Checking Brave…'}
										onClick={braveAvailable === false ? () => { navigate('/configuration'); setIsOpen(false); } : undefined}
...
<CollapseButton
						type="button"
						onClick={() => { setIsCollapsed((v) => !v); setIsExpanded(false); }}
...
<CloseButton onClick={() => { setIsOpen(false); setIsExpanded(false); setIsCollapsed(false); }} aria-label="Close assistant" title="Close assistant (Esc)">
						...
						{messages.map((message) => (
							<MessageWrapper key={message.id} $isUser={message.type === 'user'}>
								<MessageBubble $isUser={message.type === 'user'}>
									<MessageContent>{renderMessageText(message.content.replace('##LIVE_NUDGE##', '').trimEnd())}</MessageContent>
									{/* Streaming cursor */}
									...
									{message.type === 'assistant' && !message.streaming &&
										message.content.includes('##LIVE_NUDGE##') && !includeLive && (
										<LiveNudgeButton
											type="button"
									...
									{message.mcpResult && (
										<McpResultCard>
											...
											{message.mcpResult.credentialsRequired && (
												<McpCredentialHint>
													� Type <strong>&quot;Get worker token&quot;</strong> to authenticate, then
													ask again for live PingOne data.
												</McpCredentialHint>
											)}
												...
												(message.mcpResult.data as unknown[]).length > 0 && (
													<McpDataSection>
															...
															<McpJsonToggle
																onClick={() => setMessages(prev => prev.map(m =>
																	m.id === message.id && m.mcpResult
																		? { ...m, mcpResult: { ...m.mcpResult, rawJson: !m.mcpResult.rawJson } }
																		: m
																))}
															>
														...
														{message.mcpResult.rawJson
															? <McpDataPre>{JSON.stringify(message.mcpResult.data, null, 2)}</McpDataPre>
									...
									{message.webResult && (
										<BraveResultCard>
											...
											{message.webResult.credentialsRequired && (
												<McpCredentialHint>
													💡 Add your Brave Search API key in the Configuration page to enable web
													search.
												</McpCredentialHint>
											)}
					...
					<InputContainer>
						...
						/>
						<SendButton onClick={handleSend} disabled={!input.trim()} aria-label="Send message" title="Send message (Enter)">
							<span style={{ fontSize: '20px' }}>⬆</span>
						</SendButton>
						</InputContainer>
					</>)}
...
const MessageCopyBtn = styled.button<{ $copied?: boolean }>`
  ...
  white-space: nowrap;
  &:hover { background: rgba(102,126,234,0.1); color: #667eea; border-color: #667eea; }
...
const ConfirmButton = styled.button<{ $primary?: boolean }>`
  ...
  background: ${(p) => (p.$primary ? 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' : 'white')};
...
const PromptsToggleButton = styled.button<{ $active: boolean }>`
  ...
  &:hover {
    border-color: #667eea;
    background: ${(p) => (p.$active ? 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' : '#f4f4fb')};
  }
`;
