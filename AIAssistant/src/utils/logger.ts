...
// Accepts any value so callers can log arbitrary data (including caught unknowns)
...
class Logger {
	...
	warn(component: string, message: string, data?: LogData, error?: Error) {
		this.addToHistory('WARN', component, message, data, error);
		// Log to console
		if (component !== 'CONSOLE') {
			console.warn(` [${component}] ${message}`, data ?? '', error ?? '');
		}
	}
