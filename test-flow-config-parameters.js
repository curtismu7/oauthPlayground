...
const flowConfigTests = [
  {
    ...
    tests: [
      { name: 'Response Type', param: 'response_type', expected: 'code', description: 'Should use code for Authorization Code flow' },
      { name: 'Grant Type', param: 'grant_type', expected: 'authorization_code', description: 'Should be authorization_code' }
    ]
  },
  {
    category: '🎯 OAuth Scopes', 
    tests: [
      { name: 'OpenID Scope', param: 'scope', expected: 'openid', description: 'Should include openid for OIDC' },
      { name: 'Profile Scope', param: 'scope', expected: 'profile', description: 'Should include profile scope' },
      { name: 'Email Scope', param: 'scope', expected: 'email', description: 'Should include email scope' },
      { name: 'Custom Scopes', param: 'scope', expected: 'address phone offline_access', description: 'Should support additional scopes' }
    ]
  },
  {
    category: '🛡️ PKCE (Proof Key for Code Exchange)',
    tests: [
      { name: 'PKCE Enabled', param: 'code_challenge', expected: 'present', description: 'Should include code_challenge when PKCE enabled' },
      { name: 'Challenge Method', param: 'code_challenge_method', expected: 'S256', description: 'Should use S256 method (recommended)' }
    ]
  },
  {
    category: '🔐 OpenID Connect Settings',
    tests: [
      { name: 'Nonce Parameter', param: 'nonce', expected: 'present', description: 'Should include nonce for OIDC security' },
      { name: 'State Parameter', param: 'state', expected: 'present', description: 'Should include state for CSRF protection' },
      { name: 'Max Age', param: 'max_age', expected: '3600', description: 'Should include max_age when set (in seconds)' },
      { name: 'Prompt', param: 'prompt', expected: 'login', description: 'Should include prompt parameter when set' }
    ]
  },
  {
    category: '👤 User Hints',
    tests: [
      { name: 'Login Hint', param: 'login_hint', expected: 'username@domain.com', description: 'Should include login_hint when provided' },
      { name: 'ACR Values', param: 'acr_values', expected: 'urn:mace:incommon:iap:silver', description: 'Should include ACR values when set' }
    ]
  },
  {
    category: '⚙️ Custom Parameters',
    tests: [
      { name: 'Custom Param 1', param: 'custom_param', expected: 'custom_value', description: 'Should include custom parameters' },
      { name: 'Custom Param 2', param: 'another_param', expected: 'another_value', description: 'Should support multiple custom parameters' }
    ]
  }
];
  ...
  category.tests.forEach(test => {
    ...
    switch (test.param) {
      case 'response_type':
        ...
        break;
      case 'grant_type':
        passed = flowContent.includes('flowConfig.grantType ||') && flowContent.includes('authorization_code');
        break;
      case 'scope':
        passed = flowContent.includes('flowConfig.scopes.join(\' \')');
        break;
      case 'code_challenge':
        passed = flowContent.includes('flowConfig.enablePKCE') && flowContent.includes('code_challenge');
        break;
      case 'code_challenge_method':
        passed = flowContent.includes('code_challenge_method') && flowContent.includes('flowConfig.codeChallengeMethod');
        break;
      case 'nonce':
...
const tokenTests = [
  {
    ...
    check: tokenContent.includes('Bearer (Access Token)') || tokenContent.includes('getTokenTypeInfo().type'),
