...
const usageMap = new Map();  // item -> Set of files that use it
...
function analyzeFile(filePath) {
  try {
    ...
    // Export declarations
    const exportRegex = /export\s+(?:(?:const|let|var|function|class|interface|type|enum)\s+(\w+)|default\s+(?:(?:const|let|var|function|class)\s+)?(\w+)|\{([^}]+)\})/g;
...
function findUnusedVariables(filePath, content) {
  ...
  const funcRegex = /(?:function\s+(\w+)|const\s+(\w+)\s*=\s*(?:\([^)]*\)\s*=>|\([^)]*\)\s*=>\s*\{|function))/g;
