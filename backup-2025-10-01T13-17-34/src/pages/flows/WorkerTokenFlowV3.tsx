...
const WorkerTokenFlowV3: React.FC = () => {
		...
		const loadInitialCredentials = async () => {
			try {
				...
				if (workerCredentials.environmentId || workerCredentials.clientId) {
					...
					const extendedCredentials = workerCredentials as Record<string, unknown>;
					const authMethod =
						(extendedCredentials.clientAuthMethod as WorkerTokenAuthMethod) || 'client_secret_post';
					...
					if (configCredentials.environmentId || configCredentials.clientId) {
						...
						setUseJwksEndpoint(
							configCredentials.tokenAuthMethod === 'private_key_jwt' ? true : true
						);
	...
	const requestAccessToken = useCallback(async () => {
		try {
			...
			if (
				...
				sanitizedScopes !== credentials.scopes.trim()
			) {
				showFlowSuccess(
					'Scopes cleaned up',
	...
	const navigateToTokenManagement = useCallback(() => {
		...
		if (tokens?.access_token) {
			...
			showFlowError(
				'No access token available for analysis',
				'Please obtain a token first before analyzing it.'
			);
		}
	}, [tokens]);
						...
						>
							...
							>
								...
								<div style={{ marginBottom: '1rem' }}>
									...
									>
										Worker Tokens are access tokens obtained using the OAuth 2.0 Client Credentials
										Grant flow, designed for machine-to-machine (M2M) communication with no user
								...
								<div style={{ marginBottom: '1rem' }}>
									...
									>
										<li>
											Your application sends client credentials (client_id + client_secret) to
											PingOne's token endpoint
										</li>
										<li>PingOne validates the credentials and returns an access token</li>
										<li>
											Your application uses the access token to call PingOne Management API
											endpoints
										</li>
								...
								<div style={{ marginBottom: '1rem' }}>
									...
									>
										<li>
											<strong>Client Secret Post</strong> - Secret sent in request body (default)
										</li>
										<li>
											<strong>Client Secret Basic</strong> - HTTP Basic Authentication header
										</li>
										<li>
											<strong>Client Secret JWT</strong> - JWT signed with client secret (HS256)
										</li>
										<li>
											<strong>Private Key JWT</strong> - JWT signed with private key (RS256)
										</li>
									</ul>
								</div>
								...
								<div style={{ marginBottom: '1rem' }}>
									...
									>
										Worker Tokens have broad permissions - use only for trusted applications and
						...
						>
							...
							<FormField key={`auth-method-dropdown-${resetKey}-${useJwksEndpoint}`}>
								...
								>
									...
									<CollapsibleSection title="Available Methods" defaultExpanded={false}>
										<div style={{ marginBottom: '0.75rem' }}>
											...
											>
												<strong> Important:</strong> The selected method must match your PingOne
												application's "Token Endpoint Authentication Method" setting in Admin
										...
										<div style={{ display: 'grid', gap: '0.5rem' }}>
											<div
												style={{
													background:
														clientAuthMethod === 'client_secret_post' ? '#fef3c7' : '#f9fafb',
													border:
											...
											>
												...
												<div>
													<code
														style={{
															background:
																clientAuthMethod === 'client_secret_post' ? '#f59e0b' : '#f3f4f6',
															color:
																clientAuthMethod === 'client_secret_post' ? 'white' : '#374151',
											...
											<div
												style={{
													background:
														clientAuthMethod === 'client_secret_basic' ? '#fef3c7' : '#f9fafb',
													border:
											...
											>
												...
												<div>
													<code
														style={{
															background:
																clientAuthMethod === 'client_secret_basic' ? '#f59e0b' : '#f3f4f6',
															color:
																clientAuthMethod === 'client_secret_basic' ? 'white' : '#374151',
															padding: '0.2rem 0.4rem',
															borderRadius: '4px',
															fontSize: '0.8rem',
															fontWeight: '600',
											...
											<div
												style={{
													background:
														clientAuthMethod === 'client_secret_jwt' ? '#fef3c7' : '#f9fafb',
													border:
											...
											>
												...
												<div>
													<code
														style={{
															background:
																clientAuthMethod === 'client_secret_jwt' ? '#f59e0b' : '#f3f4f6',
											...
											<div
												style={{
													background:
														clientAuthMethod === 'private_key_jwt' ? '#fef3c7' : '#f9fafb',
													border:
														clientAuthMethod === 'private_key_jwt'
															? '2px solid #f59e0b'
															: '1px solid #e5e7eb',
											...
											>
												...
												<div>
													<code
														style={{
															background:
																clientAuthMethod === 'private_key_jwt' ? '#f59e0b' : '#f3f4f6',
											...
											<div
												style={{
													...
													border:
														clientAuthMethod === 'none' ? '2px solid #f59e0b' : '1px solid #e5e7eb',
													borderRadius: '6px',
													padding: '0.75rem',
													display: 'flex',
													alignItems: 'center',
									...
									{useJwksEndpoint ? (
										<div style={{ display: 'grid', gap: '1rem' }}>
											...
											>
												...
												>
													{resolvedJwksDisplayUrl || 'https://oauth-playground.vercel.app/jwks'}
												</code>
												<br />
												...
												<br />
												Use the <strong>JWT Generator JWKS</strong> tab to convert your private key
												to the correct JWKS format for PingOne.
											</div>
											<div>
				...
				content: (
					<div>
						...
						>
							...
							>
								<li>
									Your credentials are validated and formatted according to the selected
									authentication method
								</li>
								...
								<li>
									The token is displayed and can be used to call PingOne Management API endpoints
								</li>
							</ol>
						</div>
							...
							<div style={{ marginTop: '1rem' }}>
								...
								>
									...
									>
										...
										<TokenSurface hasToken={true} scrollable aria-label="Access Token">
											<div
												style={{
													fontFamily:
						...
						{!introspection ? (
							...
							>
								...
								<div style={{ marginBottom: '1.5rem' }}>
									...
									>
										Token introspection is an OAuth 2.0 mechanism (RFC 7662) that allows resource
										servers to query the authorization server to determine the active state and
										metadata of an access token. This is crucial for security and validation.
									</p>
									...
									>
										...
										<ul style={{ margin: '0', paddingLeft: '1.25rem' }}>
											<li>
												<strong>Security Validation:</strong> Verify the token is still active and
												hasn't been revoked
											</li>
											<li>
												<strong>Scope Verification:</strong> Confirm what permissions the token
												actually has
											</li>
											<li>
												<strong>Token Metadata:</strong> Get detailed information about when it was
												issued, expires, and who it belongs to
											</li>
											<li>
												<strong>Audit Trail:</strong> Track token usage and validate client identity
											</li>
										</ul>
									</div>
								</div>
								...
								>
									Click "Introspect Token" to validate your worker token and see its complete
								...
								>
									...
									<ul style={{ margin: '0.5rem 0 0 1rem', paddingLeft: '0' }}>
										<li>
											<strong>JWT Claims:</strong> iss (issuer), sub (subject), aud (audience), exp
											(expiration), iat (issued at)
										</li>
										<li>
											<strong>Token Metadata:</strong> Client ID, scope, token type, JWT ID (jti)
										</li>
										<li>
											<strong>Timing Info:</strong> When issued, expires, and becomes valid (nbf)
										</li>
										<li>
											<strong>User Claims:</strong> Username, email, name (if applicable)
										</li>
										<li>
											<strong>Complete Response:</strong> Full JSON introspection data
										</li>
									</ul>
								</div>
							</div>
				...
				<InfoBox>
					<div style={{ marginBottom: '1rem' }}>
						...
						<p style={{ margin: '0 0 1rem 0', lineHeight: '1.5' }}>
							A Worker Token is an OAuth 2.0 Client Credentials flow that allows your application to
							authenticate directly with PingOne APIs without any user interaction. It's perfect for
							server-to-server communication, background services, and automated processes.
						</p>
					</div>
					...
					<div style={{ marginBottom: '1rem' }}>
						...
						<ul style={{ margin: '0', paddingLeft: '1.5rem', lineHeight: '1.6' }}>
							<li>
								<strong>Background Services:</strong> Automated data synchronization, ETL processes
							</li>
							<li>
								<strong>API Integration:</strong> Server-to-server communication between
								applications
							</li>
							<li>
								<strong>Monitoring & Alerting:</strong> Health checks, system monitoring, automated
								reports
							</li>
							<li>
								<strong>Workflow Automation:</strong> CI/CD pipelines, deployment automation
							</li>
							<li>
								<strong>Data Management:</strong> Bulk user operations, directory synchronization
							</li>
						</ul>
					</div>
					...
					<div style={{ marginBottom: '1rem' }}>
						...
						<ul style={{ margin: '0', paddingLeft: '1.5rem', lineHeight: '1.6' }}>
							<li>
								<strong>Broad Permissions:</strong> Worker Tokens can access all PingOne Management
								API endpoints
							</li>
							<li>
								<strong>Secret Storage:</strong> Client secrets must be stored securely (environment
								variables, key vaults)
							</li>
							<li>
								<strong>Limited Scope:</strong> Use only for trusted applications and services
							</li>
							<li>
								<strong>Rotation:</strong> Regularly rotate client secrets for enhanced security
							</li>
						</ul>
					</div>
					...
					<div style={{ display: 'grid', gap: '1rem' }}>
						...
						>
							...
							>
								...
								<p style={{ margin: '0' }}>
									<strong>Use Case:</strong> Server-to-server authentication with no user
									interaction required
								</p>
							</div>
						</div>
						...
						>
							...
							>
								<li>
									Go to <strong>"Configuration"</strong> tab in your Worker Application
								</li>
								<li>
						...
						>
							...
							>
								<li>
									<strong>Client Secret Post</strong> - Secret in POST body (default)
								</li>
								<li>
									<strong>Client Secret Basic</strong> - HTTP Basic Auth header
								</li>
								<li>
									<strong>Client Secret JWT</strong> - JWT signed with secret (HS256)
								</li>
								<li>
									<strong>Private Key JWT</strong> - JWT signed with private key (RS256)
								</li>
							...
							<p style={{ margin: '0', fontSize: '0.8rem', color: '#6c757d' }}>
								<strong>Important:</strong> Must match your PingOne application's "Token Endpoint
					...
					>
						...
						<div style={{ marginBottom: '1.5rem' }}>
							...
							>
								...
								<strong>Note:</strong> Worker Token flow uses client credentials directly and
								doesn't require user authorization.
							</p>
						</div>
				...
				>
					...
					>
						...
						<div style={{ marginBottom: '1.5rem' }}>
							...
							>
								This is the introspection endpoint that will be called to validate and analyze your
								worker token.
							</p>
						</div>
