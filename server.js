...
// PingOne — Get User ID by Username
// GET /api/pingone/user/by-username/:username?environmentId=&accessToken=
// Returns { id, username, email, name } — useful for chaining into user/:userId calls
app.get('/api/pingone/user/by-username/:username', async (req, res) => {
	try {
		...
		if (!environmentId || !accessToken) {
			return res.status(400).json({
				error: 'invalid_request',
				error_description: 'Missing required query params: environmentId, accessToken',
			});
		}
		...
		if (!trimmedUsername) {
			return res.status(400).json({
				error: 'invalid_request',
				error_description: 'username path param cannot be empty',
			});
		}

		// Escape for SCIM filter
		const escaped = trimmedUsername.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
		const filterUrl = `https://api.pingone.com/v1/environments/${environmentId}/users?filter=username eq "${escaped}"&limit=1`;

		console.log(`[PingOne by Username] Lookup: ${trimmedUsername}`);

		const response = await global.fetch(filterUrl, {
			method: 'GET',
			headers: {
				Authorization: `Bearer ${accessToken}`,
				Accept: 'application/json',
		...
		if (!response.ok) {
			console.error(`[PingOne by Username] API error (${response.status}):`, data);
			return res.status(response.status).json(data);
		}
		...
		if (users.length === 0) {
			return res.status(404).json({
				error: 'not_found',
				error_description: `No user found with username: ${trimmedUsername}`,
			});
		}

		const user = users[0];
		console.log(`[PingOne by Username] Found userId: ${user.id} for username: ${trimmedUsername}`);
		...
		console.error('[PingOne by Username] Server error:', error);
		res.status(500).json({
			error: 'server_error',
			error_description: 'Internal server error during username lookup',
			details: error.message,
		});
	}
});
