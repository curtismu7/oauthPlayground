  ...
  const [redirecting, setRedirecting] = useState(false);
	...
	useEffect(() => {
		const handleCallback = () => {
			...
			console.log('🔄 [AuthCallback] Code parameter:', code ? `${code.substring(0, 10)}...` : 'NULL');
      ...
      if (errorParam) {
        ...
        return;
			...
			if (code) {
				...
				console.log('✅ [AuthCallback] Authorization code received:', code.substring(0, 10) + '...');
				...
				if (isPopup) {
					...
					window.opener.dispatchEvent(new CustomEvent('oidc-auth-code-received', {
						detail: { code, state, timestamp: Date.now() }
					}));
					
					// For debugging - don't close automatically, let user close manually
					console.log('✅ [AuthCallback] DEBUG MODE - Popup will NOT close automatically. Close it manually when done debugging.');
				...
				console.log('✅ [AuthCallback] Verification results:', {
					isPopup,
					code: verifyCode ? `${verifyCode.substring(0, 10)}... (length: ${verifyCode.length})` : 'NOT FOUND',
				...
				console.error('❌ [AuthCallback] All URL params:', Object.fromEntries(new URLSearchParams(window.location.search).entries()));
  ...
  return (
    <Container>
      <Card>
        ...
        {status === 'loading' && (
          <>
            <Title $success={false} $error={false}>Processing Authorization</Title>
            <Message>
        ...
        {status === 'success' && (
          <>
            <Title $success={true} $error={false}>Authorization Successful!</Title>
            <Message>
              Your authorization was successful. We've received your authorization code and are redirecting you back to the flow.
            </Message>
            {authCode && (
              <CodeDisplay>
            ...
            {redirecting && (
              <RedirectingMessage>
                Redirecting you back to the OIDC Authorization Code Flow with your authorization code...
              </RedirectingMessage>
            )}
            
            {status === 'success' && !redirecting && (
              <RedirectingMessage>
                ✅ DEBUG MODE: Authorization code received and saved! Check console for details. Popup will NOT close automatically - close manually when done debugging.
              </RedirectingMessage>
            )}
          </>
        )}
        ...
        {status === 'error' && (
          <>
            <Title $success={false} $error={true}>Authorization Failed</Title>
            <Message>
