	...
	static createStatusManager(config: FlowStatusConfig) {
		return {
			...
			updateProgress() {
				...
				const completedSteps = this.currentState.steps.filter(s => s.status === 'completed').length;
				const totalSteps = this.currentState.steps.length;
				this.currentState.completionPercentage = totalSteps > 0 
	...
	return React.createElement('div', {
		...
		React.createElement('div', {
			...
			React.createElement('div', { key: 'current' }, [
				React.createElement('div', { key: 'label', style: { fontWeight: '600' } }, 'Current Step:'),
				React.createElement('div', { key: 'value' }, flowState.currentStep)
			]),
			React.createElement('div', { key: 'completed' }, [
				React.createElement('div', { key: 'label', style: { fontWeight: '600' } }, 'Completed:'),
			...
			React.createElement('div', { key: 'status' }, [
				...
				React.createElement('div', { key: 'value', style: { textTransform: 'capitalize' } }, flowState.status)
			])
		])
	]);
};
