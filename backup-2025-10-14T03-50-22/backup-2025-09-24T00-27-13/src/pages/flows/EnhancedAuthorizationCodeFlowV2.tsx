...
const EnhancedAuthorizationCodeFlowV2: React.FC = () => {
	...
	const scrollToStepProgress = useCallback(() => {
		setTimeout(() => {
			...
			if (stepProgressElement) {
				...
				console.log(
					'⚠️ [EnhancedAuthorizationCodeFlowV2] Bottom step progress not found, scrolling to bottom'
				);
				window.scrollTo({
			...
			if (storedCredentials && storedCredentials.clientId) {
				const convertedCredentials = {
					...
					scopes: Array.isArray(storedCredentials.scopes)
						...
						: storedCredentials.scopes || 'openid profile email',
					responseType: 'code',
					codeChallengeMethod: 'S256',
				};
				...
				console.log(
					'⚠️ [EnhancedAuthCodeFlowV2] No stored credentials found - user needs to configure'
				);
			}
		}
	}, []);
	...
	useEffect(() => {
		...
		if (stepParam) {
			...
			if (code) {
				...
				console.log(
					'⚠️ [EnhancedAuthorizationCodeFlowV2] Step parameter found but no authorization code in URL'
				);
		...
		if (code) {
			...
			setTimeout(() => {
				console.log(
					'🔄 [EnhancedAuthorizationCodeFlowV2] Final step 5 setting after component initialization'
				);
	...
	useEffect(() => {
		if (authCode && (currentStepIndex === 4 || currentStepIndex === 5) && !showAuthSuccessModal) {
			console.log(
				'🔔 [EnhancedAuthorizationCodeFlowV2] AuthCode detected on step',
				currentStepIndex,
				', forcing modal to show'
			);
			setTimeout(() => {
				setShowAuthSuccessModal(true);
			}, 200);
	...
	useEffect(() => {
		const loadCredentials = async () => {
			try {
				...
				console.log(
					'🔧 [EnhancedAuthorizationCodeFlowV2] Step 1: Checking authz flow credentials...'
				);
				...
				if (allCredentials?.clientId && allCredentials?.environmentId) {
					...
					console.log(
						'⚠️ [EnhancedAuthorizationCodeFlowV2] Authz flow credentials incomplete or missing'
					);

					// 2. Try configuration credentials (from Configuration page)
					console.log(
						'🔧 [EnhancedAuthorizationCodeFlowV2] Step 2: Checking configuration credentials...'
					);
					...
					if (configCredentials?.clientId && configCredentials?.environmentId) {
						...
						source = 'config-page';
						console.log(
							'✅ [EnhancedAuthorizationCodeFlowV2] Found valid configuration credentials'
						);
					} else {
						console.log(
							'⚠️ [EnhancedAuthorizationCodeFlowV2] Configuration credentials incomplete or missing'
						);

						// 3. Try permanent credentials
						console.log(
							'🔧 [EnhancedAuthorizationCodeFlowV2] Step 3: Checking permanent credentials...'
						);
						...
						if (permanentCredentials?.clientId && permanentCredentials?.environmentId) {
							...
							console.log(
								'⚠️ [EnhancedAuthorizationCodeFlowV2] Permanent credentials incomplete or missing'
							);

							// 4. Try legacy pingone_config
							console.log(
								'🔧 [EnhancedAuthorizationCodeFlowV2] Step 4: Checking legacy pingone_config...'
							);
								...
								try {
									...
									if (parsedConfig?.clientId && parsedConfig?.environmentId) {
										allCredentials = {
											...
											redirectUri:
												parsedConfig.redirectUri || window.location.origin + '/authz-callback',
										...
										console.log(
											'✅ [EnhancedAuthorizationCodeFlowV2] Found valid legacy pingone_config credentials'
										);
									}
								} catch (error) {
									console.warn(
										'⚠️ [EnhancedAuthorizationCodeFlowV2] Failed to parse legacy pingone_config:',
										error
									);
								}
							}
							...
							if (!allCredentials || (!allCredentials.clientId && !allCredentials.environmentId)) {
								console.log(
									'🔧 [EnhancedAuthorizationCodeFlowV2] Step 5: Checking legacy login_credentials...'
								);
								...
								if (loginCredentials) {
									try {
										...
										if (parsedLoginCreds?.clientId && parsedLoginCreds?.environmentId) {
											...
											console.log(
												'✅ [EnhancedAuthorizationCodeFlowV2] Found valid legacy login_credentials'
											);
										}
									} catch (error) {
										console.warn(
											'⚠️ [EnhancedAuthorizationCodeFlowV2] Failed to parse legacy login_credentials:',
											error
										);
									}
								}
				...
				if (
					...
					console.log(
						'🧹 [EnhancedAuthorizationCodeFlowV2] Found test values - this indicates no real credentials are configured'
					);
					// Don't clear test values automatically, just log the issue
					console.log(
						'⚠️ [EnhancedAuthorizationCodeFlowV2] Test values detected - please configure real credentials'
					);
				}
				...
				if (!hasValidCredentials) {
					console.log(
						'⚠️ [EnhancedAuthorizationCodeFlowV2] No valid credentials found, trying environment variables...'
					);
					try {
							...
							if (envConfig.environmentId && envConfig.clientId) {
								setCredentials((prev) => ({
									...
									tokenEndpoint:
										...
										`${envConfig.apiUrl}/${envConfig.environmentId}/as/token`,
									userInfoEndpoint:
									...
									scopes: Array.isArray(envConfig.scopes)
										...
										: envConfig.scopes || 'openid profile email',
								}));
						...
						console.warn(
							'⚠️ [EnhancedAuthorizationCodeFlowV2] Failed to load from environment variables:',
							envError
						);
					}
					...
					console.log(
						'❌ [EnhancedAuthorizationCodeFlowV2] No valid credentials found in any storage location'
					);
					setCredentialsLoaded(true);
					return;
				}
				...
				setCredentials((prev) => ({
					...
					scopes: Array.isArray(allCredentials.scopes)
						...
						: allCredentials.scopes || 'openid profile email',
				}));
				...
				if (
					...
					console.log(
						`🔄 [EnhancedAuthorizationCodeFlowV2] Migrating credentials from ${source} to authz flow storage...`
					);
					try {
						...
						if (migrationSuccess) {
							console.log(
								'✅ [EnhancedAuthorizationCodeFlowV2] Credentials successfully migrated to authz flow storage'
							);
						} else {
							console.warn(
								'⚠️ [EnhancedAuthorizationCodeFlowV2] Failed to migrate credentials to authz flow storage'
							);
						}
					} catch (migrationError) {
						console.error(
							'❌ [EnhancedAuthorizationCodeFlowV2] Error during credential migration:',
							migrationError
						);
					}
				}
			...
			setTimeout(() => {
				try {
					loadCredentials()
						...
						.catch((error) => {
							console.error(
								'❌ [EnhancedAuthorizationCodeFlowV2] Failed to reload credentials after change:',
								error
							);
						});
				} catch (error) {
			...
			if (code && step === '5') {
				...
				if (false) {
					...
					if (isFreshCode) {
						...
						console.log(
							'⚠️ [EnhancedAuthorizationCodeFlowV2] Skipping auto-exchange - using old authorization code, user should start fresh flow'
						);
					}
				} else {
					console.log(
						'⚠️ [EnhancedAuthorizationCodeFlowV2] Skipping auto-exchange - already in progress, tokens exist, or not on correct step'
					);
				}
				return;
			}
	...
	const generateAuthUrl = useCallback(() => {
		...
		if (flowConfig.acrValues.length > 0) {
			...
			if (validAcrValues.length > 0) {
				...
				console.warn(
					'⚠️ [EnhancedAuthorizationCodeFlowV2] No valid ACR values found, skipping acr_values parameter'
				);
			}
		}
		...
		try {
			...
			if (tokenData.id_token) {
				try {
					...
					updateStepMessage(
						'exchange-tokens',
						`⚠️ SECURITY WARNING: ID token validation failed: ${validationError instanceof Error ? validationError.message : String(validationError)}. Tokens received but ID token is not OIDC-compliant.`
					);
				}
			}
	...
	const steps: EnhancedFlowStep[] = [
				...
				<div>
					...
					<CodeBlock>
						...
						<CodeLine>
							...
							{pkceCodes.codeChallenge || '[CODE_CHALLENGE]'}&code_challenge_method=S256
						</CodeLine>
					...
					>
						...
						<div
							style={{ marginTop: '1rem', fontSize: '0.8rem', color: '#6b7280', lineHeight: '1.4' }}
						>
			...
			content: (
				<div>
					...
					>
						...
						<MethodDescription>
							Redirects current tab - more realistic production behavior
						</MethodDescription>
					</TestingMethodCard>
		...
		},
		{
			...
			content: (
				<div>
					...
					<FormField>
						...
						<FormInput type="text" value={state} readOnly />
			...
			execute: async () => {
				...
				console.log(
					'✅ [EnhancedAuthorizationCodeFlowV2] Handle Callback executed - auto-advancing to step 5'
				);
				...
				setTimeout(() => {
					...
					console.log(
						'🚀 [EnhancedAuthorizationCodeFlowV2] Auto-advanced to step 5 (Exchange Tokens)'
					);
				}, 100);
				...
				{authCode && (
					...
					>
						...
						{tokens && (
							...
							>
								<strong>✅ Tokens Exchanged Successfully!</strong> - Access token, refresh token,
								and ID token have been received.
							</div>
						)}
						{userInfo && (
							<div
					...
					onStepChange={useCallback(
						(stepIndex) => {
							...
							if (authCode && stepIndex < 5) {
								console.log(
									'🚫 [EnhancedAuthorizationCodeFlowV2] Preventing step change to',
									stepIndex,
									'- have authCode, should stay on step 5'
								);
								return;
							}
