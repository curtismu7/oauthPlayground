...
const ERROR_MESSAGES: Record<string, string> = {
	...
	CONFIG_ENVIRONMENT_ERROR:
		'Environment configuration error. Please check your environment variables.',
...
export class ErrorHandler {
	...
	private convertToAppError = (
		...
		if (error && typeof error === 'object' && 'response' in error) {
			...
			description =
				(genericError as { description?: string }).description || 'An unexpected error occurred';
	...
	getErrorCategory = (error: AppError): ErrorCategory => {
		if (
			...
			error.code.startsWith('CONNECTION_')
		) {
		...
		if (
			...
			error.code.startsWith('REQUIRED_') ||
			error.code.startsWith('INVALID_')
		) {
			return ErrorCategory.VALIDATION;
		}
		if (
			error.code.startsWith('SECURITY_') ||
			error.code.startsWith('CSRF_') ||
	...
	getErrorStatistics = (): {
		...
		const byCategory = Object.values(ErrorCategory).reduce(
			(acc, category) => {
				acc[category] = this.errorHistory.filter(
					(error) => this.getErrorCategory(error) === category
				).length;
				return acc;
			},
		...
		const bySeverity = Object.values(ErrorSeverity).reduce(
			(acc, severity) => {
				acc[severity] = this.errorHistory.filter(
					(error) => this.getErrorSeverity(error) === severity
				).length;
				return acc;
			},
		...
		for (let attempt = 1; attempt <= maxRetries; attempt++) {
			try {
				...
				if (attempt === maxRetries) {
					this.handleError(error, `${context} (final attempt ${attempt})`);
					throw error;
				}
...
export const handleAsyncError = async <T>(
	...
	try {
		...
		throw errorHandler.handleError(error, context, userId, sessionId);
	}
};
