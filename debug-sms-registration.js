...
function makeRequest(path, data) {
    return new Promise((resolve, reject) => {
        ...
        console.log(`   Environment ID: ${data.environmentId ? '✓' : '✗ MISSING'} ${data.environmentId}`);
        console.log(`   Username: ${data.username ? '✓' : '✗ MISSING'} ${data.username}`);
        console.log(`   Worker Token: ${data.workerToken ? '✓' : '✗ MISSING'} ${data.workerToken ? '(length: ' + data.workerToken.length + ')' : ''}`);
        console.log('');
