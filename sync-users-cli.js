...
if (!workerToken) {
	console.error(
		'Worker token is required. Set PINGONE_WORKER_TOKEN environment variable or use --worker-token option'
	);
	process.exit(1);
}
...
async function mockSyncUsers() {
	...
	for (let page = 1; page <= totalPages; page++) {
		...
		console.log(
			`📄 Page ${page}/${totalPages} • ${fetchedCount.toLocaleString()} users fetched (${percentage}%)`
		);
