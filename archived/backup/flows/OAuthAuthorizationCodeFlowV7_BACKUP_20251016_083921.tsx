...
const OAuthAuthorizationCodeFlowV7: React.FC = () => {
	...
	const handleFlowVariantChange = useCallback(
		(nextVariant: 'oauth' | 'oidc') => {
			...
			const _currentAuthUrl = controller.authUrl;
			const _currentAuthCode = controller.authCode;
			...
			const reloadedCredentials = FlowCredentialService.getSharedCredentials(
				`oauth-authorization-code-v7-${nextVariant}`
			);
	...
	const handleSaveAdvancedParams = useCallback(async () => {
		...
		FlowStorageService.AdvancedParameters.set('oauth-authz-v7', {
			audience,
			resources,
			promptValues,
		});
				...
				return (
					<>
						...
						<CollapsibleSection>
							...
							>
								<CollapsibleTitle>
									<FiBook /> {flowVariant === 'oidc' ? 'OpenID Connect' : 'OAuth 2.0'} Authorization
									Code Overview
								</CollapsibleTitle>
							...
							{!collapsedSections.overview && (
								<CollapsibleContent>
									<InfoBox $variant="info">
										...
										<div>
											<InfoTitle>
												When to Use {flowVariant === 'oidc' ? 'OpenID Connect' : 'OAuth 2.0'}{' '}
												Authorization Code
											</InfoTitle>
											<InfoText>
												{flowVariant === 'oidc'
									...
									<GeneratedContentBox style={{ marginTop: '2rem' }}>
										<GeneratedLabel>
											{flowVariant === 'oidc' ? 'OIDC' : 'OAuth 2.0'} Authorization Code - PingOne
											Implementation
										</GeneratedLabel>
										<ParameterGrid>
											...
											<div style={{ gridColumn: '1 / -1' }}>
												...
												<ParameterValue>
													{flowVariant === 'oidc'
														? 'Authentication + Authorization (user identity + API access)'
														: 'Authorization (API access only)'}
												</ParameterValue>
											</div>
											<div style={{ gridColumn: '1 / -1' }}>
												<ParameterLabel>Required Scopes (PingOne)</ParameterLabel>
												<ParameterValue>
													<strong>openid</strong> scope is required for both OAuth 2.0 and OIDC
													flows in PingOne
													{flowVariant === 'oidc' &&
														' + additional OIDC scopes (profile, email, etc.)'}
												</ParameterValue>
											</div>
											<div>
												...
												<ParameterValue>
													{flowVariant === 'oidc'
														? 'OpenID Connect Core 1.0 (extends OAuth 2.0)'
														: 'OAuth 2.0 RFC 6749'}
												</ParameterValue>
											</div>
											<div style={{ gridColumn: '1 / -1' }}>
												...
												<ParameterValue>
													{flowVariant === 'oidc'
														? 'Applications needing user identity verification and API access'
														: 'API authorization without user identity requirements (but still requires openid scope in PingOne)'}
												</ParameterValue>
											</div>
										</ParameterGrid>
									</GeneratedContentBox>
								...
								<CollapsibleContent>
									...
									>
										...
										<InfoBox style={{ maxWidth: '600px', marginTop: '1rem' }}>
											<strong>💡 Tip:</strong> Save your advanced parameters to persist them across
											browser refreshes. Your saved settings will automatically load next time you
											visit this flow.
										</InfoBox>
									</div>
								</CollapsibleContent>
						...
						{flowVariant === 'oidc' && (
							<InfoBox $variant="info" style={{ marginBottom: '1.5rem' }}>
								...
								<div>
									...
									<InfoText>
										The flow will now request an ID token, include the `openid` scope automatically,
										and expose additional OpenID Connect configuration options.
									</InfoText>
								</div>
							</InfoBox>
						)}
									...
									<InfoBox $variant="info">
										...
										<div>
											...
											<InfoText>
												...
												that prevents authorization code interception attacks. It's required for
												public clients (like mobile apps) and highly recommended for all OAuth
												flows.
											</InfoText>
										</div>
									</InfoBox>
