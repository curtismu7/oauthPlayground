...
class MFARetryService {
	...
	private static rateLimitData = new Map<
		string,
		{ requests: Date[]; violations: number; lastViolation?: Date }
	>();
	 ...
	 */
	static canRetry(context: MFAOperationContext, config?: Partial<RetryConfig>): RetryStatus {
	...
	static recordRetryAttempt(
		...
		if (!success && retryNumber < retryConfig.maxAttempts) {
			...
			const jitterDelay = retryConfig.jitter ? delay + Math.random() * 1000 : delay;
	 ...
	 */
	static getRetryDelay(context: MFAOperationContext, config?: Partial<RetryConfig>): number | null {
	...
	static getRetryStatistics(): {
		...
		['CHALLENGE', 'VERIFICATION', 'ACTIVATION', 'RESEND'].forEach((operation) => {
			...
			operationStats[operation] = {
				...
				successRate:
					operationAttempts.length > 0 ? (successful / operationAttempts.length) * 100 : 0,
			};
		});

		const rateLimitViolations = Array.from(MFARetryService.rateLimitData.values()).reduce(
