...
class HelpService {
	...
	static initialize(): void {
		HelpService.loadDefaultHelpContent();
		HelpService.loadDefaultTroubleshootingGuides();
		HelpService.loadDefaultContextualHelp();

		logger.info('HelpService', 'Help service initialized', {
			helpArticles: HelpService.helpContent.size,
			troubleshootingGuides: HelpService.troubleshootingGuides.size,
			contextualHelp: HelpService.contextualHelp.size,
		});
	}
	...
	static getHelpContent(id: string): HelpContent | null {
		const content = HelpService.helpContent.get(id);
		if (content) {
			HelpService.helpMetrics.totalViews++;
			HelpService.updatePopularArticles(id, content.title);
		}
		return content || null;
	}
	...
	static searchHelpContent(
		...
		if (normalizedQuery) {
			HelpService.trackSearchQuery(normalizedQuery);
		}

		let results = Array.from(HelpService.helpContent.values());
		...
		if (normalizedQuery) {
			results.sort((a, b) => {
				const scoreA = HelpService.calculateRelevanceScore(a, normalizedQuery);
				const scoreB = HelpService.calculateRelevanceScore(b, normalizedQuery);
	...
	static getTroubleshootingGuide(id: string): TroubleshootingGuide | null {
		return HelpService.troubleshootingGuides.get(id) || null;
	}
	...
	static findTroubleshootingBySymptoms(symptoms: string[]): TroubleshootingGuide[] {
		...
		return Array.from(HelpService.troubleshootingGuides.values())
			.filter((guide) =>
			...
			.sort((a, b) => {
				...
				const matchesA = a.symptoms.filter((symptom) =>
					normalizedSymptoms.some((userSymptom) => symptom.toLowerCase().includes(userSymptom))
				).length;
	...
	static getContextualHelp(stepId: string): ContextualHelp | null {
		return HelpService.contextualHelp.get(stepId) || null;
	}
	...
	static getHelpSuggestions(context: {
		...
		if (context.step) {
			const contextualHelp = HelpService.getContextualHelp(context.step);
			if (contextualHelp) {
		...
		if (context.error) {
			const errorHelp = HelpService.searchHelpContent(context.error, {
		...
		if (context.deviceType) {
			const deviceHelp = HelpService.searchHelpContent(context.deviceType, { category: 'devices' });
	...
	static submitFeedback(
		...
		HelpService.userFeedback.set(contentId, {
		...
		// Update metrics
		if (feedback.helpful) {
			HelpService.helpMetrics.userFeedback.helpful++;
		} else {
			HelpService.helpMetrics.userFeedback.notHelpful++;
		}
		...
		if (feedback.rating) {
			const totalRatings =
				HelpService.helpMetrics.userFeedback.helpful +
				HelpService.helpMetrics.userFeedback.notHelpful;
			const currentTotal = HelpService.helpMetrics.userFeedback.averageRating * (totalRatings - 1);
			HelpService.helpMetrics.userFeedback.averageRating =
				(currentTotal + feedback.rating) / totalRatings;
		}
	...
	static getHelpMetrics(): HelpMetrics {
		return { ...HelpService.helpMetrics };
	}
	...
	static createSupportTicket(ticket: {
		...
		HelpService.helpMetrics.supportTickets++;
		...
		const allFAQ = [
			{
				...
				answer:
					'MFA is a security method that requires two or more verification factors to gain access to your account, making it much more secure than just using a password.',
				category: 'general',
			},
			{
				question: 'Why do I need to set up MFA?',
				answer:
					'MFA significantly increases your account security by requiring additional verification beyond just your password. This protects against unauthorized access even if your password is compromised.',
				category: 'general',
			},
			{
				question: 'What MFA methods are available?',
				answer:
					'You can use SMS text messages, email codes, authenticator apps (TOTP), security keys (FIDO2), or mobile push notifications depending on what your organization has enabled.',
				category: 'devices',
			},
			{
				question: "I'm not receiving SMS codes. What should I do?",
				answer:
					'Check that your phone has signal, verify the phone number is correct, and ensure SMS is not blocked. You can also try using an alternative method like email or authenticator app.',
				category: 'troubleshooting',
			},
			{
				question: 'How do I set up an authenticator app?',
				answer:
					'Download an authenticator app like Google Authenticator or Microsoft Authenticator, then scan the QR code shown during setup. The app will generate time-based codes for authentication.',
				category: 'devices',
			},
			{
				question: 'What if I lose my phone or security key?',
				answer:
					"Contact your administrator immediately. They can help you regain access and set up new authentication methods. This is why it's recommended to have multiple backup methods configured.",
				category: 'troubleshooting',
			},
			{
				question: 'Can I use multiple authentication methods?',
				answer:
					"Yes, it's recommended to set up multiple methods as backups. This ensures you can still access your account if one method becomes unavailable.",
				category: 'devices',
			},
			{
				question: 'How secure are authenticator apps compared to SMS?',
				answer:
					'Authenticator apps are generally more secure than SMS because they generate codes locally on your device and are not susceptible to SIM swapping or network interception.',
				category: 'general',
			},
		];
...
Alternative solutions include using email codes or authenticator apps.`,
		...
		defaultContent.forEach((content) => {
			HelpService.helpContent.set(content.id, content);
		});
	}
	...
	private static loadDefaultTroubleshootingGuides(): void {
		...
		defaultGuides.forEach((guide) => {
			HelpService.troubleshootingGuides.set(guide.id, guide);
		});
	}
	...
	private static loadDefaultContextualHelp(): void {
				...
				'authentication',
				{
					...
					helpItems: [
						{
							...
							content:
								'Enter your username and password to begin the authentication process. Your credentials are encrypted and transmitted securely.',
						},
						{
							type: 'tip',
							title: 'Password Security',
							content:
								'Use a strong, unique password for your account. Consider using a password manager to generate and store secure passwords.',
						},
					],
					quickActions: [
						{
					...
					faq: [
						...
						},
						{
							...
							answer:
								'Accounts are temporarily locked after multiple failed login attempts for security. Wait 15 minutes or contact support.',
						},
					],
				},
			],
					...
					helpItems: [
						{
							...
							content:
								'Select from your registered authentication devices. Each method provides secure verification of your identity.',
						},
						{
							type: 'tip',
							title: 'Backup Methods',
							content:
								"It's recommended to have multiple authentication methods set up in case one becomes unavailable.",
						},
					],
					quickActions: [
						{
					...
					faq: [
						{
							...
							answer:
								'Security keys (FIDO2) and authenticator apps are generally more secure than SMS or email.',
						},
						{
							question: 'Can I use multiple devices?',
							answer:
								'Yes, you can register multiple devices and choose which one to use each time you sign in.',
						},
					],
				},
			],
					...
					helpItems: [
						{
							...
							content:
								'Complete the verification process using your selected authentication method. Follow the prompts specific to your chosen device.',
						},
						{
							type: 'warning',
							title: 'Time Sensitive',
							content:
								'Verification codes are time-sensitive and expire quickly. Enter the code as soon as you receive it.',
						},
					],
					quickActions: [
						{
					...
					faq: [
						{
							...
							answer:
								'Verification codes typically expire within 5-10 minutes for security reasons.',
						},
						{
							question: "What if the code doesn't work?",
							answer:
								"Ensure you're entering the most recent code. If issues persist, try requesting a new code or use an alternative method.",
						},
					],
				},
			],
		...
		defaultContextualHelp.forEach(([stepId, help]) => {
			HelpService.contextualHelp.set(stepId, help);
		});
	}
	...
	private static updatePopularArticles(id: string, title: string): void {
		const existing = HelpService.helpMetrics.popularArticles.find((article) => article.id === id);

		if (existing) {
			existing.views++;
		} else {
			HelpService.helpMetrics.popularArticles.push({ id, title, views: 1 });
		}

		// Keep only top 10 and sort by views
		HelpService.helpMetrics.popularArticles.sort((a, b) => b.views - a.views);
		HelpService.helpMetrics.popularArticles = HelpService.helpMetrics.popularArticles.slice(0, 10);
	}

	private static trackSearchQuery(query: string): void {
		const existing = HelpService.helpMetrics.searchQueries.find((sq) => sq.query === query);

		if (existing) {
			existing.count++;
		} else {
			const results = HelpService.searchHelpContent(query).length;
			HelpService.helpMetrics.searchQueries.push({ query, count: 1, results });
		}

		// Keep only top 20 queries
		HelpService.helpMetrics.searchQueries.sort((a, b) => b.count - a.count);
		HelpService.helpMetrics.searchQueries = HelpService.helpMetrics.searchQueries.slice(0, 20);
	}
}
