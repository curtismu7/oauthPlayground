...
export class FlowControllerService {
	...
	static createStepValidation(config: FlowControllerConfig, controller: any): StepValidation {
		...
		const canNavigateNext = useCallback(
			(currentStep: number, totalSteps: number): boolean => {
	...
	private static validateImplicitStep(stepIndex: number, controller: any): boolean {
		// Handle null/undefined controller gracefully
		if (!controller) {
			// For steps that don't require controller state, return true
			return stepIndex === 0 || stepIndex === 4 || stepIndex === 5;
		}
	...
	private static validateClientCredentialsStep(stepIndex: number, controller: any): boolean {
		switch (stepIndex) {
			case 0:
				return true;
			case 1:
				return Boolean(controller.credentials?.clientId && controller.credentials?.clientSecret);
			case 2:
	...
	private static validateDeviceAuthorizationStep(stepIndex: number, controller: any): boolean {
		switch (stepIndex) {
			case 0:
				return true;
			case 1:
				return Boolean(controller.deviceCodeData);
			case 2:
				return Boolean(controller.tokens);
			case 3:
				return Boolean(controller.tokens);
	...
	private static validateResourceOwnerPasswordStep(stepIndex: number, controller: any): boolean {
		switch (stepIndex) {
			case 0:
				return true;
			case 1:
				return Boolean(controller.credentials?.username && controller.credentials?.password);
			case 2:
				return Boolean(controller.tokens?.access_token);
			case 3:
				return true;
	...
	private static validateJWTBearerStep(stepIndex: number, controller: any): boolean {
		switch (stepIndex) {
			case 0:
				return true;
			case 1:
				return Boolean(controller.credentials?.privateKey);
			case 2:
				return Boolean(controller.tokens?.access_token);
			case 3:
				return true;
	...
	private static validateCIBAStep(stepIndex: number, controller: any): boolean {
		switch (stepIndex) {
			case 0:
				return true;
			case 1:
				return Boolean(controller.authRequestId);
			case 2:
				return Boolean(controller.tokens);
			case 3:
				return Boolean(controller.tokens);
	...
	private static validateRedirectlessStep(stepIndex: number, controller: any): boolean {
		switch (stepIndex) {
			case 0:
				return true;
			case 1:
				return Boolean(controller.pkceCodes?.codeVerifier && controller.pkceCodes?.codeChallenge);
			case 2:
				return Boolean(controller.authUrl);
			case 3:
				return Boolean(controller.flowResponse);
			case 4:
				return Boolean(controller.tokens);
			case 5:
				return true;
	...
	private static validateTokenIntrospectionStep(stepIndex: number, controller: any): boolean {
		switch (stepIndex) {
			case 0:
				return true;
			case 1:
				return Boolean(controller.credentials?.clientId);
			case 2:
				return Boolean(controller.introspectionResult);
			case 3:
				return true;
			default:
				return false;
	...
	private static validateTokenRevocationStep(stepIndex: number, controller: any): boolean {
		switch (stepIndex) {
			...
			case 1:
				return Boolean(controller.credentials?.clientId);
			case 2:
				return Boolean(controller.revocationResult);
			case 3:
				return true;
			default:
				return false;
	...
	private static getDefaultStepRequirements(flowType: string, stepIndex: number): string[] {
		...
		if (specific?.[stepIndex as keyof typeof specific]) {
