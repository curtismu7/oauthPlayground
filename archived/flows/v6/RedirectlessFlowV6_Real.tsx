...
const { NavigationButton } = V5StepperService.createStepLayout({
	theme: 'blue',
	showProgress: true,
});
	...
	const handleRedirectlessTokenExchange = useCallback(async () => {
		if (
			...
			v4ToastManager.showError(
				'Complete above actions: Generate PKCE parameters and ensure credentials are set.'
			);
			return;
		}
		...
		try {
			...
			console.log(
				'🔐 [Redirectless V6] Making real authorization request with response_mode=pi.flow - UPDATED VERSION'
			);
			...
			if (!authData.resumeUrl) {
				throw new Error(
					'No resumeUrl received from PingOne redirectless flow - this is the updated error message'
				);
			}
				...
				return (
					<>
						...
						>
							...
							>
								<strong>Key Difference:</strong> Instead of redirecting users to PingOne's hosted
								login page, you make a direct API call to the authorization endpoint with{' '}
						...
						>
							...
							>
								<strong>⚡ Key Point:</strong> Notice there's no <code>redirect_uri</code>{' '}
								parameter! The <code>response_mode=pi.flow</code> tells PingOne to return tokens
								directly in the API response instead of redirecting to a callback URL.
							</div>
						</div>
						...
						>
							...
							>
								...
								>
									...
									POST request to PingOne's authorization endpoint with{' '}
									<code>response_mode=pi.flow</code>
				...
				{controller.tokens?.accessToken && (
					<FlowCompletionService
						config={{
							...
							description:
								'You have successfully completed the redirectless OAuth flow using response_mode=pi.flow',
							nextSteps: [
