...
const OIDCAuthorizationCodeFlowV6: React.FC = () => {
	...
	const handleNext = useCallback(() => {
		console.log('🔍 [OIDC AuthZ V6] handleNext called:', {
			currentStep,
		...
		AuthorizationCodeSharedService.Navigation.handleNext(
			...
			isStepValid,
			() => {
				console.log('✅ [OIDC AuthZ V6] Navigation allowed, moving to next step');
			...
			case 0:
				return (
					<>
						...
						<EducationalContentService flowType="oidc" defaultCollapsed={false} />
									...
									>
										...
										<div>
											...
											<InfoText style={{ marginTop: '0.75rem', color: '#78350f' }}>
												<strong>What is Nonce?</strong> The nonce (number used once) is a
												cryptographically random string that binds your client session to the ID
												token and prevents replay attacks.
											</InfoText>
											...
											<ul style={{ marginTop: '0.5rem', paddingLeft: '1.5rem', color: '#78350f' }}>
												<li>
													Your client generates a unique random nonce value for each authorization
													request
												</li>
												<li>
													The nonce is sent in the authorization request to the identity provider
												</li>
											...
											>
												⚠️ <strong>Security Critical:</strong> Without nonce validation, an attacker
												could intercept an old ID token and replay it to impersonate a user. This
												playground automatically generates and validates nonce for educational
												purposes.
											</InfoText>
											<InfoText
												style={{
											...
											>
												💡 <strong>Spec Reference:</strong> OIDC Core 1.0 Section 15.5.2 requires
												nonce validation for all flows that return ID tokens directly (Implicit and
												Hybrid). It's also strongly recommended for Authorization Code flow.
											</InfoText>
										</div>
									</InfoBox>
					...
					<>
						...
						<ResultsSection>
							...
							<PKCEGenerationService.showComponent
								...
								flowType="oidc"
								onPKCEGenerated={() => {
									console.log('[OIDC AuthZ V6] PKCE codes generated successfully');
								}}
							/>
						</ResultsSection>
					</>
								...
								<CollapsibleContent>
									<InfoBox $variant="info">
										...
										<div>
											...
											<InfoText>
												After redirect, PingOne returns an authorization code. Paste it below if it
												didn't auto-populate.
											</InfoText>
										</div>
									</InfoBox>
									<form
					...
					<>
						...
						<FlowCompletionService
							config={{
								...
								flowDescription:
									"You've successfully completed the OIDC Authorization Code flow with PKCE. You received an ID token (for user identity), access token (for API calls), and optionally a refresh token.",
	...
	return (
		<Container>
			...
			{AuthenticationModalService.showModal(
				...
				() => {
					console.log('🔧 [OIDCAuthorizationCodeFlowV6] Continuing to PingOne authentication');
					setShowRedirectModal(false);
				...
				{
					description:
						"You're about to be redirected to PingOne for OpenID Connect authentication. This will open in a new popup window for secure authentication and identity verification.",
					redirectMode: 'popup',
				}
			)}
