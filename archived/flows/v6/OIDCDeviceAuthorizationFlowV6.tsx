...
const SmartTV = styled.div<{ $isWaiting: boolean }>`
	...
	&::before {
		...
		background: ${({ $isWaiting }) =>
			$isWaiting ? 'linear-gradient(90deg, #3b82f6 0%, #2563eb 50%, #3b82f6 100%)' : '#22c55e'};
	...
	const renderIntroduction = () => (
		<>
			...
			<InfoBox $variant="info" style={{ marginBottom: '1.5rem' }}>
				...
				<div>
					...
					<InfoText style={{ marginTop: '0.75rem', color: '#0c4a6e' }}>
						<strong>Device Flow Security Model:</strong> Unlike browser-based OIDC flows (Implicit,
						Hybrid), the Device Authorization Flow does NOT use a nonce parameter because there is
						no direct browser redirect where replay attacks could occur.
					</InfoText>
					...
					<InfoText style={{ marginTop: '0.75rem', color: '#0c4a6e', fontWeight: 'bold' }}>
						✅ <strong>Security Note:</strong> While nonce is not used in device flow authorization
						requests, the ID token you receive should still be validated for signature, issuer,
						audience, and expiration to ensure authenticity.
					</InfoText>
					<InfoText
						style={{
							marginTop: '0.5rem',
					...
					>
						💡 <strong>Spec Reference:</strong> RFC 8628 (Device Authorization Grant) combined with
						OIDC Core 1.0. Nonce is only required for flows where ID tokens are returned in URL
						fragments (Implicit, Hybrid).
					</InfoText>
				</div>
			</InfoBox>
			...
			<ComprehensiveCredentialsService
				...
				scopes={
					deviceFlow.credentials?.scope || deviceFlow.credentials?.scopes || 'openid profile email'
				}
				// Change handlers
				onEnvironmentIdChange={(newEnvId) => {
					...
					if (
						...
						newEnvId.trim() &&
						deviceFlow.credentials.clientId.trim()
					) {
			...
			<InfoBox $variant="info" style={{ marginTop: '1rem' }}>
				...
				<div>
					...
					<InfoText>
						<strong>Redirect URI:</strong> Not required for OIDC Device Authorization Flow (designed
						for input-constrained devices like smart TVs, IoT devices, or CLI tools).
						<br />
						<br />
						<strong>Scopes:</strong> The <code>openid</code> scope is <strong>required</strong> for
						OIDC flows (and also required by PingOne even for OAuth). It will be automatically added
						if you remove it. Additional common scopes: <code>profile</code>, <code>email</code>,
						<code>address</code>, <code>phone</code>, <code>offline_access</code> (for refresh
						tokens).
					</InfoText>
				</div>
			</InfoBox>
									...
									{UnifiedTokenDisplayService.showTokens(
										{
											...
											...(deviceFlow.tokens.expires_in && {
												expires_in: Number(deviceFlow.tokens.expires_in),
											}),
