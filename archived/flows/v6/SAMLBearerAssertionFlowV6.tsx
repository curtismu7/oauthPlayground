...
const SAMLBearerEducationalContent = {
	...
	security: {
		...
		description:
			'SAML Bearer Assertion flow provides enterprise-grade security through XML digital signatures and established trust relationships.',
		features: [
	...
	implementation: {
		...
		description:
			'The SAML Bearer Assertion flow involves obtaining a SAML assertion from an identity provider and exchanging it for an access token.',
		steps: [
...
const SAMLBearerAssertionFlowV6: React.FC = () => {
	...
	const [samlAssertion, setSamlAssertion] = useState<SAMLAssertion>(
		SAMLAssertionService.getDefaultSAMLAssertion()
	);
			...
			try {
				...
				if (result.success && result.document) {
					...
					if (result.document.token_endpoint) {
						...
						console.log(
							'[SAML Bearer] Token endpoint auto-populated:',
					...
					if (result.document.issuer) {
						...
						console.log(
							'[SAML Bearer] Issuer and Audience auto-populated:',
		...
		<>
			<EducationalContentService
				...
				content={{
					...
					description:
						'Set up the required credentials and endpoints for SAML Bearer Assertion flow.',
					useCases: [
			...
			>
				<InfoBox $variant="error">
					...
					<div>
						...
						<InfoText>
							<strong>Educational Simulation:</strong> This demonstrates how a SAML Bearer token
							request would be sent to an OAuth 2.0 server that supports RFC 7522. The assertion
							parameter contains the Base64-encoded SAML assertion that proves the user's identity.
						</InfoText>
						<InfoText
							style={{
								marginTop: '0.5rem',
			...
			{tokenResponse && (
				...
				>
					...
					<div style={{ marginTop: '1rem', display: 'flex', gap: '0.5rem' }}>
						...
						<Button
							onClick={() =>
								CopyButtonService.copyToClipboard(JSON.stringify(tokenResponse, null, 2))
							}
							$variant="secondary"
						>
			...
			<ContentWrapper>
				...
				<InfoBox $variant="warning" style={{ marginBottom: '2rem' }}>
					...
					<div>
						...
						<InfoText>
							This is a <strong>mock/educational implementation</strong> of the SAML Bearer
							Assertion flow. PingOne does not currently support SAML Bearer assertions for client
							authentication.
						</InfoText>
