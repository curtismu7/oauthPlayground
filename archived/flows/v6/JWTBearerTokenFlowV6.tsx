	...
	useEffect(() => {
		const loadCredentials = async () => {
			...
			if (credentials) {
				...
				if (credentials.scopes || credentials.scope)
					setScopes(credentials.scopes || credentials.scope || 'read write');
			} else {
							...
							<div style={{ padding: '1.5rem' }}>
								...
								<FormGroup>
									...
									<select
										...
										onChange={(e) =>
											setJwtSignature((prev) => ({ ...prev, algorithm: e.target.value as any }))
										}
										style={{
											width: '100%',
											padding: '0.75rem',
							...
							<CollapsibleContent>
								...
								<div style={{ marginBottom: '1.5rem' }}>
									...
									<div style={{ display: 'grid', gap: '0.75rem' }}>
										...
										<div
											onClick={() =>
												setTokenEndpoint('https://{pingfederate-host}:9031/as/token.oauth2')
											}
											style={{
												padding: '0.75rem',
												background: '#f8fafc',
