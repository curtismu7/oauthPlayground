	...
	return {
		...
		resolve: {
			...
			alias: {
				...
				'@/mfa/components': path.resolve(__dirname, './src/mfa/components'),
				'@/mfa/services': path.resolve(__dirname, './src/mfa/services'),
				'@/mfa/hooks': path.resolve(__dirname, './src/mfa/hooks'),
				'@/mfa/flows': path.resolve(__dirname, './src/mfa/flows'),
				'@/mfa/types': path.resolve(__dirname, './src/mfa/types'),
				'@/mfa/utils': path.resolve(__dirname, './src/mfa/utils'),
		...
		server: {
			...
			open: `https://${process.env.FRONTEND_HOST || 'api.ping.demo'}:8000`,
			host: true,
			...
			hmr:
				...
				env.VITE_DISABLE_HMR === 'true'
					? false
					: { port: 8000, host: 'localhost', clientPort: parseInt(env.VITE_HMR_CLIENT_PORT || '8000', 10) },
