...
const TokenExchangeDebugger: React.FC = () => {
	...
	const runDiagnostics = async () => {
		...
		try {
			const credentials = credentialManager.getPermanentCredentials();
			...
			if (sessionCredentials.clientSecret) {
				...
				results.checks.push({
					...
					details:
						'This may be required if your PingOne app is configured as a confidential client',
				});
			}
			...
			if (credentials.tokenEndpoint) {
				const isValidEndpoint =
					credentials.tokenEndpoint.includes('auth.pingone.com') &&
				...
				results.checks.push({
					...
					details: isValidEndpoint
						? 'Token endpoint URL looks correct'
						: 'Token endpoint URL may be incorrect',
				});
			} else {
				results.checks.push({
					id: 'token-endpoint',
			...
			if (credentials.redirectUri) {
				const isValidUri =
					credentials.redirectUri.startsWith('http') &&
					(credentials.redirectUri.includes('localhost') ||
						credentials.redirectUri.includes('https://'));
				results.checks.push({
					id: 'redirect-uri',
					name: 'Redirect URI',
					...
					details: isValidUri
						? 'Redirect URI format looks correct'
						: 'Redirect URI should be a valid HTTP/HTTPS URL',
				});
			} else {
				results.checks.push({
					id: 'redirect-uri',
		...
		<DebuggerContainer>
			...
			<DebuggerContent>
				<p style={{ margin: '0 0 1rem 0', color: '#374151' }}>
					This tool helps diagnose 401 Unauthorized errors during token exchange. It checks your
					configuration and provides specific guidance.
				</p>
				...
				{debugResults && (
					<DebugSection>
						...
						<CodeBlock>{JSON.stringify(debugResults, null, 2)}</CodeBlock>
					</DebugSection>
				)}
				...
				{debugResults?.checks && (
					<DebugSection>
						...
						{debugResults.checks.map((check: any) => (
