...
export class SessionManager {
  ...
  createSession(userId?: string, email?: string, data: Record<string, any> = {}): Session {
    const now = Date.now();
  ...
  private saveSessionsToStorage(): void {
    try {
      ...
      errorHandler.handleError(error, 'Session storage save');
    }
  }
...
// Utility functions
export const createSession = (userId?: string, email?: string, data?: Record<string, any>): Session => {
...
export const updateSession = (sessionId: string, data: Record<string, any>): boolean => {
