...
export const useUserBehaviorTracking = (config: UseUserBehaviorTrackingConfig = {}) => {
  ...
  // Start flow tracking
  const startFlowTracking = useCallback((flowType: string, properties?: Record<string, any>) => {
  ...
  // Track flow step
  const trackFlowStepTracking = useCallback((flowType: string, stepName: string, properties?: Record<string, any>) => {
    if (!enabled || !trackFlows) return '';
...
export const useUserActionTracking = (enabled: boolean = true) => {
  ...
  const trackBlur = useCallback((element: string, properties?: Record<string, any>) => {
  ...
  const trackScroll = useCallback((scrollDepth: number, properties?: Record<string, any>) => {
