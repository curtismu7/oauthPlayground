...
async function testClearConfig() {
    ...
    try {
        ...
        const clearedValues = await page.evaluate(() => {
            const clientIdInput = document.querySelector('input[placeholder*="Required: Enter your Client ID"]');
            const clientSecretInput = document.querySelector('input[placeholder*="Required: Enter your Client Secret"]');
            
            return {
                clientId: clientIdInput ? clientIdInput.value : 'not found',
                clientSecret: clientSecretInput ? clientSecretInput.value : 'not found',
                clientIdHasError: clientIdInput ? clientIdInput.style.borderColor === 'rgb(239, 68, 68)' : false,
                clientSecretHasError: clientSecretInput ? clientSecretInput.style.borderColor === 'rgb(239, 68, 68)' : false
            };
        });
