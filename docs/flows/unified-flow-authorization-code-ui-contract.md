...
- **ID Token** (OIDC only):
  ...
  - Decoded claims display (sub, iss, aud, exp, iat, etc.)
...
- Auth Method: Access token in `Authorization: Bearer {token}` header
...
interface FlowState {
  ...
  tokens?: {
    ...
    refreshToken?: string; // If enabled
    expiresIn: number;
    tokenType: 'Bearer';
    scope: string;
  };
  
  // UserInfo (from Step 5, OIDC only)
