...
- **URL Format**: `{{authPath}}/{{envID}}/as/device_authorization`, `{{authPath}}/{{envID}}/as/token`
...
-   `device_code`: Empty (filled after device authorization request)
-   `user_code`: Empty (filled after device authorization request)
-   `verification_uri`: Default value from PingOne
-   `verification_uri_complete`: Default value from PingOne
-   `expires_in`: Default value (typically 900 seconds)
-   `interval`: Default value (typically 5 seconds)
-   `access_token`: Empty (filled after token polling)
