...
- **Cause:** (1) App.tsx registered `/flows/par` → specialty Navigate and `/flows/dpop` self-Navigate before the real FlowsPar/FlowsDpop routes. (2) `PingOnePARFlowV9.tsx` had mangled `useState`, rendered storage-only `FlowCredentialService` as JSX, called removed PKCE/messaging APIs (`getStoredPKCECodes`, `showSuccessBanner`, `controller.reset`), and mismatched child props. (3) Auth-code mock treated `configured` as always true. (4) Playwright defaulted to `:3000` and E2E still hit retired `*-v9` paths / wrong mock secret.
- **Fix:** Removed bad Navigates; mount FlowsClientCredentials at `/flows/worker-token`; restore `useState<'oauth' | 'oidc'>`; use `ComprehensiveCredentialsService` + current PKCE/messaging/restart APIs; require credentials for Continue in mock+real; Playwright baseURL `:8000`; E2E paths → `/flows/*` with `mock-client-secret`.
...
- [ ] **tokenEndpointAuthMethod must survive load:** `WorkerTokenManager.loadCredentials()` must preserve `tokenEndpointAuthMethod` from storage. Dropping it forces `client_secret_post` and causes PingOne `invalid_client` / Unsupported authentication method. Auth failures must fail fast (no 3× retry spam). See Update log "WorkerTokenManager: preserve auth method + fail fast on 401".
