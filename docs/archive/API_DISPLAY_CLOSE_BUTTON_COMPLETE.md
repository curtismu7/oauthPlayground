...
### Service Layer
- ✅ `src/v8/services/apiDisplayService.ts` (140 lines)
- ✅ `src/v8/services/__tests__/apiDisplayService.test.ts` (110 lines)
...
useEffect(() => {
  const unsubscribe = apiDisplayService.subscribe(setIsVisible);
  return () => unsubscribe();
}, []);
```
...
- ✅ Test: `apiDisplayService.test.ts`
...
- Components should use `apiDisplayService` instead
