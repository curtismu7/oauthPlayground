# Run Environment ID Fix Once

To fix the environment ID in storage, run this in your browser console:

```javascript
// Run this ONCE to fix environment ID in storage
(function() {
        ...
        if (value) {
          ...
          if (matches) {
            ...
            if (wrongIds.length > 0) {
              ...
              if (parsed.environmentId && parsed.environmentId !== CORRECT_ENV_ID) {
                console.log(`  → Fixing environmentId: ${parsed.environmentId} → ${CORRECT_ENV_ID}`);
                parsed.environmentId = CORRECT_ENV_ID;
...
4. The script will fix all environment IDs in storage
