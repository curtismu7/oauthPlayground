...
test.describe('Frontend-Backend Integration Tests', () => {
  ...
  test('UserInfo endpoint integration structure', async ({ page }) => {
    const response = await page.request.get('http://localhost:3001/api/userinfo?access_token=invalid&environment_id=test');
  ...
  test('OpenID Discovery integration', async ({ page }) => {
    const response = await page.request.get('http://localhost:3001/api/discovery?environment_id=test');
