...
const DeviceAuthorizationFlowV7: React.FC = () => {
	...
	const getDefaultScopes = useCallback(
		(variant: 'oauth' | 'oidc' = selectedVariant) =>
			variant === 'oidc' ? 'openid profile email' : 'openid read write',
		[selectedVariant]
	);

	const mapStepToDeviceCredentials = useCallback(
		(creds: StepCredentials | null | undefined): DeviceAuthCredentials => {
			const scopeValue = (() => {
				if (!creds) return getDefaultScopes();
				if (Array.isArray(creds.scopes) && creds.scopes.length > 0) {
					return creds.scopes.join(' ');
				}
				if (typeof creds.scopes === 'string' && creds.scopes.trim()) {
					return creds.scopes.trim();
				}
				if (creds.scope && creds.scope.trim()) {
					return creds.scope.trim();
				}
			...
			return {
				...
				clientSecret: creds?.clientSecret?.trim() ? creds.clientSecret : undefined,
				scopes: scopeValue,
				loginHint: creds?.loginHint?.trim() ? creds.loginHint : undefined,
				postLogoutRedirectUri: creds?.postLogoutRedirectUri?.trim()
					? creds.postLogoutRedirectUri
					: undefined,
			};
		},
		[getDefaultScopes]
	...
	const handleStepCredentialsChange = useCallback(
		(updated: StepCredentials) => {
			console.log('🔄 [Device Authorization V7] Step credentials updated:', updated);
			const mapped = mapStepToDeviceCredentials(updated);
			deviceFlow.setCredentials(mapped);
		},
		[mapStepToDeviceCredentials, deviceFlow]
	);
	...
	const ensureCredentials = useCallback(
		(updates: Partial<DeviceAuthCredentials>) => {
			...
			next.scopes = next.scopes && next.scopes.trim() ? next.scopes : defaultScopes;

			deviceFlow.setCredentials(next);
		},
		[deviceFlow, getDefaultScopes]
	);
	...
	const hasShownCredentialWarning = useRef(false);
	React.useEffect(() => {
		if (hasShownCredentialWarning.current) {
			return;
		}
