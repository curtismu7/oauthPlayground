...
### **3.5 Inventory Regression Guardrail (Run after code changes)**
```bash
# Run the comprehensive inventory checks (fail fast on known regression patterns)
./comprehensive-inventory-check.sh

# Also run the inventory’s curated prevention commands when touching flows
# (search for "Enhanced Prevention Commands" in UNIFIED_MFA_INVENTORY.md)
```

**Goal:** turn “things to look for” into a repeatable, automated *gate* so regressions don’t slip in during refactors.
