...
- **SWE-15 Development Checklist** (pre/during/post change)
- **ISSUE LOCATION MAP (Where Issues Arise)** — jump to common “regression hotspots”
- **Enhanced Prevention Commands** — copy/paste checks for known issue classes
- **Flow Compliance (MUST NOT CHANGE)** — prohibited/allowed change guardrails
- **Active Issues Prevention** — issue-specific greps to ensure fixes stay fixed
...
### 🧰 Automated Inventory Gate: `comprehensive-inventory-check.sh`

**When to run:** *every* time you touch Unified MFA flows/services/components, and before merging.

**What it should do:**
- Execute the highest-signal “Enhanced Prevention Commands” as an automated gate.
- **Fail (exit non‑zero)** on violations that represent regressions (not just print warnings).
- Print a short, grep-friendly summary of failures so it’s easy to spot in CI logs.

**Common bash pitfall to avoid (prevents false results):**
Avoid using `... | wc -l && echo "bad" || echo "good"` as a boolean test. `wc` usually exits 0 even when the count is 0, so the conditional is misleading.
Prefer:
...
**How to add a new regression guard (template):**
1. Add a new check block with a clear label.
2. Capture a count (or grep exit code) and **make it fail** when the count exceeds the allowed threshold.
3. Add the same pattern to the relevant section of the **ISSUE LOCATION MAP** so future you knows where it can arise.
