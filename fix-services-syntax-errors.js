...
function fixServicesSyntaxErrors(content) {
  ...
  fixed = fixed.replace(/logger\.(info|success|error|warn)\([^}]*{\s*([^}]*)\s*$/gm, (match, level, params) => {
    if (!match.includes('});')) {
      return match + '\n    });';
    }
    return match;
  ...
  fixed = fixed.replace(/logger\.(info|success|error|warn)\([^}]*{\s*([^}]*)\s*\n\s*try\s*{/gm, (match, level, params) => {
  ...
  fixed = fixed.replace(/{\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*:\s*([^,}]*)\s*\n\s*try/gm, (match, key, value) => {
    return `{\n      ${key}: ${value.trim()}\n    });\n\n    try`;
  });
