...
async function testV4Flow() {
	...
	try {
		...
		console.log(
			`✅ Navigation buttons found: Previous=${prevButton.length > 0}, Next=${nextButton.length > 0}, Reset=${resetButton.length > 0}`
		);
		...
		if (nextButton.length > 0) {
			...
			const generateButton = await page.$x(
				'//button[contains(text(), "Generate New PKCE Parameters")]'
			);
