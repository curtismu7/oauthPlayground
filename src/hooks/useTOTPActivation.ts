...
export interface TOTPActivationState {
  ...
  validateTOTPCode: (secret: string, code: string) => Promise<{ valid: boolean; timeWindow?: number; error?: string }>;
...
export const useTOTPActivation = (
  options: UseTOTPActivationOptions = {}
): TOTPActivationState => {
  const {
    deviceId,
    userId,
  ...
  const [backupCodeStats, setBackupCodeStats] = useState<TOTPActivationState['backupCodeStats']>(null);
  ...
  const activateDevice = useCallback(async (
    ...
    try {
      ...
      const result = await TOTPActivationService.activateTOTPDevice(
        config,
        code,
        {
  ...
  // Validate TOTP code
  const validateTOTPCode = useCallback(async (
    secret: string,
    code: string
  ...
  const validateBackupCode = useCallback((code: string): BackupCodeValidationResult | null => {
    if (!deviceId || !userId) {
      return {
        valid: false,
  ...
  const regenerateBackupCodes = useCallback(async (): Promise<BackupCode[] | null> => {
    if (!deviceId || !userId) {
      logger.warn('useTOTPActivation', 'Cannot regenerate backup codes: missing deviceId or userId');
      return null;
    }
  ...
  // Export backup codes
  const exportBackupCodes = useCallback((deviceNickname?: string): string | null => {
    if (!deviceId || !userId) {
      return null;
    }
