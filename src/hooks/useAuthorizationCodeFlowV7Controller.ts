...
const loadInitialCredentials = (variant: FlowVariant): StepCredentials => {
	...
	const mergedScopes = urlScope || (variant === 'oidc' ? 'openid profile email' : 'openid');
...
export const useAuthorizationCodeFlowController = (
	...
	const [pkceCodes, setPkceCodes] = useState<PKCECodes>(() => {
		...
		logger.info(
			'🔄 [useAuthorizationCodeFlowController] No existing PKCE codes in sessionStorage',
			'Logger info'
		);
		return {
			codeVerifier: '',
	...
	useAuthorizationFlowScroll('Authorization Code Flow V7');
	...
	useEffect(() => {
		logger.info(
			'🚀 [useAuthorizationCodeFlowV7Controller] Flow loaded, clearing PKCE codes for fresh start...'
		);
		...
		const loadData = async () => {
			try {
				...
				if (loadedCreds && (loadedCreds.environmentId || loadedCreds.clientId)) {
					logger.info('✅ [useAuthorizationCodeFlowController] Found saved credentials', {
						flowKey: persistKey,
					...
					const correctedCreds = {
						...loadedCreds,
						responseType: loadedCreds.responseType?.startsWith('code')
					...
					logger.info(
						'🔧 [useAuthorizationCodeFlowController] Corrected responseType and redirectUri:',
						{
				...
				logger.error(
					'useAuthorizationCodeFlowV7Controller',
	...
	useEffect(() => {
		...
		const handlePopupCallback = (event: CustomEvent) => {
			...
			if (code) {
				...
				logger.warn('useAuthorizationCodeFlowV7Controller', 'No code in event detail!');
			}
		};
	...
	useEffect(() => {
			...
			if (callbackProcessed && callbackProcessed !== lastProcessedTimestamp) {
				...
				if (callbackFlowType === flowKey) {
					...
					if (code && code !== authCode) {
						...
						logger.info(
							'✅ [useAuthorizationCodeFlowController] Cleared callback flags',
							'Logger info'
						);
					}
				}
		...
		return () => {
			if (pollInterval) {
				logger.info(
					'🔌 [useAuthorizationCodeFlowController] Stopping callback polling',
					'Logger info'
				);
	...
	const generatePkceCodes = useCallback(async () => {
		try {
			...
			if (pkceCodes.codeVerifier && pkceCodes.codeChallenge) {
				logger.info('🔐 [PKCE DEBUG] PKCE codes already exist, skipping regeneration');
				return;
			}
			...
			logger.info('🔐 [PKCE DEBUG] Storage key:', `${persistKey}-pkce-codes`);
	...
	const generateAuthorizationUrl = useCallback(async () => {
		...
		logger.info('🔍 [REDIRECT URI DEBUG] Detailed Analysis:', {
		...
		if (redirectUriMismatch) {
			logger.warn('useAuthorizationCodeFlowV7Controller', 'Potential mismatch detected', {
		...
		logger.info(
			'🔧 [useAuthorizationCodeFlowController] ===== AUTHORIZATION URL GENERATION =====',
			'Logger info'
		);
		logger.info(
		...
		logger.info('🔧 [useAuthorizationCodeFlowController] Full credentials:', {
		...
		logger.info('🔍 [REDIRECT URI AUDIT] Authorization Request:', {
			configuredRedirectUri: credentials.redirectUri,
		...
		try {
			...
			for (const configKey of possibleKeys) {
				...
				if (config) {
					...
					logger.info('🔧 [useAuthorizationCodeFlowController] Loaded PingOne config:', {
						key: configKey,
			...
			logger.warn('useAuthorizationCodeFlowV7Controller', 'Failed to load PingOne config', {
				detail: String(error),
			});
		}
		...
		if (pingOneConfig?.requirePushedAuthorizationRequest) {
			...
			logger.info('🔍 [REDIRECT URI AUDIT] URL Params:', {
		...
		logger.info('🔧 [useAuthorizationCodeFlowController] Stored state for validation:', state);

		// Store active flow for callback page to know where to return
		sessionStorage.setItem('active_oauth_flow', flowKey);
		logger.info('🔧 [useAuthorizationCodeFlowController] Stored active flow:', flowKey);
	...
	const handleRedirectAuthorization = useCallback(() => {
		...
		logger.info(
			'🔧 [useAuthorizationCodeFlowController] Stored flow context for V7 callback:',
			flowContext
		);
		logger.info('🔧 [useAuthorizationCodeFlowController] About to redirect to:', authUrl);
	...
	}, [authUrl, saveStepResult, credentials.redirectUri, flowKey, credentials.responseType]);
	...
	const exchangeTokens = useCallback(async () => {
		...
		try {
			logger.info('🔍 [useAuthorizationCodeFlowController] Token exchange debug:', {
			...
			if (!codeVerifier) {
				logger.info('⚠️ [PKCE DEBUG] No code_verifier in state, checking sessionStorage...');
				...
				logger.info('🔍 [PKCE DEBUG] SessionStorage check:', {
					key: pkceStorageKey,
				...
				if (storedPkceCodes?.codeVerifier) {
					...
					logger.info('⚠️ [PKCE DEBUG] No PKCE in primary storage, trying legacy keys...');
			...
			logger.info('🔍 [REDIRECT URI AUDIT] Token Exchange:', {
			...
			if (authMethod === 'client_secret_jwt' || authMethod === 'private_key_jwt') {
				logger.info(
					`🔐 [useAuthorizationCodeFlowController] Using ${authMethod} authentication`,
					'Logger info'
				);
				...
				try {
					...
					logger.info('✅ [useAuthorizationCodeFlowController] JWT assertion generated:', {
			...
			logger.info('🔍 [useAuthorizationCodeFlowController] Token exchange request:', {
			...
			if (!response.ok) {
				...
				try {
					...
					if (errorJson.error === 'invalid_grant') {
						...
						logger.error('useAuthorizationCodeFlowV7Controller', 'Detailed debugging', {
							error: errorJson,
							configuredRedirectUri: credentials.redirectUri,
			...
			storeOAuthTokens(tokenData, 'authorization_code', 'Authorization Code Flow V7');
	...
	const fetchUserInfo = async () => {
		logger.info('🔍 [fetchUserInfo] Starting user info fetch:', {
		...
		if (!tokens?.access_token) {
			logger.error('useAuthorizationCodeFlowV7Controller', 'No access token available');
		...
		if (!userInfoEndpoint) {
			logger.error(
				'useAuthorizationCodeFlowV7Controller',
				'No userinfo endpoint configured and no environment ID'
			);
		...
		logger.info('✅ [fetchUserInfo] Fetching from:', userInfoEndpoint);
	...
	const refreshTokens = useCallback(async () => {
		...
		try {
			...
			if (authMethod === 'client_secret_jwt' || authMethod === 'private_key_jwt') {
				logger.info(
					`🔐 [useAuthorizationCodeFlowController] Using ${authMethod} for refresh`,
					'Logger info'
				);
			...
			logger.info('🔍 [useAuthorizationCodeFlowController] Refresh token request:', {
				url: `${backendUrl}/api/token-exchange`,
				grant_type: 'refresh_token',
	...
	const saveCredentials = useCallback(async () => {
		...
		try {
			...
			logger.info(
				'📢 [useAuthorizationCodeFlowController] Configuration change events dispatched',
				'Logger info'
			);
			...
			logger.info(
				'✅ [useAuthorizationCodeFlowController] Credentials saved successfully',
				'Logger info'
			);
	...
	const clearPKCE = useCallback(() => {
		logger.info('🧹 [useAuthorizationCodeFlowV7Controller] Clearing PKCE codes...', 'Logger info');
