...
import { authorizationUrlValidationService, type UrlValidationResult, type UrlValidationConfig } from '../services/authorizationUrlValidationService';
...
export const useAuthorizationUrlValidation = (
  ...
  const [lastValidationResult, setLastValidationResult] = useState<UrlValidationResult | null>(null);
  ...
  const validateUrl = useCallback((url: string): UrlValidationResult => {
    const result = authorizationUrlValidationService.validateAuthorizationUrl(url, validationConfig);
  ...
  const validateAndShowModal = useCallback((
    ...
    if (shouldShowModal) {
      authorizationUrlValidationModalService.showValidationModal(
        result,
        url,
        onProceed,
        onFix
 ...
 */
export const usePreSendUrlValidation = (
  options: UseAuthorizationUrlValidationOptions
) => {
 ...
 */
export const useRealtimeUrlValidation = (
  options: UseAuthorizationUrlValidationOptions
) => {
   ...
   */
  const validateRealtime = useCallback((url: string) => {
