...
export const useOAuth2CompliantAuthorizationCodeFlow = (): [OAuth2FlowState, OAuth2FlowActions] => {
  ...
  }, [state.isConfigValid, state.pkceCodes, state.credentials, clearMessages, addError, addWarning]);
  ...
  const handleAuthorizationCallback = useCallback(async (code: string, receivedState: string) => {
    try {
      ...
      addError(oauth2ComplianceService.createErrorResponse('access_denied', errorMessage, receivedState));
      v4ToastManager.showError(errorMessage);
    }
  }, [clearMessages, addError]);
  ...
  useEffect(() => {
    ...
    if (error) {
      addError(oauth2ComplianceService.createErrorResponse(
        error as any,
