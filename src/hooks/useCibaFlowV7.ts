...
export const useCibaFlowV7 = (options: CibaFlowV7Options) => {
	...
	const timeRemaining = authRequest
		? Math.max(0, Math.floor((authRequest.expiresAt - Date.now()) / 1000))
		: 0;
	const progressPercentage = authRequest
		? Math.max(
				0,
				Math.min(
					100,
					(authRequest.expiresIn * 1000 - (authRequest.expiresAt - Date.now())) /
						(authRequest.expiresIn * 10)
				)
			)
		: 0;
