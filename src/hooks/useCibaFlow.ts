...
export const useCibaFlow = (): UseCibaFlowReturn => {
	...
	const completeWithTokens = useCallback(
		(issuedTokens: CibaTokens) => {
			...
			modernMessaging.showFooterMessage({
				type: 'status',
				message: 'CIBA request approved. Tokens issued successfully.',
				duration: 4000,
			});
			clearPollingTimer();
		},
	...
	const initiateAuthentication = useCallback(async () => {
		...
		modernMessaging.showFooterMessage({
			type: 'status',
			message: 'Mock CIBA request sent. Awaiting simulated approval.',
			duration: 4000,
		});
	...
	const cancelPolling = useCallback(() => {
		...
		modernMessaging.showFooterMessage({
			type: 'status',
			message: 'Mock polling cancelled. You can restart the flow when ready.',
			duration: 4000,
		});
