...
export const useOIDCCompliantAuthorizationCodeFlow = (): [OIDCFlowState, OIDCFlowActions] => {
	...
	const setCredentials = useCallback((credentials: OIDCCredentials) => {
		setState((prev) => ({
			...
			credentials: {
				...
				issuer: credentials.issuer || `https://auth.pingone.com/${credentials.environmentId}`,
			},
			isConfigValid: false,
			configErrors: [],
		}));
	...
	const generateAuthorizationUrl = useCallback(async () => {
		try {
			...
			if (!validation.valid) {
				throw new Error(
					`OIDC authorization request validation failed: ${validation.errors.join(', ')}`
				);
			}
	...
	const fetchUserInfo = useCallback(async () => {
		try {
			...
			if (!response.ok) {
				const errorData = await response.json().catch(() => ({}));
				throw new Error(
					errorData.error_description || `UserInfo request failed: ${response.status}`
				);
			}
