 ...
 */
async function attemptSilentTokenRetrieval(): Promise<boolean> {
	try {
		...
		if (!credentials) {
			...
			// Don't show modal or error - truly silent when silentApiRetrieval is enabled
			// User can configure credentials via the "Get Worker Token" button
			return false;
		}
...
export async function handleShowWorkerTokenModal(
	...
	if (silentApiRetrieval) {
		...
		// Silent retrieval failed - but since silentApiRetrieval is ON and not forced, don't show modal
