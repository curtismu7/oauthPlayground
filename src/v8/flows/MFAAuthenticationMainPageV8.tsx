...
export const MFAAuthenticationMainPageV8: React.FC = () => {
	...
	useEffect(() => {
		const handleTokenUpdate = async () => {
			try {
				...
				logger.error('[MFA-AUTHN-MAIN-V8] Failed to check token status in event handler:', error);
			}
		};
	...
	const handleAuthorizationApi = useCallback(async () => {
		...
		try {
			...
			logger.error(`${MODULE_TAG} Authorization API call failed:`, error);
			const errorMessage =
	...
	const handleStartMFA = useCallback(async () => {
		return startMFAAction.executeAction(async () => {
			...
			logger.info(`${MODULE_TAG} 🚀 Starting MFA Authentication`, {
			...
			if (!tokenStatus.isValid) {
				logger.warn(`${MODULE_TAG} ❌ Worker token invalid - cannot start authentication`, "Logger warning");
				modernMessaging.showBanner({
					type: 'error',
					title: 'Error',
					message: 'Please configure worker token first',
			...
			if (!credentials.environmentId) {
				logger.warn(`${MODULE_TAG} ❌ Environment ID missing - cannot start authentication`, "Logger warning");
				modernMessaging.showBanner({
					type: 'error',
					title: 'Error',
					message: 'Please configure environment ID first',
			...
			if (!credentials.deviceAuthenticationPolicyId) {
				logger.warn(`${MODULE_TAG} ❌ Policy ID missing - cannot start authentication`, "Logger warning");
				modernMessaging.showBanner({
					type: 'error',
					title: 'Error',
					message: 'Please select an MFA Policy first',
			...
			if (!usernameInput.trim()) {
				logger.info(`${MODULE_TAG} 🔄 No username provided, showing decision modal`);
				setShowUsernameDecisionModal(true);
				return;
			}
			...
			try {
				...
				if (needsDeviceSelection) {
					...
					logger.info(`${MODULE_TAG} 🔄 Device selection required, showing device list`);
					modernMessaging.showFooterMessage({
						type: 'info',
					...
					logger.info(`${MODULE_TAG} 🔢 OTP required, showing OTP modal`);
					setShowOTPModal(true);
				} else if (needsPush) {
					logger.info(`${MODULE_TAG} 📱 Push confirmation required, showing push modal`);
					...
					logger.info(`${MODULE_TAG} 🔑 FIDO2 assertion required, showing FIDO2 modal`);
					setShowFIDO2Modal(true);
				} else if (status === 'COMPLETED') {
					logger.info(`${MODULE_TAG} ✅ Authentication already completed, navigating to success`);
					modernMessaging.showFooterMessage({
						type: 'info',
					...
					logger.info(`${MODULE_TAG} 🤔 Unknown authentication status, using fallback`, {
						status,
						nextStep,
					...
					setTimeout(() => {
						if (authDevices.length > 0) {
							logger.info(`${MODULE_TAG} 🔄 Fallback: Showing device selection`, "Logger info");
							setAuthState((prev) => ({ ...prev, showDeviceSelection: true }));
						} else {
							logger.info(`${MODULE_TAG} 🔄 Fallback: No devices, showing success`);
							window.location.href = '/v8/mfa/authentication/success';
						}
					}, 1000);
				}
	...
	return (
		<>
			...
			>
					...
					sections={[
						{
							...
							content: (
								<div>
									<p>
										<strong>MFA Authentication Flow</strong> validates user identity using
										registered devices:
									</p>
									<ul>
										<li>
											<strong>Device Selection</strong> - Choose from user's registered devices
										</li>
										<li>
											<strong>Challenge Handling</strong> - OTP codes, push notifications, or FIDO2
											challenges
										</li>
										<li>
											<strong>Policy Enforcement</strong> - Respects MFA policies and device
											restrictions
										</li>
										<li>
											<strong>Username-less FIDO2</strong> - Supports passwordless authentication
										</li>
										<li>
											<strong>Real-time Status</strong> - Live updates on authentication progress
										</li>
									</ul>
									<p>
										This page provides a complete authentication experience for all MFA device
										types.
									</p>
								</div>
							),
						},
						{
							...
							content: (
								<div>
									...
									<ul>
										<li>
											<strong>Step 1: User Selection</strong> - Search and select user to
											authenticate
										</li>
										<li>
											<strong>Step 2: Device Selection</strong> - View and choose from registered
											devices
										</li>
										<li>
											<strong>Step 3: Challenge</strong> - Complete device-specific challenge (OTP,
											Push, FIDO2)
										</li>
										<li>
											<strong>Step 4: Verification</strong> - System validates response
										</li>
										<li>
											<strong>Step 5: Success</strong> - Authentication complete, access granted
										</li>
									</ul>
									<p>
										Each device type has optimized challenge flows for best user experience and
										security.
									</p>
								</div>
							),
															...
															>
																<div style={{ flex: 1 }}>
																	...
																	>
																		{selectedPolicy.skipUserLockVerification ? (
																			<>
																				<strong style={{ color: '#92400e' }}>Action:</strong> We
																				will <strong>skip</strong> checking if the user account is
																				locked before authentication. Locked accounts may be able to
																				authenticate.
																			</>
																		) : (
																			<>
																				<strong style={{ color: '#166534' }}>Action:</strong> We
																				will <strong>check</strong> the user's lock status before
																				authentication. If the account is locked, authentication
																				will be blocked.
																			</>
																		)}
																	</div>
				...
				{(() => {
					logger.info(`${MODULE_TAG} Device Selection UI Check:`, {
														...
														onSelectDevice={async (deviceId) => {
															...
															try {
																...
																let challengeId: string | null =
																	(data.challengeId as string) || null;
																...
																const links =
																	(data._links as Record<string, { href: string }>) || {};
				...
				<MFAOTPInputModal
					...
					onResendCode={async () => {
						...
						try {
							logger.info(`${MODULE_TAG} 🔄 Resending OTP using proper API approach`, {
								authenticationId: authState.authenticationId,
								deviceId: authState.selectedDeviceId,
								username: usernameInput.trim(),
							});
							...
							if (links?.cancel) {
								logger.info(
									`${MODULE_TAG} 🔄 Using cancel + re-initialize approach for OTP resend`
								, "Logger info");
								try {
							...
							if (!success) {
								logger.info(`${MODULE_TAG} 🔄 Using fallback device re-selection for OTP resend`, "Logger info");
								const reselectResult =
									await MfaAuthenticationServiceV8.selectDeviceForAuthentication(
										{
											environmentId: credentials.environmentId,
								...
								success = true;
								logger.info(`${MODULE_TAG} ✅ OTP resent successfully via direct re-selection`, "Logger info");
							}
				...
				{showPolicyInfoModal && (
					<MFAPolicyInfoModal
						...
						policy={deviceAuthPolicies.find(
							(p) => p.id === credentials.deviceAuthenticationPolicyId
						)}
					/>
				)}
