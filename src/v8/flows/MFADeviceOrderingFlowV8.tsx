...
export const MFADeviceOrderingFlowV8: React.FC = () => {
	...
	const handleManageWorkerToken = async () => {
		if (tokenStatus.isValid) {
			...
			const confirmed = await uiNotificationServiceV8.confirm({
				...
				message:
					'Worker token is currently stored.\n\nDo you want to remove it?',
				confirmText: 'Remove',
				cancelText: 'Cancel',
				severity: 'warning',
			});
	...
	const handleLoadDevices = async () => {
		...
		try {
			...
			setLoadError(
				error instanceof Error ? error.message : 'Failed to load devices from PingOne'
			);
			toastV8.error(
	...
	const handleSetAsDefault = async (deviceId: string) => {
		...
		try {
			...
			toastV8.error(
				error instanceof Error ? error.message : 'Failed to set default device'
			);
			// Revert on error
			setDevices(devices);
		} finally {
	...
	const handleRemoveOrder = async () => {
		...
		try {
			...
			await MFAServiceV8.removeUserMfaDeviceOrder(
				credentials.environmentId.trim(),
				user.id
			);
		...
		<>
			<div
				...
				style={{
					...
					minHeight: '100vh',
					overflowY: 'auto',
					paddingBottom: isApiDisplayVisible ? '450px' : '40px',
