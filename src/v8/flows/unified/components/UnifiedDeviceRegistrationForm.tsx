						...
						if (!hasReturnTarget) {
							return (
								<Button
									...
									onClick={() => {
										logger.info(
											'[UNIFIED-FLOW] Manual fallback: User clicked continue button',
											'Logger info'
										);
				...
				>
					<StandardizedCredentialExportImport
						...
						onImport={(imported) => {
							...
							modernMessaging.showFooterMessage({
								type: 'info',
								message: 'MFA credentials imported successfully',
								duration: 3000,
							});
						...
						onError={(error) => {
							...
							modernMessaging.showBanner({
								type: 'error',
								title: 'Error',
								message: `Failed to export/import credentials: ${error.message}`,
								dismissible: true,
							});
						}}
					/>
