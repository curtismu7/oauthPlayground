...
const TOTPFlowV8WithDeviceSelection: React.FC = () => {
	...
	const renderStep1WithSelection = (props: MFAFlowBaseRenderProps) => {
		const { credentials, setCredentials, mfaState, setMfaState, nav, setIsLoading, tokenStatus } =
			props;
	...
	const renderStep3QrCode = useCallback(
		(props: MFAFlowBaseRenderProps) => {
			...
			const handleContinue = () => {
				setShowQrModal(false);
				nav.markStepComplete();
				// For ACTIVE devices, show success page directly (don't navigate to Step 4)
				// The success page will be shown in renderStep3QrCode when deviceStatus is ACTIVE
			};
			...
			// If device is ACTIVE and modal is closed, show success page (same pattern as SMS/Email/WhatsApp)
			if (
										...
										<details>
											...
											<div style={{ marginTop: '12px' }}>
												...
												<p style={{ marginTop: '12px', fontSize: '12px', color: '#6b7280' }}>
													Enter this secret in your authenticator app manually. Choose "Time-based"
													and 6 digits.
												</p>
											</div>
										</details>
								...
								<div style={{ display: 'flex', gap: '12px' }}>
									{isActivationRequired ? (
										<>
											...
											<button
												type="button"
												disabled={isLoading || activationOtp.length !== otpLength}
												onClick={handleActivateDevice}
												style={{
													flex: 2,
													padding: '12px 20px',
													background:
														activationOtp.length === otpLength && !isLoading
															? '#10b981'
															: '#d1d5db',
													color: 'white',
													border: 'none',
