	...
	const handleSendOTP = async () => {
		if (!deviceState.deviceType || !deviceState.contactValue) {
			modernMessaging.showBanner({
				type: 'error',
				title: 'Error',
				message: 'Please select device type and enter contact information',
				dismissible: true,
			});
			return;
		}
		...
		try {
			...
			const response = await MFAServiceV8.initializeOneTimeDeviceAuthentication({
				...
				...(deviceState.deviceType === 'EMAIL'
					? { email: deviceState.contactValue }
					: { phone: deviceState.contactValue }),
				workerToken: token,
			});
			...
			setDeviceState((prev) => ({
				...
				...(response._links?.['otp.check']?.href && {
			...
			modernMessaging.showFooterMessage({
				type: 'info',
				message: `OTP sent to ${maskContact(deviceState.deviceType, deviceState.contactValue)}`,
				duration: 3000,
			});
		} catch (error) {
	...
	const handleValidateOTP = async () => {
		...
		try {
			...
			if (result.valid) {
				modernMessaging.showFooterMessage({
					type: 'info',
					message: 'OTP validated successfully! One-time MFA completed.',
					duration: 3000,
				});
			...
			<Card>
				<CardHeader>
					...
					<CardDescription className="text-base">
						Test one-time MFA where your application controls device information instead of storing
						it in PingOne.
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					<Alert>
						<AlertDescription>
							<strong>📚 Important:</strong> This is <strong>Phase 2</strong> - One-time Device
							Authentication. Use this when your organization maintains user contact details in your
							own database rather than storing MFA devices in PingOne.
						</AlertDescription>
					</Alert>
						...
						<TabsContent value="comparison" className="space-y-4">
							<Card>
								...
								<CardContent>
										...
										<div className="space-y-4">
											<div className="p-4 border rounded-lg">
												...
												<ul className="text-sm mt-2 space-y-1">
													...
													<li>
														• Uses <code>selectedDevice.id</code>
													</li>
									...
									<Alert>
										<AlertDescription>
											<strong>Recommendation:</strong> Start with Phase 1 (Standard Devices) for
											most implementations. Use Phase 2 (One-Time Devices) only if you have specific
											requirements to avoid storing contact information in PingOne.
										</AlertDescription>
									</Alert>
								</CardContent>
							</Card>
						...
						<TabsContent value="test" className="space-y-4">
							...
							<Card>
								<CardHeader>
									...
									<CardDescription>Choose how to send the one-time OTP</CardDescription>
								</CardHeader>
								<CardContent className="space-y-4">
									<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
										<Button
									...
									{deviceState.deviceType && (
										<div>
											...
											<Input
												...
												onChange={(e) =>
													setDeviceState((prev) => ({ ...prev, contactValue: e.target.value }))
												}
												placeholder={
													deviceState.deviceType === 'EMAIL' ? 'user@example.com' : '+15551234567'
												}
											/>
										</div>
									)}
									...
									{deviceState.otpSent && (
										<div className="space-y-4 p-4 bg-green-50 border rounded-lg">
											<Alert>
												<AlertDescription>
													OTP sent to{' '}
													{maskContact(deviceState.deviceType!, deviceState.contactValue)}
											...
											<div>
												...
												<Input
													...
													onChange={(e) =>
														setDeviceState((prev) => ({ ...prev, otpCode: e.target.value }))
													}
									...
									{!deviceState.otpSent && deviceState.deviceType && deviceState.contactValue && (
										<Button
											...
											disabled={
												deviceState.loading || !credentials.environmentId || !credentials.username
											}
											className="w-full"
											size="lg"
										>
											{deviceState.loading
												? 'Sending OTP...'
												: `Send OTP to ${maskContact(deviceState.deviceType, deviceState.contactValue)}`}
										</Button>
									)}
								</CardContent>
							</Card>
								...
								{showAdvanced && (
									<CardContent className="space-y-4">
										<div className="text-sm space-y-2">
											<p>
												<strong>API Endpoint:</strong> POST /{credentials.environmentId}
    ...
    "oneTime": {
      ...
      "${deviceState.deviceType === 'EMAIL' ? 'email' : 'phone'}": "${deviceState.contactValue || 'user@example.com'}"
											...
											{deviceState.deviceAuthId && (
												<>
													<p>
														<strong>Device Authentication ID:</strong> {deviceState.deviceAuthId}
													</p>
													{deviceState.otpCheckUrl && (
														<p>
															<strong>OTP Check URL:</strong> {deviceState.otpCheckUrl}
														</p>
													)}
												</>
											)}
