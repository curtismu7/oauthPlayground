...
import type { DeviceAuthenticationPolicy, DeviceType, MFACredentials, MFAState } from '@/v8/flows/shared/MFATypes';
...
export const AuthenticationFlowStepperV8: React.FC<AuthenticationFlowStepperV8Props> = ({
	...
	renderStep6,
	validateStep0,
	stepLabels = ['Configure', 'User Login', 'Device Selection', 'QR Code', 'Device Actions', 'API Documentation', 'Success'],
	...
	useEffect(() => {
		const checkWorkerToken = async () => {
			try {
				...
				if (!tokenStatus.hasToken && !tokenStatus.isLoading) {
					setShowWorkerTokenModal(true);
				}
			} catch (error) {
				console.error(`${MODULE_TAG} Error checking worker token:`, error);
			}
		};
	...
	const handleNext = useCallback(() => {
		if (nav.currentStep === 0) {
			if (validateStep0(credentials, WorkerTokenStatusServiceV8.checkWorkerTokenStatusSync(), nav)) {
			...
			<StepValidationFeedbackV8
				...
				onValidationRecheck={() => {
					...
					if (nav.currentStep === 0) {
						...
						if (validateStep0(credsToValidate, WorkerTokenStatusServiceV8.checkWorkerTokenStatusSync(), nav)) {
