...
import { AdminTokenExchangeConfig, TokenExchangeError, TokenExchangeErrorType } from '../types/tokenExchangeTypesV8';
...
export class TokenExchangeConfigServiceV8 {
  ...
  static async isEnabled(environmentId: string): Promise<boolean> {
    try {
      console.log(`${this.MODULE_TAG} Checking Token Exchange enablement for environment: ${environmentId}`);
      
      const config = await this.getAdminConfig(environmentId);
      
      if (!config.enabled) {
        console.warn(`${this.MODULE_TAG} Token Exchange is disabled for environment: ${environmentId}`);
        return false;
      }

      console.log(`${this.MODULE_TAG} Token Exchange is enabled for environment: ${environmentId}`);
      return true;
    } catch (error) {
      console.error(`${this.MODULE_TAG} Error checking enablement:`, error);
  ...
  static async getAdminConfig(environmentId: string): Promise<AdminTokenExchangeConfig> {
    try {
      ...
      console.log(`${this.MODULE_TAG} Retrieved admin config for environment: ${environmentId}`);
      return defaultConfig;
    } catch (error) {
      console.error(`${this.MODULE_TAG} Error getting admin config:`, error);
  ...
  static async enableTokenExchange(
    ...
    try {
      console.log(`${this.MODULE_TAG} Enabling Token Exchange for environment: ${environmentId} by admin: ${adminUserId}`);
      ...
      console.log(`${this.MODULE_TAG} Token Exchange enabled successfully:`, updatedConfig);
    } catch (error) {
      console.error(`${this.MODULE_TAG} Error enabling Token Exchange:`, error);
      throw new TokenExchangeError(
        TokenExchangeErrorType.SERVER_ERROR,
   ...
   */
  static async disableTokenExchange(
    environmentId: string,
    adminUserId: string
  ): Promise<void> {
    try {
      console.log(`${this.MODULE_TAG} Disabling Token Exchange for environment: ${environmentId} by admin: ${adminUserId}`);

      // TODO: Implement actual admin API call to PingOne
      // For now, just log the action
      console.log(`${this.MODULE_TAG} Token Exchange disabled successfully`);
    } catch (error) {
      console.error(`${this.MODULE_TAG} Error disabling Token Exchange:`, error);
      throw new TokenExchangeError(
        TokenExchangeErrorType.SERVER_ERROR,
  ...
  static async validateScopes(
    ...
    try {
      ...
      const invalidScopes = requestedScopes.filter(scope => !config.allowedScopes.includes(scope));
      
      if (invalidScopes.length > 0) {
        console.warn(`${this.MODULE_TAG} Invalid scopes requested:`, invalidScopes);
        return false;
      }

      console.log(`${this.MODULE_TAG} All scopes validated successfully:`, requestedScopes);
      return true;
    } catch (error) {
      console.error(`${this.MODULE_TAG} Error validating scopes:`, error);
      return false;
    }
  }
}
