...
export class TokenExchangeServiceV8 {
  ...
  static async exchangeToken(
    ...
    try {
      console.log(`${this.MODULE_TAG} Starting token exchange for environment: ${environmentId}`);
      ...
      const subjectTokenValidation = await this.validateToken(params.subject_token, environmentId);
      ...
      if (params.scope) {
        ...
        const scopesValid = await TokenExchangeConfigServiceV8.validateScopes(environmentId, requestedScopes);
        if (!scopesValid) {
          throw new TokenExchangeError(
      ...
      if (params.actor_token) {
        const actorTokenValidation = await this.validateToken(params.actor_token, environmentId);
      ...
      const response = await this.executeTokenExchangeWithPingOne(params, environmentId);

      console.log(`${this.MODULE_TAG} Token exchange completed successfully`);
      return response;

    } catch (error) {
      console.error(`${this.MODULE_TAG} Token exchange failed:`, error);
   ...
   */
  static async validateToken(token: string, expectedEnvironmentId: string): Promise<TokenValidationResult> {
    try {
      console.log(`${this.MODULE_TAG} Validating token for environment: ${expectedEnvironmentId}`);
      ...
      console.log(`${this.MODULE_TAG} Token validation successful:`, result);
      return result;

    } catch (error) {
      console.error(`${this.MODULE_TAG} Token validation error:`, error);
      return {
        isValid: false,
        environmentId: '',
        tokenType: 'access_token',
  ...
  private static async executeTokenExchangeWithPingOne(
    ...
    try {
      console.log(`${this.MODULE_TAG} Executing token exchange with PingOne`);
      ...
      console.log(`${this.MODULE_TAG} Token exchange request:`, {
        ...requestBody,
      ...
      const response: TokenExchangeResponse = {
        access_token: this.generateMockAccessToken(environmentId),
        token_type: 'Bearer',
        expires_in: 3600,
      ...
      console.log(`${this.MODULE_TAG} Token exchange response received:`, {
        ...response,
      ...
      console.error(`${this.MODULE_TAG} PingOne API error:`, error);
      throw new TokenExchangeError(
        TokenExchangeErrorType.SERVER_ERROR,
  ...
  static buildTokenExchangeContext(
    ...
    try {
      ...
      console.warn(`${this.MODULE_TAG} Failed to parse subject token claims:`, error);
    }
    ...
    if (params.actor_token) {
      try {
        ...
        console.warn(`${this.MODULE_TAG} Failed to parse actor token claims:`, error);
      }
    }
    ...
    return {
      requestData: {
        ...
        ...(params.resource && { resource: params.resource }),
        ...(params.audience && { audience: Array.isArray(params.audience) ? params.audience : [params.audience] })
      },
      appConfig: {
