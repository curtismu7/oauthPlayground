...
export async function validateIdToken(
	...
	try {
		...
		logger.info(' [ImplicitFlowSecurity] OpenID configuration discovered:', {
		...
		logger.info(' [ImplicitFlowSecurity] Issuer validation options:', {
		...
		const isValidIssuer =
			receivedIssuer === expectedIssuerBase ||
			receivedIssuer === expectedIssuerWithAs ||
			receivedIssuer === config.issuer;

		if (!isValidIssuer) {
			validationErrors.push(
				`Invalid issuer: expected one of [${expectedIssuerBase}, ${expectedIssuerWithAs}, ${config.issuer}], got ${receivedIssuer}`
			);
		}
		...
		if (
			...
			(!Array.isArray(aud) && aud !== options.clientId)
		) {
			validationErrors.push(
				`Invalid audience: expected ${options.clientId}, got ${JSON.stringify(aud)}`
			);
		}
		...
		if (!payload.exp || payload.exp <= Math.floor(Date.now() / 1000)) {
			validationErrors.push(
				`Token expired: exp=${payload.exp}, now=${Math.floor(Date.now() / 1000)}`
			);
		}
		...
		if (!payload.iat || payload.iat > Math.floor(Date.now() / 1000)) {
			validationErrors.push(
				`Invalid issued at: iat=${payload.iat}, now=${Math.floor(Date.now() / 1000)}`
			);
		}
		...
		if (options.expectedNonce && payload.nonce !== options.expectedNonce) {
			validationErrors.push(
				`Invalid nonce: expected ${options.expectedNonce}, got ${payload.nonce}`
			);
		}
		...
		if (payload.azp && payload.azp !== options.clientId) {
			validationErrors.push(
				`Invalid authorized party: expected ${options.clientId}, got ${payload.azp}`
			);
		}
...
export function clearSecurityData(flowType: 'oauth2' | 'oidc'): void {
	...
	const flowContextKey =
		flowType === 'oidc' ? 'oidc_implicit_v3_flow_context' : 'oauth2_implicit_v3_flow_context';
...
export function generateSecurityParameters(length: number = 32): {
	...
	logger.info(' [ImplicitFlowSecurity] Generated security parameters:', {
...
export function storeSecurityParameters(
	...
	logger.info(' [ImplicitFlowSecurity] Security parameters stored:', {
		flowType,
		stateLength: state.length,
