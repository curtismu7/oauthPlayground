...
export class JWKSCache {
  ...
  async setJWKS(key: string, jwks: JWKS, jwksUri: string, ttl?: number): Promise<void> {
    ...
    const entry: JWKSCacheEntry = {
      ...
      keyStats: {
        ...
        keyTypes: jwks.keys.reduce((acc, key) => {
          acc[key.kty] = (acc[key.kty] || 0) + 1;
          return acc;
        }, {} as Record<string, number>),
        algorithms: jwks.keys.reduce((acc, key) => {
  ...
  private evictOldest(): void {
    ...
    if (oldestKey) {
      ...
      this.stats.evictions++;
      this.stats.size = this.cache.size;
      logger.debug('JWKSCache', 'Evicted oldest entry', { key: oldestKey, timestamp: oldestTimestamp });
    }
  }
  ...
  async get(key: string): Promise<JWKSCacheEntry | null> {
    try {
      ...
      if (now > entry.expiresAt) {
        ...
        logger.debug('LocalStorageJWKSCache', 'Cache entry expired', { key, expiresAt: entry.expiresAt });
        return null;
      }
