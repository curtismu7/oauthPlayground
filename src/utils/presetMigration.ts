  ...
  try {
    ...
    for (let i = 0; i < presets.length; i++) {
      ...
      try {
        ...
        result.warnings.push(`Failed to migrate preset at index ${i}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
...
export function needsMigration(): boolean {
  try {
    ...
    return presets.some(preset => 
      !preset.metadata?.schemaVersion || 
      preset.metadata.schemaVersion !== CURRENT_SCHEMA_VERSION
    );
  } catch {
...
export function performAutoMigration(): MigrationResult {
  ...
  try {
    ...
    result.success = false;
    result.errors.push(`Auto-migration failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
