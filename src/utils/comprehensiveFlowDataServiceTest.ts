...
export class ComprehensiveFlowDataServiceTest {
	...
	private async testFlowSpecificDataManagement(): Promise<void> {
		...
		await this.runTest('Save Flow-Specific Credentials', async () => {
			...
			const success = comprehensiveFlowDataService.saveFlowCredentialsIsolated('test-flow-1', credentials);
			...
			if (loaded.logoutUrl !== 'https://test-app.com/logout') {
				throw new Error(`Expected logoutUrl 'https://test-app.com/logout', got '${loaded.logoutUrl}'`);
			}
		...
		await this.runTest('Save Comprehensive Flow Data', async () => {
			...
			const loaded = comprehensiveFlowDataService.loadFlowDataComprehensive({ flowKey: 'test-flow-2' });
	...
	private async testCredentialIsolation(): Promise<void> {
		...
		await this.runTest('Save Different Credentials for Different Flows', async () => {
			...
			const success1 = comprehensiveFlowDataService.saveFlowCredentialsIsolated('isolation-test-flow-1', flow1Creds);
			const success2 = comprehensiveFlowDataService.saveFlowCredentialsIsolated('isolation-test-flow-2', flow2Creds);
		...
		await this.runTest('Verify Credential Isolation', async () => {
			const isIsolated = comprehensiveFlowDataService.testCredentialIsolation('isolation-test-flow-1', 'isolation-test-flow-2');
		...
		await this.runTest('Same Credentials in Different Flows (Isolation)', async () => {
			...
			const success1 = comprehensiveFlowDataService.saveFlowCredentialsIsolated('same-creds-flow-1', sameCreds);
			const success2 = comprehensiveFlowDataService.saveFlowCredentialsIsolated('same-creds-flow-2', sameCreds);
			...
			const isIsolated = comprehensiveFlowDataService.testCredentialIsolation('same-creds-flow-1', 'same-creds-flow-2');
	...
	private async testEnvBackupRestore(): Promise<void> {
		...
		await this.runTest('Generate .env Content', async () => {
			...
			const saveSuccess = comprehensiveFlowDataService.saveFlowCredentialsIsolated('env-test-flow', credentials);
...
ENV_TEST_FLOW_CUSTOMPARAM2=${credentials.additionalParams?.customParam2}`;
			...
			// Test restore
			const restoreSuccess = comprehensiveFlowDataService.restoreCredentialsFromEnv('env-restore-test', envContent);
			...
			if (restored.logoutUrl !== credentials.logoutUrl) {
				throw new Error(`Expected logoutUrl '${credentials.logoutUrl}', got '${restored.logoutUrl}'`);
			}
	...
	private async testErrorHandling(): Promise<void> {
		...
		await this.runTest('Invalid Flow Key Handling', async () => {
			...
			const emptyKeyResult = comprehensiveFlowDataService.saveFlowCredentialsIsolated('', credentials);
			if (emptyKeyResult) {
				throw new Error('Should not succeed with empty flow key');
			}
			
			// Test with null flow key
			const nullKeyResult = comprehensiveFlowDataService.saveFlowCredentialsIsolated(null as any, credentials);
			if (nullKeyResult) {
		...
		await this.runTest('Invalid Credentials Handling', async () => {
			...
			const result = comprehensiveFlowDataService.saveFlowCredentialsIsolated('error-test-flow', invalidCreds);
	...
	private async testEdgeCases(): Promise<void> {
		...
		await this.runTest('Large Data Sets', async () => {
			...
			const success = comprehensiveFlowDataService.saveFlowCredentialsIsolated('large-test-flow', largeCredentials);
			...
			return { scopesCount: loaded.scopes.length, paramsCount: Object.keys(loaded.additionalParams || {}).length };
		});
		...
		await this.runTest('Special Characters', async () => {
			...
			const success = comprehensiveFlowDataService.saveFlowCredentialsIsolated('special-chars-flow', specialCredentials);
