 ...
 */
export async function saveProductionAppCredentialsWithSQLite(appId: ProductionAppId, credentials: Record<string, unknown>): Promise<void> {
	...
	try {
		switch (config.appType) {
			...
			case 'worker-token': {
				...
				await UnifiedWorkerTokenBackupServiceV8.saveWorkerTokenBackup(credentials as UnifiedWorkerTokenCredentials, {
					environmentId,
					enableBackup: true,
					backupExpiry: 7 * 24 * 60 * 60 * 1000, // 7 days
				});
 ...
 */
export async function loadProductionAppCredentialsWithSQLite(appId: ProductionAppId): Promise<Record<string, unknown> | null> {
	const config = PRODUCTION_APP_CONFIGS[appId];
	if (!config) {
		throw new Error(`Unknown app: ${appId}`);
	}
 ...
 */
export async function hasProductionAppCredentialsWithSQLite(appId: ProductionAppId): Promise<boolean> {
	try {
 ...
 */
export async function exportProductionAppCredentialsWithSQLite(appId: ProductionAppId): Promise<void> {
	const config = PRODUCTION_APP_CONFIGS[appId];
	if (!config) {
		throw new Error(`Unknown app: ${appId}`);
	}
 ...
 */
export async function importProductionAppCredentialsWithSQLite(appId: ProductionAppId, file: File): Promise<void> {
	const config = PRODUCTION_APP_CONFIGS[appId];
	if (!config) {
		throw new Error(`Unknown app: ${appId}`);
	}
	...
	try {
		...
		if (importedData.metadata.appType !== config.appType) {
			throw new Error(`Import file type (${importedData.metadata.appType}) doesn't match app type (${config.appType})`);
		}
 ...
 */
export async function verifyProductionAppsSQLiteIntegration(): Promise<{ appId: ProductionAppId; appName: string; hasBackup: boolean; error?: string }[]> {
