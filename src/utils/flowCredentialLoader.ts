...
export const loadFlowCredentials = (config: FlowCredentialConfig): FlowCredentials => {
	...
	if (fallbackToShared && (!flowCredentials.environmentId || !flowCredentials.clientId)) {
		...
		flowCredentials.environmentId =
			flowCredentials.environmentId || import.meta.env.VITE_PINGONE_ENVIRONMENT_ID || '';
		flowCredentials.clientId =
			flowCredentials.clientId || import.meta.env.VITE_PINGONE_CLIENT_ID || '';
		flowCredentials.clientSecret =
			flowCredentials.clientSecret || import.meta.env.VITE_PINGONE_CLIENT_SECRET || '';
		flowCredentials.redirectUri =
			flowCredentials.redirectUri ||
			import.meta.env.VITE_PINGONE_REDIRECT_URI ||
			getDefaultRedirectUri(flowKey);
		flowCredentials.apiUrl =
	...
	console.log(`🔐 [FlowCredentialLoader:${flowKey}] Loaded credentials:`, {
		...
		isFlowSpecific:
			!fallbackToShared ||
			(!!import.meta.env[`VITE_${flowPrefix}_ENVIRONMENT_ID`] &&
				!!import.meta.env[`VITE_${flowPrefix}_CLIENT_ID`]),
	});
 ...
 */
export const getCredentialSource = (
	flowKey: string
): { source: string; isFlowSpecific: boolean; hasFallback: boolean } => {
	const flowPrefix = getFlowPrefix(flowKey);
	const hasFlowSpecific = !!(
		import.meta.env[`VITE_${flowPrefix}_ENVIRONMENT_ID`] &&
		import.meta.env[`VITE_${flowPrefix}_CLIENT_ID`]
	);
	const hasShared = !!(
		import.meta.env.VITE_PINGONE_ENVIRONMENT_ID && import.meta.env.VITE_PINGONE_CLIENT_ID
	);
