...
class FieldEditingDiagnostic {
  ...
  private generateRecommendations(issues: FieldEditingIssue[]): string[] {
    ...
    if (issues.some(issue => issue.type === 'disabled')) {
      recommendations.push('Review all disabled attributes - ensure they are only set when appropriate');
    }

    if (issues.some(issue => issue.type === 'readonly')) {
      recommendations.push('Review all readonly attributes - ensure they are only set when appropriate');
    }
