...
describe('JWKS Utilities', () => {
  ...
  const mockJWT = 'eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6ImtleTEifQ.eyJpc3MiOiJodHRwczovL2F1dGgucGluZ29uZS5jb20vdGVzdC9hcyIsImF1ZCI6InRlc3RfY2xpZW50Iiwic3ViIjoidGVzdF91c2VyIiwiZXhwIjoxNzAwMDAwMDAwfQ.signature';
  ...
  describe('discoverJWKS', () => {
    it('should discover JWKS endpoint from discovery response', async () => {
      ...
      const result = await discoverJWKS('https://auth.pingone.com/test/as/.well-known/openid_configuration');
    ...
    it('should throw error when discovery fails', async () => {
      ...
      await expect(discoverJWKS('https://invalid.com/.well-known/openid_configuration'))
    ...
    it('should throw error when JWKS URI is missing', async () => {
      ...
      await expect(discoverJWKS('https://auth.pingone.com/test/as/.well-known/openid_configuration'))
  ...
  describe('createJWKSSet', () => {
    it('should create remote JWKS set', () => {
      ...
      expect(createRemoteJWKSet).toHaveBeenCalledWith(new URL('https://auth.pingone.com/test/as/jwks'));
      expect(result).toBe(mockJWKSSet);
    });

    it('should throw error for invalid URL', () => {
      expect(() => createJWKSSet('invalid-url'))
        .toThrow();
    });
  });
  ...
  describe('getSigningKey', () => {
    ...
    it('should throw error when fetch fails', async () => {
      ...
      await expect(getSigningKey('https://auth.pingone.com/test/as/jwks', 'key1'))
  ...
  describe('decodeJWTHeader', () => {
    ...
    it('should throw error for invalid JWT format', () => {
      expect(() => decodeJWTHeader('invalid.jwt'))
        .toThrow('Invalid JWT format');
    });

    it('should throw error for malformed header', () => {
      expect(() => decodeJWTHeader('invalidheader.payload.signature'))
        .toThrow();
    });
  });
  ...
  describe('decodeJWTPayload', () => {
    ...
    it('should throw error for invalid JWT format', () => {
      expect(() => decodeJWTPayload('invalid.jwt'))
        .toThrow('Invalid JWT format');
    });

    it('should throw error for malformed payload', () => {
      expect(() => decodeJWTPayload('header.invalidpayload.signature'))
        .toThrow();
    });
  });
  ...
  describe('validateJWT', () => {
    ...
    it('should fail validation with nonce mismatch', async () => {
      ...
      const result = await validateJWT(mockJWT, mockConfig, { ...mockOptions, nonce: 'expected_nonce' });
