...
export async function generateQRCode(url: string, options: QRCodeOptions = {}): Promise<string> {
  ...
  try {
    ...
    ctx.fillStyle = '#ffffff';
    ctx.fillRect(margin + blockSize, margin + blockSize, markerSize - 2 * blockSize, markerSize - 2 * blockSize);
    ctx.fillStyle = '#000000';
    ctx.fillRect(margin + 2 * blockSize, margin + 2 * blockSize, markerSize - 4 * blockSize, markerSize - 4 * blockSize);

    // Top-right marker
    ctx.fillStyle = '#000000';
    ctx.fillRect(size - margin - markerSize, margin, markerSize, markerSize);
    ctx.fillStyle = '#ffffff';
    ctx.fillRect(size - margin - markerSize + blockSize, margin + blockSize, markerSize - 2 * blockSize, markerSize - 2 * blockSize);
    ctx.fillStyle = '#000000';
    ctx.fillRect(size - margin - markerSize + 2 * blockSize, margin + 2 * blockSize, markerSize - 4 * blockSize, markerSize - 4 * blockSize);

    // Bottom-left marker
    ctx.fillStyle = '#000000';
    ctx.fillRect(margin, size - margin - markerSize, markerSize, markerSize);
    ctx.fillStyle = '#ffffff';
    ctx.fillRect(margin + blockSize, size - margin - markerSize + blockSize, markerSize - 2 * blockSize, markerSize - 2 * blockSize);
    ctx.fillStyle = '#000000';
    ctx.fillRect(margin + 2 * blockSize, size - margin - markerSize + 2 * blockSize, markerSize - 4 * blockSize, markerSize - 4 * blockSize);
