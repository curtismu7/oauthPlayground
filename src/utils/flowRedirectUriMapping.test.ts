...
logger.info('🧪 Testing Flow Redirect URI Mapping System');
logger.info('==========================================');
...
logger.info('4. Flow Requirements:');
logger.info(
...
logger.info('5. Flow Configurations:');
...
logger.info('6. Flows by Requirements:');
...
logger.info('7. All V6 Flows:');
v6Flows.forEach((flow) => {
	const uri = generateRedirectUriForFlow(flow.flowType);
	logger.info(`   - ${flow.flowType}: ${uri || 'No redirect URI needed'}`);
});

logger.info('\n✅ All tests completed!');
logger.info('\n📋 Summary:');
logger.info('- Centralized redirect URI mapping system is working');
logger.info('- Each flow type has its correct callback path');
logger.info('- System automatically adapts to different ports');
logger.info('- Easy to maintain and extend with new flows');
