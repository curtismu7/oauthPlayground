...
export async function getSigningKey(jwksUri: string, kid: string): Promise<JWK | null> {
  try {
    ...
    if (!key) {
      logger.warn('JWKS', 'Signing key not found', { kid, availableKids: jwks.keys.map(k => k.kid) });
      return null;
    }
...
export function decodeJWTHeader(token: string): any {
  try {
    ...
    throw new Error(`Failed to decode JWT header: ${error instanceof Error ? error.message : 'Invalid format'}`);
  }
}
...
export function decodeJWTPayload(token: string): JWTPayload {
  try {
    ...
    throw new Error(`Failed to decode JWT payload: ${error instanceof Error ? error.message : 'Invalid format'}`);
  }
}
  ...
  try {
    ...
    if (options.nonce) {
      if (!payload.nonce || payload.nonce !== options.nonce) {
        validationErrors.push(`Nonce validation failed: expected ${options.nonce}, got ${payload.nonce}`);
      }
    }
