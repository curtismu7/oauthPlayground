...
export class SecurityHeadersService {
	...
	static getSecurityHeaders(flowName: string): SecurityHeaders {
		...
		for (const config of SecurityHeadersService.securityConfigs) {
	...
	static getOAuthSecurityHeaders(): SecurityHeaders {
		return SecurityHeadersService.getSecurityHeaders('oauth');
	}
	...
	static getOIDCSecurityHeaders(): SecurityHeaders {
		return SecurityHeadersService.getSecurityHeaders('oidc');
	}
	...
	static getPingOneSecurityHeaders(): SecurityHeaders {
		return SecurityHeadersService.getSecurityHeaders('pingone');
	}
	...
	static applySecurityHeaders(response: Response, flowName: string): void {
		const headers = SecurityHeadersService.getSecurityHeaders(flowName);
			...
			if (config.required) {
				...
				if (!headerValue) {
					...
					result.warnings.push(
						`Security header ${config.name} has non-standard value: ${headerValue}`
					);
				}
			}
		}
	...
	static getSecurityHeaderConfig(headerName: string): SecurityHeaderConfig | undefined {
		return SecurityHeadersService.securityConfigs.find((config) => config.name === headerName);
	}
	...
	static getAllSecurityHeaderConfigs(): SecurityHeaderConfig[] {
		return [...SecurityHeadersService.securityConfigs];
	}
	...
	static generateSecurityHeadersReport(headers: Record<string, string>): string {
		const validation = SecurityHeadersService.validateSecurityHeaders(headers);
		...
		Object.entries(headers).forEach(([name, value]) => {
			const config = SecurityHeadersService.getSecurityHeaderConfig(name);
			if (config) {
				report += `  ✅ ${name}: ${value}\n`;
			} else {
	...
	static getSecurityScore(headers: Record<string, string>): number {
		const validation = SecurityHeadersService.validateSecurityHeaders(headers);
		const totalRequired = SecurityHeadersService.securityConfigs.filter(
