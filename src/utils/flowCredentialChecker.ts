 ...
 */
export const checkFlowCredentials = (flowType: string): { hasCredentials: boolean; status: 'configured' | 'partial' | 'missing'; missingFields: string[] } => {
  try {
    let credentials: any = {};
    ...
    if (['oauth2-client-credentials-v3', 'oidc-client-credentials-v3', 'worker-token-v3', 'oauth-resource-owner-password', 'oidc-resource-owner-password'].includes(flowType)) {
    ...
    if (['oauth-authorization-code-v3', 'enhanced-authorization-code-v3', 'oauth2-implicit-v3', 'oidc-implicit-v3', 'oidc-hybrid-v3'].includes(flowType)) {
    ...
    if (missingFields.length === 0) {
      status = 'configured';
    } else if (missingFields.length < 3) { // Some fields present but not all
      status = 'partial';
    }
