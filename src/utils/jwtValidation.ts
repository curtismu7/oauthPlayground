	...
	try {
		...
		if (!basicResult.valid) {
			return {
				...
				flowValidation: {
					valid: false,
					errors: [basicResult.error || 'Basic validation failed'],
					warnings: [],
				},
...
export function validateJWTClaims(
	...
	if (options.issuer) {
		...
		if (!payload.iss || !expectedIssuers.includes(payload.iss)) {
			errors.push(
				`Invalid issuer: expected one of [${expectedIssuers.join(', ')}], got ${payload.iss}`
			);
		}
	}
	...
	if (options.audience) {
		...
		if (
			!aud ||
			(Array.isArray(aud) && !aud.some((a) => expectedAudiences.includes(a))) ||
			(!Array.isArray(aud) && !expectedAudiences.includes(aud))
		) {
			errors.push(
				`Invalid audience: expected one of [${expectedAudiences.join(', ')}], got ${JSON.stringify(aud)}`
			);
		}
	}
 ...
 */
function validateFlowSpecificClaims(
	payload: JWTPayload,
	flowType: string
 ...
 */
export function createClaimPresenceValidator(
	claim: string
): (payload: JWTPayload) => string | null {
	return (payload: JWTPayload) => {
 ...
 */
export function createClaimTypeValidator(
	claim: string,
	expectedType: string
): (payload: JWTPayload) => string | null {
	return (payload: JWTPayload) => {
 ...
 */
export function createStringPatternValidator(
	claim: string,
	pattern: RegExp
): (payload: JWTPayload) => string | null {
	return (payload: JWTPayload) => {
