...
export async function runSafeguards(): Promise<void> {
	...
	try {
		...
		if (results.results.regressionTesting) {
			...
			logger.info(
				`Total Flows: ${results.results.regressionTesting.summary.totalFlows}`,
				'Logger info'
			);
			logger.info(
				`Passed: ${results.results.regressionTesting.summary.passedFlows}`,
				'Logger info'
			);
			logger.info(
				`Failed: ${results.results.regressionTesting.summary.failedFlows}`,
				'Logger info'
			);
			logger.info(
		...
		if (results.results.preCommitChecks) {
			...
			logger.info(
				`Total Checks: ${results.results.preCommitChecks.summary.totalChecks}`,
				'Logger info'
			);
			...
			logger.info(
				`Critical Failures: ${results.results.preCommitChecks.summary.criticalFailures}`,
				'Logger info'
			);
		}
		...
		if (results.results.errorMonitoring) {
			...
			logger.info(
				`Critical Errors: ${results.results.errorMonitoring.criticalErrors}`,
				'Logger info'
			);
		}
		...
		if (results.results.flowValidation) {
			...
			logger.info(
				`Critical Issues: ${results.results.flowValidation.criticalIssues.length}`,
				'Logger info'
			);
		}
		...
		if (results.passed) {
			...
			logger.info(
				'❌ Some safeguard checks failed. Please review the issues above.',
				'Logger info'
			);
...
export async function runQuickHealthCheck(): Promise<boolean> {
	...
	try {
		...
		logger.info(
			`Components: ${healthSummary.healthy}/${healthSummary.components} healthy`,
			'Logger info'
		);
...
export async function runFlowTest(flowName: string): Promise<boolean> {
	...
	try {
		...
		if (result) {
			logger.info(
				`Flow ${flowName}: ${result.overallPassed ? '✅ PASSED' : '❌ FAILED'}`,
				'Logger info'
			);
