...
export async function requestDeviceAuthorization(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Device authorization failed: ${response.status} ${response.statusText}. ${errorData.error_description || errorData.error || 'Unknown error'}`);
    }
 ...
 */
export function validateDeviceAuthorizationResponse(response: any): response is DeviceAuthorizationResponse {
  return (
    response &&
...
export function isPollingFailure(error: DeviceTokenError): boolean {
  return error.error === 'expired_token' || error.error === 'access_denied' || error.error === 'invalid_grant';
}
