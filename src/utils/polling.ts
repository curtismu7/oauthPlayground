...
import { DeviceTokenResponse, DeviceTokenError, isDeviceTokenError, shouldContinuePolling, isPollingFailure, getNextPollingInterval } from './deviceCode';
    ...
    return new Promise((resolve) => {
      const poll = async () => {
        ...
        if (options.maxAttempts && attempts > options.maxAttempts) {
          logger.warn('POLLING', 'Max attempts reached', { attempts, maxAttempts: options.maxAttempts });
          isRunning = false;
          resolve({
            success: false,
            attempts,
        ...
        if (options.maxDuration && currentDuration > options.maxDuration) {
          logger.warn('POLLING', 'Max duration reached', { duration: currentDuration, maxDuration: options.maxDuration });
          isRunning = false;
          resolve({
            success: false,
            attempts,
        ...
        try {
          logger.info('POLLING', 'Polling attempt', { attempt: attempts, interval: currentInterval });
          ...
          if (isDeviceTokenError(response)) {
            ...
            if (shouldContinuePolling(error)) {
              ...
              if (error.error === 'slow_down') {
                ...
                logger.info('POLLING', 'Slow down received, adjusting interval', { newInterval: currentInterval });
              }
              ...
              logger.error('POLLING', 'Polling failed', { error: error.error, description: error.error_description });
              isRunning = false;
              resolve({
                success: false,
                attempts,
            ...
            // Success!
            logger.success('POLLING', 'Polling completed successfully', { attempts, duration: currentDuration });
            isRunning = false;
