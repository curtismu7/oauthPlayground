...
export class ServiceWorkerManager {
  ...
  async register(): Promise<ServiceWorkerRegistration | null> {
    ...
    try {
      ...
      logger.error('[ServiceWorkerManager] Service worker registration failed:', error);
      throw error;
    }
  }
  ...
  async getCacheStatus(): Promise<{
    ...
    if (this.isSupported && 'caches' in window) {
      try {
        ...
        logger.error('[ServiceWorkerManager] Failed to get cache names:', error);
      }
    }
  ...
  async getCacheSize(): Promise<{
    ...
    try {
      ...
      logger.error('[ServiceWorkerManager] Failed to get cache size:', error);
      return { totalSize: 0, cacheSizes: {} };
    }
  }
  ...
  async isResourceCached(url: string): Promise<boolean> {
    ...
    try {
      ...
      logger.error('[ServiceWorkerManager] Failed to check if resource is cached:', error);
      return false;
    }
  }
