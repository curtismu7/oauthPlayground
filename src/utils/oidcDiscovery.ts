 ...
 */
export async function discoverOIDCConfiguration(options: DiscoveryOptions): Promise<DiscoveryResult> {
  try {
...
export async function fetchWithRetry(
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (response.ok) {
        logger.debug('OIDCDiscovery', 'Fetch successful', { url, attempt, status: response.status });
        return response;
      }
      
      // Don't retry on client errors (4xx)
      if (response.status >= 400 && response.status < 500) {
        logger.warn('OIDCDiscovery', 'Client error, not retrying', { url, status: response.status });
        return response;
      }
      ...
      if (attempt < retries) {
        ...
        logger.error('OIDCDiscovery', 'Fetch failed after all retries', { url, status: response.status });
        return response;
      }
 ...
 */
export function validateOIDCConfiguration(config: OIDCConfiguration): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
