...
export const FIDO2SampleApp: React.FC = () => {
	...
	const handleInitialize = async (sdkConfig: P1MFAConfig) => {
		try {
			...
			logger.error('FIDO2SampleApp', 'Failed to initialize SDK:', undefined, error);
		}
	};
	...
	const handleLoadDevices = async () => {
		...
		try {
			...
			logger.error('FIDO2SampleApp', 'Failed to load devices:', undefined, error);
		} finally {
				...
				{ label: 'Cancel', action: () => modernMessaging.hideBanner() },
				{
					...
					action: async () => {
						...
						try {
							...
							logger.error('FIDO2SampleApp', 'Failed to delete device:', undefined, error);
						}
					},
				},
			],
