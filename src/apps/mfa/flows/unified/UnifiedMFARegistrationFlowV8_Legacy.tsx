...
export const UnifiedMFARegistrationFlowV8: React.FC<UnifiedMFARegistrationFlowV8Props> = (
	...
	const handleDeviceTypeSelection = useCallback(
		(deviceType: DeviceConfigKey, environmentId: string, username: string) => {
			console.log(`${MODULE_TAG} Device type selected:`, { deviceType, environmentId, username });
			...
			console.log(`${MODULE_TAG} Saving credentials to storage IMMEDIATELY:`, {
				environmentId,
				username,
				deviceType,
			});
	...
	useEffect(() => {
		const loadCredentials = async () => {
			try {
				const savedCreds = await CredentialsServiceV8.loadCredentials(FLOW_KEY, {
					flowKey: FLOW_KEY,
					flowType: 'registration',
					includeClientSecret: false,
					includeRedirectUri: false,
				...
				if (savedCreds) {
					console.log(`${MODULE_TAG} Loaded credentials from storage:`, {
						environmentId: savedCreds.environmentId,
						username: savedCreds.username,
						deviceType: savedCreds.deviceType,
					});

					setSelectedEnvironmentId(savedCreds.environmentId || '');
					setSelectedUsername(savedCreds.username || '');
					setSelectedDeviceType((savedCreds.deviceType as DeviceConfigKey) || '');
				}
			} catch (error) {
				console.error(`${MODULE_TAG} Failed to load credentials from storage:`, error);
			}
		};
