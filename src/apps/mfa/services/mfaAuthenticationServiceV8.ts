 ...
 * @description MFA Authentication Service - MFA SERVICE
 * @version 8.3.0
 * @since 2025-01-XX
 *
 * Core MFA authentication service handling:
 * - Multi-factor authentication flows
 * - Device registration and management
 * - OTP and push notification authentication
...
// Device registration interfaces (from deprecated MFAServiceV8)
...
export interface RegisterDeviceParams extends MFACredentials {
	...
	tokenType?: 'worker' | 'user'; // Token type: 'worker' for Worker Token, 'user' for User Token (access token from Authorization Code Flow)
	userToken?: string; // User token (access token) when tokenType is 'user'
}
...
export class MfaAuthenticationServiceV8 {
	...
	// ========================================
	// DEVICE REGISTRATION METHODS (from deprecated MFAServiceV8)
	// ========================================

	/**
	 * Register a new MFA device
	 * This method was moved from the deprecated MFAServiceV8 to maintain feature parity
	 */
	static async registerDevice(params: RegisterDeviceParams): Promise<DeviceRegistrationResult> {
		...
		try {
			...
			if (params.tokenType === 'user' && params.userToken) {
				...
				accessToken = await MfaAuthenticationServiceV8.getWorkerTokenWithAutoRenew();
			...
			apiCallTrackerService.trackCall(
				'POST',
				`/v1/environments/${params.environmentId}/users/${params.username}/devices`,
				Date.now() - startTime,
				response.status
			);
	...
	/**
	 * Get MFA settings for an environment
	 * This method was moved from the deprecated MFAServiceV8 to maintain feature parity
	 */
	static async getMFASettings(environmentId: string): Promise<MFASettings> {
		console.log(`${MODULE_TAG} Getting MFA settings for environment: ${environmentId}`);

		try {
			const accessToken = await MfaAuthenticationServiceV8.getWorkerTokenWithAutoRenew();

			// Track API call for display
			const { apiCallTrackerService } = await import('@/services/apiCallTrackerService');
			const startTime = Date.now();

			// Make API call to get MFA settings
			const response = await pingOneFetch(
				`https://api.pingone.com/v1/environments/${environmentId}/mfaSettings`,
				{
					method: 'GET',
					headers: {
						Authorization: `Bearer ${accessToken}`,
			...
			if (!response.ok) {
				const errorText = await response.text();
				console.error(`${MODULE_TAG} Failed to get MFA settings:`, {
					status: response.status,
					statusText: response.statusText,
					error: errorText,
				});
				throw new Error(`Failed to get MFA settings: ${response.status} ${response.statusText}`);
			}

			const settingsData = await response.json();
			console.log(`${MODULE_TAG} ✅ MFA settings retrieved successfully`);

			return settingsData as MFASettings;
		} catch (error) {
			console.error(`${MODULE_TAG} Error getting MFA settings:`, error);
			throw error;
		}
	}

	/**
	 * Update MFA settings for an environment
	 * This method was moved from the deprecated MFAServiceV8 to maintain feature parity
	 */
	static async updateMFASettings(
		...
		console.log(`${MODULE_TAG} Updating MFA settings for environment: ${environmentId}`);
		...
		try {
			...
			// Make API call to update MFA settings
			const response = await pingOneFetch(
				`https://api.pingone.com/v1/environments/${environmentId}/mfaSettings`,
				{
					method: 'PUT',
			...
			if (!response.ok) {
				const errorText = await response.text();
				console.error(`${MODULE_TAG} Failed to update MFA settings:`, {
					status: response.status,
					statusText: response.statusText,
					error: errorText,
				});
				throw new Error(`Failed to update MFA settings: ${response.status} ${response.statusText}`);
			}

			const updatedSettings = await response.json();
			console.log(`${MODULE_TAG} ✅ MFA settings updated successfully`);

			return updatedSettings as MFASettings;
		} catch (error) {
			console.error(`${MODULE_TAG} Error updating MFA settings:`, error);
			throw error;
		}
	}
	 ...
	 * @param orderedDeviceIds - Array of device IDs in desired order
	...
	static async setUserMfaDeviceOrder(
		...
		const token = await MfaAuthenticationServiceV8.getWorkerToken();
		...
		try {
			...
			if (!response.ok) {
				const errorText = await response.text();
				console.error(`${MODULE_TAG} Failed to set device order:`, {
					status: response.status,
					statusText: response.statusText,
					error: errorText,
				});
				throw new Error(`Failed to set device order: ${response.status} ${response.statusText}`);
			}

			const result = await response.json();
			console.log(`${MODULE_TAG} ✅ Device order set successfully`, {
				userId,
				deviceCount: orderedDeviceIds.length,
			});
			...
			console.error(`${MODULE_TAG} Exception in setUserMfaDeviceOrder:`, error);
			throw error;
		}
	}
}
