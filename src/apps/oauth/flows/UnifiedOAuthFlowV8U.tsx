 ...
 * @description Unified OAuth/OIDC Flow - Single UI for all flows using real PingOne APIs - OAUTH FLOW
 * @version 8.0.0
 * @since 2024-11-16
 *
						...
						>
							...
							<WorkerTokenButton
								onTokenObtained={(token) => {
									console.log('UnifiedOAuthFlowV8U: Worker token obtained:', token);
									// Token is handled by the unified service, no additional action needed
								}}
								onModalClose={() => {
									console.log('UnifiedOAuthFlowV8U: Worker token modal closed');
								}}
								environmentId={(() => {
									try {
										const stored = localStorage.getItem('unified_worker_token');
