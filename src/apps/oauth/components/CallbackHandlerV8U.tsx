 ...
 * @description OAuth callback handler for unified flows - OAUTH APP COMPONENT
 * @version 8.0.0
 * @since 2024-11-17
 *
...
const MODULE_TAG = '[🔄 CALLBACK-HANDLER-OAUTH]';
...
export const CallbackHandlerV8U: React.FC = () => {
	...
	useEffect(() => {
		...
		MFARedirectUriServiceV8.logDebugEvent('CALLBACK_HANDLER', 'OAuth callback handler started', {
		...
		if (isDebugCallbackPage) {
			console.log(
				`${MODULE_TAG} Debug callback page detected - skipping OAuth callback handler logic`
			);
			return;
		}

		console.log(`${MODULE_TAG} 🔍 CHECKING OAUTH CALLBACK PATH:`, {
			currentPath,
			isUserLoginCallback,
		...
		logToDisk('oauth_callback_path_check', {
			currentPath,
			isUserLoginCallback,
		...
		console.log(`${MODULE_TAG} Not a user-login-callback, proceeding with OAuth flow logic`);
		...
		console.log(`${MODULE_TAG} OAuth callback received`, {
			url: window.location.href,
		...
		console.log(`${MODULE_TAG} Stored OAuth callback data in sessionStorage`, callbackData);
		...
		console.log(`${MODULE_TAG} 🚀 ========== REDIRECTING TO OAUTH FLOW ==========`);
