// src/mfa/pages/MFADeviceCreateDemo.tsx
/**
 * @module v8/pages
...
import SimplePingOneApiDisplay from '@/mfa/components/SimplePingOneApiDisplay';
import { WorkerTokenExpiryBanner } from '@/mfa/components/WorkerTokenExpiryBanner';
import { WorkerTokenModal } from '@/components/WorkerTokenModal';
import deviceCreateDemoService from '@/mfa/services/deviceCreateDemoService';
 ...
 */
export const MFADeviceCreateDemo: React.FC = () => {
	...
	const handleLookupUser = useCallback(async () => {
		...
		try {
			const user = await MFAService.lookupUserByUsername(environmentId.trim(), username.trim());
			setUserId(user.id);
	...
	const handleSubmit = useCallback(
		async (type: 'SMS' | 'EMAIL') => {
			...
			setLoading(true);
			try {
				const response = await deviceCreateDemoService.sendCreateDeviceRequest({
...
export default MFADeviceCreateDemo;
