/**
 * @file DeleteAllDevicesUtility.tsx
 * @module v8/pages
...
import { StorageService } from '@/mfa/services/storageService';
...
export const DeleteAllDevicesUtility: React.FC = () => {
	...
	const [environmentId, setEnvironmentId] = useState(() => {
		...
		try {
			const stored = StorageService.load<DeleteAllDevicesPageState>(PAGE_STORAGE_KEY);
	...
	useEffect(() => {
		try {
			...
			StorageService.save(PAGE_STORAGE_KEY, state, PAGE_STORAGE_VERSION);

			if (state.environmentId) {
				EnvironmentIdService.saveEnvironmentId(state.environmentId);
			}
		} catch (error) {
			const errorMessage = error instanceof Error ? error.message : 'Unknown error';
			logger.error(
		...
		const fetchPolicyInfo = async () => {
			...
			try {
				...
				if (config.defaultMfaPolicyId) {
					const policyData = await MFAService.readDeviceAuthenticationPolicy(
						environmentId,
	...
	useEffect(() => {
		const fetchMfaSettings = async () => {
			...
			try {
				logger.info(
					`${MODULE_TAG} Loading MFA settings for environment: ${environmentId}`,
					'Logger info'
				);
				const settings = await MFAService.getMFASettings(environmentId);
	...
	return (
		<div style={{ padding: '40px', maxWidth: '1000px', margin: '0 auto' }}>
			<WorkerTokenExpiryBanner
				onFixToken={() => openGlobalWorkerTokenModal('DeleteAllDevicesUtility-Fix')}
				marginBottom="24px"
			/>
			{/* Header */}
			<div
			...
			>
				<div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
					...
					<div>
						...
						<p style={{ margin: '4px 0 0 0', color: 'rgba(255,255,255,0.85)', fontSize: '16px' }}>
			...
			<SuperSimpleApiDisplay flowFilter="mfa" reserveSpace={true} />
		</div>
	);
};

export default DeleteAllDevicesUtility;
