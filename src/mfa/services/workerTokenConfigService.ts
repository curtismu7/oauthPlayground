/**
 * @file workerTokenConfigService.ts
 * @module v8/services
...
import { MFAConfigurationService } from './mfaConfigurationService';
...
class WorkerTokenConfigService {
	...
	/**
	 * Initialize the service by loading configuration from MFAConfigurationService
	 */
	private initialize(): void {
		...
		try {
			...
			this.config = {
				silentApiRetrieval: true, // Match MFAConfigurationService default
				showTokenAtEnd: true, // Match MFAConfigurationService default
			};
			this.isInitialized = true;
	...
	private validateConfigurationConsistency(mfaConfig: {
		...
		if (mfaSilentApi !== defaultSilentApi) {
			logger.info(`${MODULE_TAG} ⚠️ MFA silentApiRetrieval differs from default:`, {
				...
				source: 'MFAConfigurationService',
			});
		}
...
export const workerTokenConfigService = new WorkerTokenConfigService();
export default workerTokenConfigService;
