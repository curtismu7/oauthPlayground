...
export class AuthMethodService {
	 ...
	 */
	static getPingOneSupportedMethods(): AuthMethod[] {
		const configs = AuthMethodService.getAllMethodConfigs();
	 ...
	 */
	static getDisplayLabel(method: AuthMethod): string {
		return AuthMethodService.getMethodConfig(method).label;
	}
	 ...
	 */
	static getAvailableMethodsForFlow(flowType: string): AuthMethod[] {
	 ...
	 */
	static requiresClientSecret(method: AuthMethod): boolean {
		return AuthMethodService.getMethodConfig(method).requiresClientSecret;
	}

	/**
	 * Get recommended method for a flow type
	 */
	static getRecommendedMethod(flowType: string): AuthMethod {
		const normalizedFlowType = flowType.toLowerCase();
	...
	static validateMethodForFlow(
		...
		const availableMethods = AuthMethodService.getAvailableMethodsForFlow(flowType);
