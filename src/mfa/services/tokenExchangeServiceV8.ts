// src/mfa/services/tokenExchangeServiceV8.ts
...
export class TokenExchangeServiceV8 {
	...
	static async exchangeToken(
		...
		try {
			...
			logger.info(
				`${TokenExchangeServiceV8.MODULE_TAG} Token exchange completed successfully`,
				'Logger info'
			);
			return response;
		} catch (error) {
	...
	private static async executeTokenExchangeWithPingOne(
		...
		try {
			logger.info(
				`${TokenExchangeServiceV8.MODULE_TAG} Executing token exchange with PingOne`,
				'Logger info'
			);
