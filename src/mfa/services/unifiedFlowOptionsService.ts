/**
 * @file unifiedFlowOptionsService.ts
 * @module v8/services
 ...
 *
 * @example
 * const options = UnifiedFlowOptionsService.getOptionsForFlow('oauth2.1', 'oauth-authz');
 * const visibility = UnifiedFlowOptionsService.getFieldVisibility('oidc', 'hybrid');
 */

import { logger } from '../../utils/logger';
import { type FlowOptions, FlowOptionsService } from './flowOptionsService';
import { type FlowType, type SpecVersion, SpecVersionService } from './specVersionService';
...
export class UnifiedFlowOptionsService {
	...
	static getOptionsForFlow(specVersion: SpecVersion, flowType: FlowType): FlowOptions {
		...
		const baseOptions = FlowOptionsService.getOptionsForFlow(`${flowType}-v8`);
	...
	static getFieldVisibility(specVersion: SpecVersion, flowType: FlowType): FieldVisibility {
		...
		const flowOptions = FlowOptionsService.getOptionsForFlow(`${flowType}-v8`);
	...
	static getAvailableFlowsWithLabels(
		...
		return flows.map((flowType) => ({
			type: flowType,
			label: SpecVersionService.getFlowLabel(flowType),
		}));
	}
	...
	static getAllSpecVersionsWithLabels(): Array<{
		...
		return SpecVersionService.getAllSpecVersions().map((specVersion) => ({
			type: specVersion,
			label: SpecVersionService.getSpecLabel(specVersion),
			description: SpecVersionService.getSpecDescription(specVersion),
		}));
	}
}

export default UnifiedFlowOptionsService;
