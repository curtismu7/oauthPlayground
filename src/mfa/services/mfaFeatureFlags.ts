 ...
 * if (MFAFeatureFlags.isEnabled('mfa_unified_sms')) {
 *   return <UnifiedMFARegistrationFlow deviceType="SMS" />;
 * } else {
 *   return <SMSOTPConfigurationPage />;
 * }
 *
 * @example
...
export class MFAFeatureFlags {
	...
	static isEnabled(flag: MFAFeatureFlag): boolean {
		...
		const state = MFAFeatureFlags.getFlagState(flag);
		...
		const userId = MFAFeatureFlags.getUserId();
		const hash = MFAFeatureFlags.hashString(userId + flag);
	...
	static setFlag(
		...
		localStorage.setItem(MFAFeatureFlags.STORAGE_KEY, JSON.stringify(flags));
		logger.info(
	...
	static getAllFlags(): Record<MFAFeatureFlag, FeatureFlagState> {
		...
		try {
			const stored = localStorage.getItem(MFAFeatureFlags.STORAGE_KEY);
	...
	static resetAllFlags(): void {
		localStorage.removeItem(MFAFeatureFlags.STORAGE_KEY);
		logger.info(
...
if (typeof window !== 'undefined') {
	(window as Window & { mfaFlags: typeof MFAFeatureFlags }).mfaFlags = MFAFeatureFlags;
	logger.info(
