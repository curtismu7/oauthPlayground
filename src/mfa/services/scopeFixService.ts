...
export class ScopeFixService {
	...
	static analyzeAndFixScopes(options: ScopeFixOptions): ScopeFixResult {
		...
		if (flowType === 'client-credentials') {
			return ScopeFixService.fixClientCredentialsScopes(scopesList, environmentId, applicationId);
		}

		// OAuth/OIDC flow specific fixes
		return ScopeFixService.fixOAuthScopes(scopesList, flowType);
	}
	...
	private static fixClientCredentialsScopes(
		...
		if (invalidOidcScopes.length > 0 || invalidSelfMgmtScopes.length > 0) {
			...
			return {
				...
				pingOneInstructions: ScopeFixService.generatePingOneInstructions(
					environmentId,
					applicationId,
					suggestedScopes
				),
...
export default ScopeFixService;
