		...
		try {
			...
			if (stored) {
				...
				if (!data || !data.credentials) {
					logger.warn(
						'[WorkerTokenService] No credentials found in stored data',
						'Logger warning'
					);
					return null;
				}
			...
			logger.error('[WorkerTokenService] Failed to load credentials sync', error);
		}
		return null;
	}
}
...
export const workerTokenService = new WorkerTokenService();

// Make available globally for debugging
if (typeof window !== 'undefined') {
	(window as unknown as { workerTokenService: WorkerTokenService }).workerTokenService =
		workerTokenService;
}

export default workerTokenService;
