...
export const ValidationService = {
	...
	validateCredentials(
		...
		const requiredFields = ValidationService.getRequiredFields(flowType);
		...
		requiredFields.forEach((field) => {
			...
			if (rules.required && ValidationService.empty(value)) {
				errors.push(
			...
			if (ValidationService.empty(value)) {
				return;
			}
		...
		if (flowType === 'oidc') {
			const scopeResult = ValidationService.validateOIDCScopes(
		...
		if (credentials.redirectUri) {
			const redirectWarnings = ValidationService.validateRedirectUriSecurity(
	 ...
	 * @returns Validation result
	 * @example
	 * const result = ValidationService.validateUrl('http://localhost:3000', 'redirect');
	 */
	 ...
	 * @returns Validation result
	 * @example
	 * const result = ValidationService.validateUUID(environmentId, 'Environment ID');
	 */
	...
	validateMFADeviceRegistration(
		...
		switch (deviceType) {
			...
			case 'VOICE':
				...
				} else if (!ValidationService.validatePhoneNumber(params.phoneNumber)) {
					errors.push({
						field: 'phoneNumber',
			...
			case 'WHATSAPP':
				...
				} else if (!ValidationService.validateEmail(params.emailAddress)) {
					errors.push({
						field: 'emailAddress',
	...
	createBatchValidationErrors(
		...
		return Object.entries(fields).map(([field, error]) =>
			ValidationService.createValidationError(field, error.message, error.suggestion, error.code)
		);
	},
