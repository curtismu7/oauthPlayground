// src/mfa/services/tokenExchangeService.ts
...
export class TokenExchangeService {
	private static readonly MODULE_TAG = '[TokenExchangeService]';
	...
	static async exchangeToken(
		...
		try {
			logger.info(
				`${TokenExchangeService.MODULE_TAG} Starting token exchange for environment: ${environmentId}`,
				'Logger info'
			);

			// 1. CRITICAL: Validate admin enablement
			if (!(await TokenExchangeConfigService.isEnabled(environmentId))) {
			...
			const subjectTokenValidation = await TokenExchangeService.validateToken(
			...
			if (params.scope) {
				...
				const scopesValid = await TokenExchangeConfigService.validateScopes(
					environmentId,
					requestedScopes
				);
				if (!scopesValid) {
			...
			if (params.actor_token) {
				const actorTokenValidation = await TokenExchangeService.validateToken(
					params.actor_token,
					environmentId
				);
			...
			const response = await TokenExchangeService.executeTokenExchangeWithPingOne(
				params,
				environmentId
			);

			logger.info(
				`${TokenExchangeService.MODULE_TAG} Token exchange completed successfully`,
				'Logger info'
			);
			return response;
		} catch (error) {
			logger.error(`${TokenExchangeService.MODULE_TAG} Token exchange failed:`, error);
	...
	static async validateToken(
		...
		try {
			logger.info(
				`${TokenExchangeService.MODULE_TAG} Validating token for environment: ${expectedEnvironmentId}`,
				'Logger info'
			);
			...
			logger.info(`${TokenExchangeService.MODULE_TAG} Token validation successful:`, result);
			return result;
		} catch (error) {
			logger.error(`${TokenExchangeService.MODULE_TAG} Token validation error:`, error);
			return {
				isValid: false,
				environmentId: '',
				tokenType: 'access_token',
	...
	private static async executeTokenExchangeWithPingOne(
		...
		try {
			logger.info(
				`${TokenExchangeService.MODULE_TAG} Executing token exchange with PingOne`,
				'Logger info'
			);
			...
			logger.info(`${TokenExchangeService.MODULE_TAG} Token exchange request:`, {
				...requestBody,
			...
			const response: TokenExchangeResponse = {
				access_token: TokenExchangeService.generateMockAccessToken(environmentId),
				token_type: 'Bearer',
				expires_in: 3600,
			...
			logger.info(`${TokenExchangeService.MODULE_TAG} Token exchange response received:`, {
				...response,
			...
			logger.error(`${TokenExchangeService.MODULE_TAG} PingOne API error:`, error);
			throw new TokenExchangeError(
				TokenExchangeErrorType.SERVER_ERROR,
	...
	static buildTokenExchangeContext(
		...
		try {
			...
			logger.warn(
				`${TokenExchangeService.MODULE_TAG} Failed to parse subject token claims:`,
				error
			);
		}
		...
		if (params.actor_token) {
			try {
				...
				logger.warn(
					`${TokenExchangeService.MODULE_TAG} Failed to parse actor token claims:`,
					error
				);
			}
		}
