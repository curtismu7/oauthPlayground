// src/mfa/services/tokenDisplayService.ts
import { logger } from '../../utils/logger';

/**
 * @file tokenDisplayService.ts
 * @module v8/services
 ...
 * await TokenDisplayService.copyToClipboard(token, 'Access Token');
 */
...
export class TokenDisplayService {
	...
	public static decodeJWT(token: string): DecodedJWT | null {
		...
		try {
			...
			const header = JSON.parse(TokenDisplayService.base64UrlDecode(parts[0]));

			// Base64 URL decode payload
			const payload = JSON.parse(TokenDisplayService.base64UrlDecode(parts[1]));
	 ...
	 * @example
	 * const success = await TokenDisplayService.copyToClipboard(token, 'Access Token');
	 */
	 ...
	 * @example
	 * const masked = TokenDisplayService.maskToken(token, 4);
	 * // Returns: "abc1...xyz9"
	 */
	 ...
	 * @example
	 * const decoded = TokenDisplayService.decodeJWT(token);
	 * const formatted = TokenDisplayService.formatDecodedJWT(decoded);
	 */
...
Signature: ${decoded.signature}`;
	 ...
	 * @example
	 * const formatted = TokenDisplayService.formatExpiry(3600);
	 * // Returns: "1h 0m 0s"
	 */
