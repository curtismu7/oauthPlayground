 ...
 * StorageService.save('v8:authz-code', credentials, 1);
 *
 * // Load with migration
 * const data = StorageService.load('v8:authz-code', migrations);
 *
 * // Export all V8 data
...
export class StorageService {
	 ...
	 * @example
	 * StorageService.clear('v8:authz-code');
	 */
	...
	static async clearAll(): Promise<void> {
		try {
			await ensureMigration();
			await unifiedTokenStorage.clearAll();
	...
	static async exportAll(): Promise<string> {
		try {
			await ensureMigration();
			const exported = await unifiedTokenStorage.exportAll();
	 ...
	 * @example
	 * StorageService.importAll(exportedData, true);
	 */
	static async importAll(jsonData: string, overwrite = false): Promise<void> {
		try {
			await ensureMigration();
			await unifiedTokenStorage.importAll(jsonData, overwrite);
	 ...
	 * logger.info(`Storage size: ${size} bytes`, "Logger info");
	 */
	...
	static async cleanupExpired(maxAge: number): Promise<number> {
		try {
			await ensureMigration();
			const cleaned = await unifiedTokenStorage.cleanupExpired(maxAge);
	...
	static async getFlowData(flowKey: string): Promise<Array<{ key: string; data: StorageData }>> {
		try {
			...
			for (const key of allKeys) {
				const data = await StorageService.load<unknown>(key);
				if (data) {
	...
	static async clearFlowData(flowKey: string): Promise<number> {
		try {
			const flowData = await StorageService.getFlowData(flowKey);
			let cleared = 0;
