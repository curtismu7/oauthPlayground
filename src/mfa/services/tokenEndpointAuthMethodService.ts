/**
 * @file tokenEndpointAuthMethodService.ts
 * @module v8/services
 ...
 *
 * @example
 * const methods = TokenEndpointAuthMethodService.getAuthMethods('oauth-authz', 'oauth2.0', false);
 * // Returns: ['client_secret_basic', 'client_secret_post', 'client_secret_jwt', 'private_key_jwt']
 */

import { logger } from '../../utils/logger';
import { type FlowType, type SpecVersion } from './specVersionService';
...
export class TokenEndpointAuthMethodService {
	/**
	...
	static getDefaultAuthMethod(
		...
		const methods = TokenEndpointAuthMethodService.getAuthMethods(flowType, specVersion, usePKCE);
	...
	static isAuthMethodValid(
		...
		const validMethods = TokenEndpointAuthMethodService.getAuthMethods(
			flowType,
			specVersion,
			usePKCE
		);
	...
	static getAllAuthMethodsWithStatus(
		...
		return allMethods.map((method) => {
			...
			return {
				method,
				label: TokenEndpointAuthMethodService.getAuthMethodLabel(method),
				enabled,
				disabledReason,
			};
		});
