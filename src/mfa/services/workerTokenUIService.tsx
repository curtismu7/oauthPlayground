 ...
 * - WorkerTokenStatusDisplay integration
 * - Settings checkboxes with MFAConfigurationService integration
...
import { AppDiscoveryModalV8U } from '../../lab/components/AppDiscoveryModalV8U';
import type { DiscoveredApp } from '../components/AppPicker';
...
export const WorkerTokenUIService: React.FC<WorkerTokenUIServiceV8Props> = ({
	mode = 'detailed',
	showRefresh = true,
	...
	useEffect(() => {
		const initializeTokenStatus = async () => {
			try {
				...
				logger.error('[WorkerTokenUIService] Failed to check token status:', error);
			}
		};
	...
	useEffect(() => {
		const updateEffectiveEnvironmentId = async () => {
			...
			if (!effectiveId.trim()) {
				try {
					const { workerTokenService } = await import('../services/workerTokenService');
					const credentials = await workerTokenService.loadCredentials();

					if (credentials?.environmentId) {
						effectiveId = credentials.environmentId;
						logger.info('[WorkerTokenUIService] Extracted environment ID from worker token', {
							environmentId: effectiveId,
						});
					} else {
						// Fallback to sync method if async fails
						const syncCredentials = workerTokenService.loadCredentialsSync();
						if (syncCredentials?.environmentId) {
							effectiveId = syncCredentials.environmentId;
							logger.info(
								'[WorkerTokenUIService] Extracted environment ID from worker token (sync)',
								{ environmentId: effectiveId }
							);
						} else {
							// Final fallback: try global environment ID service
							const { EnvironmentIdService } = await import('./environmentIdService');
							const globalEnvId = EnvironmentIdService.getEnvironmentId();
							if (globalEnvId) {
								effectiveId = globalEnvId;
								logger.info('[WorkerTokenUIService] Using globally stored environment ID', {
									environmentId: effectiveId,
								});
							}
						}
					...
					logger.error('[WorkerTokenUIService] Error extracting environment ID:', error);
				}
			}
	...
	useEffect(() => {
		const handleConfigUpdate = () => {
			...
			logger.info('[WorkerTokenUIService] Configuration update event received', 'Logger info');
		};
	...
	const handleGetWorkerToken = useCallback(async () => {
		...
		try {
			const { handleShowWorkerTokenModal } = await import('../utils/workerTokenModalHelper');
			...
			UnifiedFlowErrorHandler.handleError(error, {
				...
				component: 'WorkerTokenUIService',
			});
		} finally {
	...
	const handleClearTokens = useCallback(async () => {
		try {
			...
			const clearedStatus = await WorkerTokenStatusService.checkWorkerTokenStatus();
		...
		async (e: React.ChangeEvent<HTMLInputElement>) => {
			...
			if (newValue) {
				try {
					...
					logger.error('[WorkerTokenUIService] Error in silent retrieval:', error);
				}
			}
		},
		[updateSilentApiRetrieval]
	...
	const _handleGetAppsConfig = useCallback(async () => {
		...
		if (!effectiveEnvironmentId.trim()) {
			try {
				...
				const { workerTokenService } = await import('./workerTokenService');
				const credentials = await workerTokenService.loadCredentials();
				...
				logger.error(
					'[WorkerTokenUIService] Failed to extract environment ID from worker token',
					{
	...
	const handleAppSelected = useCallback(
		(app: DiscoveredApp) => {
			logger.info('[WorkerTokenUIService] App selected', {
				appId: app.id,
				appName: app.name,
				context,
			});
			...
			if (context === 'mfa') {
				...
				if (app.id && onEnvironmentIdUpdate) {
					...
					modernMessaging.showFooterMessage({
						type: 'info',
						message: `Environment ID updated to: ${app.id}`,
						duration: 3000,
					});
				}
	...
	if (mode === 'minimal') {
		return (
			<div className={className} style={style}>
				{/* WorkerTokenStatusDisplay - Removed */}
			</div>
		);
	}
			...
			<WorkerTokenContainer className={className} style={style}>
				...
				<WorkerTokenModal
					...
					onClose={() => {
						...
						WorkerTokenStatusService.checkWorkerTokenStatus().then(setTokenStatus);
					}}
