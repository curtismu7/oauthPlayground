/**
 * @file sharedCredentialsService.ts
 * @module v8/services
 ...
 * - Environment ID (also stored globally via EnvironmentIdService)
 * - Client ID
 * - Client Secret
 * - Issuer URL
...
export class SharedCredentialsService {
	/**
	 * Load shared credentials from storage (localStorage only)
	...
	static async saveSharedCredentials(credentials: SharedCredentials): Promise<void> {
		...
		try {
			...
			const existing = SharedCredentialsService.loadSharedCredentialsSync();
	...
	static async mergeWithShared<T extends Record<string, unknown>>(
		...
		const shared = await SharedCredentialsService.loadSharedCredentials();
