 ...
 *
 * ⚠️ DEPRECATION NOTICE: Authentication methods are being moved to MfaAuthenticationService
 * - New code should use MfaAuthenticationService for authentication flows
 * - Authentication methods in this class will be removed in future versions
 *
 * Uses WorkerTokenService for authentication
 *
...
import { sendAnalyticsLog } from '@/mfa/utils/analyticsLogger';
 ...
 * MFAService
 *
 * Service for PingOne MFA operations using WorkerTokenService
 *
...
export class MFAService {
	...
	static async getToken(credentials?: {
		...
		if (tokenType === 'user') {
			...
			if (!userToken || userToken === 'oauth_completed') {
				// For user flows, if we don't have a real user token (or it's just the placeholder),
	...
	static getTokenScopes(token: string): string[] {
		try {
			...
			logger.warn(`${MODULE_TAG} Unable to decode token payload to extract scopes`, error);
			return [];
		}
	}
	 ...
	 */
	static async checkMfaBypassStatus(
		environmentId: string,
		userId: string
	): Promise<Record<string, unknown>> {
		const token = await MFAService.getWorkerToken();
	...
	/**
	 * Get worker token from WorkerTokenService
	...
	static async getWorkerToken(): Promise<string> {
		try {
			...
			logger.info(
				`${MODULE_TAG} Using worker token from WorkerTokenService`,
			...
			const errorMessage = error instanceof Error ? error.message : 'Unknown error';
			logger.error(
				`${MODULE_TAG} Failed to get worker token from WorkerTokenService`,
				errorMessage,
				{ error }
			);
	...
	static async lookupUserByUsername(
		...
		try {
			...
			const accessToken = await MFAService.getWorkerToken();
			logger.info(`${MODULE_TAG} Worker token obtained:`, accessToken ? 'success' : 'FAILED');
	 ...
	 * @deprecated Use UserService.listUsers() instead
	 * @param environmentId - PingOne environment ID
	...
	static async listUsers(
		...
		return UserService.listUsers(environmentId, options);
	}
	...
	static async registerDevice(params: RegisterDeviceParams): Promise<DeviceRegistrationResult> {
		...
		try {
			// Look up user by username
			const user = await MFAService.lookupUserByUsername(params.environmentId, params.username);

			// #region agent log
			sendAnalyticsLog({
				location: 'mfaService.ts:626',
			...
			if (deviceDataWithMetadata._metadata?.actualPingOneUrl) {
				const actualCallId = apiCallTrackerService.trackApiCall({
					method: (deviceDataWithMetadata._metadata.actualPingOneMethod || 'POST') as
						| 'GET'
						| 'POST'
						| 'PUT'
						| 'DELETE'
			...
			const rawPublicKeyOptions = (deviceDataWithoutMetadata as Record<string, unknown>)
			...
			const finalCheck = {
				...
				publicKeyCredentialCreationOptionsValue:
					result.publicKeyCredentialCreationOptions || 'UNDEFINED',
	...
	static async getAllDevices(
		...
		try {
			...
			logger.info(`${MODULE_TAG} API call tracked with ID:`, callId);
			...
			logger.error(`${MODULE_TAG} Get all devices error`, error);
			throw error;
		}
	}
	...
	static async updateDevice(
		...
		if (updates.name) {
			return MFAService.updateDeviceNickname(params, updates.name);
		}
	 ...
	 */
	/**
	 * @deprecated Use MfaAuthenticationService.initializeDeviceAuthentication() instead
	...
	static async initializeOneTimeDeviceAuthentication(params: {
		...
		logger.warn(
			`${MODULE_TAG} DEPRECATION WARNING: initializeOneTimeDeviceAuthentication() is deprecated. Use MfaAuthenticationService.initializeDeviceAuthentication() instead.`,
		...
		try {
			...
			// Get worker token
			const accessToken = params.workerToken || (await MFAService.getWorkerToken());
			...
			logger.error(`${MODULE_TAG} Error in one-time device authentication:`, error);
			throw error;
		}
	}
	...
	static async sendOTP(
		...
		logger.warn(
			`${MODULE_TAG} DEPRECATION WARNING: sendOTP() is deprecated. Use MfaAuthenticationService.initializeDeviceAuthentication() instead.`
		);
		logger.info(
	...
	static async resendPairingCode(params: SendOTPParams): Promise<void> {
		...
		try {
			...
			try {
				const device = await MFAService.getDevice({
					environmentId: params.environmentId,
					username: params.username,
					deviceId: params.deviceId,
				});
		...
		try {
			...
			const requestBody = {
				...
				fido2Data: {
					...
					attestation:
						fido2Data.attestation ||
						(fido2Data.attestationObject
							? JSON.stringify({
			...
			if (!response.ok) {
				...
				const errorMessage = errorData.message || errorData.error || response.statusText;
				const tokenDebugInfo = errorData.debug as
					| { tokenType?: string; tokenExpired?: boolean }
					| undefined;
				const originDebugInfo = errorData.debugInfo as
					| { origin?: string; originIsLocalhost?: boolean; hint?: string }
					| undefined;
				...
				if (response.status === 403) {
					...
					const mightBeTokenIssue =
						tokenExpired ||
						(errorLower.includes('invalid') &&
							(errorLower.includes('token') ||
								errorLower.includes('authorization') ||
								errorLower.includes('header'))) ||
	...
	static async initializeDeviceAuthentication(
		...
		logger.warn(
			`${MODULE_TAG} DEPRECATION WARNING: initializeDeviceAuthentication() is deprecated. Use MfaAuthenticationService.initializeDeviceAuthentication() instead.`
		);
		logger.info(
		...
		try {
			...
			if (params.deviceAuthenticationPolicyId) {
				try {
					policy = await MFAService.readDeviceAuthenticationPolicy(
						params.environmentId,
						params.deviceAuthenticationPolicyId
					);
					logger.info(`${MODULE_TAG} [DEVICE_SELECTION] Policy loaded successfully:`, {
						policyId: params.deviceAuthenticationPolicyId,
			...
			if (policy?.skipUserLockVerification !== true) {
				...
				logger.info(
					`${MODULE_TAG} Skipping user lock verification (skipUserLockVerification=true)`
				);
			}
			...
			if (!response.ok) {
				...
				logger.error(`${MODULE_TAG} Initialize device authentication (PingOne MFA API) failed`, {
					status: response.status,
					statusText: response.statusText,
					errorData,
					environmentId: params.environmentId,
			...
			logger.info(`${MODULE_TAG} [DEVICE_SELECTION] Evaluating device selection logic:`, {
				hasPolicy: !!policy,
				deviceSelection,
			...
			if (
				...
				if (shouldAutoSelect) {
					...
					try {
						const selectResult = await MFAService.selectDeviceForAuthentication({
							environmentId: params.environmentId,
							username: params.username,
						...
						logger.info(`${MODULE_TAG} Automatically selected first device per policy`, {
							deviceId: firstDevice.id,
					...
					logger.info(`${MODULE_TAG} [DEVICE_SELECTION] Policy requires device selection UI:`, {
						deviceSelection,
				...
				logger.info(`${MODULE_TAG} [DEVICE_SELECTION] Skipping auto-selection:`, {
					hasPreSelectedDevice: !!params.deviceId,
					needsDeviceSelection,
			...
			logger.error(`${MODULE_TAG} Initialize device authentication error`, error);
			throw error;
		}
	}
	...
	static async initializeDeviceAuthenticationAuth(
		...
		return MFAService.initializeDeviceAuthentication(params);
	}
	 ...
	 */
	/**
	 * @deprecated Use MfaAuthenticationService.readDeviceAuthentication() instead
	 * This method will be removed in the next major version
	 */
	static async readDeviceAuthentication(params: {
		...
		try {
			...
			logger.error(`${MODULE_TAG} Read device authentication error`, error);
			throw error instanceof Error ? error : new Error(String(error));
		}
	}
		...
		try {
			...
			if (!response.ok) {
				...
				if (typeof attemptsRemaining === 'number') {
					...
					logger.warn(`${MODULE_TAG} OTP validation attempts remaining:`, attemptsRemaining);
				}
			...
			logger.info(`${MODULE_TAG} OTP validated:`, validationData);
			return validationData;
		} catch (error) {
			logger.error(`${MODULE_TAG} Validate OTP for device error`, error);
			throw error;
		}
	}
	 ...
	 */
	/**
	 * @deprecated Use MfaAuthenticationService.cancelDeviceAuthentication() instead
	 * This method will be removed in the next major version
	 */
	static async cancelDeviceAuthentication(
		...
		try {
			...
			logger.info(`${MODULE_TAG} [CANCEL] Debug: Worker token before sending to backend`, {
				tokenLength: accessToken.length,
	 ...
	 */
	/**
	 * @deprecated Use MfaAuthenticationService.selectDeviceForAuthentication() instead
	 * This method will be removed in the next major version
	 */
	static async selectDeviceForAuthentication(
		...
		try {
			...
			logger.info(`${MODULE_TAG} [DEBUG] Request body being sent:`, {
				...requestBody,
			...
			if (!response.ok) {
				const errorData = responseData as PingOneResponse;
				logger.error(`${MODULE_TAG} [DEBUG] Server error response:`, {
					status: response.status,
					statusText: response.statusText,
			...
			logger.info(`${MODULE_TAG} Device selected:`, selectData);
			return selectData;
		} catch (error) {
			logger.error(`${MODULE_TAG} Select device for authentication error`, error);
			throw error;
		}
	}
	...
	static async listDeviceAuthenticationPolicies(
		...
		try {
			...
			try {
				...
				const errorMessage = error instanceof Error ? error.message : String(error);
				logger.error(`${MODULE_TAG} Network error fetching device authentication policies:`, {
					error: errorMessage,
					environmentId,
			...
			try {
				...
				logger.info(`${MODULE_TAG} Raw response text length:`, rawResponseText.length);
				logger.info(
				...
				if (!rawResponseText || !rawResponseText.trim()) {
					...
					try {
						...
						logger.error(`${MODULE_TAG} Failed to parse response as JSON:`, {
							status: response.status,
							statusText: response.statusText,
				...
				logger.error(`${MODULE_TAG} Failed to read response text:`, textError);
				responseData = {
			...
			if (!response.ok) {
				...
				logger.error(`${MODULE_TAG} Backend returned error:`, {
					status: response.status,
					statusText: response.statusText,
					errorData: errorData,
	...
	static async readDeviceAuthenticationPolicy(
		...
		try {
			...
			if (response.status === 404) {
				...
				const allPolicies = await MFAService.listDeviceAuthenticationPolicies(environmentId);
				const foundPolicy = allPolicies.find((p) => p.id === policyId);

				if (foundPolicy) {
					logger.info(`${MODULE_TAG} [DEVICE_SELECTION] Found policy via list endpoint:`, {
						id: foundPolicy.id,
						name: foundPolicy.name,
						authentication: foundPolicy.authentication,
						deviceSelection:
							foundPolicy.authentication && 'deviceSelection' in foundPolicy.authentication
								? foundPolicy.authentication.deviceSelection
								: undefined,
			...
			if (!response.ok) {
				const errorData = responseData as PingOneResponse;
				logger.error(`${MODULE_TAG} [DEVICE_SELECTION] Failed to read policy:`, {
					status: response.status,
					error: errorData,
					policyId,
				});
	...
	static async createDeviceAuthenticationPolicy(
		...
		try {
			...
			if (!response.ok) {
				...
				const errorMessage =
					errorData.message || errorData.error || response.statusText || 'Unknown error';
				logger.error(`${MODULE_TAG} Create policy error:`, {
					status: response.status,
					error: errorMessage,
	...
	static async updateDeviceAuthenticationPolicy(
		...
		try {
			...
			if (!response.ok) {
				...
				const errorMessage =
					errorData.message || errorData.error || response.statusText || 'Unknown error';
				logger.error(`${MODULE_TAG} [DEVICE_SELECTION] Update policy error:`, {
					status: response.status,
					error: errorMessage,
				});
