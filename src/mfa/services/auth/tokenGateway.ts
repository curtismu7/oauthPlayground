 ...
 * All other services (workerTokenService, MFATokenManager, etc.) must delegate here.
 *
 * Features:
...
/**
 * TokenGateway - Canonical token acquisition service
 *
 ...
 */
class TokenGateway {
	private static instance: TokenGateway | null = null;
	...
	private constructor() {
		this.log('TokenGateway initialized');
	}

	/**
	 * Get singleton instance
	 */
	static getInstance(): TokenGateway {
		if (!TokenGateway.instance) {
			TokenGateway.instance = new TokenGateway();
		}
		return TokenGateway.instance;
	}
	...
	static resetInstance(): void {
		if (TokenGateway.instance) {
			TokenGateway.instance.stopAutoRefresh();
			TokenGateway.instance.subscribers.clear();
			TokenGateway.instance = null;
		}
	}
	...
	async getWorkerTokenStatus(): Promise<TokenStatusInfo> {
		return WorkerTokenStatusService.checkWorkerTokenStatus();
	}
...
export const tokenGateway = TokenGateway.getInstance();
...
if (typeof window !== 'undefined') {
	(
		window as { tokenGateway?: typeof tokenGateway; TOKEN_GATEWAY_DEBUG?: boolean }
