...
// DualStorageService import removed - no longer needed for worker token storage
...
export class AppDiscoveryService {
	...
	static async getWorkerToken(): Promise<string | null> {
		...
		const stored = await AppDiscoveryService.getStoredWorkerToken();
		if (stored) {
			return stored;
		}
		...
		const token = await uiNotificationService.prompt({
	...
	/**
	 * Get stored worker token if valid (uses global workerTokenService)
	...
	static getStoredWorkerTokenSync(): string | null {
		try {
			...
			if (data.expiresAt && Date.now() > data.expiresAt) {
				...
				workerTokenService.clearToken().catch((err) => {
					logger.error(`${MODULE_TAG} Failed to clear expired token`, err);
				});
				return null;
			}
	...
	/**
	 * Store worker token (uses global workerTokenService)
	 ...
	 * @note This method requires credentials to be saved first via workerTokenService.saveCredentials()
	 */
	static async storeWorkerToken(token: string, expiresInSeconds?: number): Promise<void> {
		try {
			...
			const expiresIn = expiresInSeconds
				...
				: AppDiscoveryService.WORKER_TOKEN_EXPIRY;

			const expiresAt = Date.now() + expiresIn;

			// Save token using global service
			await workerTokenService.saveToken(token, expiresAt);
	...
	/**
	 * Clear stored worker token (uses global workerTokenService)
	...
	/**
	 * Get worker token expiry info (uses global workerTokenService)
	...
	static async refreshWorkerToken(): Promise<string | null> {
		...
		return AppDiscoveryService.getWorkerToken();
	}
	 ...
	 * @returns Array of discovered applications
	 * @example
	 * const apps = await AppDiscoveryService.discoverApps('12345678-1234-1234-1234-123456789012');
	 */
	static async discoverApps(environmentId: string): Promise<DiscoveredApplication[]> {
		...
		return AppDiscoveryService.discoverApplications(environmentId, workerToken);
	}
}
