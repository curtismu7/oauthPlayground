...
import { RedirectUriService } from '@/mfa/services/redirectUriService';
...
const PersistentLogger = {
	...
	log(
		...
		if (level === 'ERROR') {
			logger.error('mfaRedirectUriService', `[${category}] ${message}`, data || undefined);
		} else if (level === 'WARN') {
			logger.warn('mfaRedirectUriService', `[${category}] ${message}`, data || undefined);
		} else {
			logger.info('mfaRedirectUriService', `[${category}] ${message}`, data || undefined);
		}
	},
...
const MFARedirectUriDebugger = {
	...
	logRedirectUriDetails(flowType: string, redirectUri: string | null, fallback?: string): void {
		...
		logger.info('mfaRedirectUriService', `[Redirect URI Analysis] ${flowType}`);
	...
	logMigration(oldUri: string | undefined, newUri: string, flowType: string): void {
		...
		logger.info('mfaRedirectUriService', '[URI Migration]');
		logger.info(' Flow Type:', flowType);
	...
	logValidation(flowType: string, uri: string): void {
		...
		logger.info('mfaRedirectUriService', `[URI Validation] ${flowType}`);
		logger.info(' URI:', uri);
	...
	logAllFlowMappings(): void {
		...
		logger.info('mfaRedirectUriService', '[All Flow Mappings]');
...
export const MFARedirectUriService = {
	...
	getRedirectUri(flowType: string): string {
		// Prefer RedirectUriService (single source for V8/V8U and MFA paths and base URL)
		const from = RedirectUriService.getRedirectUriForFlow(flowType);
		const redirectUri = from || generateRedirectUriForFlow(flowType);
		const config = getFlowRedirectUriConfig(flowType);
