/**
 * @file hybridFlowIntegrationService.ts
 * @module v8/services
 ...
 *
 * @example
 * const authUrl = HybridFlowIntegrationService.generateAuthorizationUrl(credentials);
 * const tokens = await HybridFlowIntegrationService.exchangeCodeForTokens(credentials, code, codeVerifier);
 */
 ...
 */
export class HybridFlowIntegrationService {
	/**
	...
	static async generateAuthorizationUrl(
		...
		const state = HybridFlowIntegrationService.generateRandomString(32);

		// Generate nonce for OIDC (required for id_token)
		const nonce = HybridFlowIntegrationService.generateRandomString(32);
		...
		if (requiresJAR) {
			...
			try {
				...
				if (responseType.includes('code')) {
					const pkce = pkceCodes || (await HybridFlowIntegrationService.generatePKCECodes());
	...
	static decodeToken(token: string): DecodedToken {
		...
		try {
			...
			const header = JSON.parse(HybridFlowIntegrationService.base64UrlDecode(parts[0]));
			const payload = JSON.parse(HybridFlowIntegrationService.base64UrlDecode(parts[1]));
	...
	static isTokenValid(token: string): boolean {
		try {
			const decoded = HybridFlowIntegrationService.decodeToken(token);
	...
	static validateNonce(idToken: string, expectedNonce: string): boolean {
		try {
			const decoded = HybridFlowIntegrationService.decodeToken(idToken);
	...
	private static async generatePKCECodes(): Promise<{
		...
		const codeVerifier = HybridFlowIntegrationService.generateRandomString(128);

		// Generate code challenge from verifier using SHA256 (properly async)
		const codeChallenge = await HybridFlowIntegrationService.generateCodeChallenge(codeVerifier);
	...
	private static async generateCodeChallenge(codeVerifier: string): Promise<string> {
		try {
			...
			return HybridFlowIntegrationService.base64UrlEncode(codeVerifier);
		}
	}
...
export default HybridFlowIntegrationService;
