...
Why it matters: Balance between security and user experience. Different applications have different requirements.`,
	...
	static getTooltip(key: string): TooltipContent | null {
		const tooltips: Record<string, TooltipContent> = {
			'client-type': TooltipContentService.CLIENT_TYPE,
			'application-type': TooltipContentService.APPLICATION_TYPE,
			environment: TooltipContentService.ENVIRONMENT,
			specification: TooltipContentService.SPECIFICATION,
			'token-endpoint-auth': TooltipContentService.TOKEN_ENDPOINT_AUTH,
			scopes: TooltipContentService.SCOPES,
			'redirect-uri': TooltipContentService.REDIRECT_URI,
			'post-logout-redirect-uri': TooltipContentService.POST_LOGOUT_REDIRECT_URI,
			pkce: TooltipContentService.PKCE,
			'refresh-token': TooltipContentService.REFRESH_TOKEN,
			'grant-types': TooltipContentService.GRANT_TYPES,
			cors: TooltipContentService.CORS,
			'consent-flow': TooltipContentService.CONSENT_FLOW,
		};
...
export default TooltipContentService;
