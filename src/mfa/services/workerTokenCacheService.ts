/**
 * @file workerTokenCacheService.ts
 * @module v8/services
 ...
 */
export class WorkerTokenCacheService {
	private static instance: WorkerTokenCacheService;
	private readonly CACHE_KEY = 'worker-token-cache-v8';
	private readonly CACHE_DURATION = 24 * 60 * 60 * 1000; // 24 hours

	private constructor() {}

	static getInstance(): WorkerTokenCacheService {
		if (!WorkerTokenCacheService.instance) {
			WorkerTokenCacheService.instance = new WorkerTokenCacheService();
		}
		return WorkerTokenCacheService.instance;
	}
	...
	async cacheWorkerToken(
		...
		logger.info(`${MODULE_TAG} Caching worker token for validation`, 'Logger info');
		...
		try {
			...
			logger.info(`${MODULE_TAG} ✅ Worker token cached successfully`, 'Logger info');
		} catch (error) {
			logger.error(`${MODULE_TAG} ❌ Failed to cache worker token:`, error);
		}
	}
	...
	async getWorkerTokenForValidation(
		...
		logger.info(`${MODULE_TAG} Getting worker token for validation`, 'Logger info');

		// Try to get current token first
		const currentToken = await unifiedWorkerTokenService.getToken();
		if (currentToken) {
			logger.info(`${MODULE_TAG} ✅ Using current worker token`, 'Logger info');
			return {
				isValid: true,
				token: currentToken,
				source: 'current',
		...
		if (cachedToken) {
			logger.info(`${MODULE_TAG} ✅ Using cached worker token`, 'Logger info');
			return {
				isValid: true,
		...
		logger.info(`${MODULE_TAG} ❌ No worker token available for validation`, 'Logger info');
		return {
			isValid: false,
			token: null,
			source: 'none',
	...
	async updateCacheOnTokenGeneration(
		...
		logger.info(`${MODULE_TAG} Updating cache after token generation`, 'Logger info');

		const token = await unifiedWorkerTokenService.getToken();
		if (token) {
			await this.cacheWorkerToken(token, environmentId, clientId, scopes);
		} else {
			logger.warn(`${MODULE_TAG} ⚠️ No token available to cache after generation`, 'Logger warning');
		}
	}
	...
	async clearCache(): Promise<void> {
		logger.info(`${MODULE_TAG} Clearing worker token cache`, 'Logger info');

		try {
			await this.saveCachedToken(null);
			logger.info(`${MODULE_TAG} ✅ Cache cleared`, 'Logger info');
		} catch (error) {
			logger.error(`${MODULE_TAG} ❌ Failed to clear cache:`, error);
		}
	}
	...
	async cleanupExpiredCache(): Promise<void> {
		logger.info(`${MODULE_TAG} Cleaning up expired cache entries`, 'Logger info');

		const cachedToken = await this.getCachedToken();
		if (cachedToken && Date.now() > cachedToken.expiresAt) {
			await this.clearCache();
			logger.info(`${MODULE_TAG} ✅ Expired cache entry removed`, 'Logger info');
		}
	}
	...
	private async loadCachedToken(): Promise<CachedWorkerToken | null> {
		...
		try {
			...
			if (!token.token || !token.expiresAt || !token.environmentId || !token.clientId) {
				logger.warn(`${MODULE_TAG} ⚠️ Invalid cached token structure, removing`);
				await this.clearCache();
				return null;
			}
			...
			logger.error(`${MODULE_TAG} ❌ Failed to load cached token:`, error);
			await this.clearCache();
			return null;
		}
	}
	...
	private async getCachedToken(
		...
		if (Date.now() > token.expiresAt) {
			logger.info(`${MODULE_TAG} Cached token expired, removing`);
			await this.clearCache();
			return null;
		}
		...
		if (environmentId && token.environmentId !== environmentId) {
			logger.info(`${MODULE_TAG} Cached token environment mismatch, removing`);
			await this.clearCache();
			return null;
		}

		if (clientId && token.clientId !== clientId) {
			logger.info(`${MODULE_TAG} Cached token client ID mismatch, removing`);
			await this.clearCache();
			return null;
		}
...
export const workerTokenCacheService = WorkerTokenCacheService.getInstance();

// Make available globally for debugging
if (typeof window !== 'undefined') {
	(window as any).workerTokenCacheService = workerTokenCacheService;
}

export default workerTokenCacheService;
