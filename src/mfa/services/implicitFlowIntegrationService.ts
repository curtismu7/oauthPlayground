/**
 * @file implicitFlowIntegrationService.ts
 * @module v8/services
 ...
 *
 * @example
 * const service = new ImplicitFlowIntegrationService();
 ...
 */
export class ImplicitFlowIntegrationService {
	/**
	...
	static async generateAuthorizationUrl(
		...
		const state = ImplicitFlowIntegrationService.generateRandomString(32);

		// Generate nonce for OIDC (if using id_token)
		const nonce = ImplicitFlowIntegrationService.generateRandomString(32);
	...
	static decodeToken(token: string): DecodedToken {
		...
		try {
			...
			const header = JSON.parse(ImplicitFlowIntegrationService.base64UrlDecode(parts[0]));
			const payload = JSON.parse(ImplicitFlowIntegrationService.base64UrlDecode(parts[1]));
	...
	static validateNonce(idToken: string, expectedNonce: string): boolean {
		try {
			const decoded = ImplicitFlowIntegrationService.decodeToken(idToken);
...
export default ImplicitFlowIntegrationService;
