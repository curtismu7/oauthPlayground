/**
 * @file oauthIntegrationService.ts
 * @module v8/services
 ...
 *
 * @example
 * const service = new OAuthIntegrationService();
...
export class OAuthIntegrationService {
	...
	static async generatePKCECodes(): Promise<PKCECodes> {
		...
		const codeVerifier = OAuthIntegrationService.generateRandomString(128);

		// Generate code challenge from verifier using SHA256 (properly async)
		const codeChallenge = await OAuthIntegrationService.generateCodeChallenge(codeVerifier);
	...
	static async generateAuthorizationUrl(
		...
		const pkce = pkceCodes || (await OAuthIntegrationService.generatePKCECodes());

		// Generate state parameter for CSRF protection
		const state = OAuthIntegrationService.generateRandomString(32);
		...
		if (requiresJAR) {
			...
			try {
				const { jarRequestObjectService } = await import('./jarRequestObjectService');
				...
				const jarResult = await jarRequestObjectService.generateRequestObjectJWT(
					jarRequestParams,
					{
						algorithm,
						...
						if (credentials.environmentId && credentials.clientId) {
							...
							if (workerToken) {
								...
								if (appConfig) {
									...
									try {
										const { appDiscoveryService } = await import(
											'@/mfa/services/appDiscoveryService'
										);
										...
										const appWithSecret = await appDiscoveryService.fetchApplicationWithSecret(
											credentials.environmentId,
											credentials.clientId,
											workerToken
										);
								...
								window.dispatchEvent(new CustomEvent("worker-token-needed", { detail: { source: "oauthIntegrationService" } }));
...
• OAuth 2.0 Client Authentication: https://tools.ietf.org/html/rfc6749#section-2.3`;
	...
	static decodeToken(token: string): DecodedToken {
		...
		try {
			...
			const header = JSON.parse(OAuthIntegrationService.base64UrlDecode(parts[0]));
			const payload = JSON.parse(OAuthIntegrationService.base64UrlDecode(parts[1]));
	...
	private static async generateCodeChallenge(codeVerifier: string): Promise<string> {
		...
		if (crypto?.subtle) {
			...
			return OAuthIntegrationService.browserGenerateCodeChallenge(codeVerifier);
		}
		...
		return OAuthIntegrationService.base64UrlEncode(codeVerifier);
	}
