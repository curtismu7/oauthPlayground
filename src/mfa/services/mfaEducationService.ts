 ...
 * - Recovery flows
 */
export class MFAEducationService {
	/**
	...
	static getContent(key: string): MFAEducationContent {
		const content = MFAEducationService.educationContent[key];
...
- Recommended: 5 failures for most use cases`,
		...
		'policy.deviceType.configuration': {
			...
			description: `Each device type (EMAIL, OATH TOTP (RFC 6238), SMS, VOICE, MOBILE, FIDO2, WHATSAPP) can have specific configuration options in the policy.
...
**Note:** This is a policy-level setting that affects all devices registered under that policy. You cannot override it for individual devices.`,
		...
		'otp.validation': {
			...
			description:
				'Enter the one-time passcode (OTP) sent to your device or generated by your authenticator app. For SMS/Email devices, the code is sent automatically. For OATH TOTP (RFC 6238) devices, use the 6-digit code from your authenticator app (Google Authenticator, Authy, etc.). The code typically expires after 5-10 minutes.',
		...
		// ===== TOTP SPECIFIC =====
		'totp.qrCode': {
			title: 'OATH TOTP (RFC 6238) QR Code',
			description:
				'Scan this QR code with your authenticator app (Google Authenticator, Authy, Microsoft Authenticator, etc.) to set up OATH TOTP (RFC 6238). The QR code contains your secret key and account information.',
		},
		...
		'totp.verification': {
			title: 'OATH TOTP (RFC 6238) Verification',
			description:
				'After scanning the QR code, your authenticator app will generate a 6-digit code using OATH TOTP (RFC 6238). Enter this code to verify the setup is working correctly.',
		},
...
**Key Point:** Biometrics are a **method** of user verification within WebAuthn, not a separate authentication technology.`,
	...
	static getAllFactorTypes(): Array<{ key: string; content: MFAEducationContent }> {
		return [
			{ key: 'SMS', content: MFAEducationService.getContent('factor.sms') },
			{ key: 'EMAIL', content: MFAEducationService.getContent('factor.email') },
			{ key: 'WHATSAPP', content: MFAEducationService.getContent('factor.whatsapp') },
			{ key: 'TOTP', content: MFAEducationService.getContent('factor.totp') },
			{ key: 'FIDO2', content: MFAEducationService.getContent('factor.fido2') },
		];
	}
}

export default MFAEducationService;
