/**
 * @file clientCredentialsIntegrationService.ts
 * @module v8/services
 ...
 *
 * @example
 * const tokens = await ClientCredentialsIntegrationService.requestToken(credentials);
 */

import { pingOneFetch } from '@/utils/pingOneFetch';
import { logger } from '../../utils/logger';
import { ScopeFixService } from './scopeFixService';
 ...
 */
export class ClientCredentialsIntegrationService {
	/**
	...
	static getScopeFix(credentials: ClientCredentialsCredentials): ScopeFixResult {
		return ScopeFixService.analyzeAndFixScopes({
	...
	static async requestToken(credentials: ClientCredentialsCredentials): Promise<TokenResponse> {
		...
		try {
			...
			if (!credentials.scopes || credentials.scopes.trim() === '') {
				logger.error(
					`${MODULE_TAG} No scopes provided - client credentials flow requires scopes`,
					'Logger error'
				);
				throw new Error(
			...
			if (invalidScopes.length > 0) {
				logger.error(`${MODULE_TAG} Invalid scopes for client credentials flow`, {
					invalidScopes,
			...
			switch (authMethod) {
				...
				case 'client_secret_jwt': {
					...
					try {
						...
						logger.info(
							`${MODULE_TAG} Using Client Secret JWT assertion authentication`,
							'Logger info'
						);
					} catch (error) {
				...
				case 'private_key_jwt': {
					...
					try {
						...
						proxyRequestBody.client_assertion = assertion;
						logger.info(
							`${MODULE_TAG} Using Private Key JWT assertion authentication`,
							'Logger info'
						);
					} catch (error) {
			...
			if (authMethod === 'client_secret_post' && proxyRequestBody.client_secret) {
				...
				if (proxyRequestBody.client_secret.trim().length === 0) {
					logger.error(
						`${MODULE_TAG} ❌ ERROR: client_secret is empty or whitespace!`,
						'Logger error'
					);
	...
	static decodeToken(token: string): DecodedToken {
		...
		try {
			...
			const header = JSON.parse(ClientCredentialsIntegrationService.base64UrlDecode(parts[0]));
			const payload = JSON.parse(ClientCredentialsIntegrationService.base64UrlDecode(parts[1]));
	...
	static isTokenValid(token: string): boolean {
		try {
			const decoded = ClientCredentialsIntegrationService.decodeToken(token);
...
export default ClientCredentialsIntegrationService;
