 ...
 * const isValid = SpecVersionService.isFlowAvailable('oauth2.1', 'implicit');
 */
...
export class SpecVersionService {
	...
	static isFlowAvailable(specVersion: SpecVersion, flowType: FlowType): boolean {
		const available = SpecVersionService.getAvailableFlows(specVersion).includes(flowType);
	...
	static validateConfiguration(
		...
		if (!SpecVersionService.isFlowAvailable(specVersion, flowType)) {
			errors.push(`${flowType} is not available in ${specVersion}`);
		}

		// Get compliance rules
		const rules = SpecVersionService.getComplianceRules(specVersion);
...
export default SpecVersionService;
