...
export class OidcDiscoveryService {
	...
	static async discover(issuerUrl: string, environmentId?: string): Promise<DiscoveryResult> {
		...
		try {
			...
			const normalized = OidcDiscoveryService.normalizeIssuerUrl(issuerUrl);

			// Try cache first
			if (typeof window !== 'undefined') {
				try {
					const { getCachedDiscoveryDocument } = await import('./discoveryCacheService');
...
Original error: ${errorMessage}`;
			...
			if (typeof window !== 'undefined') {
				try {
					const { cacheDiscoveryDocument } = await import('./discoveryCacheService');
	...
	static async discoverFromInput(input: string): Promise<DiscoveryResult> {
		...
		if (OidcDiscoveryService.isValidEnvironmentId(input)) {
			...
			return OidcDiscoveryService.discover(issuerUrl);
		}

		// Otherwise treat as issuer URL
		if (OidcDiscoveryService.isValidIssuerUrl(input)) {
			return OidcDiscoveryService.discover(input);
		}
