...
import { backendConnectivityService } from './backendConnectivityService';

const MODULE_TAG = '[UserService]';
...
export class UserService {
	 ...
	 * const result = await UserService.listUsers(envId);
	 *
	 * // Search for users
	 * const searched = await UserService.listUsers(envId, { search: 'curtis' });
	 *
	 * // Paginate through users
	 * const page2 = await UserService.listUsers(envId, { limit: 200, offset: 200 });
	 * ```
	 */
	static async listUsers(
		...
		try {
			...
			if (!accessToken) {
				window.dispatchEvent(new CustomEvent("worker-token-needed", { detail: { source: "userService" } }));
	...
	static async searchAllUsers(
		...
		while (page < maxPages) {
			logger.info(
				`${MODULE_TAG} searchAllUsers: fetching page ${page + 1} at offset ${offset}`,
				'Logger info'
			);

			const result = await UserService.listUsers(environmentId, {
				search,
				limit,
				offset,
				region,
	 ...
	 * const users = await UserService.fetchAllUsers(envId);
	 *
	...
	static async fetchAllUsers(
		...
		try {
			while (hasMore && fetchedPages < maxPages) {
				const result = await UserService.listUsers(environmentId, { limit, offset });
