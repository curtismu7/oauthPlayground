/**
 * @file unifiedMFAResumeStepResolver.ts
 * @module v8/services
...
export class UnifiedMFAResumeStepResolver {
	/**
	...
	private static resolveFromServerSession(
		...
		if (serverSessionStep) {
			...
			if (UnifiedMFAResumeStepResolver.isValidStep(step) && step !== 0) {
				return {
					step,
	...
	private static resolveFromStateParam(context: StepResolverContext): StepResolutionResult | null {
		...
		if (callbackStep) {
			...
			if (UnifiedMFAResumeStepResolver.isValidStep(step) && step !== 0) {
				...
				if (!timestamp || UnifiedMFAResumeStepResolver.isStateExpired(timestamp)) {
	...
	private static resolveFromClientStorage(
		...
		if (flowState) {
			try {
				...
				if (
					parsed.currentStep &&
					UnifiedMFAResumeStepResolver.isValidStep(parsed.currentStep) &&
	...
	public static resolveResumeStep(currentUrl: string = window.location.href): StepResolutionResult {
		const correlationId = UnifiedMFAResumeStepResolver.generateCorrelationId();
		const startTime = Date.now();

		// Build context
		const searchParams = new URL(window.location.href).searchParams;
		const { sessionStorage, localStorage } = UnifiedMFAResumeStepResolver.getStorageData();

		const context: StepResolverContext = {
			currentUrl: UnifiedMFAResumeStepResolver.sanitizeUrl(currentUrl),
			searchParams,
			sessionStorage,
			localStorage,
			correlationId,
			redirectParams: UnifiedMFAResumeStepResolver.sanitizeRedirectParams(searchParams),
		};
		...
		const sources = [
			{
				resolver: UnifiedMFAResumeStepResolver.resolveFromServerSession,
				source: StepResolutionSource.SERVER_SESSION,
			},
			{
				resolver: UnifiedMFAResumeStepResolver.resolveFromStateParam,
				source: StepResolutionSource.STATE_PARAM,
			},
			{
				resolver: UnifiedMFAResumeStepResolver.resolveFromClientStorage,
				source: StepResolutionSource.CLIENT_STORAGE,
			},
			{
				resolver: UnifiedMFAResumeStepResolver.resolveFromLastKnownStep,
				source: StepResolutionSource.LAST_KNOWN_STEP,
			},
		];
		...
		for (const { resolver, source } of sources) {
			try {
				const result = resolver.call(UnifiedMFAResumeStepResolver, context);
				if (result) {
					...
					UnifiedMFAResumeStepResolver.logDecision(
						correlationId,
						context,
						result,
						rejectedSources
		...
		const fallbackResult = UnifiedMFAResumeStepResolver.resolveFallbackStep2(context);
		UnifiedMFAResumeStepResolver.logDecision(
			correlationId,
			context,
			fallbackResult,
	...
	public static isRedirectResumeScenario(): boolean {
		const searchParams = new URL(window.location.href).searchParams;
		const { sessionStorage } = UnifiedMFAResumeStepResolver.getStorageData();
...
export default UnifiedMFAResumeStepResolver;
