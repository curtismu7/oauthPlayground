/**
 * @file preFlightValidationService.ts
 * @module v8/services
...
import { ConfigCheckerService } from './configCheckerService';
 ...
 */
export const PreFlightValidationService = {
	/**
	...
	async validateOAuthConfig(options: PreFlightValidationOptions): Promise<{
		...
		try {
			...
			if (flowType === 'oauth-authz' || flowType === 'hybrid') {
				...
				const complianceRules = SpecVersionService.getComplianceRules(specVersion);
...
✅ Your PingOne application requires signed request objects (JAR), and you have provided the necessary signing credentials. The OAuth Playground will automatically generate a signed JWT request object when you generate the authorization URL.
 ...
 For more information about JAR, see the "JAR (JWT-secured Authorization Request)" section in Advanced Options.`
	...
	async validateBeforeAuthUrl(
		...
		const redirectUriResult = await PreFlightValidationService.validateRedirectUri(
			options.credentials,
			options.workerToken
		);

		// 2. Validate OAuth configuration
		const oauthConfigResult = await PreFlightValidationService.validateOAuthConfig(options);
