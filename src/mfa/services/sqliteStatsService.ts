 ...
 */
export class SQLiteStatsService {
	...
	private static async executeWithMonitoring<T>(
		...
		if (circuitBreakerOpen) {
			if (SQLiteStatsService.shouldResetCircuitBreaker()) {
				SQLiteStatsService.resetCircuitBreaker();
			} else {
			...
			while (attempt < MAX_RETRIES) {
				try {
					...
					if (connectionErrors >= CIRCUIT_BREAKER_THRESHOLD) {
						SQLiteStatsService.openCircuitBreaker();
	...
	static async getUserCount(environmentId: string): Promise<SQLiteUserStats> {
		...
		// Check cache first
		const cached = SQLiteStatsService.userCountCache.get(environmentId);
		if (cached && Date.now() - cached.timestamp < SQLiteStatsService.CACHE_TTL) {
		...
		try {
			const response = await SQLiteStatsService.executeWithMonitoring(
			...
			// Cache the result
			SQLiteStatsService.userCountCache.set(environmentId, {
				count,
				timestamp: Date.now(),
			});
	...
	static async getSyncMetadata(environmentId: string): Promise<SQLiteSyncMetadata> {
		...
		try {
			...
			const countResult = await SQLiteStatsService.getUserCount(environmentId);
	...
	static clearCache(): void {
		SQLiteStatsService.userCountCache.clear();
	...
	static clearCacheForEnvironment(environmentId: string): void {
		SQLiteStatsService.userCountCache.delete(environmentId);
		logger.info(`${MODULE_TAG} Cache cleared for ${environmentId}`, 'Logger info');
	}
}

export default SQLiteStatsService;
