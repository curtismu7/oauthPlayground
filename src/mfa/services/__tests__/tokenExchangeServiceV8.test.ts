// src/mfa/services/__tests__/tokenExchangeServiceV8.test.ts
...
describe('TokenExchangeServiceV8', () => {
	...
	describe('exchangeToken', () => {
		it('should fail when Token Exchange is disabled', async () => {
			...
			await expect(TokenExchangeServiceV8.exchangeToken(params, mockEnvironmentId)).rejects.toThrow(
	...
	describe('validateToken', () => {
		it('should validate a valid token', async () => {
			const result = await TokenExchangeServiceV8.validateToken(
				mockSubjectToken,
				mockEnvironmentId
			);
