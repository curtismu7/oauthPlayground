/**
 * @file appDiscoveryService.test.ts
 * @module v8/services/__tests__
 * @description Tests for AppDiscoveryService
 * @version 8.0.0
 * @since 2024-11-16
 */

import { AppDiscoveryService, DiscoveredApplication } from '../appDiscoveryService';
...
describe('AppDiscoveryService', () => {
	beforeEach(async () => {
		...
		await workerTokenService.clearCredentials();
	...
	describe('worker token management', () => {
		it('should store and retrieve worker token', async () => {
			...
			await AppDiscoveryService.storeWorkerToken(token);

			const retrieved = await AppDiscoveryService.getStoredWorkerToken();
		...
		it('should clear worker token', async () => {
			await AppDiscoveryService.storeWorkerToken('test-token');
			await AppDiscoveryService.clearWorkerToken();
		...
		it('should validate worker token format', () => {
			expect(AppDiscoveryService.isValidWorkerToken('short')).toBe(false);
			expect(AppDiscoveryService.isValidWorkerToken('this-is-a-valid-worker-token-xyz')).toBe(
				true
			);
		});
	...
	describe('app configuration', () => {
		it('should get app config for auto-fill', () => {
			const config = AppDiscoveryService.getAppConfig(mockApp);
		...
		it('should set PKCE based on app requirements', () => {
			...
			const configWithPkce = AppDiscoveryService.getAppConfig(appWithPkce);
			expect(configWithPkce.usePkce).toBe(true);

			const appWithoutPkce = { ...mockApp, pkceRequired: false, pkceEnforced: false };
			const configWithoutPkce = AppDiscoveryService.getAppConfig(appWithoutPkce);
	...
	describe('application lookup', () => {
		it('should find application by ID', () => {
			const apps = [mockApp, { ...mockApp, id: 'app-456', name: 'Another App' }];
			const found = AppDiscoveryService.getApplicationById(apps, 'app-123');
		...
		it('should return null for non-existent application', () => {
			const apps = [mockApp];
			const found = AppDiscoveryService.getApplicationById(apps, 'non-existent');
	...
	describe('app type handling', () => {
		it('should handle different app types', () => {
			...
			const spaConfig = AppDiscoveryService.getAppConfig(spaApp);
			const nativeConfig = AppDiscoveryService.getAppConfig(nativeApp);
			const serviceConfig = AppDiscoveryService.getAppConfig(serviceApp);
	...
	describe('default values', () => {
		it('should provide default values for missing fields', () => {
			...
			const config = AppDiscoveryService.getAppConfig(minimalApp);
