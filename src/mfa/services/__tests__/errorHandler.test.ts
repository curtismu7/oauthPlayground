 ...
 * @description Tests for ErrorHandler
 * @version 8.0.0
 * @since 2024-11-16
 */
...
describe('ErrorHandler', () => {
	...
	describe('getUserMessage', () => {
		...
		it('should handle string errors', () => {
			const message = ErrorHandler.getUserMessage('access_denied');
	...
	describe('getCode', () => {
		it('should return error code', () => {
			const error = new Error('invalid_grant');
			expect(ErrorHandler.getCode(error)).toBe('INVALID_GRANT');
		});

		it('should return undefined for unknown errors', () => {
			const error = new Error('Something weird');
			expect(ErrorHandler.getCode(error)).toBe('UNKNOWN_ERROR');
		});
	});
	...
	describe('fromOAuthError', () => {
		it('should create error from OAuth error response', () => {
			...
			const error = ErrorHandler.fromOAuthError(errorResponse);
