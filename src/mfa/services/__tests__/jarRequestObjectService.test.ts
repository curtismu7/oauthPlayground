/**
 * @file jarRequestObjectService.test.ts
 * @module v8/services/__tests__
 * @description Tests for JARRequestObjectService (JWT-secured Authorization Request)
 * @version 8.0.0
 * @since 2025-01-XX
 */
...
import {
	JARRequestObjectService,
	type JARSigningConfig,
	jarRequestObjectService,
	type OAuthAuthorizationParams,
} from '../jarRequestObjectService';

describe('JARRequestObjectService', () => {
...
-----END PRIVATE KEY-----`,
	...
	describe('buildRequestObjectPayload', () => {
		it('should build request object payload with required parameters', () => {
			const service = new JARRequestObjectService();
	...
	describe('generateRequestObjectJWT - HS256', () => {
		it('should generate signed request object JWT with HS256', async () => {
			const result = await jarRequestObjectService.generateRequestObjectJWT(
				mockAuthParams,
				mockHS256Config
			);
