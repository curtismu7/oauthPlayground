...
export class MfaAuthenticationService {
	...
	static async initializeDeviceAuthentication(
		...
		try {
			...
			let policy: import('@/mfa/flows/shared/MFATypes').DeviceAuthenticationPolicy | null = null;
			...
			let user: import('./mfaService').UserLookupResult | null = null;
			...
			if (policy?.skipUserLockVerification !== true && params.username) {
				...
				const { MFAService } = await import('./mfaService');
				try {
					user = await MFAService.lookupUserByUsername(params.environmentId, params.username);
			...
			const authPath = MfaAuthenticationService.getAuthBaseUrl(
				params.region,
				params.customDomain
			);
			...
			logger.info(`${MODULE_TAG} Sending request to backend:`, {
		...
		try {
			...
			if (options?.isUserId) {
				...
				if (uuidRegex.test(usernameOrUserId)) {
					...
					const { MFAService } = await import('./mfaService');
					const user = await MFAService.lookupUserByUsername(environmentId, usernameOrUserId);
					userId = user.id as string;
				}
			}
	...
	static async selectDeviceForAuthentication(
		...
		try {
			...
			try {
				const authData = await MfaAuthenticationService.readDeviceAuthentication(
					params.environmentId,
					userId,
	...
	private static async getWorkerTokenWithAutoRenew(): Promise<string> {
		...
		const { MFAConfigurationService } = await import('@/mfa/services/mfaConfigurationService');
		...
		if (needsRenewal) {
			if (!autoRenewalEnabled) {
				...
				try {
					...
					await workerTokenService.saveToken(newToken, expiresAt);
					logger.info(
	...
	static async cancelDeviceAuthentication(
		...
		try {
			...
			// Determine base URL - use custom domain if provided, otherwise use region-based domain
			const authPath = MfaAuthenticationService.getAuthBaseUrl(region, customDomain);
	...
	static async checkFIDO2Assertion(
		...
		try {
			...
			if (!finalEnvironmentId) {
				// Use 'mfa-flow-v8' to match the flow key used in MFAAuthenticationMainPage
			...
			// 	body: JSON.stringify({
			// 		location: 'mfaAuthenticationService.ts:1724',
			...
			// 	headers: { 'Content-Type': 'application/json' },
			// 	body: JSON.stringify({
			// 		location: 'mfaAuthenticationService.ts:1774',
			...
			// 	headers: { 'Content-Type': 'application/json' },
			// 	body: JSON.stringify({
			// 		location: 'mfaAuthenticationService.ts:1802',
	...
	static async resendOTPForActiveDevice(params: {
		...
		try {
			...
			try {
				authData = await MfaAuthenticationService.readDeviceAuthentication(
					params.environmentId,
					userId,
					params.authenticationId,
					{
			...
			const links = MfaAuthenticationService.extractAvailableLinks(authData);
			...
			if (links.cancel) {
				...
				try {
					...
					await MfaAuthenticationService.cancelDeviceAuthentication(
						params.environmentId,
						params.username,
						params.authenticationId,
						params.region,
					...
					const { MFAService } = await import('./mfaService');
					const newAuthResult = await MFAService.sendOTP({
						environmentId: params.environmentId,
						username: params.username,
						deviceId: params.deviceId,
					});
					...
					const newAuthData = await MfaAuthenticationService.readDeviceAuthentication(
						params.environmentId,
						userId,
					...
					logger.info(
						`${MODULE_TAG} Successfully resent OTP via cancel + re-initialize`,
						'Logger info'
					);
					return newAuthData;
				} catch (cancelError) {
			...
			const reselectResult = await MfaAuthenticationService.selectDeviceForAuthentication(
				{
					environmentId: params.environmentId,
					username: params.username,
					userId,
			...
			try {
				const updatedAuthData = await MfaAuthenticationService.readDeviceAuthentication(
					params.environmentId,
					userId,
					params.authenticationId,
					{
