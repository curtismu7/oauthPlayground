...
export class ToastNotifications {
	...
	static success(message: string, options?: { duration?: number }): void {
		logger.info(`${ToastNotifications.MODULE_TAG} Success:`, message);
	...
	static error(message: string, _options?: { duration?: number }): void {
		logger.info(`${ToastNotifications.MODULE_TAG} Error:`, message);
		const shortMessage = ToastNotifications.extractShortSummary(message);
	...
	static warning(message: string, _options?: { duration?: number }): void {
		logger.info(`${ToastNotifications.MODULE_TAG} Warning:`, message);
	...
	static info(message: string, _options?: { duration?: number }): void {
		logger.info(`${ToastNotifications.MODULE_TAG} Info:`, message);
	...
	static copiedToClipboard(itemName: string): void {
		ToastNotifications.success(`${itemName} copied to clipboard`);
	}
	...
	static validationError(fields: string[]): void {
		...
		ToastNotifications.error(`Please fill in required fields: ${fieldList}`);
	}
	...
	static networkError(operation: string): void {
		ToastNotifications.error(`Network error during ${operation}. Please check your connection.`);
	}
	...
	static stepCompleted(stepNumber: number): void {
		ToastNotifications.success(`Step ${stepNumber} completed`);
	}
	...
	static flowCompleted(): void {
		ToastNotifications.success(' OAuth Flow Complete!', { duration: 8000 });
	}
	...
	static processing(operation: string): void {
		ToastNotifications.info(`${operation}...`);
	}
	...
	static credentialsSaved(): void {
		ToastNotifications.success('Credentials saved successfully');
	}
	...
	static credentialsLoaded(): void {
		ToastNotifications.success('Credentials loaded successfully');
	}
	...
	static pkceGenerated(): void {
		ToastNotifications.success('PKCE parameters generated successfully');
	}
	...
	static authUrlGenerated(): void {
		ToastNotifications.success('Authorization URL generated successfully');
	}
	...
	static tokenExchangeSuccess(): void {
		ToastNotifications.success('Tokens exchanged successfully');
	}
	...
	static tokenIntrospectionSuccess(): void {
		ToastNotifications.success('Token introspection completed successfully');
	}
	...
	static userInfoFetched(): void {
		ToastNotifications.success('User information retrieved successfully');
	}
	...
	static appDiscoverySuccess(): void {
		ToastNotifications.success('Application discovered successfully');
	}
	...
	static configurationChecked(): void {
		ToastNotifications.success('Configuration check completed');
	}
	...
	static flowReset(): void {
		ToastNotifications.success('Flow reset. Tokens cleared, credentials preserved.');
	}
	...
	static scopeRequired(scope: string): void {
		ToastNotifications.warning(`Added required "${scope}" scope for compliance`);
	}
	...
	static discoveryEndpointLoaded(): void {
		ToastNotifications.success('Discovery endpoint loaded successfully');
	}
	...
	static environmentIdExtracted(): void {
		ToastNotifications.success('Environment ID extracted from discovery');
	}
	...
	static formattedError(message: string, detail?: string): void {
		const formattedMessage = detail
			? `Message: ${message} | Detail: ${detail}`
			: `Message: ${message}`;
		ToastNotifications.error(formattedMessage);
	}
	...
	static formattedWarning(message: string, detail?: string): void {
		const formattedMessage = detail
			? `Message: ${message} | Detail: ${detail}`
			: `Message: ${message}`;
		ToastNotifications.warning(formattedMessage);
	}
	...
	static formattedInfo(message: string, detail?: string): void {
		const formattedMessage = detail
			? `Message: ${message} | Detail: ${detail}`
			: `Message: ${message}`;
		ToastNotifications.info(formattedMessage);
	}
	...
	static mfaOperationError(operation: string, reason: string): void {
		...
		const detail = reason;
		ToastNotifications.formattedError(message, detail);
	}
...
export const toast = ToastNotifications;
