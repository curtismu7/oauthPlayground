...
export const MFAConfigurationStep: React.FC<MFAConfigurationStepV8Props> = ({
	credentials,
	setCredentials,
	tokenStatus,
	deviceAuthPolicies,
	...
	React.useEffect(() => {
		if (!credentials.environmentId && tokenStatus.isValid) {
			const workerCreds = workerTokenService.loadCredentialsSync();
	...
	React.useEffect(() => {
		...
		if (credentials.userToken !== userToken) {
			if (credentials.userToken) {
				logger.info(
					`[⚙️ MFA-CONFIG-STEP-V8] ✅ Syncing token from credentials to local state`,
					'Logger info'
				);
	...
	const getWorkerTokenStatusDisplay = () => {
		return {
			icon: WorkerTokenStatusService.getStatusIcon(tokenStatus.status),
			message: tokenStatus.message,
			color: WorkerTokenStatusService.getStatusColor(tokenStatus.status),
							...
							<button
								...
								onClick={async () => {
									if (tokenStatus.isValid) {
										...
										const _newStatus = WorkerTokenStatusService.checkWorkerTokenStatus();
										// #region agent log
										// #endregion
