...
import { MFADeviceLimitModal } from '@/mfa/components/MFADeviceLimitModal';
import { MFANavigation } from '@/mfa/components/MFANavigation';
import { MFASettingsModal } from '@/mfa/components/MFASettingsModal';
import { MFAUserDisplay } from '@/mfa/components/MFAUserDisplay';
import { MFAWaitScreen } from '@/mfa/components/MFAWaitScreen';
import StepActionButtons from '@/mfa/components/StepActionButtons';
import StepValidationFeedback from '@/mfa/components/StepValidationFeedback';
import { UserLoginModal } from '@/mfa/components/UserLoginModal';
import { WorkerTokenModal } from '@/components/WorkerTokenModal';
import { WorkerTokenPromptModal } from '@/mfa/components/WorkerTokenPromptModal';
...
import { UnifiedMFAResumeStepResolver } from '@/mfa/services/unifiedMFAResumeStepResolver';
import {
...
export const MFAFlowBase: React.FC<MFAFlowBaseProps> = ({
	deviceType,
	renderStep0,
	renderStep1,
	renderStep2,
	...
	useEffect(() => {
		...
		// Set initial state
		setApiDisplayVisible(apiDisplayService.isVisible());
		return unsubscribe;
	}, []);
	...
	useEffect(() => {
		...
		if (!UnifiedMFAResumeStepResolver.isRedirectResumeScenario()) {
			return;
		}

		// Use the fool-proof step resolver
		const resolutionResult = UnifiedMFAResumeStepResolver.resolveResumeStep();
	...
	const [credentials, setCredentials] = useState<MFACredentials>(() => {
		...
		if (!stored.userToken) {
			try {
				const userLoginCreds = CredentialsService.loadCredentials('user-login-v8', {
				...
				logger.warn(`${MODULE_TAG} Initial user token sync failed (non-critical):`, error);
			}
		}
	...
	const fetchDeviceAuthPolicies = useCallback(async () => {
		...
		const currentStatus = await WorkerTokenStatusService.checkWorkerTokenStatus();
	...
	useEffect(() => {
		...
		const initialStatus = WorkerTokenStatusService.checkWorkerTokenStatusSync();
	...
	const syncUserTokenFromUserLogin = useCallback(() => {
		try {
			...
			logger.warn(`${MODULE_TAG} Backup sync failed (non-critical):`, error);
		}
	...
	useEffect(() => {
		...
		// #region agent log
		sendAnalyticsLog({
			location: 'MFAFlowBase.tsx:319',
			message: 'Checking for OAuth callback code in MFAFlowBase',
			data: {
				hasCode: !!code,
		...
		if (code && hasUserLoginState && !showUserLoginModal) {
			...
			if (receivedState !== storedState) {
				logger.error(
					`${MODULE_TAG} ❌ OAuth state mismatch - possible CSRF attack`,
					'Logger error'
				);
				modernMessaging.showBanner({
					type: 'error',
		...
		if (isOAuthCallbackReturn && credentials.userToken?.trim()) {
			...
			if (storedFlowState) {
				try {
					...
					logger.info(`${MODULE_TAG} Found stored flow state after OAuth callback:`, flowState);
					...
					if (flowState.deviceStatus === 'ACTIVATION_REQUIRED' && flowState.deviceId) {
						logger.info(`${MODULE_TAG} Device requires activation, skipping registration step`);
					...
					logger.error(`${MODULE_TAG} Failed to parse stored flow state:`, error);
					sessionStorage.removeItem('mfa_flow_state_after_oauth');
				}
			}
			...
			setTimeout(() => {
				if (nav.currentStep === 0) {
					...
					logger.info(`${MODULE_TAG} User was on Step 0, validating and advancing normally`);
					if (validateStep0(credentials, tokenStatus, nav)) {
						...
						logger.info(`${MODULE_TAG} Step 0 validation failed, staying on Step 0`);
					}
					...
					logger.info(`${MODULE_TAG} User was on Step ${nav.currentStep}, validating current step`);
		...
		if (hasWorkerTokenError && !showWorkerTokenPromptModal) {
			...
			void (async () => {
				try {
					...
					logger.warn(
						`${MODULE_TAG} Config error - not showing modal automatically`,
						'Logger warning'
					);
				}
			})();
	...
	const isNextDisabled = () => {
		...
		if (nav.currentStep === 0) {
			...
			try {
				...
				logger.warn(`${MODULE_TAG} Backup credential check failed, using current state:`, error);
			}
			const isTokenValid =
		...
		<div className="mfa-flow-v8">
			...
			<div className="flow-container">
				...
				<div className="step-content-wrapper" style={{ paddingBottom: '12px' }}>
					{nav.isTransitioning ? (
						<MFAWaitScreen message={`Loading ${stepLabels[nav.currentStep] || 'next step'}...`} />
					) : (
						renderStepContent()
					)}
				</div>
				...
				<StepValidationFeedback
					...
					onValidationRecheck={() => {
						logger.info(
							`${MODULE_TAG} Rechecking validation after worker token refresh`,
							'Logger info'
						);
						...
						if (nav.currentStep === 0) {
							...
							if (validateStep0(credsToValidate, tokenStatus, nav)) {
								logger.info(
									`${MODULE_TAG} Step 0 validation passed after worker token refresh`,
									'Logger info'
								);
							} else {
								logger.info(
					...
					onWorkerTokenRefresh={async () => {
						...
						modernMessaging.showFooterMessage({
							type: 'info',
							message: 'Worker token refreshed successfully!',
							duration: 3000,
						});
						...
						<button
							...
							onClick={async () => {
								...
								try {
									...
									await MFAService.cancelDeviceAuthentication({
										...credentials,
										authenticationId: mfaState.authenticationId,
									...
									modernMessaging.showFooterMessage({
										type: 'info',
										message: 'Authentication canceled successfully',
										duration: 3000,
									});
								} catch (error) {
									...
									modernMessaging.showBanner({
										type: 'error',
										title: 'Error',
										message: `Failed to cancel authentication: ${errorMessage}`,
										dismissible: true,
									});
				...
				<StepActionButtons
					...
					onPrevious={() => {
						...
						if (deviceType === 'TOTP') {
							...
							// Note: Modal state is managed in TOTPFlow, so we just navigate
						...
						if (nav.canGoPrevious) {
							...
							logger.warn(
								`${MODULE_TAG} Previous button clicked but canGoPrevious is false`,
								'Logger warning'
							);
						}
					}}
					onNext={() => {
						if (nav.currentStep === 0) {
							...
							if (!credentials.environmentId?.trim() || !credentials.username?.trim()) {
								logger.info(`${MODULE_TAG} Missing credentials, showing UserLoginModal`);
			...
			{showWorkerTokenModal &&
				(() => {
					...
					try {
						// Use existing MFAConfigurationService from imports
						const config = MFAConfigurationService.loadConfiguration();
						const currentTokenStatus = WorkerTokenStatusService.checkWorkerTokenStatusSync();
				...
				<UserLoginModal
					...
					onCredentialsSaved={() => {
						...
						if (savedCreds.environmentId || savedCreds.clientId) {
							...
							logger.info(
								`${MODULE_TAG} Synced saved credentials from User Login Modal`,
								'Logger info'
							);
						}
					}}
