/**
 * @file MobileOTPConfigurationPage.tsx
 * @module v8/flows/types
 * @description Mobile/OTP Configuration and Education Page
 * @version 8.0.0
 *
 * This page provides:
 * - Mobile/OTP education and information
...
import { UserLoginSection } from '@/mfa/components/UserLoginSection';
...
import { OAuthIntegrationService } from '@/mfa/services/oauthIntegrationService';
...
export const MobileOTPConfigurationPage: React.FC = () => {
	...
	const [credentials, setCredentials] = useState<MFACredentials>(() => {
		...
		// #region agent log
		sendAnalyticsLog({
			location: 'MobileOTPConfigurationPage.tsx:45',
	...
	useEffect(() => {
		...
		// #region agent log
		sendAnalyticsLog({
			location: 'MobileOTPConfigurationPage.tsx:140',
			message: 'MobileOTPConfigurationPage checking for callback code',
			data: {
				hasCode: !!code,
				hasError: !!error,
		...
		const processCallback = async () => {
			// #region agent log
			sendAnalyticsLog({
				location: 'MobileOTPConfigurationPage.tsx:159',
			...
			if (code && state) {
				...
				if (state !== hasUserLoginState) {
					// #region agent log
					sendAnalyticsLog({
						location: 'MobileOTPConfigurationPage.tsx:173',
				...
				try {
					...
					// #region agent log
					sendAnalyticsLog({
						location: 'MobileOTPConfigurationPage.tsx:186',
					...
					const correctRedirectUri = MFARedirectUriService.getRedirectUri('unified-mfa-v8');
					...
					// #region agent log
					sendAnalyticsLog({
						location: 'MobileOTPConfigurationPage.tsx:211',
					...
					setCredentials((prev) => {
						...
						// #region agent log
						sendAnalyticsLog({
							location: 'MobileOTPConfigurationPage.tsx:224',
					...
					// #region agent log
					sendAnalyticsLog({
						location: 'MobileOTPConfigurationPage.tsx:228',
	...
	const loadPolicies = useCallback(async () => {
		...
		try {
			...
			const policies = await MFAService.listDeviceAuthenticationPolicies(
				credentials.environmentId
			);
			...
			if (tokenStatus.isValid) {
				modernMessaging.showBanner({
					type: 'error',
					title: 'Error',
					message: `Failed to load policies: ${errorMessage}`,
					dismissible: true,
				});
			}
		} finally {
	...
	useEffect(() => {
		// #region agent log
		sendAnalyticsLog({
			location: 'MobileOTPConfigurationPage.tsx:467',
		...
		CredentialsService.saveCredentials('mfa-flow-v8', credentials);

		// Verify the save by immediately reading it back
		setTimeout(() => {
			const saved = CredentialsService.loadCredentials('mfa-flow-v8', {
				flowKey: 'mfa-flow-v8',
				flowType: 'oidc',
				includeClientSecret: false,
				includeRedirectUri: false,
			...
			// #region agent log
			sendAnalyticsLog({
				location: 'MobileOTPConfigurationPage.tsx:479',
	...
	return (
		<div style={{ minHeight: '100vh', background: '#f9fafb' }}>
			...
			>
				...
				{/* Registration Flow Type Selection - SMS/Email specific - MOVED ABOVE MFAConfigurationStep */}
				<div
					style={{
						background: 'white',
						borderRadius: '8px',
				...
				>
					...
					<div style={{ fontSize: '14px', color: '#6b7280', lineHeight: 1.6 }}>
						<p style={{ margin: '0 0 12px 0' }}>
							<strong>Mobile-based OTP (One-Time Password)</strong> authentication sends a temporary
