...
import { fetchPhoneFromPingOne } from '@/mfa/services/phoneAutoPopulationService';
import { WorkerTokenStatusService } from '@/mfa/services/workerTokenStatusService';
import { WorkerTokenUIService } from '@/mfa/services/workerTokenUIService'; // NEW - Enhanced UI service
...
import { isValidPhoneFormat, validateAndNormalizePhone } from '@/mfa/utils/phoneValidation';
...
const MobileFlowV8WithDeviceSelection: React.FC = () => {
	...
	const renderStep0 = useMemo(() => {
		return (props: MFAFlowBaseRenderProps) => {
			...
			//
			// Always show Step 0 so users can configure token type and see the new UI
							...
							{shouldPromptForNickname ? (
								<div style={{ marginBottom: '12px' }}>
									...
									>
										...
										<MFAInfoButton contentKey="device.nickname" displayMode="tooltip" />
									</div>
									<input
		...
		return (props: MFAFlowBaseRenderProps) => {
			...
			return (
				<div className="step-content">
					<h2>
						Send OTP Code
						<MFAInfoButton contentKey="factor.sms" displayMode="modal" />
					</h2>
						...
						<div style={{ padding: '16px 20px' }}>
							...
							<div style={{ marginBottom: '16px' }}>
								...
								>
									...
									<MFAInfoButton contentKey="otp.validation" displayMode="modal" />
								</label>
								<MFAOTPInput
										...
										if (isActivationRequired || mfaState.deviceActivateUri) {
											...
											try {
												...
												const activationResult =
													await MFAService.activateDevice(activationParams);
									...
									onClick={async () => {
										...
										try {
											...
											if (mfaState.authenticationId && mfaState.deviceId) {
												const { MfaAuthenticationService } = await import(
													'@/mfa/services/mfaAuthenticationService'
												);
												...
												await MfaAuthenticationService.resendOTPForActiveDevice({
													environmentId: credentials.environmentId,
													username: credentials.username,
													userId,
													authenticationId: mfaState.authenticationId,
...
export const MobileFlow: React.FC = () => {
