/**
 * @file FIDO2ConfigurationPage.tsx
...
export const FIDO2ConfigurationPage: React.FC = () => {
	...
	const [isApiDisplayVisible, setIsApiDisplayVisible] = useState(apiDisplayService.isVisible());

	// FIDO2 Configuration state - load from service
	const [fido2Config, setFido2Config] = useState(() => {
		return MFAConfigurationService.loadConfiguration().fido2;
	});

	// Education content
	const webauthnContent = MFAEducationService.getContent('fido2.webauthn');
	const authenticatorContent = MFAEducationService.getContent('fido2.authenticator');
	const publicKeyContent = MFAEducationService.getContent('fido2.publicKey');
	const phishingContent = MFAEducationService.getContent('security.phishingResistance');
	const passkeysVsWebAuthnContent = MFAEducationService.getContent('fido2.passkeys.vs.webauthn');
	const passkeysVsDeviceBindingContent = MFAEducationService.getContent(
		'fido2.passkeys.vs.device.binding'
	);
	const biometricsVsWebAuthnContent = MFAEducationService.getContent(
	...
	useEffect(() => {
		const loadDeviceAuthPolicies = async () => {
			...
			try {
				const policies = await MFAService.listDeviceAuthenticationPolicies(environmentId);
					...
					>
						...
						<div style={{ marginBottom: '20px' }}>
							...
							>
								...
								<MFAInfoButton contentKey="fido2.webauthn" displayMode="tooltip" />
							</h3>
						...
						<div style={{ marginBottom: '20px' }}>
							...
							>
								...
								<MFAInfoButton contentKey="fido2.authenticator" displayMode="tooltip" />
							</h3>
							<p style={{ margin: 0, fontSize: '14px', color: '#6b7280', lineHeight: '1.6' }}>
						...
						<div style={{ marginBottom: '20px' }}>
							...
							>
								...
								<MFAInfoButton contentKey="fido2.publicKey" displayMode="tooltip" />
							</h3>
							<p style={{ margin: 0, fontSize: '14px', color: '#6b7280', lineHeight: '1.6' }}>
										...
										>
											<input
												...
												onChange={() => {
													...
													const fullConfig = MFAConfigurationService.loadConfiguration();
													MFAConfigurationService.saveConfiguration({
														...fullConfig,
														fido2: newConfig,
													});
												}}
