...
import { TOTPExpiredModal } from '@/mfa/components/TOTPExpiredModal';
...
} from '@/mfa/services/mfaConfigurationService';
import {
	type DeviceRegistrationResult,
	MFAService,
	type SendOTPParams,
} from '@/mfa/services/mfaService';
...
import { MFAConfigurationStep } from '../shared/MFAConfigurationStep';
...
const TOTPConfigureStep: React.FC<TOTPConfigureStepProps> = (props) => {
	...
	React.useEffect(() => {
		...
		if (registrationFlowType === 'user' && props.credentials.tokenType !== 'user') {
			...
			logger.info(
				`${MODULE_TAG} Admin Flow selected - ensuring Worker Token is used`,
				'Logger info'
			);
			isSyncingRef.current = true;
		...
		<>
			...
			>
				...
				>
					...
					<MFAInfoButton contentKey="mfa.registrationFlowType" displayMode="tooltip" />
				</label>
...
const TOTPFlowV8WithDeviceSelection: React.FC = () => {
	...
	useEffect(() => {
		const handleConfigUpdate = (event: Event) => {
			...
			const newConfig = customEvent.detail || MFAConfigurationService.loadConfiguration();
	...
	const tokenStatusRef = React.useRef<ReturnType<
		typeof WorkerTokenStatusService.checkWorkerTokenStatus
	> | null>(null);
	...
	const navigateSafely = useCallback(
		(nav: ReturnType<typeof useStepNavigation>, targetStep: number | 'previous' | 'hub') => {
			try {
				if (targetStep === 'hub') {
			...
			if (
				...
				try {
					...
					if (authResult.nextStep === 'COMPLETED') {
						...
						modernMessaging.showFooterMessage({
							type: 'info',
							message: 'Device selected. Please enter the TOTP code from your authenticator app.',
							duration: 3000,
						});
					} else {
						nav.markStepComplete();
			...
			const handleRegisterDevice = async () => {
				...
				try {
					...
					if (result.deviceId && registrationCredentials.nickname) {
						try {
							await MFAService.updateDeviceNickname(
								{
									environmentId: credentials.environmentId,
					...
					if (!secret && !keyUri && result.deviceId) {
						...
						try {
							const deviceDetails = await MFAService.getDevice({
								...credentials,
								deviceId: result.deviceId,
							} as SendOTPParams);
					...
					modernMessaging.showFooterMessage({
						type: 'info',
						message: 'Device registered. Scan the QR code to complete setup.',
						duration: 3000,
					});
				} catch (error) {
					const errorMessage = error instanceof Error ? error.message : 'Unknown error';
					...
					if (isDeviceLimitError) {
						...
						modernMessaging.showBanner({
							type: 'error',
							title: 'Error',
							message: `Registration failed: ${formattedError.userFriendlyMessage}`,
							dismissible: true,
						});
					}
				} finally {
									...
									>
										...
										>
											...
											<button
												type="button"
												onClick={async () => {
													const success = await TokenDisplayService.copyToClipboard(
														currentTotpSecret,
														'TOTP Secret'
													);
													if (success) {
													...
													onClick={async () => {
														if (
															...
															modernMessaging.showBanner({
																type: 'error',
																title: 'Error',
																message: 'Missing device information. Cannot delete device.',
																dismissible: true,
															});
															return;
														}
						...
						<div style={{ padding: '12px 16px' }}>
							...
							<div style={{ marginBottom: '16px' }}>
								...
								>
									...
									<MFAInfoButton contentKey="otp.validation" displayMode="tooltip" />
								</label>
								<MFAOTPInput
									...
									onClick={async () => {
										...
										try {
											...
											modernMessaging.showBanner({
												type: 'error',
												title: 'Error',
												message: `Validation failed: ${normalizedError}`,
												dismissible: true,
											});
											setIsLoading(false);
										}
	...
	// Also sync when mfaState changes (passed from MFAFlowBase)
	...
	React.useEffect(() => {
		...
		const checkStep2 = () => {
			// We need to check the actual nav.currentStep from MFAFlowBase
				...
				onRegenerate={async () => {
					...
					if (currentMfaState?.deviceId && currentCredentials && currentTokenStatus) {
						try {
							...
							modernMessaging.showFooterMessage({
								type: 'info',
								message: 'Expired device deleted. Please register a new device.',
								duration: 3000,
							});
							...
							// Go back to device selection - we need to get nav from MFAFlowBase
