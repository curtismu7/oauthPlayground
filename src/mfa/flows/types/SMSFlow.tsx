...
const SMSFlowV8WithDeviceSelection: React.FC = () => {
	...
	const renderStep2Register = useCallback(
		(props: MFAFlowBaseRenderProps) => {
			...
			const { WorkerTokenStatusService } = require('@/mfa/services/workerTokenStatusService');
			...
			if (deviceRegisteredActive && deviceRegisteredActive.status === 'ACTIVE' && !showModal) {
				...
				successData.deviceType = 'SMS' as DeviceType;
				return (
					<MFASuccessPage
						{...props}
						credentials={{ ...credentials, deviceType: 'SMS' as DeviceType }}
