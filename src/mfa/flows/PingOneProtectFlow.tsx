...
import { type Credentials, CredentialsService } from '@/mfa/services/credentialsService';
...
export const PingOneProtectFlow: React.FC = () => {
	// Worker token state
	const [tokenStatus, setTokenStatus] = useState(() => {
		return WorkerTokenStatusService.checkWorkerTokenStatusSync();
	});
	...
	useEffect(() => {
		const syncCredentials = async () => {
			const stored = await CredentialsService.loadCredentials(FLOW_KEY, {
				flowKey: FLOW_KEY,
				flowType: 'oauth',
	...
	useEffect(() => {
		const saveTimeout = setTimeout(() => {
			void CredentialsService.saveCredentials(FLOW_KEY, {
	...
	const makeApiCall = useCallback(
		async (
			...
			if (!credentials.environmentId || !credentials.workerToken) {
				uiNotificationService.showError('Please configure environment ID and worker token first');
	...
	const fetchRiskPolicies = useCallback(async () => {
		await riskPoliciesSpinner.executeWithSpinner(
			async () => {
				...
				uiNotificationService.showSuccess('Risk policies fetched successfully');
			},
			{
				onSuccess: () => {
				...
				onError: (error) => {
					...
					uiNotificationService.showError('Failed to fetch risk policies');
				},
			}
		);
	...
	const createRiskEvaluation = useCallback(async () => {
		await riskEvaluationSpinner.executeWithSpinner(
			async () => {
				...
				uiNotificationService.showSuccess(`Risk evaluation completed: ${riskLevel} risk`);
			},
			{
				onSuccess: () => {
					// Success handled in main function
				...
				onError: (error) => {
					...
					uiNotificationService.showError('Failed to create risk evaluation');
				},
			}
		);
	...
	const updateRiskEvaluation = useCallback(
		async (evaluationId: string, completionStatus: 'SUCCESS' | 'FAILED') => {
			try {
				...
				uiNotificationService.showSuccess(`Risk evaluation updated: ${completionStatus}`);
				return data;
			} catch (error) {
				logger.error(`${MODULE_TAG} Failed to update risk evaluation:`, error);
				uiNotificationService.showError('Failed to update risk evaluation');
				return null;
			}
		},
		[makeApiCall]
	...
	const provideFeedback = useCallback(
		async (evaluationId: string, feedback: 'POSITIVE' | 'NEGATIVE') => {
			try {
				...
				uiNotificationService.showSuccess(`Feedback provided: ${feedback}`);
				return data;
			} catch (error) {
				logger.error(`${MODULE_TAG} Failed to provide feedback:`, error);
				uiNotificationService.showError('Failed to provide feedback');
				return null;
			}
		},
		[makeApiCall]
...
export default PingOneProtectFlow;
