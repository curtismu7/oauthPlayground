...
import { MFAReportingService } from '@/mfa/services/mfaReportingService';
...
export const MFAReportingFlow: React.FC = () => {
	...
	const [showTokenAtEnd, _setShowTokenAtEnd] = useState(() => {
		try {
			return MFAConfigurationService.loadConfiguration().workerToken.showTokenAtEnd || false;
		} catch {
			return false;
		}
	});
		...
		return await reportingSpinner.withSpinner(async () => {
			...
			try {
				...
				if (selectedReport === 'user-authentications') {
					...
					result = await MFAReportingService.getUserAuthenticationReports(params);
				} else if (selectedReport === 'device-authentications') {
					result = await MFAReportingService.getDeviceAuthenticationReports({
						environmentId: credentials.environmentId,
						region,
						customDomain,
					});
					...
					const reportResult = await MFAReportingService.createMFADevicesReport(deviceParams);
					...
					if (selectedReport === 'mfa-enabled') {
						...
						const reportResult =
							await MFAReportingService.createMFAEnabledDevicesReport(mfaParams);
						// Extract entries from the report result
						// biome-ignore lint/suspicious/noExplicitAny: Report result requires any type
						result = (reportResult as any)._embedded?.entries || [];
...
export default MFAReportingFlow;
