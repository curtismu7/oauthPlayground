 ...
 *
 * **Legacy Mode (default):**
 * - SMSFlow: SMS device registration and OTP validation
 * - EmailFlow: Email device registration and OTP validation
 * - TOTPFlow: TOTP device registration and validation
 * - FIDO2Flow: FIDO2 device registration and WebAuthn validation
 * - MobileFlow: Mobile app pairing
 * - WhatsAppFlow: WhatsApp OTP validation
 *
 * **Unified Mode (feature flag controlled):**
 * - UnifiedMFARegistrationFlow: Single component handling all 6 device types
...
import { MFAErrorBoundary } from '@/mfa/components/MFAErrorBoundary';
import { MFAFlowSkeleton } from '@/mfa/components/MFASkeletonLoader';
...
const UnifiedMFARegistrationFlow = React.lazy(() =>
	import('./unified/UnifiedMFARegistrationFlow').then((module) => ({
 ...
 * - Feature Flags: Controls gradual rollout per device type (MFAFeatureFlags)
 * - Factory: Creates legacy components dynamically (MFAFlowComponentFactory)
 * - Unified: Single component for all device types (UnifiedMFARegistrationFlow)
 */
export const MFARouterFlow: React.FC = () => {
	logger.info(`${MODULE_TAG} Initializing MFA flow router`, 'Logger info');
	...
	React.useEffect(() => {
		const handleDeviceTypeChange = (event: Event) => {
			...
			if (newDeviceType !== deviceType) {
				logger.info(
					`${MODULE_TAG} Device type changed from ${deviceType} to ${newDeviceType}`,
					'Logger info'
				);
