...
import { CreatePolicyModal } from '@/mfa/components/CreatePolicyModal';
...
import { MFAService, type MFASettings } from '@/mfa/services/mfaService';
...
export const MFAConfigurationPage: React.FC = () => {
	...
	const loadPingOneSettings = useCallback(async (envId: string) => {
		...
		try {
			...
			UnifiedFlowErrorHandler.handleError(error, {
				operation: 'load-pingone-mfa-settings',
	...
	const loadSelectedPolicy = useCallback(async (envId: string, policyId: string) => {
		...
		try {
			...
			UnifiedFlowErrorHandler.handleError(error, {
				operation: 'load-device-auth-policy',
				component: MODULE_TAG,
			});
		} finally {
	...
	const handleSavePingOneSettings = async () => {
		...
		try {
			await MFAService.updateMFASettings(environmentId, pingOneSettings);
			setHasPingOneSettingsChanges(false);
			...
			UnifiedFlowErrorHandler.handleError(error, {
				operation: 'save-pingone-mfa-settings',
				component: MODULE_TAG,
			});
		} finally {
	...
	const handleSavePolicy = async () => {
		...
		try {
			...
			UnifiedFlowErrorHandler.handleError(error, {
				operation: 'save-device-auth-policy',
				component: MODULE_TAG,
			});
		} finally {
	...
	const handleCreatePolicy = async (name: string, description: string) => {
		if (!environmentId) {
			modernMessaging.showBanner({
				type: 'error',
				title: 'Error',
				message: 'Environment ID is required to create a policy.',
				dismissible: true,
			});
			return;
		}
		...
		try {
			...
			const newPolicy = await MFAService.createDeviceAuthenticationPolicy(
				environmentId,
				{
					name: name.trim(),
			...
			UnifiedFlowErrorHandler.handleError(error, {
				operation: 'create-device-auth-policy',
				component: MODULE_TAG,
			});
	...
	const handleResetPingOneSettings = async () => {
		...
		try {
			await MFAService.resetMFASettings(environmentId);
			modernMessaging.showFooterMessage({
				type: 'info',
			...
			UnifiedFlowErrorHandler.handleError(error, {
				operation: 'reset-pingone-mfa-settings',
				component: MODULE_TAG,
			});
		}
	};
			...
			{selectedPolicy && (
				...
				>
					...
					>
						...
						<MFAInfoButton contentKey="otp.failure.coolDown.duration" displayMode="tooltip" />
					</h4>
					<div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
						...
						<div style={{ flex: 1, minWidth: '200px' }}>
							...
							>
								Time Unit
								<MFAInfoButton contentKey="otp.failure.coolDown.timeUnit" displayMode="tooltip" />
							</label>
							<select
					...
					<div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
						<ToggleSetting
							label={
								<>
									Pairing Disabled
									<MFAInfoButton contentKey="policy.pairingDisabled" displayMode="tooltip" />
								</>
							}
			...
			{selectedPolicy && (
				<div style={{ marginTop: '20px' }}>
					...
					>
						...
						<MFAInfoButton contentKey="policy.deviceType.configuration" displayMode="tooltip" />
					</h4>
						...
						<>
							...
							>
								...
								<div style={{ margin: 0, fontSize: '13px', color: '#1e40af', lineHeight: '1.5' }}>
									...
									<MFAInfoButton contentKey="mfa.settings" displayMode="tooltip" />
								</div>
							</div>
...
export default MFAConfigurationPage;
