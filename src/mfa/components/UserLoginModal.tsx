...
import { RedirectUriValidator } from '@/mfa/components/RedirectUriValidator';
import {
	AppDiscoveryService,
	type DiscoveredApplication,
} from '@/mfa/services/appDiscoveryService';
import { AuthMethodService, type AuthMethod } from '@/mfa/services/authMethodService';
...
import { MFARedirectUriService } from '@/mfa/services/mfaRedirectUriService';
import {
	type OAuthCredentials,
	OAuthIntegrationService,
} from '@/mfa/services/oauthIntegrationService';
import { useRedirectURIRouting } from '@/mfa/services/redirectURIRoutingService';
...
import {
	type SessionInfo,
	UserAuthenticationSuccessPage,
} from './UserAuthenticationSuccessPage';
...
export const UserLoginModal: React.FC<UserLoginModalV8Props> = ({
	isOpen,
	onClose,
	onTokenReceived,
	onCredentialsSaved,
	...
	// Check if worker token is available (from workerTokenService)
	...
	const [authMethod, setAuthMethod] = useState<AuthMethod>('client_secret_post');
	...
	const handlePreFlightValidation = async () => {
		...
		try {
			...
			// Store appConfig for RedirectUriValidator
	...
	useEffect(() => {
		if (isOpen) {
			...
			const saved = CredentialsService.loadCredentials(FLOW_KEY, {
				flowKey: FLOW_KEY,
				flowType: 'oauth',
			...
			if (isMfaFlow && MFARedirectUriService.needsMigration(saved.redirectUri)) {
				logger.warn(
					'[ USER-LOGIN-MODAL-V8] MIGRATION: Updating to unified MFA callback URI',
					'Logger warning'
				);
				const migrated = MFARedirectUriService.migrateCredentials(saved, 'unified-mfa-v8');
				CredentialsService.saveCredentials(FLOW_KEY, migrated);
			}
			...
			if (saved.environmentId || saved.clientId || saved.redirectUri || finalEnvironmentId) {
				...
				// #region agent log
				sendAnalyticsLog({
					location: 'UserLoginModal.tsx:134',
					message:
	...
	useEffect(() => {
		// #region agent log
		sendAnalyticsLog({
			location: 'UserLoginModal.tsx:189',
		...
		const checkCallback = async () => {
			...
			// #region agent log
			sendAnalyticsLog({
				location: 'UserLoginModal.tsx:207',
			...
			if (code && storedState) {
				...
				try {
					...
					// #region agent log
					sendAnalyticsLog({
						location: 'UserLoginModal.tsx:258',
					...
					// #region agent log
					sendAnalyticsLog({
						location: 'UserLoginModal.tsx:260',
							...
							if (workerToken) {
								...
								if (appConfig?.tokenEndpointAuthMethod) {
									...
									if (currentAuthMethod !== pingOneAuthMethod) {
										...
										credentials = {
											...credentials,
											clientAuthMethod: pingOneAuthMethod as AuthMethod,
											tokenEndpointAuthMethod: pingOneAuthMethod as AuthMethod,
										};
									}
								}
							}
					...
					const tokenResponse = await OAuthIntegrationService.exchangeCodeForTokens(
						{
							environmentId: credentials.environmentId,
							clientId: credentials.clientId,
					...
					// #region agent log
					sendAnalyticsLog({
						location: 'UserLoginModal.tsx:282',
					...
					const currentCredentials =
						CredentialsService.loadCredentials(FLOW_KEY, {
							flowKey: FLOW_KEY,
							flowType: 'oauth',
					...
					CredentialsService.saveCredentials(FLOW_KEY, updatedCredentials);
					...
					// #region agent log
					sendAnalyticsLog({
						location: 'UserLoginModal.tsx:377',
					...
					// #region agent log
					sendAnalyticsLog({
						location: 'UserLoginModal.tsx:308',
	...
	const handleSaveCredentials = useCallback(() => {
		...
		try {
			...
			// Save credentials using CredentialsService
			const FLOW_KEY = 'user-login-v8';
			const credsToSave = {
				environmentId: environmentId.trim(),
				clientId: clientId.trim(),
			...
			CredentialsService.saveCredentials(FLOW_KEY, credsToSave);
			modernMessaging.showFooterMessage({
				type: 'info',
	...
	const handleLogin = async () => {
		...
		// #region agent log
		sendAnalyticsLog({
			location: 'UserLoginModal.tsx:614',
		...
		// Save credentials using CredentialsService (always use user-login-callback for User Login Flow)
		const FLOW_KEY = 'user-login-v8';
		const credsToSave = {
			environmentId: environmentId.trim(),
			clientId: clientId.trim(),
		...
		try {
			...
			const { authorizationUrl, state, codeVerifier } =
				await OAuthIntegrationService.generateAuthorizationUrl(credentials);
			...
			// #region agent log
			sendAnalyticsLog({
				location: 'UserLoginModal.tsx:657',
				...
				<div style={{ padding: '24px' }}>
					...
					<div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
											...
											onAppSelected={async (app: DiscoveredApp) => {
												try {
													...
													if (environmentId.trim()) {
														try {
															...
															if (workerToken) {
																...
																const fetchedApp =
																	await AppDiscoveryService.fetchApplicationWithSecret(
																		environmentId.trim(),
																		app.id,
																		workerToken
																	);
													...
													if (appWithSecret.tokenEndpointAuthMethod) {
														...
														if (validMethods.includes(methodValue as AuthMethod)) {
															setAuthMethod(methodValue as AuthMethod);
														} else {
															logger.warn(
						...
						<div>
							...
							>
								{AuthMethodService.getAvailableMethodsForFlow('authorization-code').map(
									(method) => (
										<option key={method} value={method}>
											{AuthMethodService.getDisplayLabel(method)}
										</option>
									)
								)}
							</select>
							<small
								style={{ display: 'block', marginTop: '4px', fontSize: '12px', color: '#6b7280' }}
							>
								{AuthMethodService.getMethodConfig(authMethod)?.description ||
									'Token endpoint authentication method'}
							</small>
						</div>
