...
export const MFASettingsModal: React.FC<MFASettingsModalV8Props> = ({
	isOpen,
	onClose,
	environmentId,
}) => {
	...
	const fetchSettings = async () => {
		setIsLoading(true);
		try {
			const data = await MFAService.getMFASettings(environmentId);
			setSettings(data);
		} catch (error) {
	...
	const handleSave = async () => {
		setIsSaving(true);
		try {
			await MFAService.updateMFASettings(environmentId, settings);
