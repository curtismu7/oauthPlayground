...
export const WorkerTokenStatusDisplay: React.FC<WorkerTokenStatusDisplayV8Props> = ({
	mode = 'detailed',
	showRefresh = true,
	className = '',
	refreshInterval = 5,
	...
	const [config, setConfig] = useState(() => MFAConfigurationService.loadConfiguration());
	...
	const updateTokenStatus = useCallback(async () => {
		try {
			const v8Status = await WorkerTokenStatusService.checkWorkerTokenStatus();
			...
			try {
				...
				try {
					...
					const creds = CredentialsService.loadCredentials('mfa-flow-v8', {
						flowKey: 'mfa-flow-v8',
						flowType: 'oidc',
					...
					logger.warn('[WorkerTokenStatusDisplay] Failed to load MFA credentials:', credError);
				...
				logger.warn('[WorkerTokenStatusDisplay] Failed to fetch additional data:', dataError);
				// Don't let additional data failure break the main status
			}
		} catch (error) {
			logger.error('[WorkerTokenStatusDisplay] Failed to check token status:', error);
		} finally {
			setIsLoading(false);
		}
	}, []);
	...
	const handleRefresh = async () => {
		...
		try {
			...
			setConfig(MFAConfigurationService.loadConfiguration());
			...
			UnifiedFlowErrorHandler.handleError(error, {
				...
				component: 'WorkerTokenStatusDisplay',
			});
		} finally {
	...
	useEffect(() => {
		...
		const handleTokenUpdate = async () => {
			logger.debug('WorkerTokenStatusDisplay', 'Worker token update event received');
			...
			const v8Status = WorkerTokenStatusService.checkWorkerTokenStatusSync();
		...
		try {
			...
			if (tokenStatus.isValid) {
				try {
					...
					logger.warn('[WorkerTokenStatusDisplay] Failed to fetch PingOne config:', error);
	...
	const handleSaveConfig = async () => {
		...
		try {
			...
			const newConfig = MFAConfigurationService.loadConfiguration();
			if (!newConfig.oauth) {
				newConfig.oauth = {
			...
			MFAConfigurationService.saveConfiguration(newConfig);
			...
			if (tokenStatus.isValid && fullTokenData?.token) {
				try {
					...
					logger.warn('[WorkerTokenStatusDisplay] Failed to sync config with PingOne:', error);
					modernMessaging.showBanner({
						type: 'warning',
						title: 'Warning',
						message: 'OAuth configuration saved locally, but failed to sync with PingOne',
	...
	const handleGetWorkerTokenForConfig = () => {
		...
		window.dispatchEvent(
			new CustomEvent('open-worker-token-modal', {
				detail: { source: 'WorkerTokenStatusDisplay' },
			})
		);
	};
...
export default WorkerTokenStatusDisplay;
