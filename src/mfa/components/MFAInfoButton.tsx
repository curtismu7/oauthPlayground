 ...
 * content from MFAEducationService in a modal or tooltip.
 *
 * @example
 * <MFAInfoButton contentKey="factor.sms" />
...
export interface MFAInfoButtonV8Props {
	/** Education content key from MFAEducationService */
	contentKey: string;
 ...
 * MFA Info Button V8
 *
 * Displays educational content from MFAEducationService in either:
 * - Tooltip mode: Shows on hover (for quick reference)
 * - Modal mode: Shows on click (for detailed information)
 */
export const MFAInfoButton: React.FC<MFAInfoButtonV8Props> = ({
	contentKey,
	displayMode = 'tooltip',
	label,
	...
	const content = useMemo(() => {
		return MFAEducationService.getContent(contentKey);
	}, [contentKey]);
	...
	const handleClose = React.useCallback(() => {
		logger.info(`${MODULE_TAG} Closing modal`, 'Logger info');
		setIsOpen(false);
	}, []);
								...
								{content.securityLevel && (
									<div
										style={{
											...
											padding: '4px 8px',
											background: MFAEducationService.getSecurityLevelColor(
												content.securityLevel
											),
											color: 'white',
											borderRadius: '4px',
