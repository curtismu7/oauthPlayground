...
export const MFADeviceManager: React.FC<MFADeviceManagerV8Props> = ({
	environmentId,
	username,
	onUsernameChange,
}) => {
	...
	const loadDevices = useCallback(async () => {
		await loadDevicesSpinner.executeWithSpinner(
			async () => {
				...
				const deviceList = await MFAService.getAllDevices({
					environmentId,
					username,
				});
	...
	const handleAllowBypass = async () => {
		...
		try {
			...
			await MFAService.allowMfaBypass(environmentId, user.id);
	...
	const handleCheckBypass = async () => {
		...
		try {
			const user = await MFAService.lookupUserByUsername(environmentId, username);
			const status = await MFAService.checkMfaBypassStatus(environmentId, user.id);
	...
	const _handleSetDeviceOrder = async () => {
		...
		try {
			...
			await MFAService.setUserMfaDeviceOrder(environmentId, user.id, orderedDeviceIds);
			modernMessaging.showFooterMessage({
				type: 'info',
	...
	const _handleRemoveDeviceOrder = async () => {
		...
		try {
			const user = await MFAService.lookupUserByUsername(environmentId, username);
			await MFAService.removeUserMfaDeviceOrder(environmentId, user.id);
			modernMessaging.showFooterMessage({
				type: 'info',
	...
	const handleUnblock = async (deviceId: string) => {
		...
		setProcessingDeviceId(deviceId);
		try {
			await MFAService.unblockDevice({
				environmentId,
				username,
				deviceId,
			});
	...
	const handleAdminActivate = async (deviceId: string) => {
		...
		try {
			...
			if (device.type === 'TOTP') {
				...
				_activationResult = await MFAService.activateDevice({
					environmentId,
					username,
					deviceId,
				...
				_activationResult = await MFAService.activateFIDO2Device({
					environmentId,
					username,
					deviceId,
				...
				{showInfo && (
					...
					>
						...
						<ul style={{ margin: '8px 0 0 18px' }}>
							...
							<li>
								Each card lists the factor type, nickname, contact details, and current status.
							</li>
							<li>
								Action buttons let you rename, block/unblock, or delete devices—mirroring what
								PingOne admins do in production.
							</li>
							<li>
