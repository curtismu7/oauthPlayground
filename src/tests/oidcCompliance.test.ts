...
describe('OIDCComplianceService', () => {
	...
	describe('OIDC Authorization Request Validation', () => {
		...
		it('should require openid scope', () => {
			...
			expect(result.errors).toContain(
				'invalid_request: scope must include "openid" for OIDC requests'
			);
		});
		...
		it('should require nonce for implicit flows', () => {
			...
			expect(result.valid).toBe(false);
			expect(result.errors).toContain(
				'invalid_request: nonce is required when response_type includes id_token'
			);
		});
	...
	describe('Claims Request Validation', () => {
		...
		it('should validate essential parameter type', () => {
			...
			expect(result.valid).toBe(false);
			expect(result.errors).toContain(
				'invalid_request: claims.userinfo.name.essential must be boolean'
			);
		});
		...
		it('should validate values parameter type', () => {
			...
			expect(result.valid).toBe(false);
			expect(result.errors).toContain(
				'invalid_request: claims.userinfo.locale.values must be array'
			);
		});
	});
	...
	describe('ID Token Validation', () => {
		...
		it('should validate issuer claim', async () => {
			...
			expect(result.valid).toBe(false);
			expect(result.errors).toContain(
				'Invalid issuer: expected https://auth.pingone.com/test, got https://wrong-issuer.com'
			);
		});
		...
		it('should validate audience claim', async () => {
			...
			expect(result.valid).toBe(false);
			expect(result.errors).toContain(
				'Invalid audience: client_id client-123 not found in aud claim'
			);
		});
	...
	describe('Hash Validation', () => {
		it('should validate at_hash format', () => {
			const result = service.validateAtHash(
				'access-token',
		...
		it('should validate c_hash format', () => {
			const result = service.validateCHash(
				'auth-code',
				'dBjftJeZ4CVP-mB92K27uhbUJU1p1r_wW1gFWFOEjXk'
			);
			expect(result).toBe(true);
