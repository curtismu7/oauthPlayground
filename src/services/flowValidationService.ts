...
export class FlowValidationService {
  ...
  static canNavigateNext(currentStep: number, flowType: string, controller: FlowController): boolean {
    return this.validateStep(currentStep, flowType, controller) && 
  ...
  static getValidationRules(flowType: string): ValidationRule[] {
    const rules: Record<string, ValidationRule[]> = {
      'authorization-code': [
        ...
        {
          stepIndex: 1,
          validator: (controller) => !!(controller.pkceCodes?.codeVerifier && controller.pkceCodes?.codeChallenge),
      ...
      'client-credentials': [
        ...
        {
          stepIndex: 1,
          validator: (controller) => !!(controller.credentials?.clientId && controller.credentials?.clientSecret),
  ...
  static validateCredentials(credentials: Record<string, unknown>, requiredFields: string[]): boolean {
  ...
  static getRequiredFields(flowType: string): string[] {
    const requiredFields: Record<string, string[]> = {
      ...
      'resource-owner-password': ['environmentId', 'clientId', 'clientSecret', 'username', 'password'],
  ...
  static getStepType(stepIndex: number, flowType: string): 'introduction' | 'configuration' | 'authorization' | 'token-exchange' | 'validation' | 'completion' | 'security' {
