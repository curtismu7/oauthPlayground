...
export class FlowStateService {
  ...
  static createStepValidationPatterns(flowType: string): Record<string, (controller: FlowController) => boolean> {
    const patterns: Record<string, Record<string, (controller: FlowController) => boolean>> = {
      'authorization-code': {
        '0': () => true, // Introduction step
        '1': (controller) => !!(controller.pkceCodes?.codeVerifier && controller.pkceCodes?.codeChallenge),
      ...
      'client-credentials': {
        '0': () => true, // Introduction step
        '1': (controller) => !!(controller.credentials?.clientId && controller.credentials?.clientSecret),
