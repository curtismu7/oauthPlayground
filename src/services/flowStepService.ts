...
export class FlowStepService {
   ...
   */
  private static createImplicitStepContent(stepIndex: number, controller: any, theme: string): StepContent {
    switch (stepIndex) {
      case 0:
        return {
          type: 'introduction',
          components: [
            {
              ...
              content: 'The Implicit Flow is considered legacy and less secure than Authorization Code with PKCE.',
            },
            {
              type: 'info',
              props: { variant: 'info' },
              content: 'This flow returns tokens directly in the URL fragment, making them vulnerable to interception.',
            },
          ],
          collapsibleSections: [
            {
          ...
          components: [
            {
              type: 'code',
              props: { language: 'json' },
              content: controller.tokens ? JSON.stringify(controller.tokens, null, 2) : 'No tokens received',
            },
          ],
        };
      case 3:
  ...
  private static createAuthorizationCodeStepContent(stepIndex: number, controller: any, theme: string): StepContent {
    // Similar structure to implicit but with authorization code specific content
    return this.createDefaultStepContent(stepIndex, controller, theme);
  }

  private static createClientCredentialsStepContent(stepIndex: number, controller: any, theme: string): StepContent {
    // Client credentials specific content
    return this.createDefaultStepContent(stepIndex, controller, theme);
  }

  private static createDeviceAuthorizationStepContent(stepIndex: number, controller: any, theme: string): StepContent {
    // Device authorization specific content
    return this.createDefaultStepContent(stepIndex, controller, theme);
  }

  private static createResourceOwnerPasswordStepContent(stepIndex: number, controller: any, theme: string): StepContent {
    // Resource owner password specific content
    return this.createDefaultStepContent(stepIndex, controller, theme);
  }

  private static createJWTBearerStepContent(stepIndex: number, controller: any, theme: string): StepContent {
    // JWT bearer specific content
    return this.createDefaultStepContent(stepIndex, controller, theme);
  }

  private static createCIBAStepContent(stepIndex: number, controller: any, theme: string): StepContent {
    // CIBA specific content
    return this.createDefaultStepContent(stepIndex, controller, theme);
  }

  private static createHybridStepContent(stepIndex: number, controller: any, theme: string): StepContent {
    // Hybrid specific content
    return this.createDefaultStepContent(stepIndex, controller, theme);
  }

  private static createRedirectlessStepContent(stepIndex: number, controller: any, theme: string): StepContent {
    // Redirectless specific content
    return this.createDefaultStepContent(stepIndex, controller, theme);
  }

  private static createTokenIntrospectionStepContent(stepIndex: number, controller: any, theme: string): StepContent {
    // Token introspection specific content
    return this.createDefaultStepContent(stepIndex, controller, theme);
  }

  private static createTokenRevocationStepContent(stepIndex: number, controller: any, theme: string): StepContent {
    // Token revocation specific content
    return this.createDefaultStepContent(stepIndex, controller, theme);
  }

  private static createUserInfoStepContent(stepIndex: number, controller: any, theme: string): StepContent {
    // User info specific content
    return this.createDefaultStepContent(stepIndex, controller, theme);
  }

  private static createDefaultStepContent(stepIndex: number, controller: any, theme: string): StepContent {
    return {
      type: 'introduction',
      components: [
        {
