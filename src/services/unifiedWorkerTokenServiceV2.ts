...
import type { UnifiedWorkerTokenCredentials, UnifiedWorkerTokenStatus, WorkerTokenValidationResult } from './unifiedWorkerTokenService';
...
class UnifiedWorkerTokenServiceV2 {
   ...
   */
  async saveToken(token: string, expiresAt?: number, tokenMetadata?: {
    tokenType?: string;
    expiresIn?: number;
    scope?: string;
  }): Promise<void> {
  ...
  private broadcastTokenUpdate(tokenData: {
    ...
    const credentials = workerTokenRepository.loadCredentials();
