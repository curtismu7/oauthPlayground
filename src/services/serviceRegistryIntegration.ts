...
import { serviceRegistry, ServiceCategory } from './serviceRegistry';
...
export async function initializeServiceRegistry(): Promise<void> {
  ...
  try {
    ...
    console.log(`[ServiceRegistryIntegration] Registry health: ${health.healthy ? 'HEALTHY' : 'UNHEALTHY'}`);
...
export function performHealthCheck(): {
  ...
  const services = Array.from(serviceRegistry.getAllServices().entries()).map(([name, service]) => ({
    name,
