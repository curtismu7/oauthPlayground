...
export interface MfaPolicy {
    ...
    name: string;
    enabled: boolean;
    deviceSelection?: 'ALWAYS_DISPLAY_DEVICES' | 'DEFAULT_TO_FIRST_DEVICE' | 'PROMPT_TO_SELECT_DEVICE';
    sms?: {
        enabled: boolean;
...
export class EnhancedPingOneMfaService {
    ...
    private static getBaseUrl(environmentId: string): string {
        return `${this.BASE_URL}/environments/${environmentId}`;
    }
    ...
    static async getDevices(credentials: MfaCredentials): Promise<MfaDevice[]> {
        const url = `${this.getBaseUrl(credentials.environmentId)}/users/${credentials.userId}/mfaDevices`;
        const response = await this.makeRequest<{ _embedded: { mfaDevices: MfaDevice[] } }>(
            url,
            {
                method: 'GET',
                headers: this.getDefaultHeaders(credentials.accessToken)
            }
        );
    ...
    static async getDevice(credentials: MfaCredentials, deviceId: string): Promise<MfaDevice> {
        const url = `${this.getBaseUrl(credentials.environmentId)}/users/${credentials.userId}/mfaDevices/${deviceId}`;
        return this.makeRequest<MfaDevice>(
            url,
            {
                method: 'GET',
                headers: this.getDefaultHeaders(credentials.accessToken)
    ...
    static async activateDevice(
        ...
        const url = `${this.getBaseUrl(credentials.environmentId)}/users/${credentials.userId}/mfaDevices/${deviceId}/activate`;
        return this.makeRequest<MfaDevice>(
            url,
            {
                method: 'POST',
    ...
    static async sendChallenge(
        ...
        const url = `${this.getBaseUrl(credentials.environmentId)}/users/${credentials.userId}/mfaDevices/${deviceId}/challenges`;
        return this.makeRequest<{ challengeId: string; expiresAt: string }>(
            url,
            {
                method: 'POST',
                headers: this.getDefaultHeaders(credentials.accessToken),
    ...
    static async verifyChallenge(
        ...
        const url = `${this.getBaseUrl(credentials.environmentId)}/users/${credentials.userId}/mfaDevices/${deviceId}/challenges/${challengeId}`;
        return this.makeRequest<{ valid: boolean; expiresAt: string }>(
            url,
            {
                method: 'POST',
                headers: this.getDefaultHeaders(credentials.accessToken),
    ...
    static async createFido2Device(
        ...
        const device = await this.createDevice(credentials, 'FIDO2', {
            name: options.name || 'Security Key'
        });

        // Get registration options from PingOne
        const url = `${this.getBaseUrl(credentials.environmentId)}/users/${credentials.userId}/mfaDevices/${device.id}/registrationOptions`;
        const registrationOptions = await this.makeRequest<any>(
            url,
            {
                method: 'GET',
                headers: this.getDefaultHeaders(credentials.accessToken)
    ...
    static async getMfaPolicies(
        ...
        if (!response.ok) {
            ...
            throw new Error(
                ...
                errorBody?.message ||
                'Unable to load PingOne MFA policies'
            );
        }
    ...
    static async getMfaPolicy(
        ...
        const url = `${this.getBaseUrl(environmentId)}/mfaSettings/${policyId}`;
        return this.makeRequest<MfaPolicy>(
            url,
            {
                method: 'GET',
    ...
    static getEnabledDeviceTypes(policy: MfaPolicy): Array<'SMS' | 'EMAIL' | 'TOTP' | 'VOICE' | 'FIDO2' | 'MOBILE'> {
