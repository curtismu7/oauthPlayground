...
export type BuilderAppType = 'OIDC_WEB_APP' | 'OIDC_NATIVE_APP' | 'SINGLE_PAGE_APP' | 'WORKER' | 'SERVICE' | 'SAML_APP';
export type TokenEndpointMethod = 'client_secret_basic' | 'client_secret_post' | 'client_secret_jwt' | 'private_key_jwt' | 'none';
...
const BUILT_IN_PRESETS: ConfigurationPreset[] = [
...

  {
    ...
    configuration: {
      ...
      pkceEnforcement: 'OPTIONAL',
      scopes: ['p1:read:user', 'p1:update:user', 'p1:create:user', 'p1:delete:user', 'p1:create:application', 'p1:update:application', 'p1:read:application'],
  ...
  // OIDC Web Applications
  {
    ...
    description: 'High-security OIDC web application for enterprise environments with strict compliance',
    category: 'built-in',
...

  {
    ...
    configuration: {
      enabled: true,
      redirectUris: ['http://localhost:3000/callback', 'https://localhost:3000/callback', 'http://localhost:8080/callback'],
      postLogoutRedirectUris: ['http://localhost:3000', 'https://localhost:3000', 'http://localhost:8080'],
      grantTypes: ['authorization_code', 'refresh_token'],
      responseTypes: ['code'],
  ...
  // OAuth Authorization Code Flow Presets
  {
    ...
    description: 'Standard OAuth 2.0 Authorization Code flow for web applications with delegated authorization',
    category: 'built-in',
    appType: 'OIDC_WEB_APP',
    configuration: {
      enabled: true,
...

  {
    ...
    description: 'OpenID Connect Authorization Code flow for web applications with authentication and authorization',
    category: 'built-in',
    appType: 'OIDC_WEB_APP',
    configuration: {
      enabled: true,
...

  {
    ...
    description: 'Worker application optimized for token-based API access and background processing',
    category: 'built-in',
    appType: 'WORKER',
    configuration: {
      enabled: true,
...
class PresetManagerServiceImpl implements PresetManagerService {
  ...
  applyPreset(presetId: string): FormDataState | null {
    ...
    return {
      ...
      postLogoutRedirectUris: preset.configuration.postLogoutRedirectUris || defaultFormData.postLogoutRedirectUris,
