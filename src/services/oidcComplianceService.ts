...
export class OIDCComplianceService {
	...
	async validateIDToken(
		...
		try {
			...
			errors.push(
				`ID token parsing failed: ${error instanceof Error ? error.message : 'Unknown error'}`
			);
			return { valid: false, errors };
		}
	}
	 ...
	 */
	validateAtHash(_accessToken: string, atHash: string, algorithm: string = 'RS256'): boolean {
		try {
	 ...
	 */
	validateCHash(_code: string, cHash: string, algorithm: string = 'RS256'): boolean {
		try {
	...
	createOIDCErrorResponse(
		...
		const response: { error: OIDCErrorCode; error_description?: string; state?: string } = {
			error,
		};
	...
	validateOIDCFlow(
		...
		if (userInfo && idTokenValidation.claims) {
			const userInfoValidation = this.validateUserInfoResponse(
				userInfo,
				idTokenValidation.claims.sub
			);
		...
		if (tokenResponse.access_token && idTokenValidation.claims?.at_hash) {
			const atHashValid = this.validateAtHash(
				tokenResponse.access_token,
				idTokenValidation.claims.at_hash
			);
			if (!atHashValid) {
