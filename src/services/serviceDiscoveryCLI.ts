...
import { ServiceDiscoveryService, FlowType, ServiceCategory } from './serviceDiscoveryService.js';
    ...
    if (options.details) {
      ...
      if (service.dependencies?.length > 0) {
        ...
        service.dependencies.forEach((dep: any) => {
          console.log(`     • ${dep.name}: ${dep.purpose}${dep.required ? ' (required)' : ' (optional)'}`);
        });
      }
  ...
  static run(): void {
    try {
      ...
      if (services.length === 0) {
        console.log('No services found matching your criteria. Try adjusting your search or use --help for options.');
        return;
      }
