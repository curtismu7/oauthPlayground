 ...
 */
function getDefaultFieldRule(): FieldRule {
  return {
    visible: true,
    editable: true,
    required: true,
...
function getDeviceAuthorizationRules(): FlowFieldRules {
  return {
    ...
    redirectUri: {
      ...
      editable: false,
      required: false,
      explanation: "Device flow doesn't use browser redirects - user authenticates on a separate device",
    ...
    responseType: {
      ...
      editable: false,
      required: false,
      explanation: "Device flow doesn't use authorization endpoint - uses device authorization endpoint",
      specReference: "https://datatracker.ietf.org/doc/html/rfc8628#section-3.1"
    },
    loginHint: {
      visible: true,
...
function getCIBARules(): FlowFieldRules {
  return {
    ...
    clientId: {
      ...
      required: true,
      explanation: "OAuth 2.0 Client Identifier",
      specReference: "https://openid.net/specs/openid-client-initiated-backchannel-authentication-core-1_0.html"
    },
    clientSecret: {
      visible: true,
      editable: true,
    ...
    redirectUri: {
      ...
      specReference: "https://openid.net/specs/openid-client-initiated-backchannel-authentication-core-1_0.html#rfc.section.1"
    },
    scope: {
      visible: true,
      editable: true,
    ...
    responseType: {
      ...
      editable: false,
      required: false,
      explanation: "CIBA doesn't use authorization endpoint - uses backchannel authentication endpoint",
      specReference: "https://openid.net/specs/openid-client-initiated-backchannel-authentication-core-1_0.html"
    },
    loginHint: {
      visible: true,
      ...
      specReference: "https://openid.net/specs/openid-client-initiated-backchannel-authentication-core-1_0.html#rfc.section.7.1"
    },
    postLogoutRedirectUri: {
      visible: false,
      editable: false,
...
function getJWTBearerRules(): FlowFieldRules {
  return {
    ...
    responseType: {
      ...
      editable: false,
      required: false,
      explanation: "JWT Bearer flow doesn't use authorization endpoint - direct token request with JWT",
      specReference: "https://datatracker.ietf.org/doc/html/rfc7523#section-2.1"
    },
    loginHint: {
      visible: false,
...
function getSAMLBearerRules(): FlowFieldRules {
  return {
    ...
    responseType: {
      ...
      editable: false,
      required: false,
      explanation: "SAML Bearer flow doesn't use authorization endpoint - direct token request with SAML assertion",
      specReference: "https://datatracker.ietf.org/doc/html/rfc7522#section-2.1"
    },
    loginHint: {
      visible: false,
...
export function getFieldRules(
  ...
  console.warn(`[fieldRulesService] Unknown flow type: ${flowType}, using Authorization Code rules`);
  return isOIDC ? getOIDCAuthorizationCodeRules() : getAuthorizationCodeRules();
}
...
export function getValidResponseTypes(flowType: string, isOIDC: boolean): string[] {
  ...
  // Hybrid Flow (OIDC only)
  if (normalized.includes('hybrid')) {
    return ['code', 'id_token', 'token id_token', 'code id_token', 'code token', 'code token id_token'];
  }
