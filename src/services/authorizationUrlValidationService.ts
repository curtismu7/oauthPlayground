...
class AuthorizationUrlValidationService {
	 ...
	 */
	validateAuthorizationUrl(url: string, config: UrlValidationConfig): UrlValidationResult {
		...
		try {
			...
			result.errors.push(
				`URL_VALIDATION_ERROR: ${error instanceof Error ? error.message : 'Failed to parse URL'}`
			);
	...
	private validateFlowSpecificRequirements(
		...
		if (config.flowType !== 'device') {
			...
			if (invalidResponseTypes.length > 0) {
				result.errors.push(
					`URL_VALIDATION_ERROR: Invalid response_type(s): ${invalidResponseTypes.join(', ')}`
				);
			}
			...
			if (missingResponseTypes.length > 0) {
				result.errors.push(
					`URL_VALIDATION_ERROR: Missing required response_type(s): ${missingResponseTypes.join(', ')}`
				);
			}
		}
	}
	...
	private validateScopes(
		...
		if (!validation.isValid) {
			result.errors.push(
				...validation.errors.map((err: string) => `SCOPE_VALIDATION_ERROR: ${err}`)
			);
		}

		if (validation.warnings.length > 0) {
			result.warnings.push(
				...validation.warnings.map((warn: string) => `SCOPE_VALIDATION_WARNING: ${warn}`)
			);
		}
		...
		if (requiresOpenId && !scopes.includes('openid')) {
			...
			result.suggestions.push(
				'Add "openid" to the scope parameter for PingOne authorization flows'
			);
		}
		...
		if (pingOneScopes.length > 0) {
			result.warnings.push(
				`URL_VALIDATION_WARNING: Using PingOne-specific scopes: ${pingOneScopes.join(', ')}`
			);
		}
	}
	...
	private validatePingOneRequirements(
		...
		if (parsedUrl.maxAge) {
			const maxAge = parseInt(parsedUrl.maxAge, 10);
	...
	getValidationSummary(result: UrlValidationResult): {
		...
		if (!isValid) {
			...
			message =
				'The authorization URL has warnings but can proceed. Consider addressing these issues.';
			canProceed = true;
		} else {
