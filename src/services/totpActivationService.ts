...
class TOTPActivationService {
	...
	static async activateTOTPDevice(
		...
		try {
			...
			const validation = await TOTPActivationService.validateTOTPCode(
				config.secret,
				activationCode,
				{
			...
			if (!validation.valid) {
				TOTPActivationService.recordValidationAttempt(
					config.deviceId,
					config.userId,
					activationCode,
					false,
			...
			if (options?.generateBackupCodes !== false) {
				backupCodes = await TOTPActivationService.generateBackupCodes(
					config.deviceId,
					config.userId
				);
			}
	...
	static async validateTOTPCode(
		...
		try {
			...
			for (let skew = -allowedSkew; skew <= allowedSkew; skew++) {
				...
				const expectedCode = TOTPActivationService.generateTOTPCode(secret, timeStep);
	...
	static async generateBackupCodes(
		deviceId: string,
		userId: string,
		count: number = TOTPActivationService.BACKUP_CODE_COUNT
	): Promise<BackupCode[]> {
		try {
			...
			for (let i = 0; i < count; i++) {
				const code = TOTPActivationService.generateSecureBackupCode();
				codes.push({
					code,
			...
			const existingSet = TOTPActivationService.backupCodeSets.get(deviceId);
			...
			TOTPActivationService.backupCodeSets.set(deviceId, backupCodeSet);
	...
	static validateBackupCode(
		...
		try {
			const backupCodeSet = TOTPActivationService.backupCodeSets.get(deviceId);
			...
			const matchingCode = backupCodeSet.codes.find((c) => c.code === code && !c.used);
			...
			const shouldRegenerate =
				remainingCodes <= Math.floor(TOTPActivationService.BACKUP_CODE_COUNT / 2);
	...
	static getBackupCodeStats(
		...
		const shouldRegenerate = remaining <= Math.floor(TOTPActivationService.BACKUP_CODE_COUNT / 2);
	 ...
	 */
	static async regenerateBackupCodes(deviceId: string, userId: string): Promise<BackupCode[]> {
		logger.info('TOTPActivationService', 'Regenerating backup codes', {
			deviceId,
			userId,
		});

		return TOTPActivationService.generateBackupCodes(deviceId, userId);
	}
	...
	static getValidationAttempts(deviceId: string): TOTPValidationAttempt[] {
		return TOTPActivationService.validationAttempts.get(deviceId) || [];
	}
	...
	static isDeviceLocked(deviceId: string): {
		...
		const attempts = TOTPActivationService.validationAttempts.get(deviceId) || [];
		const recentAttempts = attempts.filter((attempt) => {
			const cutoff = new Date(
				Date.now() - TOTPActivationService.VALIDATION_WINDOW_MINUTES * 60 * 1000
			);
		...
		if (failedAttempts.length >= TOTPActivationService.MAX_VALIDATION_ATTEMPTS) {
			const oldestFailedAttempt = failedAttempts[0];
			const lockoutTime = new Date(
				oldestFailedAttempt.timestamp.getTime() +
					TOTPActivationService.VALIDATION_WINDOW_MINUTES * 60 * 1000
			);
		...
		return {
			locked: false,
			attemptsRemaining: TOTPActivationService.MAX_VALIDATION_ATTEMPTS - failedAttempts.length,
		};
	}
	...
	static clearValidationAttempts(deviceId: string): void {
		TOTPActivationService.validationAttempts.delete(deviceId);
	...
	private static generateSecureBackupCode(): string {
		...
		const array = new Uint8Array(TOTPActivationService.BACKUP_CODE_LENGTH);
		crypto.getRandomValues(array);

		for (let i = 0; i < TOTPActivationService.BACKUP_CODE_LENGTH; i++) {
	...
	private static generateTOTPCode(secret: string, timeStep: number): string {
		...
		const hash = TOTPActivationService.hmacSha1(secret, timeStep.toString());
	...
	private static recordValidationAttempt(
		...
		const attempt: TOTPValidationAttempt = {
			...
			timeWindow,
			error,
			ipAddress: TOTPActivationService.getCurrentIP(),
			userAgent: TOTPActivationService.getCurrentUserAgent(),
		};
		...
		TOTPActivationService.validationAttempts.set(deviceId, recentAttempts);
	}
