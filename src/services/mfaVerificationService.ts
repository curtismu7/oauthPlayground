...
class MFAVerificationService {
	...
	static async completeMFAVerification(
		...
		try {
			...
			const sessionId = MFAVerificationService.getCurrentSessionId(userId);
			...
			MFAVerificationService.recordVerificationEvent(userId, verificationEvent);

			// Calculate authentication level and risk score
			const authLevel = MFAVerificationService.calculateAuthenticationLevel(
				deviceType,
				verificationMethod
			);
			const riskScore = await MFAVerificationService.calculateRiskScore(
				userId,
				deviceId,
				deviceType
			);
			const complianceFlags = MFAVerificationService.generateComplianceFlags(
				deviceType,
				verificationMethod,
				riskScore
			);
			...
			const completionData: MFACompletionData = {
				...
				sessionId,
				ipAddress: MFAVerificationService.getCurrentIP(),
				userAgent: MFAVerificationService.getCurrentUserAgent(),
				riskScore,
				authenticationLevel: authLevel,
				complianceFlags,
				auditTrail: MFAVerificationService.getVerificationEvents(userId),
			};

			MFAVerificationService.completionData.set(userId, completionData);

			// Update session with MFA completion
			const sessionUpdated = await MFAVerificationService.updateSessionWithMFA(userId, {
				sessionId,
				mfaCompleted: true,
			...
			if (sessionUpdated) {
				...
				MFAVerificationService.recordVerificationEvent(userId, sessionEvent);
			}
			...
			MFAVerificationService.recordVerificationEvent(userId, failureEvent);
	...
	static recordVerificationAttempt(
		...
		const attempts = MFAVerificationService.verificationAttempts.get(userId) || [];
		...
		MFAVerificationService.verificationAttempts.set(userId, recentAttempts);
	...
	static getMFACompletionData(userId: string): MFACompletionData | null {
		return MFAVerificationService.completionData.get(userId) || null;
	}
	...
	static getVerificationEvents(userId: string): MFAVerificationEvent[] {
		return MFAVerificationService.verificationEvents.get(userId) || [];
	}
	...
	static getVerificationAttempts(userId: string): VerificationAttempt[] {
		return MFAVerificationService.verificationAttempts.get(userId) || [];
	}
	...
	static hasRecentMFACompletion(userId: string, withinMinutes: number = 30): boolean {
		const completionData = MFAVerificationService.completionData.get(userId);
	...
	static getVerificationStats(userId: string): {
		...
		const attempts = MFAVerificationService.verificationAttempts.get(userId) || [];
		const events = MFAVerificationService.verificationEvents.get(userId) || [];
		...
		const deviceUsage = attempts.reduce(
			(acc, attempt) => {
		...
		const mostUsedDevice = Object.keys(deviceUsage).reduce(
			(a, b) => (deviceUsage[a] > deviceUsage[b] ? a : b),
			Object.keys(deviceUsage)[0]
		);
	...
	static clearVerificationData(userId: string): void {
		MFAVerificationService.verificationEvents.delete(userId);
		MFAVerificationService.verificationAttempts.delete(userId);
		MFAVerificationService.completionData.delete(userId);
	...
	private static recordVerificationEvent(userId: string, event: MFAVerificationEvent): void {
		...
		MFAVerificationService.verificationEvents.set(userId, recentEvents);
	}
	...
	private static async calculateRiskScore(
		...
		const recentFailures = attempts.filter(
			(a) => !a.success && a.timestamp > new Date(Date.now() - 60 * 60 * 1000) // Last hour
		).length;
