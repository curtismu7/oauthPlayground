...
class EnvironmentServiceV8 {
	...
	static async getEnvironment(
		...
		const response = await pingOneFetch(
			`${EnvironmentServiceV8.BASE_PATH}/${id}?${params.toString()}`
		);
	...
	static async updateEnvironmentStatus(
		...
		const response = await pingOneFetch(
			`${EnvironmentServiceV8.BASE_PATH}/${id}/status?${params.toString()}`,
			{
				method: 'PUT',
				headers: {
					'Content-Type': 'application/json',
	...
	static async performBulkOperation({
		...
		const response = await pingOneFetch(
			`${EnvironmentServiceV8.BASE_PATH}/bulk?${params.toString()}`,
			{
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
	...
	static async getEnvironmentUsage(
		...
		const response = await pingOneFetch(
			`${EnvironmentServiceV8.BASE_PATH}/${id}/usage?${params.toString()}`
		);
	...
	static async getEnvironmentCapabilities(
		...
		const response = await pingOneFetch(
			`${EnvironmentServiceV8.BASE_PATH}/${id}/capabilities?${params.toString()}`
		);
