...
export class FlowContextService {
	...
	static saveFlowContext(flowId: string, context: FlowContext): boolean {
		try {
			...
			if (!validation.valid) {
				logger.warn('FlowContextService', '[FlowContextService] Invalid flow context:', {
			...
			const enhancedContext: FlowContext = {
				...
				metadata: {
					...context.metadata,
					sessionId: FlowContextService.generateSessionId(),
					flowId,
			...
			if (contextString.length > FlowContextService.MAX_CONTEXT_SIZE) {
				logger.warn('FlowContextService', '[FlowContextService] Flow context exceeds size limit');
				return false;
			}

			// Determine storage key based on flow type
			const storageKey = FlowContextService.getStorageKey(context.flowType);

			sessionStorage.setItem(storageKey, contextString);

			logger.info('FlowContextService', `[FlowContextService] Saved flow context for ${flowId}:`, {
				arg0: {
			...
			logger.error(
				'FlowContextService',
				'[FlowContextService] Failed to save flow context:',
				undefined,
				error as Error
			);
			return false;
	 ...
	 */
	static getFlowContext(_flowId?: string): FlowContext | null {
		try {
			// Try different storage keys
			const storageKeys = [
				FlowContextService.FLOW_CONTEXT_KEY,
				FlowContextService.TOKEN_MANAGEMENT_CONTEXT_KEY,
			...
			for (const key of storageKeys) {
				...
				if (contextString) {
					...
					const validation = FlowContextService.validateFlowContext(context);
					if (!validation.valid) {
						logger.warn('FlowContextService', `[FlowContextService] Invalid context from ${key}:`, {
							arg0: validation.errors,
						});
					...
					if (FlowContextService.isContextExpired(context)) {
						logger.warn('FlowContextService', `[FlowContextService] Expired context from ${key}`);
						FlowContextService.clearFlowContext(key);
						continue;
					}
					...
					logger.info(
						'FlowContextService',
						`[FlowContextService] Retrieved valid flow context from ${key}:`,
						{
							arg0: {
								flowType: context.flowType,
								returnPath: context.returnPath,
			...
			logger.info('FlowContextService', '[FlowContextService] No valid flow context found');
			return null;
		} catch (error) {
			logger.error(
				'FlowContextService',
				'[FlowContextService] Failed to get flow context:',
				undefined,
				error as Error
			);
			return null;
	...
	static clearFlowContext(flowId?: string): void {
		try {
			...
			logger.info('FlowContextService', '[FlowContextService] Cleared flow context');
		} catch (error) {
			logger.error(
				'FlowContextService',
				'[FlowContextService] Failed to clear flow context:',
				undefined,
				error as Error
			);
		}
	...
	static buildReturnPath(
		...
		try {
			...
			logger.info('FlowContextService', `[FlowContextService] Built return path for ${flowType}:`, {
				arg0: basePath,
			});
			return basePath;
		} catch (error) {
			logger.error(
				'FlowContextService',
				'[FlowContextService] Failed to build return path:',
				undefined,
				error as Error
			);
			return '/dashboard';
	...
	static validateFlowContext(context: FlowContext): FlowContextValidationResult {
		...
		try {
			...
			if (context.returnPath && !FlowContextService.isValidReturnPath(context.returnPath)) {
				errors.push('returnPath contains invalid characters or patterns');
			}

			// Size validation
			const contextSize = JSON.stringify(context).length;
			if (contextSize > FlowContextService.MAX_CONTEXT_SIZE) {
				errors.push(
					`Context size (${contextSize}) exceeds maximum allowed (${FlowContextService.MAX_CONTEXT_SIZE})`
				);
			}

			// Age validation
			if (context.timestamp && FlowContextService.isContextExpired(context)) {
				warnings.push('Context is older than maximum allowed age');
			}

			// Flow type validation
			if (context.flowType && !FlowContextService.isValidFlowType(context.flowType)) {
	...
	static handleRedirectReturn(callbackData: any): RedirectResult {
		try {
			...
			if (!context) {
				logger.info(
					'FlowContextService',
					'[FlowContextService] No flow context found, using default redirect'
				);
				return {
					success: true,
			...
			if (!FlowContextService.validateRedirectSecurity(context)) {
				logger.warn(
					'FlowContextService',
					'[FlowContextService] Security validation failed, using safe redirect'
				);
			...
			logger.info('FlowContextService', '[FlowContextService] Redirect return successful:', {
				arg0: {
					flowType: context.flowType,
					redirectUrl,
				},
			...
			logger.error(
				'FlowContextService',
				'[FlowContextService] Failed to handle redirect return:',
				undefined,
				error as Error
			);
			return {
	...
	static validateRedirectSecurity(context: FlowContext): boolean {
		try {
			...
			if (!FlowContextService.isValidReturnPath(context.returnPath)) {
				return false;
			}
			...
			logger.error(
				'FlowContextService',
				'[FlowContextService] Security validation error:',
				undefined,
				error as Error
			);
			return false;
	...
	private static getStorageKey(flowType: string): string {
		...
		if (tokenManagementFlows.includes(flowType)) {
			return FlowContextService.TOKEN_MANAGEMENT_CONTEXT_KEY;
		}
		...
		return FlowContextService.FLOW_CONTEXT_KEY;
	}

	private static isContextExpired(context: FlowContext): boolean {
		return Date.now() - context.timestamp > FlowContextService.MAX_CONTEXT_AGE_MS;
	}
	...
	static safeJsonParse(jsonString: string): any {
		try {
			...
			logger.warn(
				'FlowContextService',
				'[FlowContextService] Failed to parse JSON:',
				undefined,
				error as Error
			);
			return null;
	 ...
	 */
	static updateFlowContext(flowId: string, updates: Partial<FlowContext>): boolean {
		const existingContext = FlowContextService.getFlowContext(flowId);
		if (!existingContext) {
			return false;
		}
		...
		return FlowContextService.saveFlowContext(flowId, updatedContext);
	}
}
