import { FiShield, FiKey, FiLock, FiZap } from 'react-icons/fi';

export type AuthMethod = 'client_secret_basic' | 'client_secret_post' | 'client_secret_jwt' | 'private_key_jwt';
...
export class AuthMethodService {
  /**
   * Get configuration for all supported authentication methods
   */
  ...
  static getMethodConfig(method: AuthMethod): AuthMethodConfig {
    const configs = this.getAllMethodConfigs();
  ...
  static getSupportedMethods(): AuthMethod[] {
    return Object.keys(this.getAllMethodConfigs()) as AuthMethod[];
  }
  ...
  static isValidMethod(method: string): method is AuthMethod {
    return method in this.getAllMethodConfigs();
  }

  /**
   * Get the icon component for an authentication method
   */
  ...
  static getDisplayName(method: AuthMethod): string {
    return this.getMethodConfig(method).authMethod
      .split('_')
      .map(word => word.charAt(0).toUpperCase() + word.slice(1) + (word === 'jwt' ? '' : ''))
      .join(' ');
  }
}
...
// React component for selecting authentication method
export const AuthMethodSelector: React.FC<AuthMethodSelectorProps> = ({
  ...
  return (
    <div className={className}>
      <div className="mb-4">
        <label htmlFor="auth-method-select" className="block text-sm font-medium text-gray-700 mb-1">
      ...
      <div className="p-4 border border-gray-200 rounded-lg bg-gray-50">
        <div className="flex items-center mb-3">
          ...
          <div>
            <h3 className="font-medium text-gray-900">{AuthMethodService.getDisplayName(selectedMethod)}</h3>
