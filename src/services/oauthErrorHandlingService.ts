...
export class OAuthErrorHandlingService {
	...
	static parseOAuthError(error: any, context: OAuthErrorContext): OAuthErrorDetails {
		...
		return {
			message: OAuthErrorHandlingService.getUserFriendlyMessage(errorType, context),
			troubleshootingSteps: OAuthErrorHandlingService.getTroubleshootingSteps(errorType, context),
			errorType,
			recoveryActions: OAuthErrorHandlingService.getRecoveryActions(errorType, context),
			correlationId,
		};
	}
	...
	private static getUserFriendlyMessage(
		errorType: OAuthErrorDetails['errorType'],
		context: OAuthErrorContext
	): string {
		const flowName = OAuthErrorHandlingService.getFlowDisplayName(context.flowType);
	...
	private static getTroubleshootingSteps(
		...
		switch (errorType) {
			case 'invalid_credentials':
				return OAuthErrorHandlingService.getInvalidCredentialsSteps(context);
			case 'forbidden':
				return OAuthErrorHandlingService.getForbiddenSteps(context);
			case 'not_found':
				return OAuthErrorHandlingService.getNotFoundSteps(context);
			case 'network':
				return OAuthErrorHandlingService.getNetworkErrorSteps();
			case 'server_error':
				return OAuthErrorHandlingService.getServerErrorSteps();
			case 'invalid_grant':
				return OAuthErrorHandlingService.getInvalidGrantSteps(context);
			case 'invalid_scope':
				return OAuthErrorHandlingService.getInvalidScopeSteps(context);
			case 'unauthorized_client':
				return OAuthErrorHandlingService.getUnauthorizedClientSteps(context);
			case 'unsupported_grant_type':
				return OAuthErrorHandlingService.getUnsupportedGrantTypeSteps(context);
			default:
				return OAuthErrorHandlingService.getGenericErrorSteps(flowName);
		}
	}
...
4. **Application Type**: Ensure your application is configured as:
   ...
   - **Grant Types**: "${OAuthErrorHandlingService.getRequiredGrantTypes(context.flowType)}" must be enabled
...
1. **Check Scopes**: Your application may not have the required scopes
   - Required: ${OAuthErrorHandlingService.getRequiredScopes(context.flowType)}
...
4. **User Permissions**: Ensure your user account has the necessary permissions`;
	...
	private static getNotFoundSteps(_context: OAuthErrorContext): string {
		return `
🔧 **Troubleshooting Steps:**
...
4. **Alternative Region**: Try using a different PingOne region if available`;
	...
	private static getInvalidGrantSteps(_context: OAuthErrorContext): string {
		return `
🔧 **Troubleshooting Steps:**
...
1. **Grant Types**: Ensure your application supports the required grant types
   - Required: ${OAuthErrorHandlingService.getRequiredGrantTypes(context.flowType)}
...
1. **Grant Type**: Verify the grant type is correct for this flow
   - Expected: ${OAuthErrorHandlingService.getRequiredGrantTypes(context.flowType)}
...
3. **Flow Type**: Verify you're using the correct flow for your use case
   ...
   - Device Authorization: For devices with limited input capabilities`;
	...
	private static getGenericErrorSteps(_flowName: string): string {
		return `
🔧 **Troubleshooting Steps:**
...
💡 **Need Help?** Check the PingOne documentation or contact your PingOne administrator.`;
	...
	private static getRequiredGrantTypes(flowType: OAuthErrorContext['flowType']): string {
		switch (flowType) {
			...
			case 'hybrid':
				return 'Authorization Code, Implicit';
			case 'token_exchange':
				return 'Client Credentials';
			case 'mfa':
				return 'Client Credentials, Authorization Code';
			default:
				return 'varies by flow';
		}
	}
	...
	private static getRequiredScopes(flowType: OAuthErrorContext['flowType']): string {
		switch (flowType) {
			case 'authorization_code':
				return '`openid profile email` (PingOne requires openid for all flows)';
			case 'implicit':
				return '`openid profile email` (PingOne requires openid for all flows)';
			case 'client_credentials':
				return '`p1:read:user p1:update:user` (or your API scopes)';
			case 'device_authorization':
				return '`openid profile email` (consistent for both OAuth and OIDC)';
			case 'hybrid':
				return '`openid profile email` (OIDC hybrid flows)';
			case 'token_exchange':
				return '`urn:ietf:params:oauth:token-exchange` (token exchange scope)';
			case 'mfa':
				return '`p1:read:user p1:update:user p1:read:device p1:update:device` (MFA operations)';
			default:
				return 'varies by flow';
		}
	}
