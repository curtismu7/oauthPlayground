...
export class FlowFactory {
  ...
  static createFlow(config: FlowFactoryConfig): FlowFactoryResult {
    ...
    if (!flowConfig) {
      throw new Error(`Flow type '${flowType}' not found. Available flows: ${Array.from(FlowConfigService.getAllFlowConfigs().map(f => f.flowType)).join(', ')}`);
    }
  ...
  static getFlowStatistics(): {
    ...
    const flowsByVariant = flows.reduce((acc, flow) => {
      acc[flow.flowVariant] = (acc[flow.flowVariant] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    const flowsByTheme = flows.reduce((acc, flow) => {
