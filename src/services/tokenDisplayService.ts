...
class TokenDisplayService {
	 ...
	 */
	public static getTokenInfo(token: string, type: 'access' | 'id' | 'refresh', flow?: string): TokenInfo {
		return {
			type,
	...
	public static logTokenRender(tokenInfo: TokenInfo): void {
		...
		console.log(`[${timestamp}][🔐 TOKEN-DISPLAY-V6][INFO] Rendered ${tokenInfo.type} Token panel (length=${tokenInfo.length}, isJWT=${tokenInfo.isJWT}, flow=${tokenInfo.flow || 'unknown'})`);
	}
	...
	public static logTokenCopy(tokenInfo: TokenInfo): void {
		const timestamp = new Date().toISOString().replace('T', ' ').substring(0, 19);
		console.log(`[${timestamp}][🔐 TOKEN-DISPLAY-V6][INFO] Copied ${tokenInfo.type} Token (length=${tokenInfo.length}, flow=${tokenInfo.flow || 'unknown'})`);
	}
	...
	public static logTokenDecode(tokenInfo: TokenInfo, success: boolean): void {
		...
		console.log(`[${timestamp}][🔐 TOKEN-DISPLAY-V6][${level}] Decode ${tokenInfo.type} Token (length=${tokenInfo.length}, isJWT=${tokenInfo.isJWT}, success=${success}, flow=${tokenInfo.flow || 'unknown'})`);
	}
	 ...
	 */
	public static getTokenLabel(tokenType: 'access' | 'id' | 'refresh', isOIDC: boolean = false): string {
		switch (tokenType) {
			case 'access':
				return 'Access Token';
			case 'id':
