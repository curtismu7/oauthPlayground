...
export class QRCodeService {
	...
	static async generateTOTPQRCode(config: TOTPConfig): Promise<QRCodeResult> {
		try {
			...
			logger.error(
				'QrCodeService',
				'[QRCodeService] Failed to generate TOTP QR code:',
				undefined,
				error as Error
			);
			throw new Error(
	...
	static generateManualEntryCode(secret: string): string {
		try {
			...
			logger.info('QrCodeService', '[QRCodeService] Generated manual entry code');
			return formatted;
		} catch (error) {
			logger.error(
				'QrCodeService',
				'[QRCodeService] Failed to generate manual entry code:',
				undefined,
				error as Error
			);
			throw new Error(
	...
	static validateTOTPCode(
		...
		try {
			...
			logger.warn('QrCodeService', '[QRCodeService] TOTP code validation failed');
			return { valid: false, error: 'Invalid code or code has expired' };
		} catch (error) {
			logger.error(
				'QrCodeService',
				'[QRCodeService] TOTP validation error:',
				undefined,
				error as Error
			);
			return {
	...
	static generateTOTPSecret(length: number = 32): string {
		try {
			...
			logger.info('QrCodeService', '[QRCodeService] Generated TOTP secret');
			return secret;
		} catch (error) {
			logger.error(
				'QrCodeService',
				'[QRCodeService] Failed to generate TOTP secret:',
				undefined,
				error as Error
			);
			throw new Error(
	...
	private static async generateQRCodeDataUrl(totpUri: string): Promise<string> {
		try {
			...
			logger.error(
				'QrCodeService',
				'[QRCodeService] Failed to generate QR code data URL:',
				undefined,
				error as Error
			);
	...
	private static generateTOTPCode(
		...
		try {
			...
			logger.error(
				'QrCodeService',
				'[QRCodeService] TOTP code generation error:',
				undefined,
				error as Error
			);
