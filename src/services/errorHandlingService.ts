...
const ERROR_PATTERNS = {
	...
	[ErrorType.AUTHORIZATION]: [/access_denied/i, /insufficient_scope/i, /forbidden/i, /403/i],
	[ErrorType.VALIDATION]: [/invalid_request/i, /invalid_grant/i, /missing/i, /required/i, /400/i],
	[ErrorType.TIMEOUT]: [/timeout/i, /took too long/i, /408/i],
	[ErrorType.RATE_LIMIT]: [/rate limit/i, /too many requests/i, /429/i],
...
export class ErrorHandlingService {
	 ...
	 */
	static determineSeverity(errorType: ErrorType, _context?: ErrorContext): ErrorSeverity {
		switch (errorType) {
	 ...
	 */
	static getUserFriendlyMessage(errorType: ErrorType, _context?: ErrorContext): string {
		const messages = {
			[ErrorType.NETWORK]:
			...
			[ErrorType.AUTHORIZATION]:
				"You don't have permission to perform this action. Please check your access rights or contact your administrator.",
	...
	static shouldContactSupport(errorType: ErrorType): boolean {
		return [ErrorType.SERVER_ERROR, ErrorType.CONFIGURATION, ErrorType.UNKNOWN].includes(errorType);
	}
	...
	private static generateErrorResponse(
		flowError: FlowError
	): Omit<ErrorResponse, 'recoveryOptions' | 'correlationId'> {
		return {
			type: flowError.type,
	...
	static getErrorAnalytics(errorType?: ErrorType): ErrorAnalytics[] {
		if (errorType) {
			return ErrorHandlingService.analyticsStore.has(errorType)
				? [ErrorHandlingService.analyticsStore.get(errorType)!]
				: [];
		}
...
export const getUserFriendlyMessage =
	ErrorHandlingService.getUserFriendlyMessage.bind(ErrorHandlingService);
