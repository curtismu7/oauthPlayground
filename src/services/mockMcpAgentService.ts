...
const MOCK_TOOLS: MockToolsListResponse['tools'] = [
  { name: 'mock_get_token', description: 'Obtain a mock worker token (simulated client credentials)', inputSchema: { type: 'object', properties: { clientId: { type: 'string' }, clientSecret: { type: 'string' } } } },
  { name: 'mock_token_exchange', description: 'Exchange a subject token for a new token with broader scope (RFC 8693)', inputSchema: { type: 'object', properties: { subject_token: { type: 'string' }, requested_scope: { type: 'string' } } } },
  { name: 'mock_list_users', description: 'List mock users (requires access token)', inputSchema: { type: 'object', properties: { access_token: { type: 'string' } } } },
];
...
export function callTool(name: MockToolName, args: Record<string, unknown>): MockToolCallResult {
  ...
  if (name === 'mock_token_exchange') {
    ...
    if (!subjectToken) {
      return { success: false, tool: 'mock_token_exchange', error: 'subject_token required', request: args };
    }
  ...
  if (name === 'mock_list_users') {
    ...
    if (!token) {
      return { success: false, tool: 'mock_list_users', error: 'access_token required', request: args };
    }
    const users = [{ id: 'u1', name: { given: 'Alice', family: 'User' } }, { id: 'u2', name: { given: 'Bob', family: 'Demo' } }];
    return {
      success: true,
