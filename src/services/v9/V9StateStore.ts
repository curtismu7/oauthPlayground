// src/services/v9/V9StateStore.ts
// Ephemeral in-memory + sessionStorage-backed store for V9 mock flows.
// Tracks authorization codes, tokens, device codes with expirations and safe consumption.
//
// Migrated from V7MStateStore.ts — updated prefix/keys to v9.
...
type StoreShape = {
	authCodes: Record<string, V9AuthorizationCodeRecord>;
	tokens: Record<string, V9TokenRecord>;
	deviceCodes: Record<string, V9DeviceCodeRecord>;
};

const SESSION_KEY = 'v9:state';
let memoryStore: StoreShape = loadFromSession();

export const V9StateStore = {
	// Authorization Codes
	saveAuthorizationCode(record: V9AuthorizationCodeRecord): void {
		memoryStore.authCodes[record.code] = record;
		persist();
	},
	getAuthorizationCode(code: string): V9AuthorizationCodeRecord | undefined {
	...
	consumeAuthorizationCode(code: string): V9AuthorizationCodeRecord | undefined {
	...
	// Tokens
	saveToken(key: string, record: V9TokenRecord): void {
		memoryStore.tokens[key] = record;
		persist();
	},
	getToken(key: string): V9TokenRecord | undefined {
	...
	saveDeviceCode(record: V9DeviceCodeRecord): void {
		memoryStore.deviceCodes[record.deviceCode] = record;
		persist();
	},
	getDeviceCode(deviceCode: string): V9DeviceCodeRecord | undefined {
	...
	getDeviceCodeByUserCode(userCode: string): V9DeviceCodeRecord | undefined {
