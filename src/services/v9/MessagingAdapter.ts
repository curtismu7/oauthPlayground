...
class V8ToastAdapter implements IMessagingAdapter {
  ...
  clear(): void {
    ...
    console.log('[MessagingAdapter] V8 toast messages auto-dismiss');
  }
}
  ...
  success(message: string, config?: Partial<MessageConfig>): void {
    if (config?.recoveryActions || config?.dismissible === true) {
      ...
      modernMessaging.showBanner({
        ...
        actions: config?.recoveryActions?.map(action => ({
          label: action.label,
...
function getMessagingAdapter(): IMessagingAdapter {
  ...
  try {
    ...
    if (testState !== undefined) {
      console.log('[MessagingAdapter] Using V9 Modern Messaging');
      return new V9ModernMessagingAdapter();
    }
  } catch (err) {
    console.log('[MessagingAdapter] V9 Modern Messaging not available, falling back to V8 Toast');
  }

  // Check if V8 toast is available
  try {
    if (typeof toastV8 === 'object' && (toastV8 as any).success) {
      console.log('[MessagingAdapter] Using V8 Toast');
      return new V8ToastAdapter();
    }
  } catch (err) {
    console.log('[MessagingAdapter] V8 Toast not available');
  }
...
class ConsoleMessagingAdapter implements IMessagingAdapter {
  success(message: string, config?: Partial<MessageConfig>): void {
    console.log(`✅ [SUCCESS] ${config?.title ? `${config.title}: ` : ''}${message}`);
  }
  ...
  info(message: string, config?: Partial<MessageConfig>): void {
    console.info(`ℹ️ [INFO] ${config?.title ? `${config.title}: ` : ''}${message}`);
  }

  clear(): void {
    console.log('[MessagingAdapter] Console messages cleared');
  }
}
...
export function withMessagingAdapter<P extends object>(
  ...
  return function WrappedComponent(props: P) {
    ...
    return React.createElement(ComponentType, { ...(props as P & { messaging: IMessagingAdapter }), messaging });
  };
}
