// src/services/flowComponentService.ts
...
export class FlowComponentService {
   ...
   */
  static createFlowContainer(theme: string = 'blue') {
    return styled.div`
      min-height: 100vh;
    ...
    return styled.div<{ $variant?: 'warning' | 'info' | 'success' | 'danger' }>`
      background: ${({ $variant }) => {
        switch ($variant) {
          ...
          case 'info': return 'linear-gradient(135deg, #dbeafe 0%, #bfdbfe 100%)';
   ...
   */
  static createCollapsibleSectionComponent(config: CollapsibleSectionConfig) {
    const Section = this.createCollapsibleSection();
    const HeaderButton = this.createCollapsibleHeaderButton();
    const Title = this.createCollapsibleTitle();
    const ToggleIcon = this.createCollapsibleToggleIcon();
    const Content = this.createCollapsibleContent();
  ...
  /**
   * Create a requirements indicator component
   */
  static createRequirementsIndicatorComponent(config: RequirementsIndicatorConfig) {
    const Indicator = this.createRequirementsIndicator();
    const Icon = this.createRequirementsIcon();
    const Text = this.createRequirementsText();

    return (
      <Indicator $variant={config.variant}>
        <Icon>
          <FiAlertCircle />
        </Icon>
        <Text>
   ...
   */
  static createActionButtonsComponent(actions: ActionConfig[]) {
