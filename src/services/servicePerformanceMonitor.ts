...
import { ServiceInstance, ServicePerformanceMetrics, serviceRegistry } from './serviceRegistry';
...
export class ServicePerformanceMonitor extends EventEmitter {
	...
	startMonitoring(): void {
		if (this.isMonitoring) {
			console.log(
				'[ServicePerformanceMonitor] Performance monitoring already running, skipping...'
			);
			return;
		}
	...
	private updateServiceMetrics(name: string, service: ServiceInstance): void {
		...
		const _throughput = (metrics.totalRequests / timeSinceCreation) * 1000;
	...
	private createAlert(
		...
		const existingAlert = Array.from(this.alerts.values()).find(
			(alert) => alert.serviceName === serviceName && alert.level === level && !alert.resolved
		);
		...
		console.warn(
			`[ServicePerformanceMonitor] ${level.toUpperCase()} Alert: ${serviceName} - ${message}`
		);
	...
	private onServiceError(event: {
		...
		service.errorCount++;
		service.performanceMetrics.errorRate = (service.errorCount / service.accessCount) * 100;
	...
	getPerformanceSummary(): {
		...
		const totalResponseTime = services.reduce(
			(sum, service) => sum + service.performanceMetrics.averageResponseTime,
			0
		);
