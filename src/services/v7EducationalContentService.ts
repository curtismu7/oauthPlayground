...
export class V7EducationalContentService {
		...
		const contentMap: Record<V7FlowName, V7EducationalContent> = {
			'oauth-authorization-code-v7': {
				...
				overview: {
					...
					description: 'The most secure OAuth 2.0 flow, using a two-step process to obtain access tokens. It involves redirecting the user to an authorization server, obtaining an authorization code, and then exchanging that code for tokens.',
					keyConcepts: [
					...
					flowDiagram: 'User → Client → Authorization Server → User → Authorization Server → Client → Resource Server'
				},
				stepByStepGuide: {
					step: 1,
					title: 'Authorization Request',
					description: 'The client redirects the user to the authorization server with the required parameters.',
					...
					quizzes: [
						{
							...
							correctAnswer: 1,
							explanation: 'The state parameter is used to prevent CSRF attacks by allowing the client to verify that the authorization response corresponds to the original request.'
						},
						{
							...
							correctAnswer: 2,
							explanation: 'The response_type parameter is required and must be "code" for the authorization code flow.'
						}
					],
					scenarios: [
						{
							title: 'Mobile App Authorization',
							description: 'A mobile app needs to access a user\'s photos from a cloud service. How should it implement OAuth 2.0?',
							solution: 'Use the authorization code flow with PKCE. The app should redirect to the authorization server, obtain an authorization code, and exchange it for an access token.',
							learningPoints: [
			...
			'oidc-authorization-code-v7': {
				...
				overview: {
					...
					description: 'Extends OAuth 2.0 to provide identity information through ID tokens. It follows the same pattern as OAuth 2.0 but adds identity layer with ID tokens and user information.',
					keyConcepts: [
					...
					flowDiagram: 'User → Client → Authorization Server → User → Authorization Server → Client → UserInfo Endpoint'
				},
				stepByStepGuide: {
					step: 1,
					title: 'Authorization Request',
					description: 'The client redirects the user to the authorization server with OIDC-specific parameters.',
					...
					quizzes: [
						{
							...
							correctAnswer: 1,
							explanation: 'The nonce parameter is used to prevent replay attacks by ensuring that each ID token is unique and cannot be reused.'
						},
						{
							...
							correctAnswer: 2,
							explanation: 'The "openid" scope is required for OpenID Connect flows to indicate that the client is requesting an ID token.'
						}
					],
					scenarios: [
						{
							title: 'Single Sign-On Implementation',
							description: 'A company wants to implement SSO across multiple applications. How should they use OpenID Connect?',
							solution: 'Use OpenID Connect with a centralized identity provider. Each application should validate ID tokens and use the access token for API calls.',
							learningPoints: [
	...
	static getAllEducationalContent(): V7EducationalContent[] {
		...
		return flowNames.map(flowName => this.getEducationalContent(flowName));
	}
	...
	static getSpecificationReferences(flowName: V7FlowName): Array<{
		...
		const content = this.getEducationalContent(flowName);
...
export default V7EducationalContentService;
