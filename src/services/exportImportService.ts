...
class ExportImportServiceImpl implements ExportImportService {
		...
		try {
			const exportConfig: ExportedConfiguration = {
				...
				metadata: {
					...
					description:
						metadata.description || formData.description || 'Exported application configuration',
	...
	validateImportedConfiguration(config: any): ImportValidationResult {
		...
		try {
			...
			if (!config.version) {
				...
				errors.push(
					`Unsupported configuration version: ${config.version}. Supported versions: ${SUPPORTED_VERSIONS.join(', ')}`
				);
			}
			...
			if (!config.appType) {
				...
				!['OIDC_WEB_APP', 'OIDC_NATIVE_APP', 'SINGLE_PAGE_APP', 'WORKER', 'SERVICE'].includes(
					config.appType
				)
			) {
	 ...
	 */
	exportPresetAsConfiguration(_presetId: string): void {
		try {
	...
	private sanitizeConfiguration(config: FormDataState): FormDataState {
		return {
			...
			responseTypes: Array.isArray(config.responseTypes)
				...
				: [],
			scopes: Array.isArray(config.scopes) ? config.scopes.filter((scope) => scope?.trim()) : [],
	...
	private checkSecurityIssues(
		...
		if (config.accessTokenValiditySeconds > 86400) {
			// > 24 hours
			warnings.push(
				'Access token validity period is very long (>24 hours). Consider shorter periods for better security.'
			);
		}

		if (config.refreshTokenValiditySeconds > 2592000) {
			// > 30 days
			warnings.push(
				'Refresh token validity period is very long (>30 days). Consider shorter periods for better security.'
			);
		}
