...
class CallbackUriService {
  ...
  private normalizeOverride(uri?: string | null): string | null {
    ...
    return trimmed.startsWith('/') ? `${this.config.origin}${trimmed}` : `${this.config.origin}/${trimmed}`;
  }
  ...
  private getCallbackTypesForFlow(flowKey: string): { redirect: keyof FlowCallbackUris; logout: keyof FlowCallbackUris } {
    const normalized = flowKey.toLowerCase();

    if (normalized.includes('worker-token') || normalized.includes('client-credentials')) {
  ...
  getCallbackUriForFlow(flowType: string): {
    ...
    const baseFlowMap = {
      authorization_code: {
        ...
        logoutNote: 'Handles logout redirects for Authorization Code Flow. Required for RP-initiated logout.',
      },
      implicit: {
  ...
  getRedirectUriCatalog(): Array<{
    ...
    return FLOW_REDIRECT_URI_MAPPING.map((config) => {
      ...
      const redirectUri = config.requiresRedirectUri ? this.getCallbackUri(redirect) : 'Not required';
      const logoutUri = this.getCallbackUri(logout);
      const defaultRedirectUri = config.requiresRedirectUri ? this.buildDefaultUri(redirect) : 'Not required';
  ...
  applyFlowOverrides(overrides: Record<string, { redirectUri?: string; logoutUri?: string }>): void {
...
export const createCallbackUriService = (config?: Partial<CallbackUriConfig>) => new CallbackUriService(config);
