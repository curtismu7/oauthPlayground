	...
	private async _loadCredentialsFromSQLite(): Promise<UnifiedWorkerTokenCredentials | null> {
		try {
			...
			const credentials: UnifiedWorkerTokenCredentials = {
				...
				authzScopes: c.authzScopes ? String(c.authzScopes).split(/\s+/).filter(Boolean) : undefined,
			};
			return credentials;
		} catch (error) {
		...
		try {
			...
			const body = {
				...
				credentials: {
					...
					authzScopes: Array.isArray(credentials.authzScopes)
						? credentials.authzScopes.join(' ')
						: null,
				},
