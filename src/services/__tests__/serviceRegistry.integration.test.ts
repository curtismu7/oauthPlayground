...
import { AlertLevel, ServicePerformanceMonitor } from '../servicePerformanceMonitor';
import { ServiceCategory, ServiceRegistry, ServiceState } from '../serviceRegistry';
...
class MockCredentialService {
	...
	async validateCredentials(credentials: CredentialPayload | null): Promise<boolean> {
...
class MockFlowService {
	...
	async processFlow(flowData: FlowDataPayload): Promise<{ isValid: boolean; processed: true }> {
...
class MockApiService {
	...
	async makeRequest(endpoint: string): Promise<ApiResponse> {
...
describe('Service Registry Integration Tests', () => {
	...
	describe('Service Registration and Discovery', () => {
		it('should register services with metadata', () => {
			registry.register('credential-service', () => new MockCredentialService(), {
	...
	describe('Service Lifecycle Management', () => {
		it('should track service states correctly', async () => {
			registry.register('test-service', () => new MockCredentialService(), {
				name: 'test-service',
				dependencies: [],
				category: ServiceCategory.UTILITY,
			});
		...
		it('should shutdown all services correctly', async () => {
			registry.register('service-1', () => new MockCredentialService('Service1'), {
				name: 'service-1',
				dependencies: [],
				category: ServiceCategory.UTILITY,
			});

			registry.register('service-2', () => new MockCredentialService('Service2'), {
				name: 'service-2',
				dependencies: [],
				category: ServiceCategory.UTILITY,
			});
	...
	describe('Service Categorization and Filtering', () => {
		beforeEach(async () => {
			...
			registry.register('flow-service', () => new MockFlowService(new MockCredentialService()), {
				name: 'flow-service',
				dependencies: [],
				category: ServiceCategory.FLOW,
			});

			registry.register('api-service', () => new MockApiService(), {
				name: 'api-service',
				dependencies: [],
	...
	describe('Performance Monitoring Integration', () => {
		...
		it('should track service access patterns', async () => {
			registry.register('access-tracked-service', () => new MockCredentialService(), {
	...
	describe('Registry Health Monitoring', () => {
		it('should report overall registry health', async () => {
			registry.register('healthy-service', () => new MockCredentialService(), {
		...
		it('should report healthy registry when all services are healthy', async () => {
			registry.register('service-1', () => new MockCredentialService(), {
				name: 'service-1',
				dependencies: [],
				category: ServiceCategory.UTILITY,
			});

			registry.register('service-2', () => new MockCredentialService(), {
				name: 'service-2',
				dependencies: [],
				category: ServiceCategory.UTILITY,
			});
