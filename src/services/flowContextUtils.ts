...
export class FlowContextUtils {
  ...
  static buildAuthorizationUrl(
    ...
    try {
      ...
      const flowId = this.initializeOAuthFlow(flowType, currentStep, flowState);
  ...
  static validateFlowIntegrity(): {
    ...
    try {
      ...
      for (let i = 0; i < sessionStorage.length; i++) {
        ...
        if (key && key.startsWith('flow_state_')) {
          orphanedStates++;
        }
      }
  ...
  static emergencyCleanup(): void {
    try {
      ...
      console.log(`[FlowContextUtils] Emergency cleanup completed. Removed ${flowContextKeys.length + keysToRemove.length} items.`);
    } catch (error) {
