...
class SectionsViewModeManager {
  ...
  private scheduleWrite(pageKey: string, state: Record<string, boolean>): void {
    ...
    this.writeTimeout = setTimeout(async () => {
      ...
      try {
        await Promise.all(
          operations.map(([key, data]) => this.saveViewState(key, data))
        );
      } catch (error) {
...
export function useSectionsViewMode(pageKey: string, sectionIds: string[]) {
  ...
  const collapseAll = React.useCallback(() => {
    ...
    setExpandedStates(
      Object.fromEntries(sectionIds.map(id => [id, false]))
    );
  }, [pageKey, sectionIds]);

  const isSectionExpanded = React.useCallback((sectionId: string) => {
