...
class ComprehensiveFlowDataService {
	...
	saveSharedGlobalConfig(config: Partial<SharedGlobalConfig>): boolean {
		try {
			...
			const updated: SharedGlobalConfig = {
				preferences: {
					autoDiscovery:
						config.preferences?.autoDiscovery ?? existing?.preferences?.autoDiscovery ?? true,
					cacheDiscovery:
						config.preferences?.cacheDiscovery ?? existing?.preferences?.cacheDiscovery ?? true,
					showAdvancedOptions:
						config.preferences?.showAdvancedOptions ??
						existing?.preferences?.showAdvancedOptions ??
						false,
					defaultRegion:
						config.preferences?.defaultRegion ?? existing?.preferences?.defaultRegion ?? 'us',
				},
				lastUpdated: Date.now(),
			};
	...
	getAllFlowStorageKeys(): string[] {
		return Object.keys(localStorage).filter((key) => key.startsWith(this.FLOW_DATA_PREFIX));
	}
...
${Object.entries(credentials.additionalParams || {})
	.map(([key, value]) => `${envKey}_${key.toUpperCase()}=${value}`)
	.join('\n')}
