...
export class PingOneMfaService {
    ...
    static async getRegisteredDevices(credentials: MfaCredentials): Promise<MfaDevice[]> {
        try {
            ...
            throw new Error(`Failed to retrieve MFA devices: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
        ...
        try {
            ...
            switch (config.type) {
                case 'SMS':
                    ...
                    device = await this.createSmsDevice(credentials, config.phoneNumber);
                    break;
                ...
                case 'EMAIL':
                    ...
                    device = await this.createEmailDevice(credentials, config.emailAddress);
                    break;

                case 'TOTP':
                    setupData = await this.createTotpDeviceWithQRCode(credentials, {
                ...
                case 'VOICE':
                    ...
                    device = await this.createVoiceDevice(credentials, config.phoneNumber);
                    break;
                ...
                case 'MOBILE':
                    device = await this.createDevice(credentials, 'MOBILE', {
            ...
            if (config.nickname && config.nickname !== device.deviceName) {
                try {
                    await this.updateDevice(credentials, device.id, {
            ...
            if (!setupData && device.type !== 'FIDO2') {
                setupData = await this.getDeviceSetupData(credentials, device, {
                    issuer: config.issuer,
                    accountName: config.accountName,
            ...
            console.log(`[PingOneMfaService] Successfully registered ${config.type} device: ${device.id}`);
    ...
    static async activateDeviceWithCode(
        ...
        try {
            ...
            const devices = await this.getRegisteredDevices(credentials);
            ...
            if (device.type === 'TOTP' && activationData.totpCode) {
                // Validate TOTP code
                const totpResult = await this.validateTotpCode(device, activationData.totpCode);
                validationResult = {
                ...
                const activationResult = await this.activateDevice(credentials, {
                    deviceId,
    ...
    static async initiateChallenge(
        ...
        try {
            ...
            const challengeResponse = await this.sendChallenge(credentials, {
                deviceId,
                challengeType
            });
            ...
            const result: ChallengeResult = {
                ...
                expiresAt: challengeResponse.expiresAt ? new Date(challengeResponse.expiresAt) : new Date(Date.now() + 5 * 60 * 1000), // 5 minutes default
                deliveryStatus: challengeResponse.deliveryStatus,
                retryAllowed: challengeResponse.retryAllowed !== false,
                nextRetryAt: challengeResponse.nextRetryAt ? new Date(challengeResponse.nextRetryAt) : undefined
            };
            ...
            throw new Error(`Challenge initiation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    static async validateChallenge(
        ...
        try {
            ...
            const validationResponse = await fetch(
                `${this.baseUrl}/environments/${credentials.environmentId}/users/${credentials.userId}/authentications/${challengeId}`,
                {
                    method: 'POST',
                    headers: {
    ...
    static async sendOTP(
        ...
        return this.initiateChallenge(credentials, deviceId, method);
    }
    ...
    static async validateOTP(
        ...
        const result = await this.validateChallenge(credentials, challengeId, code);
        return {
            ...result,
            deviceId
        };
            ...
            if (activeDevices.length === 1 && activeDevices[0].id === deviceId) {
                if (!confirmation?.userConfirmed) {
                    return {
                        ...
                        error: 'This is your only active MFA device. Deleting it will disable MFA protection for your account.'
                    };
                }
            }

            await this.deleteDevice(credentials, deviceId);
    ...
    static async getDevices(credentials: MfaCredentials): Promise<MfaDevice[]> {
        const response = await fetch(
            `${this.baseUrl}/environments/${credentials.environmentId}/users/${credentials.userId}/devices`,
            {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${credentials.workerToken}`,
    ...
    static async activateDevice(
        ...
        const response = await fetch(
            `${this.baseUrl}/environments/${credentials.environmentId}/users/${credentials.userId}/devices/${request.deviceId}`,
            {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${credentials.workerToken}`,
    ...
    static async sendChallenge(
        ...
        const response = await fetch(
            `${this.baseUrl}/environments/${credentials.environmentId}/users/${credentials.userId}/devices/${request.deviceId}/authentications`,
            {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${credentials.workerToken}`,
    ...
    static async createSmsDevice(
        ...
        return this.createDevice(credentials, 'SMS', {
            phone: phoneNumber
        });
    }
    ...
    static async createEmailDevice(
        ...
        return this.createDevice(credentials, 'EMAIL', {
            email: emailAddress
        });
    }
    ...
    static async createTotpDevice(
        ...
        const device = await this.createDevice(credentials, 'TOTP', {});
    ...
    static async createVoiceDevice(
        ...
        return this.createDevice(credentials, 'VOICE', {
            phone: phoneNumber
        });
    }
    ...
    static async updateDevice(
        ...
        const response = await fetch(
            `${this.baseUrl}/environments/${credentials.environmentId}/users/${credentials.userId}/devices/${deviceId}`,
            {
                method: 'PATCH',
                headers: {
                    'Authorization': `Bearer ${credentials.workerToken}`,
    ...
    static async generateDeviceQRCode(
        ...
        try {
            ...
            const account = accountName || `user-${credentials.userId}@${issuer.toLowerCase().replace(/\s+/g, '-')}`;
    ...
    static async getDeviceSetupData(
        ...
        try {
            ...
            if (device.type === 'TOTP' && (device.secret || device.pairingKey)) {
                try {
                    const qrCodeData = await this.generateDeviceQRCode(
                        credentials,
                        device,
                        issuer,
                        options?.accountName
                    ...
                    console.warn('[PingOneMfaService] QR code generation failed, providing manual setup only:', qrError);
                    
                    // Fallback to manual entry only
                    if (device.secret || device.pairingKey) {
                        const manualKey = QRCodeService.generateManualEntryCode(device.secret || device.pairingKey || '');
                        setupData.alternativeMethods.manualEntry = {
                            key: manualKey,
                            instructions: includeInstructions ? [
            ...
            console.log(`[PingOneMfaService] Generated setup data for ${device.type} device ${device.id}`);
            
            return setupData;
        } catch (error) {
            console.error('[PingOneMfaService] Failed to get device setup data:', error);
            throw new Error(`Setup data generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    static async createTotpDeviceWithQRCode(
        ...
        try {
            ...
            const device = await this.createTotpDevice(credentials);
            ...
            const setupData = await this.getDeviceSetupData(credentials, device, {
            ...
            throw new Error(`TOTP device creation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
