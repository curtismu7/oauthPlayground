...
export interface TokenRequest {
  grantType: 'authorization_code' | 'refresh_token' | 'client_credentials' | 'urn:ietf:params:oauth:grant-type:token-exchange';
  code?: string;
  refreshToken?: string;
  redirectUri?: string;
  scope?: string;
...
export class TokenManagementService {
  ...
  async introspectToken(
    ...
    try {
      ...
      const requestBody = this.buildIntrospectionRequestBody(token, tokenTypeHint, resourceId, resourceSecret);
   ...
   */
  private buildTokenRequestBody(request: TokenRequest, authMethod: TokenAuthMethod): FormData | string {
