...
import { V7MAuthorizationCodeRecord, V7MStateStore } from './V7MStateStore';
import { generateV7MTokens, V7MTokenSeed, V7MTokenTtls } from './V7MTokenGenerator';

export type V7MAuthorizeRequest = {
	response_type:
		| 'code'
		| 'id_token token'
		| 'id_token'
...
export type V7MAuthorizeResult =
	| {
			type: 'redirect';
			url: string;
			tokens?: {
...
function authorizeIssueTokens(
	...
	const tokens = generateV7MTokens(seed, nowEpochSeconds, ttls, needsIdToken);
...
export function authorizeAccessDenied(req: V7MAuthorizeRequest): V7MAuthorizeResult {
	return {
		type: 'redirect',
...
function error(errorCode: string, description?: string): V7MAuthorizeResult {
	return { type: 'error', error: errorCode, error_description: description };
}

function buildCode(req: V7MAuthorizeRequest, nowEpochSeconds: number): string {
	const raw = [
		'v7m',
		req.client_id,
		req.redirect_uri,
