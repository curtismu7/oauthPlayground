// src/services/v7m/V7MCIBAService.ts
...
import { generateV7MTokens, type V7MTokenSeed, type V7MTokenTtls } from './V7MTokenGenerator';

export type V7MCIBADeliveryMode = 'poll' | 'ping' | 'push';

export type V7MCIBABackchannelRequest = {
	client_id: string;
	scope: string;
	login_hint?: string;
	...
	delivery_mode?: V7MCIBADeliveryMode;
};

export type V7MCIBABackchannelResponse =
	| { auth_req_id: string; expires_in: number; interval: number }
	| { error: string; error_description?: string };

export type V7MCIBATokenResult =
	| {
			access_token: string;
			id_token: string;
			token_type: 'Bearer';
...
export const V7MCIBAService = {
	...
	requestBackchannelAuth(
		req: V7MCIBABackchannelRequest,
		expiresInSeconds = 120,
		intervalSeconds = 5
	): V7MCIBABackchannelResponse {
		if (!req.client_id) {
	 ...
	 */
	pollForToken(auth_req_id: string, ttls?: Partial<V7MTokenTtls>): V7MCIBATokenResult {
		const rec = store.get(auth_req_id);
		const now = Math.floor(Date.now() / 1000);
		...
		const tokens = generateV7MTokens(seed, now, resolvedTtls, true);
