...
private
translateAuthResult(authResult: AuthResult)
: LegacyAuthResult
{
...
private
translateTokenValidation(tokenValidation: TokenValidation)
...
private
translateUserInfo(userInfo: any)
: LegacyUserInfo
{
	return {
...
export class LegacyAuthServiceAdapterFactory {
	...
	static createBatchAdapters(
		...
		for (const serviceName of services) {
			adapters[serviceName] = LegacyAuthServiceAdapterFactory.createAdapter(
				serviceName,
				unifiedAuthService,
				options
			);
...
export class ServiceMigrationHelper {
	...
	static async testAdapterCompatibility(
		...
		for (const testCase of testCases) {
			try {
				const result = await ServiceMigrationHelper.runTestCase(adapter, testCase);
				results.push(result);
			} catch (error) {
				results.push({
