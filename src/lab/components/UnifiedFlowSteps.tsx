...
import type { TokenResponse } from '@/mfa/services/oauthIntegrationService';
...
import { TokenOperationsService } from '@/mfa/services/tokenOperationsService';

// Create module-specific logger
const _log = createModuleLogger('src/lab/components/UnifiedFlowSteps.tsx');

import { ButtonSpinner } from '@/components/ui';
import { getRedirectUriForUnifiedFlow } from '@/platform/UnifiedRedirectUriService';
...
export const UnifiedFlowSteps: React.FC<UnifiedFlowStepsProps> = ({
	...
	// Educational content collapsed state — persisted so survives refresh/restart
	const [isQuickStartCollapsed, setIsQuickStartCollapsed] = usePersistedCollapse(
		flowType,
		'quick-start',
		false
	);
	const [pkceOverviewCollapsed, setPkceOverviewCollapsed] = usePersistedCollapse(
		flowType,
		'pkce-overview',
		false
	);
	const [pkceDetailsCollapsed, setPkceDetailsCollapsed] = usePersistedCollapse(
		flowType,
		'pkce-details',
		false
	);
	const [authRequestOverviewCollapsed, setAuthRequestOverviewCollapsed] = usePersistedCollapse(
		flowType,
		'auth-request-overview',
		false
	);
	const [authRequestDetailsCollapsed, setAuthRequestDetailsCollapsed] = usePersistedCollapse(
		flowType,
		'auth-request-details',
		false
	);
	const [deviceCodeOverviewCollapsed, setDeviceCodeOverviewCollapsed] = usePersistedCollapse(
		flowType,
		'device-code-overview',
		false
	);
	const [deviceCodeDetailsCollapsed, setDeviceCodeDetailsCollapsed] = usePersistedCollapse(
		flowType,
		'device-code-details',
		false
	);
	const [clientCredentialsOverviewCollapsed, setClientCredentialsOverviewCollapsed] =
		usePersistedCollapse(flowType, 'client-credentials-overview', false);
	const [clientCredentialsDetailsCollapsed, setClientCredentialsDetailsCollapsed] =
		usePersistedCollapse(flowType, 'client-credentials-details', false);
	const [authzCodeOverviewCollapsed, setAuthzCodeOverviewCollapsed] = usePersistedCollapse(
		flowType,
		'authz-code-overview',
		false
	);
	const [authzCodeDetailsCollapsed, setAuthzCodeDetailsCollapsed] = usePersistedCollapse(
		flowType,
		'authz-code-details',
		false
	);
	const [hybridOverviewCollapsed, setHybridOverviewCollapsed] = usePersistedCollapse(
		flowType,
		'hybrid-overview',
		false
	);
	const [hybridDetailsCollapsed, setHybridDetailsCollapsed] = usePersistedCollapse(
		flowType,
		'hybrid-details',
		false
	);
	const [implicitOverviewCollapsed, setImplicitOverviewCollapsed] = usePersistedCollapse(
		flowType,
		'implicit-overview',
		false
	);
	const [implicitDetailsCollapsed, setImplicitDetailsCollapsed] = usePersistedCollapse(
		flowType,
		'implicit-details',
		false
	);
	const [preflightValidationCollapsed, setPreflightValidationCollapsed] = usePersistedCollapse(
		flowType,
	...
	useEffect(() => {
		const checkTokenStatus = async () => {
			try {
				...
				const { checkWorkerTokenStatus } = await import('@/mfa/services/workerTokenStatusService');
				...
				logger.info('[WORKER TOKEN VALIDATION] Full diagnostic:', {
					step: currentStep,
				...
				if (flowRequiresWorkerToken && currentStep > 0 && !status.isValid) {
					...
					logger.warn('[WORKER TOKEN VALIDATION] No valid worker token on configuration step', {
						status: status.status,
						message: status.message,
						unifiedStatus,
					});
				} else if (status.isValid) {
					logger.info('[WORKER TOKEN VALIDATION] ✅ Token is valid', {
						step: currentStep,
						expiresIn: status.minutesRemaining,
					});
				}
			} catch (error) {
				logger.error('[WORKER TOKEN VALIDATION] Failed to check worker token status', error);
	...
	useEffect(() => {
		if (
			...
			logger.warn('[PKCE VALIDATION] PKCE codes required but missing - redirecting to PKCE step', {
				currentStep,
	...
	const showTokenSuccessModal = useCallback(
		(tokens: TokenResponse) => {
			logger.info('✅ Tokens received - proceeding to next step', 'Logger info');
		...
		async (accessToken: string, environmentId: string): Promise<Record<string, unknown> | null> => {
			...
			try {
				...
				if (!discoveryResult.success || !discoveryResult.data?.userInfoEndpoint) {
					...
					logger.info('Fetching UserInfo via backend proxy', { userInfoEndpoint });
				}
	...
	useEffect(() => {
		if (prevFlowTypeRef.current !== flowType) {
			logger.info(`${MODULE_TAG} Flow type changed - clearing tokens and flow state`, {
			...
			PKCEStorageServiceV8U.clearPKCECodes(prevFlowKey).catch((err) => {
				logger.error(
					'UnifiedFlowSteps',
					`Failed to clear PKCE codes for previous flow type`,
					undefined,
					err
				);
			});
			...
			logger.info(
				`${MODULE_TAG} Cleared tokens and flow state due to flow type change`,
				'Logger info'
			);
	...
	useEffect(() => {
		...
		>[1];
		CredentialsService.saveCredentials(`${flowType}-v8u`, credsForSave);
	...
	useEffect(() => {
		if (currentStep === 0) {
			...
			if (errors.length === 0 && !completedSteps.includes(0)) {
				logger.info(`${MODULE_TAG} Step 0 validation passed - marking as complete`, 'Logger info');
				setCompletedSteps((prev) => [...prev, 0]);
			} else if (errors.length > 0 && completedSteps.includes(0)) {
				// Remove from completed steps if errors appear
				logger.info(
					`${MODULE_TAG} Step 0 validation failed - removing from completed steps`,
					'Logger info'
				);
	...
	useEffect(() => {
		...
		if (currentStep === 1 && (flowType === 'oauth-authz' || flowType === 'hybrid')) {
			...
			if (canProceed && !completedSteps.includes(1)) {
				logger.info(`${MODULE_TAG} Step 1 (PKCE) validation passed - marking as complete`);
	...
	useEffect(() => {
		if (currentStep === 4 && (flowType === 'oauth-authz' || flowType === 'hybrid')) {
			...
			logger.info(
				`${MODULE_TAG} ========== TOKEN EXCHANGE VALIDATION DEBUG ==========`,
				'Logger info'
			);
			logger.info(`${MODULE_TAG} Current Step:`, currentStep);
			logger.info(`${MODULE_TAG} Flow Type:`, flowType);
			logger.info(`${MODULE_TAG} Credentials at validation time:`, {
			...
			logger.info(`${MODULE_TAG} Flow State:`, {
			...
			logger.info(`${MODULE_TAG} Validation Errors Found:`, errors.length);
			// Set validation errors if any are found
			if (errors.length > 0) {
				logger.warn('UnifiedFlowSteps', `Missing required fields on token exchange step`, {
					usePKCE: isPKCERequired,
					pkceEnforcement: credentials.pkceEnforcement,
					hasAuthorizationCode: !!flowState.authorizationCode,
				...
				logger.info(`${MODULE_TAG} ✅ All validation checks passed`, 'Logger info');
				setValidationErrorsState([]);
			}
			logger.info(
				`${MODULE_TAG} ========== TOKEN EXCHANGE VALIDATION DEBUG END ==========`,
				'Logger info'
			);
		}
	}, [
	...
	useEffect(() => {
		if (flowState.tokens?.accessToken) {
			...
			if (tokenStep >= 0 && !completedSteps.includes(tokenStep)) {
				logger.info(`${MODULE_TAG} Tokens available - auto-marking step ${tokenStep} as complete`, {
					flowType,
					tokenStep,
	...
	useEffect(() => {
		if (flowState.tokens?.accessToken) {
			...
			try {
				...
				logger.info(`${MODULE_TAG} Tokens auto-saved to sessionStorage`, {
					flowType,
				...
				try {
					...
					logger.info(
						`${MODULE_TAG} Enhanced state management updated with new tokens`,
						'Logger info'
					);
				} catch {
				...
				logger.error('UnifiedFlowSteps', `Failed to save tokens to sessionStorage`, undefined, err);
			}
		}
	...
	useEffect(() => {
		if (flowType === 'device-code' && flowState.deviceCode) {
			...
			try {
				...
				logger.info(`${MODULE_TAG} Device code data auto-saved to sessionStorage`, {
				...
				logger.error(
					'UnifiedFlowSteps',
					`Failed to save device code data to sessionStorage`,
					undefined,
					err
				);
			}
	...
	useEffect(() => {
		if (flowState.authorizationCode && (flowType === 'oauth-authz' || flowType === 'hybrid')) {
			try {
				...
				logger.info(`${MODULE_TAG} Authorization code auto-saved to sessionStorage`, {
	...
	useEffect(() => {
		...
		if (currentStep === 3 && (flowType === 'oauth-authz' || flowType === 'hybrid')) {
			logger.info(`${MODULE_TAG} Step 3 mounted - checking for callback data`, { flowType });
			...
			if (flowType === 'hybrid') {
				...
				if (
					...
					logger.info(`${MODULE_TAG} Hybrid flow: Parsing full callback URL`, {
						hasFragmentTokens,
						hasQueryCode,
					...
					try {
						...
						if (codeFromResult && !detectedCode) {
							...
							logger.info(`${MODULE_TAG} Hybrid flow: Code extracted from parseCallbackFragment`, {
								code: codeFromResult,
							});
						}
						...
						logger.info(`${MODULE_TAG} Hybrid flow: Callback parsed successfully`, {
						...
						logger.error(
							'UnifiedFlowSteps',
							`Failed to parse callback for hybrid flow`,
							undefined,
							err
						);
					}
			...
			logger.info(`${MODULE_TAG} Checking sessionStorage`, { hasData: !!callbackDataStr });

			if (callbackDataStr) {
				try {
					const callbackData = JSON.parse(callbackDataStr);
					logger.info(`${MODULE_TAG} Found callback data in sessionStorage`, callbackData);

					if (callbackData.code) {
						callbackUrl = callbackData.fullUrl || window.location.href;
						detectedCode = callbackData.code;
						detectedState = callbackData.state;
						logger.info(`${MODULE_TAG} Extracted from sessionStorage`, {
							code: detectedCode,
							state: detectedState,
						});
					...
					logger.error(
						'UnifiedFlowSteps',
						`Failed to parse callback data from sessionStorage`,
						undefined,
						err
					);
				}
			...
			if (!detectedCode) {
				...
				logger.info(`${MODULE_TAG} Checking URL params`, { hasCode: !!code, hasState: !!state });

				if (code) {
					logger.info(`${MODULE_TAG} Found authorization code in URL params`, 'Logger info');
					callbackUrl = window.location.href;
					detectedCode = code;
					detectedState = state;
			...
			if (flowType === 'hybrid') {
				...
				if (detectedCode && !flowState.authorizationCode) {
					...
					hasUpdates = true;
					logger.info(`${MODULE_TAG} Hybrid flow: Authorization code extracted from detectedCode`, {
						code: detectedCode,
					});
					...
					if (!hasCode || !hasState) {
						logger.warn(
							'UnifiedFlowSteps',
							`Fallback callback URL does not contain required OAuth parameters`,
							{
								callbackUrl,
								hasCode,
								hasState,
					...
					try {
						...
						logger.error(
							'UnifiedFlowSteps',
							`Failed to parse authorization code for hybrid flow`,
							undefined,
							err
						);
				...
				if (fragmentTokens && !flowState.tokens?.accessToken) {
					...
					try {
						...
						logger.info(`${MODULE_TAG} Hybrid flow: Tokens saved to sessionStorage`, 'Logger info');
					} catch (err) {
						logger.error(
							'UnifiedFlowSteps',
							`Failed to save hybrid tokens to sessionStorage`,
							undefined,
							err
						);
					}
				...
				if (hasUpdates) {
					...
					if (updates.authorizationCode && updates.tokens) {
						...
						modernMessaging.showFooterMessage({
							type: 'info',
							message: '✅ Callback URL parsed automatically! Authorization code extracted.',
							duration: 3000,
						});
					} else if (updates.tokens) {
						modernMessaging.showFooterMessage({
							type: 'info',
							message: '✅ Tokens extracted automatically!',
							duration: 3000,
						});
					}
					...
					logger.warn('UnifiedFlowSteps', `Hybrid flow: No updates applied`, {
			...
			if (
				...
				logger.info(`${MODULE_TAG} Auto-parsing callback URL and extracting code`, { callbackUrl });
				...
				try {
					...
					logger.info(`${MODULE_TAG} Successfully parsed callback`, { extractedCode: parsed.code });
					...
					logger.info(
						`${MODULE_TAG} Authorization code extracted - showing success modal`,
						'Logger info'
					);
				} catch (err) {
					logger.error('UnifiedFlowSteps', `Failed to auto-parse callback URL`, undefined, err);
					...
					modernMessaging.showBanner({
						type: 'warning',
						title: 'Warning',
						message: 'Callback URL detected - click "Parse Callback URL" to continue',
						dismissible: true,
					});
				}
			} else if (!detectedCode && !hasFragment) {
				logger.info(
					`${MODULE_TAG} No callback data found - user will need to paste URL manually`,
					'Logger info'
				);
			}
		}
		...
		try {
			...
			if (!fragment) {
				...
				try {
					...
					logger.info(`${MODULE_TAG} Tokens saved to sessionStorage`, {
	...
	useEffect(() => {
		...
		if (
			...
			if (callbackDataStr) {
				try {
					const callbackData = JSON.parse(callbackDataStr);
					logger.info(`${MODULE_TAG} Restoring callback data from sessionStorage`, {
			...
			if (fragment && (fragment.includes('access_token') || fragment.includes('id_token'))) {
				...
				(async () => {
					try {
						...
						modernMessaging.showFooterMessage({
							type: 'info',
							message: 'Tokens extracted automatically from URL fragment',
							duration: 3000,
						});
					} catch (err) {
						logger.error('UnifiedFlowSteps', `Failed to auto-parse fragment`, undefined, err);
				...
				if (storedTokens) {
					try {
						const tokens = JSON.parse(storedTokens);
						logger.info(`${MODULE_TAG} Restoring tokens from sessionStorage on step 2`, {
							hasAccessToken: !!tokens.accessToken,
						});
						setFlowState((prev) => ({
							...prev,
						...
						modernMessaging.showFooterMessage({
							type: 'info',
							message: 'Tokens restored from previous session',
							duration: 3000,
						});
					} catch (err) {
						logger.error(
							'UnifiedFlowSteps',
							`Failed to restore tokens from sessionStorage`,
							undefined,
							err
						);
					}
															...
															<button
																...
																onClick={async () => {
																	try {
																		...
																		const { uiNotificationService } = await import(
																			'@/mfa/services/uiNotificationService'
																		);
																		const { PreFlightValidationService } = await import(
																			'@/mfa/services/preFlightValidationService'
																		);
																		const { workerTokenService } = await import(
																			'@/mfa/services/workerTokenService'
																		);
																		const { CredentialsService } = await import(
																			'@/mfa/services/credentialsService'
																		);
																		...
																		if (confirmed) {
																			...
																			const { SharedCredentialsService } = await import(
																				'@/mfa/services/sharedCredentialsService'
																			);
																			const sharedCreds =
																			...
																			if (
																				...
																				SharedCredentialsService.saveSharedCredentialsSync(
																					sharedCreds
																				);
																				// Also save to disk asynchronously (non-blocking)
																				SharedCredentialsService.saveSharedCredentials(
																					sharedCreds
																				).catch(() => {
																					logger.warn(
																						'UnifiedFlowSteps',
																			...
																			modernMessaging.showFooterMessage({
																				type: 'info',
																				message: `Fixed ${fixesApplied.length} error(s): ${fixesApplied.join(', ')}`,
																				duration: 3000,
																			});
																			...
																			const newValidationResult =
																				await PreFlightValidationService.validateBeforeAuthUrl({
																					specVersion,
																					flowType,
																			...
																			if (newValidationResult.errors.length > 0) {
																				...
																				modernMessaging.showBanner({
																					type: 'error',
																					title: 'Error',
																					message: 'Some errors remain after fixes',
																					dismissible: true,
																				});
																				...
																				modernMessaging.showBanner({
																					type: 'warning',
																					title: 'Warning',
																					message: 'Pre-flight validation warnings remain',
																					dismissible: true,
																				});
																			} else {
																				setValidationWarnings([]);
	...
	const renderStep1PKCE = () => {
		...
		const handlePKCEChange = (codes: PKCECodes) => {
			...
			if (codes.codeVerifier && codes.codeChallenge) {
				...
				logger.warn('UnifiedFlowSteps', `handlePKCEChange called but codes are incomplete`);
			}
		};

		const handlePKCEGenerate = async () => {
			setIsGeneratingPKCE(true);
			logger.info(`${MODULE_TAG} PKCE codes generated`, 'Logger info');
		...
		async (flowId: string, stateValue: string, resumeUrl?: string) => {
			...
			try {
				...
				if (!authCode) {
					...
					logger.error('UnifiedFlowSteps', `Authorization code not found in resume response`, {
				...
				modernMessaging.showFooterMessage({
					type: 'info',
					message: '✅ Authorization code received from PingOne redirectless flow!',
					duration: 3000,
				});
	...
	const handleRedirectlessTokenExchange = useCallback(
		async (authCode: string, codeVerifier: string) => {
			...
			try {
				...
				modernMessaging.showFooterMessage({
					type: 'info',
					message: '✅ Tokens obtained successfully via redirectless authentication!',
					duration: 3000,
				});
			...
			try {
				...
				if (status === 'MUST_CHANGE_PASSWORD') {
					...
					if (userId) {
						...
						modernMessaging.showBanner({
							type: 'warning',
							title: 'Warning',
							message: '⚠️ Password change is required. Please update your password to continue.',
							dismissible: true,
						});
					} else {
						...
						try {
							...
							const { workerTokenService } = await import('@/mfa/services/workerTokenService');
							const workerToken = await workerTokenService.getToken();
							...
							logger.error('UnifiedFlowSteps', `Failed to lookup user:`, undefined, lookupError);
							const errorMsg =
				...
				if (status === 'READY_TO_RESUME' && resumeUrl) {
					...
					if (accessToken) {
						...
						modernMessaging.showFooterMessage({
							type: 'info',
							message: '✅ Authentication completed successfully! Tokens received.',
							duration: 3000,
						});
						...
						if (codeVerifier) {
							...
							modernMessaging.showFooterMessage({
								type: 'info',
								message: '✅ Authorization code received! Proceeding to token exchange.',
								duration: 3000,
							});
						...
						if (resumeUrl) {
							...
							logger.warn(
								'UnifiedFlowSteps',
								`Flow COMPLETED but no code, tokens, or resumeUrl found`,
								{
				...
				logger.error('UnifiedFlowSteps', `Failed to submit credentials:`, undefined, errorMessage);
	...
	const handlePasswordChange = useCallback(
		async (_oldPassword: string, newPassword: string) => {
			...
			try {
				...
				modernMessaging.showFooterMessage({
					type: 'info',
					message: 'Please try signing in again with your new password.',
					duration: 3000,
				});
				...
				logger.error('UnifiedFlowSteps', `Password change failed:`, undefined, error);
	...
	const handleStartRedirectlessAuth = useCallback(async () => {
		...
		if (!isRedirectless || !flowState.authorizationUrl) {
			logger.warn(
				'UnifiedFlowSteps',
				`Cannot start redirectless auth - missing URL or not in redirectless mode`
			);
			return;
		}
	...
	const handleProceedWithPingOneRequest = useCallback(async () => {
		...
		try {
			...
			if (flowId) {
				logger.warn(
					'UnifiedFlowSteps',
					`Unexpected flow status "${flowStatus}" but have flowId - showing modal as fallback to allow manual credential entry`
				);
				setShowRedirectlessModal(true);
				setIsLoading(false);
				return;
			...
			logger.error('UnifiedFlowSteps', `No flowId in redirectless response:`, undefined, flowData);
	...
	const renderStep1AuthUrl = () => {
		const handleGenerateAuthUrl = async () => {
			...
			try {
				...
				let workerToken = await workerTokenService.getToken();
				...
				const validationPromise = PreFlightValidationService.validateBeforeAuthUrl({
					specVersion,
					flowType,
				...
				logger.info(`${MODULE_TAG} Pre-flight validation result:`, {
				...
				if (validationResult.errors.length > 0 || validationResult.warnings.length > 0) {
					...
					if (validationResult.errors.length > 0) {
						...
						if (fixableErrors.length > 0) {
							...
							if (confirmed) {
								...
								const { CredentialsService } = await import('@/mfa/services/credentialsService');
								const flowKey = `${specVersion}-${flowType}-v8u`;
								// updatedCredentials is compatible with Credentials interface
								CredentialsService.saveCredentials(flowKey, updatedCredentials);
								...
								const sharedCreds =
									SharedCredentialsService.extractSharedCredentials(updatedCredentials);
								if (
									sharedCreds.environmentId ||
									sharedCreds.clientId ||
									sharedCreds.clientAuthMethod
								...
								if (newValidationResult.errors.length > 0) {
									...
									modernMessaging.showBanner({
										type: 'warning',
										title: 'Warning',
										message: 'Pre-flight validation warnings - check details below',
										dismissible: true,
									});
									setIsPreFlightValidating(false);
									setPreFlightStatus('');
									...
									modernMessaging.showFooterMessage({
										type: 'info',
										message: `✅ Pre-flight validation passed! Fixed ${fixesApplied.length} error(s).`,
										duration: 3000,
									});
									setIsPreFlightValidating(false);
									setPreFlightStatus('');
					...
					if (validationResult.warnings.length > 0) {
						...
						modernMessaging.showBanner({
							type: 'warning',
							title: 'Warning',
							message: '⚠️ Pre-flight validation warnings - check details below',
							dismissible: true,
						});
						setIsPreFlightValidating(false);
						setPreFlightStatus('');
					...
					logger.info(
						`${MODULE_TAG} Pre-flight validation passed with no errors or warnings`,
						'Logger info'
					);
					modernMessaging.showFooterMessage({
						type: 'info',
				...
				if (errorMessage.includes('timed out')) {
					modernMessaging.showBanner({
						type: 'error',
						title: 'Error',
						message: 'Pre-flight validation timed out - continuing anyway',
						dismissible: true,
					});
				} else {
					modernMessaging.showBanner({
						type: 'error',
						title: 'Error',
						message: 'Pre-flight validation encountered an error - continuing anyway',
						dismissible: true,
					});
				}
				...
				if (isPreFlightValidating) {
					logger.warn(
						'UnifiedFlowSteps',
						`Pre-flight validation spinner still active in finally block - clearing it`
					);
					setIsPreFlightValidating(false);
					setPreFlightStatus('');
				}
			...
			try {
				...
				if (flowState.codeVerifier && flowState.codeChallenge) {
					...
					if (storedPKCE?.codeChallengeMethod && storedPKCE.codeChallengeMethod !== 'S256') {
						logger.warn(
							'UnifiedFlowSteps',
							`Stored PKCE codes have codeChallengeMethod='${storedPKCE.codeChallengeMethod}' instead of 'S256'. ` +
										...
										<button
											...
											onClick={async () => {
												try {
													...
													const { WorkerTokenStatusService } = await import(
														'@/mfa/services/workerTokenStatusService'
													);
													...
													if (tokenStatus.isValid) {
														...
														const newWorkerToken = await workerTokenService.getToken();
														const newValidationResult =
															await PreFlightValidationService.validateBeforeAuthUrl({
																specVersion,
																flowType,
														...
														if (newValidationResult.errors.length > 0) {
															...
															modernMessaging.showBanner({
																type: 'error',
																title: 'Error',
																message: 'Pre-flight validation failed - check error details below',
																dismissible: true,
															});
														} else if (newValidationResult.warnings.length > 0) {
															const warningMessage = [
													...
													modernMessaging.showBanner({
														type: 'error',
														title: 'Error',
														message: 'Failed to retrieve worker token. Please try again.',
														dismissible: true,
													});
												} finally {
													setIsLoading(false);
											...
											onClick={async () => {
												try {
													...
													if (confirmed) {
														...
														await SharedCredentialsService.saveCredentials({
													...
													modernMessaging.showBanner({
														type: 'error',
														title: 'Error',
														message: `❌ Failed to fix errors: ${error instanceof Error ? error.message : 'Unknown error'}`,
														dismissible: true,
													});
												} finally {
													setIsLoading(false);
				...
				{flowState.authorizationUrl && (
					<div style={{ marginTop: '24px' }}>
						...
						<ColoredUrlDisplay
							...
							editable={true}
							onChange={(newUrl) => {
								logger.info(`${MODULE_TAG} Authorization URL edited`, { newUrl });
								setFlowState((prev) => ({
									...prev,
						...
						{credentials.responseMode === 'pi.flow' || credentials.useRedirectless ? (
							...
							>
								<button
									...
									onClick={() => {
										logger.info(
											`${MODULE_TAG} Opening authorization URL for authentication`,
											'Logger info'
										);
	...
	const renderStep2Callback = () => {
		...
		if (isRedirectlessCallback) {
			logger.warn(
				'UnifiedFlowSteps',
				`Redirectless mode (response_mode=pi.flow) is enabled but user is on callback step - this shouldn't happen in redirectless mode`
			);
			...
			try {
				...
				if (flowType === 'hybrid') {
					...
					if (!hasCode || !hasState) {
						logger.warn(
							'UnifiedFlowSteps',
							`Manual parse callback URL does not contain required OAuth parameters`,
							{
					...
					if (updates.tokens) {
						modernMessaging.showFooterMessage({
							type: 'info',
							message: 'Authorization code and tokens extracted successfully!',
							duration: 3000,
						});
					} else {
						modernMessaging.showFooterMessage({
							type: 'info',
							message: 'Authorization code extracted successfully!',
							duration: 3000,
						});
					}
				} else {
					...
					if (callbackUrlForParsing) {
						...
						if (!hasCode || !hasState) {
							logger.warn(
								'UnifiedFlowSteps',
								`Authorization code flow callback URL does not contain required OAuth parameters`,
								{
									callbackUrl: callbackUrlForParsing,
									hasCode,
									hasState,
		...
		// Check for errors in callback data (from sessionStorage) or URL (when CallbackHandler redirects with csrf_risk, it does not store sessionStorage)
		const callbackDataStr = sessionStorage.getItem('v8u_callback_data');
		...
		if (callbackDataStr) {
			try {
				...
				if (callbackData.error) {
					...
					logger.error('UnifiedFlowSteps', `Error in callback:`);
				}
			} catch (err) {
				logger.error('UnifiedFlowSteps', `Failed to parse callback data:`, undefined, err);
			}
		}

		// When csrf_risk or state_expired, CallbackHandler redirects without storing — error is only in URL
		const urlError = searchParams.get('error');
		const urlReason = searchParams.get('reason');
		if (urlError === 'csrf_risk' || urlError === 'state_expired') {
			callbackError = callbackError || urlError;
			callbackErrorDescription =
				callbackErrorDescription ||
				urlReason ||
				(urlError === 'csrf_risk'
					? 'CSRF validation failed. Go back and try again.'
					: 'State expired. Go back and try again.');
		}
					...
					>
						<div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
							...
							<div style={{ flex: 1 }}>
								...
								{(callbackError === 'csrf_risk' || callbackError === 'state_expired') && (
	...
	const handleRequestDeviceAuth = useCallback(async () => {
		...
		try {
			...
			const oauthConfigResult = await PreFlightValidationService.validateOAuthConfig({
				specVersion,
				flowType,
				credentials,
				...(workerToken && { workerToken }),
			...
			const fixableErrors = PreFlightValidationService.analyzeFixableErrors(
			...
			if (validationResult.errors.length > 0) {
				...
				if (fixableErrors.length > 0) {
					...
					if (confirmed) {
						...
						// Re-run validation
						setPreFlightStatus(' Re-validating configuration...');
						const newOAuthConfigResult = await PreFlightValidationService.validateOAuthConfig({
							specVersion,
							flowType,
							credentials: updatedCredentials,
							...(workerToken && { workerToken }),
						...
						const newFixableErrors = PreFlightValidationService.analyzeFixableErrors(
						...
						if (newOAuthConfigResult.errors.length > 0) {
							...
							modernMessaging.showFooterMessage({
								type: 'info',
								message: '✅ All errors fixed! Pre-flight validation passed!',
								duration: 3000,
							});
						}
					} else {
			...
			logger.error('UnifiedFlowSteps', `Pre-flight validation error:`, undefined, validationError);
			modernMessaging.showBanner({
				type: 'error',
				title: 'Error',
				message: 'Pre-flight validation encountered an error - continuing anyway',
		...
		try {
			...
			modernMessaging.showFooterMessage({
				type: 'info',
				message: 'Device authorization request successful',
				duration: 3000,
			});
		} catch (err) {
	...
	}, [credentials, flowState, nav, currentStep, flowType, onCredentialsChange, specVersion]);
	...
	const renderStep2Poll = () => {
		const handleStopPolling = () => {
			logger.info(`${MODULE_TAG} User requested to stop polling`, 'Logger info');
			pollingAbortRef.current = true;
			isPollingExecutingRef.current = false;
		...
		const handlePollForTokens = async () => {
			...
			if (pollingAbortRef.current) {
				logger.info(`${MODULE_TAG} Polling aborted - not starting new polling`, 'Logger info');
			...
			if (isPollingExecutingRef.current || isLoading || flowState.pollingStatus?.isPolling) {
				logger.info(`${MODULE_TAG} Polling already in progress, skipping duplicate call`, {
			...
			logger.info(`${MODULE_TAG} Starting polling for tokens`, 'Logger info');
			setIsLoading(true);
			setError(null);
			autoPollTriggeredRef.current = true;
			...
			const performPoll = async (): Promise<TokenResponse | null> => {
				...
				if (pollingAbortRef.current) {
					logger.info(`${MODULE_TAG} Polling aborted at start of performPoll`, 'Logger info');
					return null;
				}
				...
				try {
					...
					if (pollingAbortRef.current) {
						logger.info(`${MODULE_TAG} Polling aborted after reading device code`, 'Logger info');
						return null;
					}

					// Validate device code before sending
					if (!currentDeviceCode || currentDeviceCode === '') {
						logger.error('UnifiedFlowSteps', `Device code is missing or empty`, {
					...
					logger.info(`${MODULE_TAG} Polling request via backend proxy`, {
					...
					try {
						...
						if (!response.ok) {
							logger.error('UnifiedFlowSteps', `Polling response error (attempt ${pollCount}):`, {
								status: response.status,
								statusText: response.statusText,
						...
						logger.error('UnifiedFlowSteps', `Fetch error during polling (attempt ${pollCount}):`, {
							error: fetchError,
							tokenEndpoint,
					...
					if (response.ok) {
						logger.info(`${MODULE_TAG} [DEBUG] Response is OK (200) - attempting to parse tokens`);
						...
						logger.info(
							`${MODULE_TAG} ✅ Tokens received successfully on attempt ${pollCount}`,
							'Logger info'
						);
					...
					logger.info(`${MODULE_TAG} Poll response (attempt ${pollCount}):`, {
						status: response.status,
					...
					if (errorData.error === 'access_denied') {
						logger.error('UnifiedFlowSteps', `User denied authorization`, {
							error: errorData.error,
					...
					if (errorData.error === 'expired_token' || errorData.error === 'invalid_grant') {
						logger.error('UnifiedFlowSteps', `Device code expired or invalid`, {
							error: errorData.error,
							description: errorData.error_description,
						});
						throw new Error(
					...
					logger.error('UnifiedFlowSteps', `Token polling error`, {
						error: errorData.error,
						description: errorData.error_description,
					});
					throw new Error(
					...
					logger.warn('UnifiedFlowSteps', `Polling error (attempt ${pollCount}/${maxAttempts})`);
			...
			const pollLoop = async () => {
				...
				for (let attempt = 0; attempt < maxAttempts; attempt++) {
					...
					if (pollingAbortRef.current) {
						logger.info(`${MODULE_TAG} Polling stopped by user`, 'Logger info');
						setIsLoading(false);
						setLoadingMessage('');
					...
					logger.info(
						`${MODULE_TAG} [DEBUG] Device code being used:`,
						`${deviceCodeRef.current?.substring(0, 8)}...`
					);

					const tokens = await performPoll();
					logger.info(
						`${MODULE_TAG} [DEBUG] Poll result:`,
						tokens ? 'TOKENS RECEIVED' : 'NO TOKENS (will continue polling)'
					);

					// Check again after async operation
					if (pollingAbortRef.current) {
						logger.info(`${MODULE_TAG} Polling stopped after poll attempt`, 'Logger info');
						setIsLoading(false);
						isPollingExecutingRef.current = false;
						setFlowState((prev) => ({
							...prev,
					...
					if (attempt < maxAttempts - 1) {
						...
						if (pollingAbortRef.current) {
							logger.info(`${MODULE_TAG} Polling stopped before wait`, 'Logger info');
							setIsLoading(false);
							isPollingExecutingRef.current = false;
							setFlowState((prev) => ({
								...prev,
						...
						logger.info(
							`${MODULE_TAG} Waiting ${currentInterval}s before next poll...`,
							'Logger info'
						);
						...
						if (pollingAbortRef.current) {
							logger.info(`${MODULE_TAG} Polling stopped after wait`, 'Logger info');
							setIsLoading(false);
							isPollingExecutingRef.current = false;
							setFlowState((prev) => ({
								...prev,
				...
				modernMessaging.showBanner({
					type: 'error',
					title: 'Error',
					message: 'Polling timeout - authorization not completed',
					dismissible: true,
				});
					...
					<div style={{ marginTop: '32px', marginBottom: '32px' }}>
						{(() => {
							try {
								return (
									<DynamicDeviceFlow
										...
										onStateUpdate={(newState: unknown) => {
											logger.info(`${MODULE_TAG} Device flow state updated`, newState);
										}}
										onComplete={(tokens: unknown) => {
											logger.info(`${MODULE_TAG} Device authorization completed`, tokens);
										}}
								...
								logger.error(
									'UnifiedFlowSteps',
									`Error rendering DynamicDeviceFlow:`,
									undefined,
									error
								);
								return (
									...
									onClick={async () => {
										try {
											...
											modernMessaging.showBanner({
												type: 'error',
												title: 'Error',
												message: 'Failed to copy user code',
												dismissible: true,
											});
										}
									}}
			...
			if (flowType === 'client-credentials') {
				...
				try {
					...
					if (workerToken && credentials.environmentId && credentials.clientId) {
						try {
							const { ConfigCheckerService } = await import(
								'@/mfa/services/configCheckerService'
							);
							const fetchedConfig = await ConfigCheckerService.fetchAppConfig(
								credentials.environmentId,
								credentials.clientId,
								workerToken
							);
								...
								<CollapsibleContent>
									...
									<InfoBox $variant="success">
										...
										<div>
											...
											<InfoText>
												Client Credentials flow works best with custom resources that define your
												own API endpoints and scopes. Custom resources give you full control over
												what your client application can access.
											</InfoText>
											<InfoList>
												<li>
												...
												<li>
													<strong>Why use custom resources?</strong> They provide precise access
													control for your specific application needs and business logic
												</li>
												<li>
													<strong>How to create:</strong> In PingOne, go to Resources → Resource
													Servers → Add Resource Server, then define your custom scopes
												</li>
												<li>
											...
											<InfoText style={{ marginTop: '12px', fontStyle: 'italic' }}>
												<strong>Note:</strong> Unlike other flows, Client Credentials cannot access
												user data or identity information. It's designed specifically for
												machine-to-machine access to your protected resources.
											</InfoText>
										</div>
									</InfoBox>
	...
	const renderStep3ExchangeTokens = () => {
		const handleExchangeTokens = async () => {
			logger.info(`${MODULE_TAG} ========== TOKEN EXCHANGE DEBUG START ==========`, 'Logger info');
			...
			if (storedPKCE) {
				...
				logger.info(`${MODULE_TAG} ✅ Loaded PKCE codes from bulletproof storage`, {
				...
				logger.error(
					'UnifiedFlowSteps',
					`CRITICAL: No PKCE codes found in ANY of 4 storage locations!`
				);
				// If PKCE is enabled but codes are missing, this is a critical error
				if (isPKCERequired) {
					logger.error(
						'UnifiedFlowSteps',
						`CRITICAL: PKCE enabled but no codes found in sessionStorage or flowState`
					);
					logger.error(
						'UnifiedFlowSteps',
						`User must either: 1) Go to Step 0 (Configuration) and generate PKCE codes in Advanced Options, or 2) Disable PKCE in configuration`
					);
				}
			}
			...
			logger.info(`${MODULE_TAG} Credentials:`, {
				environmentId: credentials.environmentId,
				clientId: credentials.clientId,
			...
			logger.info(`${MODULE_TAG} Flow Type:`, flowType);
			logger.info(`${MODULE_TAG} Spec Version:`, specVersion);

			// Validate required fields before attempting token exchange
			if (!flowState.authorizationCode) {
				logger.error('UnifiedFlowSteps', `VALIDATION FAILED: Missing authorization code`);
			...
			if (!credentials.clientId || !credentials.clientId.trim()) {
				logger.error('UnifiedFlowSteps', `VALIDATION FAILED: Missing client ID`);
				const errorMsg =
					'Client ID is required for token exchange. Credentials may have been lost. Please check your configuration.';
				setError(errorMsg);
				setValidationErrors([errorMsg]);
				logger.error('UnifiedFlowSteps', `Missing clientId in credentials:`, {
					credentials,
					hasClientId: !!credentials.clientId,
			...
			if (!credentials.environmentId || !credentials.environmentId.trim()) {
				logger.error('UnifiedFlowSteps', `VALIDATION FAILED: Missing environment ID`);
				const errorMsg =
					'Environment ID is required for token exchange. Please check your configuration.';
				setError(errorMsg);
				setValidationErrors([errorMsg]);
				logger.error('UnifiedFlowSteps', `Missing environmentId in credentials:`, {
					credentials,
					hasEnvironmentId: !!credentials.environmentId,
				});
				return;
			...
			if (!isPKCERequired && (!credentials.redirectUri || !credentials.redirectUri.trim())) {
				logger.error(
					'UnifiedFlowSteps',
					`VALIDATION FAILED: PKCE not required and missing redirect URI`
				);
				const errorMsg =
			...
			logger.info(`${MODULE_TAG} ✅ All validations passed`, 'Logger info');
			logger.info(`${MODULE_TAG} Preparing token exchange request with:`, {
				flowType,
				environmentId: credentials.environmentId,
				clientId: credentials.clientId,
				hasClientSecret: !!credentials.clientSecret,
			...
			if (
				...
				try {
					const { workerTokenService } = await import('@/mfa/services/workerTokenService');
					const { ConfigCheckerService } = await import('@/mfa/services/configCheckerService');
					const workerToken = await workerTokenService.getToken();
			...
			try {
				...
				if (credentials.enableRefreshToken && !tokens.refresh_token) {
					logger.warn('UnifiedFlowSteps', `Refresh token expected but not received`, {
						enableRefreshToken: credentials.enableRefreshToken,
						scopesRequested: credentials.scopes,
					...
					try {
						...
						if (userInfo) {
							...
							modernMessaging.showBanner({
								type: 'warning',
								title: 'Warning',
								message: 'UserInfo could not be fetched, but tokens were received',
								dismissible: true,
							});
				...
				if (
					...
					logger.error(
						'UnifiedFlowSteps',
						`PKCE mismatch detected: Authorization URL was generated with 'plain' method`
					);
					...
					try {
						...
						if (storedPKCE?.codeChallengeMethod === 'plain') {
							...
							logger.info(`${MODULE_TAG} ✅ Old PKCE codes cleared`, 'Logger info');
						}
					} catch (clearError) {
						logger.error(
							'UnifiedFlowSteps',
					...
					modernMessaging.showBanner({
						type: 'error',
						title: 'Error',
						message: 'PKCE method mismatch - old codes cleared, please regenerate',
						dismissible: true,
					});
					...
					modernMessaging.showBanner({
						type: 'error',
						title: 'Error',
						message: 'PKCE required - please enable PKCE and restart the flow',
						dismissible: true,
					});
				} else {
					setError(message);
	...
	const handleFetchUserInfo = useCallback(async () => {
		...
		const canCallUserInfo = TokenOperationsService.isOperationAllowed(
			flowType,
			credentials.scopes,
			'userinfo'
		);
		...
		if (!canCallUserInfo) {
			...
			logger.warn('UnifiedFlowSteps', `${errorMsg}`);
			modernMessaging.showBanner({
				type: 'error',
				title: 'Error',
				message: errorMsg,
	...
	const handleIntrospectToken = useCallback(
		async (tokenType: 'access' | 'refresh' | 'id' = selectedTokenType) => {
			logger.info(`${MODULE_TAG} ========== TOKEN INTROSPECTION START ==========`, 'Logger info');
			logger.info(`${MODULE_TAG} Introspecting ${tokenType} token`, 'Logger info');
			logger.info(`${MODULE_TAG} Credentials check:`, {
				hasEnvironmentId: !!credentials.environmentId,
				hasClientId: !!credentials.clientId,
				hasClientSecret: !!credentials.clientSecret,
				hasAccessToken: !!flowState.tokens?.accessToken,
			...
			if (
				...
				logger.error('UnifiedFlowSteps', `${errorMsg}`, {
					environmentId: credentials.environmentId,
					clientId: credentials.clientId,
			...
			try {
				...
				logger.info(`${MODULE_TAG} Discovering endpoints from`, { issuerUrl });

				const discoveryResult = await OidcDiscoveryService.discover(issuerUrl);
				logger.info(`${MODULE_TAG} Discovery result:`, {
				...
				if (!discoveryResult.success || !discoveryResult.data?.introspectionEndpoint) {
					...
					logger.warn(
						'UnifiedFlowSteps',
						`Discovery failed or no introspection endpoint, using fallback`,
						{
				...
				logger.info(`${MODULE_TAG} Introspecting token at endpoint`, { introspectionEndpoint });
				...
				logger.info(`${MODULE_TAG} Sending introspection request via proxy:`, {
					tokenType,
				...
				logger.info(`${MODULE_TAG} Introspection response status:`, {
					status: response.status,
					statusText: response.statusText,
					ok: response.ok,
				});
				...
				if (!response.ok) {
					...
					logger.error('UnifiedFlowSteps', `Introspection error response:`, undefined, errorText);
					throw new Error(
				...
				logger.info(`${MODULE_TAG} Introspection data received:`, data);

				setIntrospectionData(data);
				setIntrospectionTokenType(tokenType);
				modernMessaging.showFooterMessage({
					type: 'info',
					message: `${tokenName} introspected successfully!`,
					duration: 3000,
				});
				logger.info(
					`${MODULE_TAG} ========== TOKEN INTROSPECTION SUCCESS ==========`,
					'Logger info'
				);
			} catch (err) {
				const message = err instanceof Error ? err.message : 'Failed to introspect token';
				logger.error('UnifiedFlowSteps', `Introspection error:`, undefined, err);
				...
				logger.info(
					`${MODULE_TAG} ========== TOKEN INTROSPECTION FAILED ==========`,
					'Logger info'
				);
			} finally {
				setIntrospectionLoading(false);
	...
	const handleRefreshToken = useCallback(async () => {
		...
		if (!credentials.environmentId || !credentials.clientId) {
			modernMessaging.showBanner({
				type: 'error',
				title: 'Error',
				message: 'Missing credentials required for token refresh',
				dismissible: true,
			});
			return;
		}
		...
		try {
			const { OAuthIntegrationService } = await import('@/mfa/services/oauthIntegrationService');
			...
			const newTokenResponse = await OAuthIntegrationService.refreshAccessToken(
				{
					environmentId: credentials.environmentId,
					clientId: credentials.clientId,
			...
			logger.info(`${MODULE_TAG} ✅ Token refresh successful`, {
			...
			logger.error('UnifiedFlowSteps', `Token refresh failed`, { error: message });
	...
	const renderStep6IntrospectionUserInfo = () => {
		...
		const operationRules = TokenOperationsService.getOperationRules(flowType, scopesForRules);

		// Debug logging
		logger.info(`${MODULE_TAG} [INTROSPECTION] Operation rules check`, {
			flowType,
					...
					<div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
						...
						<h3
							style={{
								...
								fontSize: '18px',
								color:
									operationRules.canIntrospectAccessToken && hasAccessToken ? '#92400e' : '#6b7280',
								flex: 1,
							}}
						>
							Token Introspection
								...
								<li style={{ marginTop: '4px' }}>
									...
									{!operationRules.canIntrospectIdToken && hasIdToken && (
										<>
											...
											>
												<strong>Why?</strong> ID tokens are JWTs designed to be validated locally by
												your application, not by calling the introspection endpoint.
												<br />
												<br />
							...
							<>
								...
								>
									...
									<ul style={{ margin: '8px 0 0 0', paddingLeft: '20px', lineHeight: '1.6' }}>
										...
										<li>This allows users to stay authenticated without re-entering credentials</li>
									</ul>
								</div>
			...
			>
				...
				>
					...
					>
						...
						>
							The device has been authorized successfully and tokens have been received. You can now
							proceed to view your tokens and use them for API calls.
						</div>
						{flowState.tokens?.expiresIn && (
							<div
								style={{
					...
					<div style={{ flex: 1, overflowY: 'auto', marginBottom: '16px', minHeight: 0 }}>
						...
						{(() => {
							if (callbackDetails.allParams.id_token) {
								const decoded = TokenDisplayService.decodeJWT(callbackDetails.allParams.id_token);
								if (decoded?.payload) {
	...
	const renderStepContent = () => {
		...
		if (prevStepRef.current !== currentStep || prevFlowTypeRef.current !== flowType) {
			logger.info(`${MODULE_TAG} [STEP ROUTING] Rendering step content`, {
				currentStep,
				flowType,
				usePKCE: isPKCERequired,
				pkceEnforcement: credentials.pkceEnforcement,
		...
		switch (currentStep) {
			...
			case 1:
				...
				if (flowType === 'oauth-authz' || flowType === 'hybrid') {
					logger.info(`${MODULE_TAG} [STEP ROUTING] Showing PKCE step (Step 1) for ${flowType}`);
					return renderStep1PKCE();
				}
				// For implicit flow, show Authorization URL step
				logger.info(
					`${MODULE_TAG} [STEP ROUTING] Showing Auth URL step for ${flowType}`,
					'Logger info'
				);
					...
					onClose={async () => {
						...
						if (tokenStatus.isValid) {
							...
							try {
								...
								logger.error(
									'UnifiedFlowSteps',
									`Error re-running validation after token retrieval:`,
									undefined,
									error
								);
								modernMessaging.showBanner({
									type: 'error',
									title: 'Error',
									message: 'Failed to re-run validation. Please try again.',
									dismissible: true,
								});
							} finally {
								setIsLoading(false);
