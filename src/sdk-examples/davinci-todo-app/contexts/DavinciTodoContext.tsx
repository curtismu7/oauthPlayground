// src/sdk-examples/davinci-todo-app/contexts/DavinciTodoContext.tsx
// React Context for DaVinci Todo App state management

import { createContext, ReactNode, useContext, useEffect, useReducer } from 'react';
import DavinciTodoService, {
	DavinciCollector,
	DavinciFlow,
	Todo,
} from '../services/davinciTodoService';
...
// Reducer function
function davinciTodoReducer(
	state: typeof initialState,
	action: DavinciTodoAction
): typeof initialState {
	switch (action.type) {
		case 'SET_LOADING':
			return {
...
interface DavinciTodoContextType {
	...
	startFlow: (
		flowId: string
	) => Promise<{ sessionId: string; firstCollector: DavinciCollector | null }>;
	submitCollector: (
		sessionId: string,
...
export function DavinciTodoProvider({ children }: DavinciTodoProviderProps) {
	...
	useEffect(() => {
		const storedUser = DavinciTodoService.getCurrentUser();
		if (storedUser) {
	...
	const initializeClient = async () => {
		setLoading(true);
		try {
			const result = await DavinciTodoService.initializeClient();
			if (!result.success) {
	...
	const getFlows = async () => {
		setLoading(true);
		try {
			const flows = await DavinciTodoService.getFlows();
			return flows;
		} catch (error) {
			const errorMessage = error instanceof Error ? error.message : 'Unknown error';
			setError(errorMessage);
	...
	const startFlow = async (flowId: string) => {
		setLoading(true);
		try {
			const result = await DavinciTodoService.startFlow(flowId);
			return result;
		} catch (error) {
			const errorMessage = error instanceof Error ? error.message : 'Unknown error';
			setError(errorMessage);
	...
	const submitCollector = async (
		...
		setLoading(true);
		try {
			const result = await DavinciTodoService.submitCollector(sessionId, collectorName, data);
			return result;
		} catch (error) {
			const errorMessage = error instanceof Error ? error.message : 'Unknown error';
			setError(errorMessage);
	...
	const createTodo = async (title: string, description?: string) => {
		setLoading(true);
		try {
			const newTodo = await DavinciTodoService.createTodo(title, description);
			addTodo(newTodo);
			return newTodo;
		} catch (error) {
			const errorMessage = error instanceof Error ? error.message : 'Unknown error';
	...
	const updateTodoService = async (todoId: string, updates: Partial<Todo>) => {
		setLoading(true);
		try {
			const updatedTodo = await DavinciTodoService.updateTodo(todoId, updates);
	...
	const deleteTodoService = async (todoId: string) => {
		setLoading(true);
		try {
			await DavinciTodoService.deleteTodo(todoId);
			deleteTodo(todoId);
		} catch (error) {
			const errorMessage = error instanceof Error ? error.message : 'Unknown error';
			setError(errorMessage);
	...
	const getCurrentUser = () => {
		return DavinciTodoService.getCurrentUser();
	};

	const setCurrentUser = (user: { id: string; email: string; name: string }) => {
		DavinciTodoService.setCurrentUser(user);
		setUser(user);
	};

	const clearCurrentUser = () => {
		DavinciTodoService.clearCurrentUser();
		setUser(null);
	};

	const validateCollectorData = (collector: DavinciCollector, data: Record<string, unknown>) => {
		return DavinciTodoService.validateCollectorData(collector, data);
	};

	const getClientStatus = () => {
		return DavinciTodoService.getClientStatus();
	};

	const getConfiguration = () => {
		return DavinciTodoService.getConfiguration();
	};

	const cleanup = () => {
		DavinciTodoService.cleanup();
		resetState();
	};
