 ...
 */
export function validateScopesForFlow(
	flowType: string,
	...
	if (!mapping.requiresOpenId && normalizedScopes.includes('openid')) {
		warnings.push(
			`Flow "${mapping.flowName}" does not require "openid" scope. Worker tokens use Management API scopes (p1:read:*, p1:write:*).`
		);
	}
	...
	if (mapping.category === 'worker-token') {
		...
		if (foundOidcScopes.length > 0) {
			warnings.push(
				`Worker tokens should use Management API scopes, not OIDC scopes (${foundOidcScopes.join(', ')}).`
			);
		}
	}
	...
	return {
		...
		warnings,
		normalizedScopes:
			mapping.requiresOpenId && !normalizedScopes.includes('openid')
