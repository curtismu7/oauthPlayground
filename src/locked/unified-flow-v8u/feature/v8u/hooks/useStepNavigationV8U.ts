...
export interface UseStepNavigationV8UReturn {
 ...
 *
 * @example
 * const nav = useStepNavigationV8UU(4);
 *
 ...
 */
export const useStepNavigationV8U = (
	totalSteps: number,
	options: UseStepNavigationV8UOptions = {}
): UseStepNavigationV8UReturn => {
	const {
		initialStep = 0,
		onStepChange,
		onValidationChange,
	} = options as UseStepNavigationV8UOptions;
	...
	const goToStep = useCallback(
		(step: number) => {
			...
			logger.info(`${MODULE_TAG} Going to step`, { from: currentStep, to: step });
	...
	const markStepComplete = useCallback(() => {
		if (!completedSteps.includes(currentStep)) {
			logger.info(`${MODULE_TAG} Marking step as complete`, { step: currentStep });
			setCompletedSteps((prev) => [...prev, currentStep]);
		}
...
export default useStepNavigationV8U;
