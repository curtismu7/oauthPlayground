...
export class DualStorageServiceV8 {
	...
	static async save<T>(options: DualStorageOptions, data: T): Promise<DualStorageResult<void>> {
		...
		try {
			...
			logger.info(
				`${MODULE_TAG} Saved to browser storage: ${options.browserStorageKey}`,
				'Logger info'
			);
		} catch (error) {
	...
	static async load<T>(options: DualStorageOptions): Promise<DualStorageResult<T>> {
		...
		try {
			...
			if (browserData) {
				...
				logger.info(
					`${MODULE_TAG} Loaded from browser storage: ${options.browserStorageKey}`,
					'Logger info'
				);
	...
	static async delete(options: DualStorageOptions): Promise<DualStorageResult<void>> {
		...
		try {
			...
			logger.info(
				`${MODULE_TAG} Deleted from browser storage: ${options.browserStorageKey}`,
				'Logger info'
			);
		} catch (error) {
