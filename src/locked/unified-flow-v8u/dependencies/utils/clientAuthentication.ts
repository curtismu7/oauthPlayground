...
export const createClientAssertion = async (
	...
	try {
		if (algorithm === 'HS256') {
			...
			return await new SignJWT(claims).setProtectedHeader({ alg: 'HS256' }).sign(secretKey);
		} else {
			// Sign with private key (RSA)
			const { SignJWT, importPKCS8 } = await import('jose');
			const privateKey = await importPKCS8(secret, 'RS256');
			return await new SignJWT(claims).setProtectedHeader({ alg: 'RS256' }).sign(privateKey);
		}
	} catch (error) {
		throw new Error(
			`Failed to create client assertion: ${error instanceof Error ? error.message : String(error)}`
		);
	}
};
 ...
 */
export const getAuthMethodSecurityLevel = (
	method: ClientAuthMethod
): {
	...
	switch (method) {
		case 'none':
			return {
				level: 'Low',
				description:
					'No client authentication. Relies on PKCE for security. Suitable for public clients that cannot store secrets.',
				icon: '',
			};
		case 'client_secret_post':
			return {
				level: 'Medium',
				description:
					'Client secret sent in POST body. Simple but secret is visible in request body.',
				icon: '',
			};
		case 'client_secret_basic':
			return {
				level: 'Medium',
				description:
					'Client secret sent via HTTP Basic Authentication. More secure than POST method.',
				icon: '',
			};
		case 'client_secret_jwt':
			return {
				level: 'High',
				description:
					'JWT signed with client secret (HS256). Prevents secret exposure and includes timing protection.',
				icon: '',
			};
		case 'private_key_jwt':
			return {
				level: 'Highest',
				description:
					'JWT signed with private key (RS256). No shared secrets, asymmetric cryptography. Enterprise grade.',
				icon: '',
			};
	}
};
