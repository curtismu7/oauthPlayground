...
import EnhancedFlowInfoCard from '../../components/EnhancedFlowInfoCard.tsx';
import FlowConfigurationRequirements from '../../components/FlowConfigurationRequirements.tsx';
import FlowSequenceDisplay from '../../components/FlowSequenceDisplay.tsx';
import { PasswordChangeModal } from '../../components/PasswordChangeModal.tsx';
import RedirectlessLoginModal from '../../components/RedirectlessLoginModal.tsx';
import { type PKCECodes, PKCEService } from '../../services/pkceService.tsx';
import { WorkerTokenVsClientCredentialsEducationModalV8 } from '../../v8/components/WorkerTokenVsClientCredentialsEducationModalV8.tsx';
import { CredentialsServiceV8 } from '../../v8/services/credentialsServiceV8.ts';
import type { TokenResponse } from '../../v8/services/oauthIntegrationServiceV8.ts';
import { OidcDiscoveryServiceV8 } from '../../v8/services/oidcDiscoveryServiceV8.ts';
import { type FlowType, type SpecVersion } from '../../v8/services/specVersionServiceV8.ts';
import { TokenDisplayServiceV8 } from '../../v8/services/tokenDisplayServiceV8.ts';
import { TokenOperationsServiceV8 } from '../../v8/services/tokenOperationsServiceV8.ts';
...
export const UnifiedFlowSteps: React.FC<UnifiedFlowStepsProps> = ({
		...
		if (flowType === 'implicit' || flowType === 'hybrid') {
			...
			if (storedTokens) {
				try {
					...
					logger.error(`${MODULE_TAG} Failed to parse stored ${flowType} tokens`, err);
				}
			}
			...
			if (storedTokens) {
				try {
					...
					logger.error(`${MODULE_TAG} Failed to parse stored client-credentials tokens`, err);
				}
			}
			...
			if (storedDeviceCode) {
				try {
					...
					logger.error(`${MODULE_TAG} Failed to parse stored device code data`, err);
				}
			}
		}
	...
	const showTokenSuccessModal = useCallback(
		(tokens: TokenResponse) => {
			logger.info(`${MODULE_TAG} ✅ Tokens received - proceeding to next step`, "Logger info");
		...
		async (accessToken: string, environmentId: string): Promise<Record<string, unknown> | null> => {
			try {
				...
				if (!discoveryResult.success || !discoveryResult.data?.userInfoEndpoint) {
					...
					logger.warn(`${MODULE_TAG} Discovery failed, using fallback UserInfo endpoint`, {
						userInfoEndpoint,
						discoveryError: discoveryResult.error,
					});
				} else {
					userInfoEndpoint = discoveryResult.data.userInfoEndpoint;
					logger.info(`${MODULE_TAG} Fetching UserInfo via backend proxy`, { userInfoEndpoint });
				}
		...
		if (currentStep === 3 && (flowType === 'oauth-authz' || flowType === 'hybrid')) {
			...
			if (flowType === 'hybrid' && !flowState.tokens?.accessToken) {
				...
				if (fragment && (fragment.includes('access_token') || fragment.includes('id_token'))) {
					logger.info(`${MODULE_TAG} Hybrid flow: Fragment detected in URL`, {
					...
					try {
						...
						logger.info(`${MODULE_TAG} Hybrid flow: Tokens extracted from fragment`, {
							hasAccessToken: !!fragmentTokens?.access_token,
							hasIdToken: !!fragmentTokens?.id_token,
						});
					} catch (err) {
						logger.error(`${MODULE_TAG} Failed to parse fragment for hybrid flow`, err);
					}
				}
			}
			...
			if (flowType === 'hybrid') {
				...
				if (detectedCode && callbackUrl && !flowState.authorizationCode) {
					try {
						...
						hasUpdates = true;
						logger.info(`${MODULE_TAG} Hybrid flow: Authorization code extracted`, {
							code: parsed.code,
						});
					} catch (err) {
	...
	const renderStep3ExchangeTokens = () => {
		const handleExchangeTokens = async () => {
			...
			if (isPKCERequired && !effectiveCodeVerifier) {
				logger.error(`${MODULE_TAG} ❌ VALIDATION FAILED: PKCE required but code verifier missing`, "Logger error");
