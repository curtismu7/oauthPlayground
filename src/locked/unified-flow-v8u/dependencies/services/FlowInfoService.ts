...
const flowConfigs: Record<string, DetailedFlowInfo> = {
...
const normalizeFlowKey = (flowType: string): string => {
	...
	return flowConfigs[trimmed] ? trimmed : flowType;
};
...
const getFlowInfo = (flowType: string): DetailedFlowInfo | null => {
	...
	const key = normalizeFlowKey(flowType);
	return flowConfigs[key] || null;
};
...
const generateFlowInfoCard = (flowType: string): FlowInfoCardData | null => {
	...
	const flowInfo = getFlowInfo(flowType);
	if (!flowInfo) {
		return null;
	}
	...
	return {
		header: {
			...
			badge: getCategoryBadge(flowInfo.flowCategory),
			icon: getFlowIcon(flowInfo.flowName),
		},
		keyDetails: {
		...
		additionalInfo: {
			complexity: getComplexityLabel(flowInfo.complexity),
			securityLevel: getSecurityLevelLabel(flowInfo.securityLevel),
			userInteraction: getUserInteractionLabel(flowInfo.userInteraction),
			backendRequired: flowInfo.backendRequired,
		},
	};
};

const getAvailableFlowTypes = (): string[] => Object.keys(flowConfigs);

const getFlowsByCategory = (category: DetailedFlowInfo['flowCategory']): string[] =>
	Object.entries(flowConfigs)
		.filter(([, config]) => config.flowCategory === category)
		.map(([flowType]) => flowType);

const getFlowsByComplexity = (complexity: DetailedFlowInfo['complexity']): string[] =>
	Object.entries(flowConfigs)
		.filter(([, config]) => config.complexity === complexity)
		.map(([flowType]) => flowType);

const getFlowsBySecurityLevel = (securityLevel: DetailedFlowInfo['securityLevel']): string[] =>
	Object.entries(flowConfigs)
		.filter(([, config]) => config.securityLevel === securityLevel)
		.map(([flowType]) => flowType);

const searchFlows = (criteria: {
...
const getRelatedFlows = (flowType: string): string[] => {
	const flowInfo = getFlowInfo(flowType);
	return flowInfo?.relatedFlows ?? [];
};

const getCommonIssues = (flowType: string): Array<{ issue: string; solution: string }> => {
	const flowInfo = getFlowInfo(flowType);
 ...
 */
const getImplementationNotes = (flowType: string): string[] => {
	const flowInfo = getFlowInfo(flowType);
 ...
 */
const getDocumentationLinks = (flowType: string): Array<{ title: string; url: string }> => {
	const flowInfo = getFlowInfo(flowType);
	return flowInfo?.documentationLinks ?? [];
};

// Private helper methods
const getCategoryBadge = (category: DetailedFlowInfo['flowCategory']): string => {
	switch (category) {
		case 'standard':
			return 'Standard';
		case 'experimental':
...
const getSecurityLevelLabel = (securityLevel: DetailedFlowInfo['securityLevel']): string => {
...
const getUserInteractionLabel = (userInteraction: DetailedFlowInfo['userInteraction']): string => {
