...
export class RedirectStateManager {
	...
	static preserveFlowState(flowId: string, state: FlowState): boolean {
		try {
			...
			logger.info(`[RedirectStateManager] Preserved flow state for ${flowId}:`, {
			...
			logger.error('[RedirectStateManager] Failed to preserve flow state:', error);
			return false;
		}
	}
	...
	static restoreFlowState(flowId: string): FlowState | null {
		try {
			...
			logger.info(`[RedirectStateManager] Restored flow state for ${flowId}:`, {
			...
			logger.error('[RedirectStateManager] Failed to restore flow state:', error);
			return null;
		}
	}
	...
	static handleRedirectReturn(callbackData: CallbackData): RedirectResult {
		try {
			...
			const flowState = RedirectStateManager.restoreFlowState(
				flowContext.metadata?.flowId || flowContext.flowType
			);

			// Handle OAuth errors
			if (callbackData.error) {
				logger.warn('[RedirectStateManager] OAuth error in callback:', callbackData.error);
			...
			const redirectUrl = RedirectStateManager.buildSuccessRedirectUrl(flowContext, callbackData);

			// Clean up after successful redirect
			RedirectStateManager.clearFlowState(flowContext.metadata?.flowId || flowContext.flowType);
			FlowContextService.clearFlowContext();

			logger.info('[RedirectStateManager] Redirect return successful:', {
			...
			logger.error('[RedirectStateManager] Failed to handle redirect return:', error);
			return {
				success: false,
				redirectUrl: '/dashboard',
	...
	static validateRedirectSecurity(context: FlowContext, callbackData: CallbackData): boolean {
		try {
			...
			if (!contextValidation.valid) {
				logger.warn('[RedirectStateManager] Invalid flow context:', contextValidation.errors);
				return false;
			}
			...
			if (callbackData && typeof callbackData === 'object') {
				...
				const dangerousPatterns = [/<script/i, /javascript:/i, /data:/i, /vbscript:/i, /on\w+=/i];
			...
			logger.error('[RedirectStateManager] Security validation error:', error);
			return false;
		}
	}
	...
	static createRedirectContext(
		...
		try {
			...
			// Preserve flow state
			const stateSaved = RedirectStateManager.preserveFlowState(flowId, flowState);
			if (!stateSaved) {
				FlowContextService.clearFlowContext(flowId);
				return false;
			}

			logger.info(`[RedirectStateManager] Created redirect context for ${flowType}:`, {
				flowId,
				currentStep,
				returnPath,
			});
			...
			logger.error('[RedirectStateManager] Failed to create redirect context:', error);
			return false;
		}
	}
	...
	static cleanupExpiredStates(): void {
		try {
			...
			for (let i = 0; i < sessionStorage.length; i++) {
				...
				if (key?.startsWith(RedirectStateManager.FLOW_STATE_PREFIX)) {
					try {
						...
						if (stateString) {
							...
							if (
								state._timestamp &&
								Date.now() - state._timestamp > RedirectStateManager.MAX_STATE_AGE_MS
							) {
	...
	private static getStateKey(flowId: string): string {
		return `${RedirectStateManager.FLOW_STATE_PREFIX}${flowId}`;
	}
	...
	private static buildSuccessRedirectUrl(context: FlowContext, callbackData: CallbackData): string {
		try {
			...
			logger.error('[RedirectStateManager] Failed to build success redirect URL:', error);
	...
	private static buildErrorRedirectUrl(context: FlowContext, callbackData: CallbackData): string {
		try {
			...
			logger.error('[RedirectStateManager] Failed to build error redirect URL:', error);
			return context.returnPath;
		}
	}
}
