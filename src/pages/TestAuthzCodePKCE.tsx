...
const Status = styled.span<{ type: 'success' | 'error' | 'pending' }>`
  display: inline-block;
  padding: 4px 8px;
  border-radius: 4px;
  font-size: 12px;
...
const TestAuthzCodePKCE: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (krogerCreds) {
      setConfig({
        ...
        redirectUri: 'https://localhost:3000/test-authz-pkce',
        scopes: Array.isArray(krogerCreds.scopes) ? krogerCreds.scopes.join(' ') : 'openid profile email'
      });
  ...
  useEffect(() => {
    if (autoExchangeEnabled && autoExchange && authCode && pkceData && config.envId && config.clientId && config.clientSecret && !tokenData) {
  ...
  }, [autoExchange, authCode, pkceData, config, tokenData]);
        ...
        <div>
          <Section>
            ...
            <div style={{display: 'flex', gap: '10px'}}>
              ...
              <Button onClick={handleClearStorage} style={{background: '#e53e3e'}}>Clear Storage</Button>
            </div>
          </Section>
          ...
          <Section>
            ...
            {pkceData && (
              <CodeBlock>
                <strong>Code Verifier:</strong> {pkceData.codeVerifier}<br/>
                <strong>Length:</strong> {pkceData.codeVerifier.length} characters<br/>
                <strong>Code Challenge:</strong> {pkceData.codeChallenge}<br/>
          ...
          <Section>
            ...
            {authUrl && (
              <>
                ...
                <Button onClick={() => window.open(authUrl, '_blank')} style={{marginTop: '10px'}}>
          ...
          <Section>
            ...
            {tokenData && (
              <CodeBlock style={{marginTop: '10px'}}>
                <strong>Access Token:</strong> {tokenData.access_token.substring(0, 50)}...<br/>
                ...
                <strong>ID Token:</strong> {tokenData.id_token ? 'Present ✓' : 'Not present'}<br/>
                <strong>Refresh Token:</strong> {tokenData.refresh_token ? 'Present ✓' : 'Not present'}
              </CodeBlock>
            )}
          </Section>
          ...
          <Section>
            ...
            <Button onClick={() => {
              ...
              handleVerifyPKCE();
            }}>Verify PKCE Implementation</Button>
          </Section>
        </div>
            ...
            <Section>
              ...
              <div>
                <div style={{margin: '8px 0'}}>
                  PKCE Generation: <Badge type={results.pkceGeneration ? 'pass' : 'fail'}>
                    {results.pkceGeneration ? 'PASS' : 'FAIL'}
                  </Badge>
                </div>
                <div style={{margin: '8px 0'}}>
                  PKCE Storage: <Badge type={results.pkceStorage ? 'pass' : 'fail'}>
                    {results.pkceStorage ? 'PASS' : 'FAIL'}
                  </Badge>
                </div>
                <div style={{margin: '8px 0'}}>
                  PKCE in Token Exchange: <Badge type={results.pkceInExchange ? 'pass' : 'fail'}>
                    {results.pkceInExchange ? 'PASS' : 'FAIL'}
                  </Badge>
                </div>
                <div style={{margin: '8px 0'}}>
                  Token Receipt: <Badge type={results.tokenReceipt ? 'pass' : 'fail'}>
                    {results.tokenReceipt ? 'PASS' : 'FAIL'}
                  </Badge>
                </div>
                <div style={{margin: '8px 0'}}>
                  ID Token: <Badge type={results.idToken ? 'pass' : 'fail'}>
                    {results.idToken ? 'PASS' : 'FAIL'}
                  </Badge>
                </div>
                <div style={{margin: '8px 0'}}>
                  Refresh Token: <Badge type={results.refreshToken ? 'pass' : 'fail'}>
