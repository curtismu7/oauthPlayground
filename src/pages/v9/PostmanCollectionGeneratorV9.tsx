...
export const PostmanCollectionGenerator: React.FC = () => {
	...
	// Map V9FlowType to UnifiedVariation types
	...
	const getVariationsForSpec = (specVersion: V9SpecVersion): UnifiedVariation[] => {
		const flows = V9SpecVersionService.getAvailableFlows(specVersion);
	...
	const getCredentials = () => {
		...
		const unifiedCreds = V9FlowCredentialService.load();
		...
		const mfaCreds = V9FlowCredentialService.load();
		...
		if (mfaCreds?.userEmail) credentials.userEmail = mfaCreds.userEmail;
	...
	const generateFilteredUnifiedCollection = (credentials: {
		...
		const allSpecVersions: V9SpecVersion[] = [];
		...
		const allFlows = new Set<V9FlowType>();
		for (const spec of allSpecVersions) {
			for (const flow of V9SpecVersionService.getAvailableFlows(spec)) {
				allFlows.add(flow);
			}
		}
						...
						<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
							...
							>
								{expandedUnifiedVariations ? (
									<BootstrapIcon icon="chevron-down" size={20} />
								) : (
									<BootstrapIcon icon="chevron-right" size={20} />
								)}
