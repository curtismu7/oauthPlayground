...
export const OAuthOIDCData = {
	comparisonTable: [
		...
		},
		{
			...
			oauth: "✅ Supported (RFC 9126) - Pushes authorization request parameters to server before redirect",
			oidc: "✅ Supported - Enhanced security for OIDC flows"
		},
		{
			feature: "RAR (Rich Authorization Requests)",
			oauth: "✅ Supported (RFC 9396) - Allows fine-grained authorization with complex permission structures",
	...
	trainingContent: {
		oauth: {
			...
			description: "OAuth 2.0 is an authorization framework that enables applications to obtain limited access to user accounts on an HTTP service.",
			keyPoints: [
			...
			oauthOnlyNote: "Note: Client Credentials Flow is OAuth 2.0 only - it does not apply to OIDC since there is no user authentication involved (machine-to-machine only).",
			flows: [
				{
		...
		oidc: {
			...
			description: "OIDC is an identity layer built on top of OAuth 2.0 that adds authentication capabilities.",
			keyPoints: [
			...
			optionalParameters: {
				...
				description: "These parameters enhance the user experience in user-facing flows but do NOT apply to Client Credentials Flow (no user involved).",
				parameters: [
					{
						name: "login_hint",
					...
					{
						name: "acr_values",
						description: "Requested Authentication Context Class Reference (e.g., multi-factor authentication)",
						applicableFlows: ["Authorization Code", "PKCE", "Device Authorization"],
						notApplicableTo: ["Client Credentials"],
		...
		security: {
			...
			practices: [
				...
				},
				{
					...
					description: "Push authorization parameters to the server before redirect to prevent parameter tampering and leakage in browser history/logs"
				},
				{
					name: "Implement RAR (Rich Authorization Requests)",
					description: "Use RAR for fine-grained, complex authorization scenarios requiring detailed permission structures beyond simple scopes"
				},
				{
			...
			advancedFeatures: {
				par: {
					...
					description: "PAR allows clients to push authorization request parameters directly to the authorization server via a back-channel request before initiating the user authorization flow.",
					benefits: [
				...
				rar: {
					...
					description: "RAR extends OAuth 2.0 to support complex, fine-grained authorization requirements that go beyond simple scope strings.",
					benefits: [
	...
	const highlightText = (text: string): React.ReactNode => {
		...
		return parts.map((part, i) =>
			part.toLowerCase() === searchTerm.toLowerCase() ?
				<mark key={i} className="bg-yellow-300 px-1">{part}</mark> : part
		);
	};
				...
				<div className="bg-white rounded-lg shadow-lg p-8 mb-6">
					<div className="flex items-center gap-4 mb-4">
						...
						<div className="flex-1">
							...
							<p className="text-gray-600 mt-2">Complete guide to modern authentication and authorization</p>
						</div>
					</div>
					...
					<div className="p-6">
									...
									<tbody>
										{OAuthOIDCData.comparisonTable
											.filter(row =>
												...
												filterContent(typeof row.oauth === 'string' ? row.oauth : row.oauth.text || '') ||
												filterContent(typeof row.oidc === 'string' ? row.oidc : row.oidc.text || '')
											)
											.map((row, idx) => (
												<tr key={idx} className={idx % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
													<td className="p-4 font-medium text-gray-700 border-b">{highlightText(row.feature)}</td>
									...
									<div className="flex items-start gap-3">
										...
										<div>
											...
											<p className="text-blue-800">
												<strong>OAuth 2.0</strong> answers "what can they access?" while <strong>OIDC</strong> answers "who are they?"
												OIDC is essentially OAuth 2.0 + identity layer.
											</p>
										</div>
									</div>
								</div>
						...
						{activeTab === 'oauth' && (
							<div>
								...
								<p className="text-gray-600 mb-6">{OAuthOIDCData.trainingContent.oauth.description}</p>
								...
								<h3 className="text-xl font-bold text-gray-800 mb-4">OAuth 2.0 Authorization Flows</h3>
										...
										.map((flow) => {
											...
											return (
												<div key={actualIdx} className="border rounded-lg overflow-hidden">
													<button
														onClick={() => setExpandedFlow(expandedFlow === actualIdx.toString() ? null : actualIdx.toString())}
														className="w-full bg-gray-100 hover:bg-gray-200 p-4 flex items-center justify-between transition"
													>
														<div className="text-left">
															<h4 className="font-semibold text-gray-800">{highlightText(flow.name)}</h4>
															<p className="text-sm text-gray-600">{highlightText(flow.description)}</p>
														</div>
														<RefreshCw
															size={20}
																...
																<div className="bg-amber-50 p-3 rounded border border-amber-200 mb-4">
																	<div className="flex items-start gap-2">
																		...
																		<p className="text-sm text-amber-800">
																			<strong>OAuth 2.0 Only:</strong> This flow does not apply to OIDC since there is no user involved.
																			OIDC requires user authentication to provide identity information via ID Tokens.
																		</p>
																	</div>
																</div>
															)}
															...
															{flow.security && (
																<div className="bg-red-50 p-4 rounded border border-red-200">
																	<div className="flex items-center gap-2 mb-3">
																		...
																		<h5 className="font-semibold text-red-900">Security Best Practices for {flow.name}</h5>
																	</div>
																	<ul className="space-y-2">
																		{flow.security.map((practice: string, practiceIdx: number) => (
																			<li key={practiceIdx} className="flex items-start gap-2 text-sm text-red-800">
																				<CheckCircle size={16} className="mt-0.5 flex-shrink-0 text-red-600" />
							...
							<div>
								...
								<div className="grid md:grid-cols-2 gap-6 mb-6">
									<div className="border rounded-lg p-6 bg-white">
										...
										<p className="text-gray-600 mb-4">{OAuthOIDCData.trainingContent.oidc.idToken.description}</p>
										...
										<ul className="space-y-1">
											{OAuthOIDCData.trainingContent.oidc.idToken.claims.map((claim, idx) => (
												<li key={idx} className="text-sm text-gray-600 font-mono bg-gray-50 p-2 rounded">
													{claim}
												</li>
											))}
										</ul>
								...
								<div className="border rounded-lg p-6 bg-gradient-to-br from-indigo-50 to-purple-50 mb-6">
									...
									<div className="space-y-3">
										{OAuthOIDCData.trainingContent.oidc.optionalParameters.parameters.map((param, idx) => (
											<div key={idx} className="bg-white rounded-lg p-4 border border-indigo-200">
												<div className="flex items-start justify-between mb-2">
													...
													<div className="text-xs text-gray-500">
														...
														<div className="text-red-600">Not for: {param.notApplicableTo.join(', ')}</div>
													</div>
												</div>
							...
							<div>
								...
								<div className="space-y-6">
									...
									<div className="bg-white border rounded-lg p-6">
										...
										<p className="text-gray-600 mb-4">{OAuthOIDCData.trainingContent.security.advancedFeatures.par.description}</p>

										<div className="grid md:grid-cols-2 gap-6">
											<div>
												<h5 className="font-semibold text-gray-700 mb-2">Benefits</h5>
												<ul className="space-y-1">
													{OAuthOIDCData.trainingContent.security.advancedFeatures.par.benefits.map((benefit: string, idx: number) => (
											...
											<div>
												...
												<ol className="space-y-1">
													{OAuthOIDCData.trainingContent.security.advancedFeatures.par.howItWorks.map((step: string, idx: number) => (
														<li key={idx} className="text-sm text-gray-600 flex items-start gap-2">
									...
									<div className="bg-white border rounded-lg p-6">
										...
										<p className="text-gray-600 mb-4">{OAuthOIDCData.trainingContent.security.advancedFeatures.rar.description}</p>

										<div className="grid md:grid-cols-2 gap-6 mb-4">
											<div>
												<h5 className="font-semibold text-gray-700 mb-2">Benefits</h5>
												<ul className="space-y-1">
													{OAuthOIDCData.trainingContent.security.advancedFeatures.rar.benefits.map((benefit: string, idx: number) => (
														<li key={idx} className="text-sm text-gray-600 flex items-start gap-2">
															<CheckCircle size={14} className="mt-0.5 text-green-600 flex-shrink-0" />
															{benefit}
														</li>
											...
											<div>
												...
												<ul className="space-y-1">
													{OAuthOIDCData.trainingContent.security.advancedFeatures.rar.useCases.map((useCase: string, idx: number) => (
														<li key={idx} className="text-sm text-gray-600 flex items-start gap-2">
															<CheckCircle size={14} className="mt-0.5 text-blue-600 flex-shrink-0" />
															{useCase}
														</li>
													))}
												</ul>
										...
										<div className="bg-gray-50 rounded-lg p-4 border">
											<h5 className="font-semibold text-gray-700 mb-2">{OAuthOIDCData.trainingContent.security.advancedFeatures.rar.example.description}</h5>
											<pre className="text-xs bg-white p-3 rounded border overflow-x-auto">
												<code>{OAuthOIDCData.trainingContent.security.advancedFeatures.rar.example.json}</code>
											</pre>
										</div>
									</div>
								</div>
