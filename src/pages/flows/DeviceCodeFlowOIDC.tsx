...
const DeviceCodeFlowOIDC: React.FC = () => {
	...
	useEffect(() => {
		const loadCredentials = () => {
			try {
				...
				const deviceCredentials = credentialManager.loadFlowCredentials(
					"oidc-device-code-v3",
				);
				...
				if (deviceCredentials.environmentId || deviceCredentials.clientId) {
					setFlowState((prev) => ({
						...
						config: {
							...prev.config,
							environmentId:
								deviceCredentials.environmentId || prev.config.environmentId,
					...
					logger.info(
						"DEVICE-CODE-V3",
						"Loaded device flow-specific credentials",
					);
				} else {
					...
					if (configCredentials.environmentId || configCredentials.clientId) {
						setFlowState((prev) => ({
							...
							config: {
								...prev.config,
								environmentId:
									configCredentials.environmentId || prev.config.environmentId,
						...
						if (config?.environmentId || config?.clientId) {
							setFlowState((prev) => ({
								...
								config: {
									...prev.config,
									environmentId:
										config?.environmentId || prev.config.environmentId,
	...
	useEffect(() => {
		if (
			...
			!flowState.config.environmentId.includes("{")
		) {
	...
	const requestDeviceAuth = useCallback(async () => {
		...
		try {
			...
			if (
				...
				throw new Error(
					"Device authorization endpoint and Client ID are required",
				);
			}
	...
	const steps: DeviceCodeStep[] = [
		{
			...
			description:
				"Set up your PingOne application credentials for device code flow",
			status: "pending",
			canExecute: Boolean(
				flowState.config.deviceAuthorizationEndpoint &&
					flowState.config.clientId,
							...
							{step.id === "configure" && (
								<CredentialsSection>
									...
									>
										...
										>
											The Device Code Flow is designed for devices with limited
											input capabilities. Users will scan a QR code or enter a
											user code manually to authorize the device on a separate
											device with full browser capabilities.
										</p>
										...
										<div style={{ marginTop: "1.5rem" }}>
											...
											>
												<li>
													...
													(smart TVs, IoT devices, CLI tools)
												</li>
												<li>
													Eliminates the need to type complex credentials on
													constrained devices
												</li>
												<li>
													Uses a separate device (phone, computer) for
													authentication
												</li>
												<li>
													Provides secure authorization without storing
										...
										<div style={{ marginTop: "1.5rem" }}>
											...
											>
												<li>
													User codes expire after a short time (typically 15
													minutes)
												</li>
												...
												<li>
													Authorization happens on a trusted device with full
													browser
												</li>
											</ul>
										</div>
							...
							{step.id === "request-authorization" && (
								<div>
									...
									>
										...
										>
											Request a device authorization from PingOne. This will
											generate a user code and verification URL that the user
											can use on their phone or computer to authorize the
											device.
										</p>
										...
										<div style={{ marginTop: "1.5rem" }}>
											...
											>
												<li>
													PingOne generates a unique user code (e.g.,
													"ABCD-EFGH")
												</li>
												<li>A QR code is created with the verification URL</li>
												<li>
													The device polls the token endpoint for authorization
												</li>
												<li>
													User scans QR code or visits URL on their
													phone/computer
												</li>
												<li>User enters the code and authorizes the device</li>
												<li>
													Device receives access tokens after successful
													authorization
												</li>
											</ul>
										</div>
									...
									>
										...
										<div>
											...
											<p style={{ margin: 0, color: "#6b7280" }}>
												Click the button below to request a user code and
												verification URL from PingOne.
											</p>
										</div>
									</div>
								</div>
							...
							{step.id === "verify-device" && flowState.userCode && (
								<div>
									...
									>
										...
										>
											Complete device verification by having the user scan the
											QR code or enter the user code on their phone or computer.
											The device will automatically poll for tokens once
										...
										<div style={{ marginTop: "1.5rem" }}>
											...
											>
												<li>
													Use your phone or computer to scan the QR code below
												</li>
												<li>
													Alternatively, visit the verification URL and enter
													the user code
												</li>
												<li>Log in with your PingOne credentials</li>
												<li>Grant permission to the application</li>
												<li>
													The device will automatically receive tokens after
													authorization
												</li>
											</ul>
										</div>
