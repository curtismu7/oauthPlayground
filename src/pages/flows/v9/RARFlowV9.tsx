...
const RARFlowV9: React.FC = () => {
	...
	const generateAuthorizationUrl = useCallback(() => {
		...
		const url = `https://auth.pingone.com/${environmentId}/as/authorization.oauth2?` +
			`response_type=code&` +
								...
								<CollapsibleContent>
									...
									{authorizationUrl && (
										<GeneratedContentBox>
											...
											<HelperText>
												This URL contains RAR authorization details that specify exactly what permissions are being requested.
											</HelperText>
											
											<ParameterGrid>
												<ParameterLabel>Authorization Details:</ParameterLabel>
												<ParameterValue>
													<CodeBlock>{JSON.stringify([generateRARAuthorizationDetails()], null, 2)}</CodeBlock>
												</ParameterValue>
											</ParameterGrid>
										</GeneratedContentBox>
									)}
									...
									{tokens && (
										<GeneratedContentBox>
											...
											<InfoBox $variant="success" style={{ marginTop: '1rem' }}>
												...
												<InfoText>
													The token includes RAR authorization details that grant specific permissions as configured in step 2.
												</InfoText>
											</InfoBox>
										</GeneratedContentBox>
									)}
							...
							{!collapsedSections.completion && (
								<CollapsibleContent>
									<InfoBox $variant="success">
										...
										<InfoText>
											You have successfully completed the Rich Authorization Requests flow demonstration.
											The token you received includes specific authorization details that grant precise permissions
											as configured in the RAR parameters.
										</InfoText>
									</InfoBox>
								...
								{STEP_METADATA.map((step, index) => (
									<div
										...
										style={{
											...
											backgroundColor: index <= currentStep ? V9_COLORS.PRIMARY_BLUE : V9_COLORS.LIGHT_BLUE_BG,
