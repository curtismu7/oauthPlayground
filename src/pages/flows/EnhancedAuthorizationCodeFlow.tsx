...
const EnhancedAuthorizationCodeFlow: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (savedCredentials) {
      ...
      setCredentials({
        ...
        scopes: Array.isArray(config.scopes) ? config.scopes : config.scopes?.split(' ') || ['openid', 'profile', 'email'],
        authEndpoint: config.authorizationEndpoint || `https://auth.pingone.com/${config.environmentId}/as/authorize`,
        tokenEndpoint: config.tokenEndpoint || `https://auth.pingone.com/${config.environmentId}/as/token`,
        userInfoEndpoint: config.userInfoEndpoint || `https://auth.pingone.com/${config.environmentId}/as/userinfo`,
      });
    }
  }, [config]);
  ...
  const handleSaveCredentials = useCallback(() => {
    try {
      ...
      if (success) {
        setSaveStatus({ type: 'success', message: 'Credentials saved successfully!' });
  ...
  const buildAuthorizationUrl = useCallback(() => {
    ...
    if (flowConfig.acrValues.length > 0) {
      ...
      if (validAcrValues.length > 0) {
        ...
        console.warn('⚠️ [EnhancedAuthorizationCodeFlow] No valid ACR values found, skipping acr_values parameter');
      }
    }
  ...
  const exchangeCodeForTokens = useCallback(async () => {
    ...
    const tokenEndpoint = credentials.tokenEndpoint || `https://auth.pingone.com/${credentials.environmentId}/as/token`;
  ...
  const validateTokens = useCallback(async () => {
    ...
    const userInfoEndpoint = credentials.userInfoEndpoint || `https://auth.pingone.com/${credentials.environmentId}/as/userinfo`;
...
const credentials = {
  ...
  scopes: ${JSON.stringify(credentials.scopes || ['openid', 'profile', 'email'], null, 2)}
      ...
      execute: async () => {
        ...
        return { status: 'Credentials configured', credentials: { ...credentials, clientSecret: '***' } };
      },
      debugInfo: { credentials: { ...credentials, clientSecret: credentials.clientSecret ? '***' : undefined } },
      tips: [
...
const authUrl = '${credentials.authEndpoint || `https://auth.pingone.com/${credentials.environmentId}/as/authorize`}?' +
  new URLSearchParams({
    ...
    nonce: '${flowConfig.nonce || 'RANDOM_NONCE'}'${flowConfig.enablePKCE && pkceData ? `,
...
console.log('State:', state);`,
      execute: async () => {
        if (!authCode) {
          throw new Error('No authorization code available. Please complete the authorization step first.');
        }
      ...
      debugInfo: { hasAuthCode: !!authCode, authCodeLength: authCode?.length },
      tips: [
...
const tokenResponse = await fetch('${credentials.tokenEndpoint || `https://auth.pingone.com/${credentials.environmentId}/as/token`}', {
  ...
  body: new URLSearchParams({
    ...
    redirect_uri: '${credentials.redirectUri || getCallbackUrlForFlow('authorization-code')}',
    client_id: '${credentials.clientId || 'YOUR_CLIENT_ID'}',
    client_secret: '${credentials.clientSecret ? '***' : 'YOUR_CLIENT_SECRET'}'${flowConfig.enablePKCE && pkceData ? `,
...
console.log('User Info:', userInfo);`,
  ...
  ], [credentials, flowConfig, pkceData, authUrl, authCode, tokens]);
