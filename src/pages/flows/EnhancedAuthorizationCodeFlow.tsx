...
import EnhancedStepFlow, {
	type EnhancedFlowStep,
} from "../../components/EnhancedStepFlow";
...
const EnhancedAuthorizationCodeFlow: React.FC = () => {
		...
		if (savedCredentials) {
			...
			setCredentials({
				...
				userInfoEndpoint:
					...
					`https://auth.pingone.com/${config.environmentId}/as/userinfo`,
			});
		}
	}, [config]);
	...
	const buildAuthorizationUrl = useCallback(() => {
		...
		const params = new URLSearchParams({
			...
			redirect_uri:
				credentials.redirectUri || getCallbackUrlForFlow("authorization-code"),
		...
		if (flowConfig.acrValues.length > 0) {
			...
			if (validAcrValues.length > 0) {
				...
				console.log(
					" [EnhancedAuthorizationCodeFlow] Added valid ACR values:",
					validAcrValues,
				);
			} else {
				console.warn(
	...
	const validateTokens = useCallback(async () => {
		...
		logger.info(
			"EnhancedAuthCodeFlow",
			"Validating tokens with UserInfo endpoint",
			{
		...
		() => [
			{
				...
				description:
					"Configure your PingOne OAuth credentials for the authorization flow.",
...
const credentials = {
  ...
  scopes: ${JSON.stringify(credentials.scopes || ["openid", "profile", "email"], null, 2)}
			...
			},
			{
				...
				description:
					"Generate Proof Key for Code Exchange (PKCE) codes for enhanced security.",
				category: "preparation",
...
console.log('Authorization URL:', authUrl);`,
			...
			},
			{
				...
				description:
					"Redirect the user to PingOne for authentication and authorization.",
				category: "authorization",
				canSkip: false,
...
window.open(authUrl, '_blank');`,
				execute: async () => {
					...
					console.log(
						" [EnhancedAuthorizationCodeFlow] Popup - Current path:",
						currentPath,
					);
					console.log(
						" [EnhancedAuthorizationCodeFlow] Popup - Return path:",
						returnPath,
					);
					...
					return {
						...
						authUrl,
						instruction:
							"Complete authentication in the popup window, then return here",
					};
				},
...
console.log('Tokens received:', tokens);`,
			...
			},
			{
				...
				description:
					"Validate the received tokens by calling the UserInfo endpoint.",
				category: "validation",
				canSkip: true,
				isOptional: true,
				...
				<CredentialsContent>
					...
					<CredentialActions>
						...
						<ActionButton
							...
							onClick={() => {
								setCredentials({});
								persistentCredentials.removeFlowCredentials(
									"authorization-code",
								);
							}}
						>
