...
import PingOneMfaService, { type MfaDevice, type MfaCredentials } from '../../services/pingOneMfaService';
...
const PingOneMFAFlowV6: React.FC = () => {
	...
	const initiateMfaChallenge = useCallback(async (device: any) => {
		...
		try {
			...
			v4ToastManager.showError(`Failed to send MFA challenge: ${error instanceof Error ? error.message : 'Unknown error'}`);
		} finally {
			setIsLoading(false);
		}
	...
	const completeActivation = useCallback(async () => {
		...
		try {
			...
			setActivationCode('');
			v4ToastManager.showSuccess(`${activatingDevice.type} device activated successfully with real PingOne API!`);
			
		} catch (error) {
			console.error('[P1MFA] Activation failed:', error);
			v4ToastManager.showError(`Device activation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
		} finally {
			setIsLoading(false);
		}
	...
	const loadRealDevices = useCallback(async () => {
		...
		try {
			...
			v4ToastManager.showError(`Failed to load devices: ${error instanceof Error ? error.message : 'Unknown error'}`);
		} finally {
			setIsLoading(false);
		}
	}, [workerToken, credentials]);
	...
	const handleSaveContactInfo = useCallback((deviceId: string) => {
	...
	React.useEffect(() => {
		const timeoutId = setTimeout(() => {
			if (credentials.clientId || credentials.clientSecret || credentials.environmentId || credentials.username) {
		...
		return (
				...
				{devicesNeedingInfo.length > 0 && (
					<InfoBox $variant="warning" style={{ marginBottom: '1rem' }}>
						...
						<InfoContent>
							...
							<InfoText>
								{devicesNeedingInfo.length} device{devicesNeedingInfo.length > 1 ? 's' : ''} need{devicesNeedingInfo.length === 1 ? 's' : ''} contact information. 
								Click the "Edit" button next to each device to add phone numbers, email addresses, or device names.
								<br />
								<strong>Devices needing info:</strong> {devicesNeedingInfo.map(d => `${d.type} Device`).join(', ')}
							</InfoText>
						</InfoContent>
					</InfoBox>
				)}
			...
			<DeviceTableBody>
				{mfaDevices.length > 0 ? (
					mfaDevices.map((device) => {
						...
						return (
							<DeviceTableRow 
								...
								style={{ 
									cursor: stepContext === 'selection' && device.status === 'ACTIVE' ? 'pointer' : 'default'
								}}
							>
								{/* Device Type Column */}
								<DeviceTableCell>
								...
								<DeviceTableCell>
									<DeviceStatusBadge $status={device.status.toLowerCase()}>
										{device.status === 'CHECKING' && <SpinningIcon><FiRefreshCw size={12} /></SpinningIcon>}
										...
										{device.status === 'ACTIVATION_REQUIRED' ? 'NEEDS OTP' : device.status.replace('_', ' ')}
									</DeviceStatusBadge>
								</DeviceTableCell>
					...
					<DeviceTableRow>
						<DeviceTableCell colSpan={5} style={{ textAlign: 'center', padding: '2rem', color: '#6b7280' }}>
							<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem' }}>
								<FiSmartphone size={24} style={{ opacity: 0.5 }} />
								<span>No MFA devices registered yet</span>
								<span style={{ fontSize: '0.875rem', opacity: 0.7 }}>
									{stepContext === 'registration' ? 'Register your first device using the form below' : 'Go back to Step 1 to register devices'}
								</span>
							</div>
						</DeviceTableCell>
					</DeviceTableRow>
		...
		<Container>
			...
			<div style={{ marginBottom: '2rem', padding: '1rem', background: '#f9fafb', borderRadius: '0.5rem', border: '1px solid #e5e7eb' }}>
				<div style={{ display: 'flex', gap: '1rem', marginBottom: '1rem' }}>
					{['Setup', 'Device Mgmt', 'Selection', 'Challenge', 'Exchange', 'Complete'].map((step, index) => (
						<div 
							key={index}
							style={{
								flex: 1,
					...
					<Collapsible.CollapsibleSection>
						...
						<InfoBox $variant="info">
							...
							<InfoContent>
								...
								<InfoText>
									Enter your real PingOne application credentials to obtain an actual worker token from PingOne. 
									This makes real API calls to PingOne's token endpoint and returns a valid access token for MFA device management.
								</InfoText>
							</InfoContent>
						</InfoBox>

						<div style={{ margin: '1rem 0', padding: '1rem', background: '#e0f2fe', borderRadius: '0.5rem', border: '1px solid #0284c7' }}>
							...
							<div style={{ fontSize: '0.75rem', color: '#0c4a6e', lineHeight: '1.5' }}>
								<div><strong>Endpoint:</strong> POST https://auth.pingone.{'{region}'}/v1/environments/{'{environmentId}'}/as/token</div>
								<div><strong>Grant Type:</strong> client_credentials (always for worker tokens)</div>
								<div><strong>Auth Method:</strong> {credentials.authMethod}</div>
								<div><strong>Scope:</strong> p1:read:user p1:update:user p1:create:device p1:read:device p1:update:device p1:delete:device</div>
								<div><strong>Auto-Save:</strong> Credentials and tokens are automatically saved to localStorage</div>
							</div>
						</div>

						<div style={{ margin: '1rem 0', padding: '1rem', background: '#f0f9ff', borderRadius: '0.5rem', border: '1px solid #0ea5e9' }}>
							<h4 style={{ margin: '0 0 0.5rem 0', fontSize: '0.875rem', fontWeight: '600', color: '#0c4a6e' }}>
								🔐 Worker Token Authentication Methods
							</h4>
							<div style={{ fontSize: '0.75rem', color: '#0c4a6e', lineHeight: '1.5' }}>
								<div><strong>Client Secret Basic:</strong> Credentials sent in Authorization header (Base64 encoded) - Most common</div>
								<div><strong>Client Secret Post:</strong> Credentials sent in request body as form parameters</div>
								<div><strong>Private Key JWT:</strong> Client authenticates using signed JWT with private key - Most secure</div>
								<div><strong>Client Secret JWT:</strong> Client authenticates using signed JWT with shared secret</div>
								<div style={{ marginTop: '0.5rem', fontStyle: 'italic' }}><strong>Note:</strong> Worker tokens always use client_credentials grant type</div>
							</div>
						</div>

						{/* Phase 1: Educational Content about Device Statuses */}
						<div style={{ margin: '1.5rem 0', padding: '1rem', background: '#fef3c7', borderRadius: '0.5rem', border: '1px solid #fbbf24' }}>
							...
							<div style={{ fontSize: '0.75rem', color: '#92400e', lineHeight: '1.6' }}>
								<div style={{ marginBottom: '0.5rem' }}><strong>ACTIVATION_REQUIRED:</strong> Device needs OTP activation. The OTP is sent to the phone number or email address specified in the device resource's properties.</div>
								<div style={{ marginBottom: '0.5rem' }}><strong>PENDING_ACTIVATION:</strong> Device registered but awaiting user activation (QR scan, pairing key, etc.)</div>
								<div style={{ marginBottom: '0.5rem' }}><strong>ACTIVE:</strong> Device ready for MFA challenges and authentication</div>
								<div style={{ marginBottom: '0.5rem' }}><strong>BLOCKED:</strong> Device temporarily disabled by administrator</div>
								<div><strong>ERROR:</strong> Device has configuration or connectivity issues</div>
							</div>
						</div>
						...
						<div style={{ margin: '1rem 0', padding: '1rem', background: '#e0f2fe', borderRadius: '0.5rem', border: '1px solid #0284c7' }}>
							...
							<div style={{ fontSize: '0.75rem', color: '#0c4a6e', lineHeight: '1.5' }}>
								<div><strong>Endpoint:</strong> POST /environments/{'{environmentId}'}/users/{'{userId}'}/devices/{'{deviceId}'}</div>
								<div><strong>Content-Type:</strong> application/vnd.pingidentity.device.activate+json</div>
								<div><strong>Purpose:</strong> Activate devices with ACTIVATION_REQUIRED status using valid OTP</div>
								<div><strong>Process:</strong> OTP sent to phone/email → User enters OTP → Device becomes ACTIVE</div>
							</div>
						</div>
						...
						<div style={{ marginBottom: '1rem' }}>
							<ActionButton
								...
								disabled={isLoading || !credentials.clientId || !credentials.clientSecret || !credentials.environmentId || !credentials.authMethod}
								style={{ width: '100%' }}
							>
								{isLoading ? (
									<>
						...
						{apiRequestDetails && (
							<div style={{ margin: '1.5rem 0', padding: '1rem', background: '#f8fafc', borderRadius: '0.5rem', border: '1px solid #e2e8f0' }}>
								<h4 style={{ margin: '0 0 1rem 0', fontSize: '1rem', fontWeight: '600', color: '#1f2937', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
								...
								<div style={{ marginBottom: '1rem' }}>
									<div style={{ fontSize: '0.875rem', marginBottom: '0.5rem' }}>
										<strong>URL:</strong> <code style={{ background: '#f1f5f9', padding: '0.25rem 0.5rem', borderRadius: '0.25rem' }}>{apiRequestDetails.url}</code>
									</div>
									<div style={{ fontSize: '0.875rem', marginBottom: '0.5rem' }}>
										<strong>Method:</strong> <code style={{ background: '#f1f5f9', padding: '0.25rem 0.5rem', borderRadius: '0.25rem' }}>{apiRequestDetails.method}</code>
									</div>
									<div style={{ fontSize: '0.875rem', marginBottom: '0.5rem' }}>
										<strong>Auth Method:</strong> <code style={{ background: '#f1f5f9', padding: '0.25rem 0.5rem', borderRadius: '0.25rem' }}>{apiRequestDetails.authMethod}</code>
									</div>
								</div>
						...
						{apiResponseDetails && (
							<div style={{ margin: '1.5rem 0', padding: '1rem', background: '#f0fdf4', borderRadius: '0.5rem', border: '1px solid #bbf7d0' }}>
								<h4 style={{ margin: '0 0 1rem 0', fontSize: '1rem', fontWeight: '600', color: '#1f2937', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
								...
								<div style={{ marginBottom: '1rem' }}>
									<div style={{ fontSize: '0.875rem', marginBottom: '0.5rem' }}>
										<strong>Status:</strong> <code style={{ background: apiResponseDetails.status === 200 ? '#dcfce7' : '#fee2e2', padding: '0.25rem 0.5rem', borderRadius: '0.25rem' }}>{apiResponseDetails.status} {apiResponseDetails.statusText}</code>
									</div>
									<div style={{ fontSize: '0.875rem', marginBottom: '0.5rem' }}>
										<strong>Timestamp:</strong> <code style={{ background: '#f1f5f9', padding: '0.25rem 0.5rem', borderRadius: '0.25rem' }}>{new Date(apiResponseDetails.timestamp).toLocaleString()}</code>
									</div>
								</div>
						...
						{mfaDevices.length > 0 ? (
							...
							<InfoBox $variant="info">
								...
								<InfoContent>
									...
									<InfoText>
										Register your first MFA device using the form below. Once registered, devices will appear in the table.
									</InfoText>
								</InfoContent>
							</InfoBox>
						)}
					...
					<Collapsible.CollapsibleSection>
						...
						<InfoBox $variant="info">
							...
							<InfoContent>
								...
								<InfoText>
									PingOne supports multiple MFA device types: SMS, Email, Voice, TOTP (Authenticator Apps), 
									FIDO2 (Security Keys), Mobile (PingID App), and Push Notifications. Each device type has 
									specific registration and activation requirements.
								</InfoText>
							</InfoContent>
						</InfoBox>
						...
						<div style={{ margin: '1rem 0', padding: '1rem', background: '#f9fafb', borderRadius: '0.5rem' }}>
							...
							<div style={{ fontSize: '0.75rem', color: '#6b7280', lineHeight: '1.5' }}>
								<div><strong>Device Registration:</strong> POST /environments/{'{environmentId}'}/users/{'{userId}'}/devices</div>
								<div><strong>Device Activation:</strong> POST /environments/{'{environmentId}'}/users/{'{userId}'}/devices/{'{deviceId}'}/oathTokens</div>
								<div><strong>Device Management:</strong> PUT /environments/{'{environmentId}'}/users/{'{userId}'}/devices/{'{deviceId}'}</div>
								<div><strong>Block Device:</strong> PUT /environments/{'{environmentId}'}/users/{'{userId}'}/devices/{'{deviceId}'} (status: BLOCKED)</div>
								<div><strong>Delete Device:</strong> DELETE /environments/{'{environmentId}'}/users/{'{userId}'}/devices/{'{deviceId}'}</div>
								<div><strong>Send MFA Challenge:</strong> POST /environments/{'{environmentId}'}/users/{'{userId}'}/devices/{'{deviceId}'}/authentications</div>
								<div><strong>Verify Challenge:</strong> POST /environments/{'{environmentId}'}/users/{'{userId}'}/devices/{'{deviceId}'}/authentications/{'{authenticationId}'}/check</div>
							</div>
						</div>

						<div style={{ margin: '1rem 0', padding: '1rem', background: '#ecfdf5', borderRadius: '0.5rem', border: '1px solid #10b981' }}>
							...
							<div style={{ fontSize: '0.75rem', color: '#065f46', lineHeight: '1.5' }}>
								<div><strong>Device Ordering:</strong> Use ↑↓ arrows to reorder devices for user preference</div>
								<div><strong>Nicknames:</strong> Click edit icon to add friendly names for easier identification</div>
								<div><strong>Block/Unblock:</strong> Temporarily disable devices without deleting them</div>
								<div><strong>Activation:</strong> Handle ACTIVATION_REQUIRED and PENDING_ACTIVATION statuses</div>
								<div><strong>Testing:</strong> Test active devices to verify MFA functionality</div>
							</div>
						</div>
						...
						{mfaDevices.length > 0 ? (
							<>
								<InfoBox $variant="info">
									...
									<InfoContent>
										...
										<InfoText>
											Devices with "PENDING_ACTIVATION" status need to be activated first using the "Activate" button. 
											Once activated, you can select and test devices for MFA challenges.
										</InfoText>
									</InfoContent>
								</InfoBox>

								<div style={{ margin: '1rem 0', padding: '1rem', background: '#fef3c7', borderRadius: '0.5rem', border: '1px solid #fbbf24' }}>
									...
									<div style={{ fontSize: '0.75rem', color: '#92400e', lineHeight: '1.5' }}>
										<div style={{ marginBottom: '0.25rem' }}><strong>ACTIVATION_REQUIRED:</strong> Device needs OTP activation. The OTP is sent to the phone number or email address specified in the device resource's properties. Use the activation API with content-type application/vnd.pingidentity.device.activate+json.</div>
										<div style={{ marginBottom: '0.25rem' }}><strong>PENDING_ACTIVATION:</strong> Device registered but awaiting user activation (QR scan, pairing key, etc.)</div>
										<div style={{ marginBottom: '0.25rem' }}><strong>BLOCKED:</strong> Device temporarily disabled. Can be unblocked by administrator.</div>
										<div><strong>ACTIVE:</strong> Device ready for MFA challenges and authentication.</div>
									</div>
								</div>
							...
							<InfoBox $variant="warning">
								...
								<InfoContent>
									...
									<InfoText>
										You need to register at least one MFA device before you can test MFA challenges.
										Go back to Step 1 to register a device.
									</InfoText>
								</InfoContent>
							</InfoBox>
						)}
							...
							<InfoBox $variant="success">
								...
								<InfoContent>
									...
									<InfoText>
										...
										{(selectedDevice.type === 'TOTP' || selectedDevice.type === 'PUSH') && 'Authenticator App'}.
					...
					<Collapsible.CollapsibleSection>
						...
						<InfoBox $variant="success">
							...
							<InfoContent>
								...
								<InfoText>
									MFA challenge has been sent to your {selectedDevice?.type.toLowerCase()} device. 
									Enter the verification code below to complete the authentication.
								</InfoText>
							</InfoContent>
						</InfoBox>
			...
			{/* Step Navigation */}
			<div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '2rem', padding: '1rem', background: '#f9fafb', borderRadius: '0.5rem' }}>
				<ActionButton
					$variant="secondary"
				...
				<ActionButton
					...
					onClick={() => {
						if (currentStep === 0) {
							if (!credentials.clientId || !credentials.clientSecret || !credentials.environmentId || !credentials.authMethod) {
				...
				>
					{(() => {
						...
						if (currentStep === 1) return credentials.username ? "Continue to Device Selection" : "Enter Username First";
						if (currentStep === 2) return mfaDevices.length > 0 ? "Continue to MFA Challenge" : "Register Device First";
						return "Next";
					})()}
					<FiArrowRight />
				</ActionButton>
					...
					<ActivationContent onClick={(e) => e.stopPropagation()}>
						<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
							<h3 style={{ margin: 0, fontSize: '1.25rem', fontWeight: '600' }}>
								{activatingDevice.status === 'ACTIVATION_REQUIRED' ? 'Enter OTP Code' : 'Activate'} {activatingDevice.type} Device
							</h3>
							<button 
						...
						{activatingDevice.status === 'ACTIVATION_REQUIRED' && (
							<InfoBox $variant="info">
								...
								<InfoContent>
									...
									<InfoText>
										This device has ACTIVATION_REQUIRED status. An OTP (One-Time Password) has been sent to the 
										{activatingDevice.type === 'SMS' && ` phone number ${activatingDevice.phoneNumber}`}
										{activatingDevice.type === 'EMAIL' && ` email address ${activatingDevice.emailAddress}`}
										{activatingDevice.type === 'VOICE' && ` phone number ${activatingDevice.phoneNumber} via voice call`}
								...
								<InfoBox $variant="info">
									...
									<InfoContent>
										...
										<InfoText>
											...
											or manually enter the secret key, then provide the 6-digit code to activate.
										</InfoText>
									</InfoContent>
								</InfoBox>
							...
							<>
								<InfoBox $variant="info">
									...
									<InfoContent>
										...
										<InfoText>
											Install the PingID mobile app and use the pairing key below to pair your device, 
											then enter the activation code from the app.
										</InfoText>
									</InfoContent>
								</InfoBox>
						...
						<div style={{ margin: '1.5rem 0' }}>
							<label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '500' }}>
								{activatingDevice.status === 'ACTIVATION_REQUIRED' ? 'OTP Code *' : 'Activation Code *'}
							</label>
							<input
								type="text"
								value={activationCode}
								onChange={(e) => setActivationCode(e.target.value)}
								placeholder={activatingDevice.status === 'ACTIVATION_REQUIRED' ? 'Enter OTP from SMS/Email/Voice' : 'Enter 6-digit activation code'}
								style={{
									width: '100%',
									padding: '0.75rem',
									border: '1px solid #d1d5db',
							...
							{activatingDevice.status === 'ACTIVATION_REQUIRED' && (
								<div style={{ fontSize: '0.75rem', color: '#6b7280', marginTop: '0.5rem', textAlign: 'center' }}>
									Check your {activatingDevice.type === 'SMS' ? 'text messages' : activatingDevice.type === 'EMAIL' ? 'email inbox' : 'phone for voice call'} for the OTP
								</div>
							)}
						</div>
