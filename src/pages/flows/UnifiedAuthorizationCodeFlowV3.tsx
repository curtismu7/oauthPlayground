...
const UnifiedAuthorizationCodeFlowV3: React.FC<UnifiedFlowProps> = ({
	flowType,
}) => {
	...
	const _performanceMonitor = usePerformanceMonitor(
		`UnifiedAuthorizationCodeFlowV3-${flowType}`,
	);
				...
				if (allCredentials.environmentId && allCredentials.clientId) {
					...
					if (pingoneConfig) {
						try {
							...
							console.warn(
								`! [${flowType.toUpperCase()}-V3] Failed to parse pingone_config:`,
								e,
							);
						}
					}
	...
	const [hasUnsavedCredentialChanges, setHasUnsavedCredentialChanges] =
		useState(false);
	const [originalCredentials, setOriginalCredentials] = useState<
		typeof credentials | null
	>(null);
				...
				console.log(
					`💾 [${flowType.toUpperCase()}-V3] Saved flow configuration:`,
					{
						...
						nonce: newConfig.nonce
							? `${newConfig.nonce.substring(0, 10)}...`
							: "none",
						state: newConfig.state
							? `${newConfig.state.substring(0, 10)}...`
							: "none",
	...
	useEffect(() => {
					...
					console.log(
						`🔍 [${flowType.toUpperCase()}-V3] setCredentials called - previous state:`,
						{
							...
							clientId: previous.clientId
								? `${previous.clientId.substring(0, 8)}...`
								: "none",
		...
		const handleStorageChange = (e: StorageEvent) => {
			if (
				e.key &&
				(e.key.includes("pingone") ||
					e.key.includes("credentials") ||
	...
	const exchangeRefreshToken = useCallback(async () => {
		...
		try {
			...
			if (response.ok && result.access_token) {
				...
				sessionStorage.setItem(
					`${flowType}_v3_new_access_token`,
					result.access_token,
				);
				if (result.refresh_token) {
					sessionStorage.setItem(
						`${flowType}_v3_new_refresh_token`,
						result.refresh_token,
					);
				}
				if (result.id_token && flowType === "oidc") {
					sessionStorage.setItem(
						`${flowType}_v3_new_id_token`,
						result.id_token,
					);
				}
			...
			if (authCode) {
				...
				if (stepManager.isInitialized) {
					...
					const skipCredentialsStep =
						localStorage.getItem(`${flowType}_v3_skip_credentials_step`) ===
						"true";
	...
	const handlePopupAuthorizationWithModal = useCallback(() => {
		...
		const globalFlowConfig = JSON.parse(
			localStorage.getItem(globalFlowConfigKey) || "{}",
		);
	...
	const exchangeTokens = useCallback(async () => {
		...
		if (!authCode || !credentials.environmentId || !credentials.clientId) {
			throw new Error(
				"Authorization code, environment ID, and client ID are required",
			);
		}
		...
		try {
			...
			const storedRedirectUri =
				sessionStorage.getItem(`${flowType}_v3_redirect_uri`) ||
				credentials.redirectUri;
			...
			console.log(
				`🔍 [${flowType.toUpperCase()}-V3] COMPREHENSIVE REDIRECT URI DEBUG FOR TOKEN EXCHANGE:`,
				{
					...
					allRedirectKeys: Object.keys(sessionStorage).filter((key) =>
						key.includes("redirect"),
					),
					sessionValues: {
						[`${flowType}_v3_redirect_uri`]: sessionStorage.getItem(
							`${flowType}_v3_redirect_uri`,
						),
						[`${flowType}_v3_state`]: sessionStorage.getItem(
							`${flowType}_v3_state`,
						),
						[`${flowType}_v3_nonce`]: sessionStorage.getItem(
							`${flowType}_v3_nonce`,
						),
			...
			const authMethod: ClientAuthMethod =
				...
				(credentials.clientSecret ? "client_secret_post" : "none");
			...
			console.log(
				`🔍 [${flowType.toUpperCase()}-V3] Simplified token exchange request:`,
				{
					...
					redirect_uri_match:
						finalRequestBody.redirect_uri === credentials.redirectUri
							? "MATCH"
							: "MISMATCH",
			...
			if (
				!finalRequestBody.client_id ||
				finalRequestBody.client_id.trim() === ""
			) {
				throw new Error("Client ID is required for token exchange");
			}
			if (
				!finalRequestBody.environment_id ||
				finalRequestBody.environment_id.trim() === ""
			) {
			...
			if (
				!finalRequestBody.code_verifier ||
				finalRequestBody.code_verifier.trim() === ""
			) {
...
Original Error: ${errorData.error_description || errorData.error}
          `.trim();
			...
			sessionStorage.setItem(
				`${flowType}_v3_access_token`,
				tokenData.access_token || "",
			);
			sessionStorage.setItem(
				`${flowType}_v3_refresh_token`,
				tokenData.refresh_token || "",
			);
			sessionStorage.setItem(
				`${flowType}_v3_id_token`,
				tokenData.id_token || "",
			);
			...
			if (flowType === "oidc" && tokenData.id_token) {
				try {
					...
					console.warn(
						`! [${flowType.toUpperCase()}-V3] ID token validation error:`,
						idTokenError,
					);
				}
			}
			...
			if (flowType === "oidc" && tokenData.access_token) {
				try {
					...
					console.warn(
						`! [${flowType.toUpperCase()}-V3] User info fetch failed:`,
						userInfoError,
					);
				}
			}
		...
		if (flowType === "oidc") {
			...
			baseSteps.push(
				...
				// Add refresh token step after OIDC token validation
				{
					...
					completed:
						hasStepResult("refresh-token-exchange") ||
						Boolean(newTokensFromRefresh),
					isFinalStep: true,
				},
			);
		} else {
						...
						<div>
							<InfoBox type="info">
								...
								<div>
									...
									<br />
									Your authorization code has been successfully exchanged for
									OAuth tokens. In OAuth 2.0 (unlike OIDC), you receive only{" "}
									<strong>Access Tokens</strong> and{" "}
								...
								<div style={{ margin: "1.5rem 0" }}>
									...
									{tokens.refresh_token && (
										...
										>
											...
											>
												<strong>Refresh Token Purpose:</strong> Used to obtain
												new access tokens when they expire, without requiring
												user re-authentication. Long-lived (typically 30 days)
												and should be stored securely.
												<br />
												<br />
												<strong>How to use:</strong> Send a POST request to the
												token endpoint with grant_type=refresh_token to get a
												new access token.
											</p>
											<code
												style={{
