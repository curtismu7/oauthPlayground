...
const AuthorizationCodeFlow: React.FC = () => {
	...
	console.log(
		"AuthorizationCodeFlow: showCredentials initial state:",
		showCredentials,
	);
	...
	const [hasAttemptedAuthorization, setHasAttemptedAuthorization] =
		useState<boolean>(false);
	...
	useEffect(() => {
		...
		try {
			...
			console.warn(
				" [AuthorizationCodeFlow] Could not clear persisted history:",
				error,
			);
		}
		...
		const loadCredentials = () => {
			try {
				...
				console.log(
					" [AuthorizationCodeFlow] Loading credentials:",
					allCredentials,
				);
				...
				setCredentials((prev) => ({
					...
					scopes: Array.isArray(allCredentials.scopes)
						...
						: allCredentials.scopes || prev.scopes,
					authorizationEndpoint:
						allCredentials.authEndpoint || prev.authorizationEndpoint,
					tokenEndpoint: allCredentials.tokenEndpoint || prev.tokenEndpoint,
					userInfoEndpoint:
						allCredentials.userInfoEndpoint || prev.userInfoEndpoint,
					clientAuthMethod:
						allCredentials.tokenAuthMethod || prev.clientAuthMethod,
				}));

				logger.info(
					"Loaded credentials from credential manager",
					JSON.stringify({
				...
				logger.warn(
					"Failed to load credentials from credential manager",
					String(error),
				);
			}
		};
		...
		loadCredentials();
		console.log(
			" [AuthorizationCodeFlow] Cleared all flow states and loaded credentials on mount",
		);
	}, []);
		...
		const handleCredentialChange = () => {
			const loadCredentials = () => {
				try {
					const allCredentials = credentialManager.getAllCredentials();
					console.log(
						" [AuthorizationCodeFlow] Reloading credentials after change:",
						allCredentials,
					);
					...
					logger.warn(
						"Failed to reload credentials from credential manager",
						String(error),
					);
				}
			};
	...
	const saveCredentials = useCallback(async () => {
		try {
			...
			console.log(
				" [AuthorizationCodeFlow] Saving permanent credentials:",
				permanentCreds,
			);

			// Save permanent credentials (Environment ID, Client ID, etc.)
			const permanentSuccess =
				credentialManager.savePermanentCredentials(permanentCreds);
	...
	const loadCredentials = useCallback(() => {
		try {
			...
			console.error(
				" [AuthorizationCodeFlow] Failed to load credentials:",
				error,
			);
	...
	const generatePKCECodes = useCallback(async () => {
		try {
			...
			console.log(" [AuthorizationCodeFlow] PKCE codes generated:", {
			...
			console.log(
				" [AuthorizationCodeFlow] Stored code_verifier in sessionStorage",
			);
			...
			console.error(
				" [AuthorizationCodeFlow] Failed to generate PKCE codes:",
				error,
			);
	...
	const generateAuthUrl = useCallback(
		(pkceChallenge?: string) => {
			...
			console.log(
				" [AuthorizationCodeFlow] Generating authorization URL...",
				{ credentials, pkceCodes, challengeToUse },
			);
			...
			console.log(
				" [AuthorizationCodeFlow] Stored state in sessionStorage:",
				generatedState,
			);
			...
			console.log(
				" [AuthorizationCodeFlow] Authorization URL generated:",
				url,
			);
			setAuthUrl(url);
	...
	const handleAuthorization = useCallback(() => {
		console.log(" [AuthorizationCodeFlow] handleAuthorization called", {
			testingMethod,
			authUrl,
		});
		if (testingMethod === "popup") {
			...
			console.log(
				" [AuthorizationCodeFlow] Opening popup with URL:",
				authUrl,
			);
			...
			if (popup) {
				...
				const messageHandler = (event: MessageEvent) => {
					console.log(
						" [AuthorizationCodeFlow] Message received from popup:",
						event.data,
						"origin:",
						event.origin,
					);
					...
					if (event.data.type === "oauth-callback") {
						...
						console.log(" [AuthorizationCodeFlow] OAuth callback received:", {
							callbackCode,
							callbackState,
							error,
							expectedState: state,
			...
			// Full redirect
			console.log(" [AuthorizationCodeFlow] Full redirect triggered", {
				authUrl,
				testingMethod,
			});
			...
			if (!authUrl) {
				console.error(
					" [AuthorizationCodeFlow] No authUrl available for redirect",
				);
	...
	const exchangeCodeForTokens = useCallback(async () => {
		try {
			...
			console.log(
				" [AuthorizationCodeFlow] Using client authentication method:",
	...
	const handleRedirectModalProceed = useCallback(() => {
		if (testingMethod === "popup") {
			...
			console.log(
				" [AuthorizationCodeFlow] Popup - Current path:",
				currentPath,
			);
			console.log(
				" [AuthorizationCodeFlow] Popup - Correct path:",
				correctPath,
			);
			console.log(
				" [AuthorizationCodeFlow] Popup - Return path:",
				returnPath,
			);
			...
			console.log(
				" [AuthorizationCodeFlow] Stored flow context for popup callback:",
				flowContext,
			);
			console.log(
				" [AuthorizationCodeFlow] Popup - Flow context stored in sessionStorage:",
			...
			console.log(
				" [AuthorizationCodeFlow] Stored flow context for callback:",
				flowContext,
			);
			console.log(
				" [AuthorizationCodeFlow] Flow context stored in sessionStorage:",
				sessionStorage.getItem("flowContext"),
			);
	...
	const steps: EnhancedFlowStep[] = [
				...
				<div>
					...
					<FormField>
						...
						<FormInput
							...
							onChange={(e) =>
								setCredentials((prev) => ({ ...prev, scopes: e.target.value }))
							}
					...
					<FormField>
						...
						<FormSelect
							...
							onChange={(e) =>
								setCredentials((prev) => ({
									...prev,
									clientAuthMethod: e.target.value as ClientAuthMethod,
								}))
							}
						>
					...
					<InfoBox type="info">
						...
						<div>
							...
							locally in your browser and are not transmitted to any external
							servers.
						</div>
					</InfoBox>
					...
					{credentialsSaved && (
						<InfoBox type="success">
							...
							<div>
								...
								<br />
								Your OAuth credentials have been saved and are ready to use. You
		...
		},
		{
					...
					{isAuthorizing && (
						<InfoBox type="info">
							...
							<div>
								...
								<br />
								{testingMethod === "popup"
									? "Popup window opened. Please complete authentication in the popup."
					...
					>
						<button
							onClick={() => {
								...
								console.log(
									" [AuthorizationCodeFlow] Opening redirect modal with URL:",
									authUrl,
								);
							...
							style={{
								padding: "0.75rem 1.5rem",
								backgroundColor:
									testingMethod === "popup" ? "#3b82f6" : "#10b981",
								color: "white",
								border: "none",
								borderRadius: "0.5rem",
								fontSize: "0.875rem",
			...
			execute: async () => {
				...
				console.log(
					" [AuthorizationCodeFlow] Build Authorization URL step executing...",
				);
				console.log(
					" [AuthorizationCodeFlow] Current credentials:",
					credentials,
				);
				console.log(
					" [AuthorizationCodeFlow] Current PKCE codes:",
					pkceCodes,
				);
					...
					if (popup) {
						...
						const messageHandler = (event: MessageEvent) => {
							...
							if (event.data.type === "oauth-callback") {
								...
								console.log(
									" [AuthorizationCodeFlow] OAuth callback received:",
									{ callbackCode, callbackState, error, expectedState: state },
								);
								if (error) {
									logger.error("Authorization error received", error);
									setIsAuthorizing(false);
			...
			canExecute: Boolean(
				authUrl && authUrl.trim().length > 0 && hasAttemptedAuthorization,
			),
		},
		{
			id: "handle-callback",
			...
			execute: async () => {
				if (!authCode) {
					return {
						success: false,
						error:
							"Authorization code is required. Please complete the redirect step first.",
					};
				}
		...
		},
		{
			...
			content: (
				<div>
					<CodeBlock>
						...
						<CodeComment>
							// Response will contain user profile information
						</CodeComment>
					</CodeBlock>
					...
					{isGettingUserInfo && (
						<InfoBox type="info">
							...
							<div>
								...
								<br />
								Making request to UserInfo endpoint to get authenticated user
								profile...
							</div>
						</InfoBox>
					)}
			...
			execute: async () => {
				if (!tokens?.access_token) {
					return {
						success: false,
						error:
							"Access token is required. Please complete the token exchange step first.",
					};
				}
				await getUserInfo();
				return { success: true };
			...
			canExecute: Boolean(
				tokens?.access_token && tokens.access_token.trim().length > 0,
			),
		},
	];
				...
				<CollapsibleContent $isOpen={showCredentials}>
					<div>
						...
						<ButtonGroup>
							...
							<Button onClick={loadCredentials} $variant="secondary">
								<FiRefreshCw />
								Load Saved
							</Button>
						</ButtonGroup>
