...
export const useFlowVariantSwitching = (
		...
		async (newVariant: FlowVariant) => {
			...
			try {
				...
				const reloadedCredentials =
					await FlowCredentialService.loadSharedCredentials(credentialKey);
	...
	const currentVariantConfig = useMemo(
		() => getVariantConfig(flowVariant),
		[flowVariant, getVariantConfig]
	);

	// Validate variant compatibility
	const validateVariantCompatibility = useCallback(
		(variant: FlowVariant, credentials: FlowCredentials) => {
	...
	const getVariantResponseType = useCallback(
		(variant: FlowVariant) => {
	...
	const getVariantScopes = useCallback(
		(variant: FlowVariant) => {
			const config = getVariantConfig(variant);
			return {
	...
	const autoSwitchVariant = useCallback(
	...
	useEffect(() => {
		const storedVariant = sessionStorage.getItem(
			'oauth-authorization-code-v7-variant'
		) as FlowVariant;
