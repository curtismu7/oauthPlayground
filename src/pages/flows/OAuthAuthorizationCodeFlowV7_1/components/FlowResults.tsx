...
const TokenCard = styled.div<{ $type: 'access' | 'id' | 'refresh' }>`
  background: ${(props) => {
		switch (props.$type) {
			case 'access':
				return UI_CONSTANTS.STATUS.SUCCESS_BACKGROUND;
			case 'id':
				return UI_CONSTANTS.STATUS.INFO_BACKGROUND;
			case 'refresh':
				return UI_CONSTANTS.COLORS.GRAY_50;
			default:
  ...
  border: 1px solid ${(props) => {
		switch (props.$type) {
			case 'access':
				return UI_CONSTANTS.STATUS.SUCCESS_BORDER;
			case 'id':
				return UI_CONSTANTS.STATUS.INFO_BORDER;
			case 'refresh':
...
const TokenTitle = styled.div<{ $type: 'access' | 'id' | 'refresh' }>`
  ...
  color: ${(props) => {
		switch (props.$type) {
			...
			case 'id':
				return UI_CONSTANTS.STATUS.INFO_COLOR;
			case 'refresh':
