...
export class FlowErrorBoundary extends Component<Props, State> {
  ...
  private handleRetry = () => {
    ...
    if (newRetryCount < 3) {
      const delay = Math.pow(2, newRetryCount) * 1000; // 1s, 2s, 4s
    ...
    if (this.state.hasError) {
      ...
      return (
        <ErrorContainer>
          ...
          <ErrorTitle>
            Something went wrong in {flowName}
          </ErrorTitle>
          
          <ErrorMessage>
            An unexpected error occurred while processing the OAuth flow. 
            This might be due to a network issue, invalid configuration, or a temporary problem.
          </ErrorMessage>
