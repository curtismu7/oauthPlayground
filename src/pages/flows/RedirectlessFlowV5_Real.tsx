...
// Real OAuth implementation with educational notes about response_mode=pi.flow
...
const STEP_METADATA = [
	{ title: 'Step 0: Introduction & Setup', subtitle: 'Understand the Redirectless Flow' },
	{ title: 'Step 1: PKCE Parameters', subtitle: 'Generate secure verifier and challenge' },
	{
		title: 'Step 2: Authorization Request',
		subtitle: 'Build and launch the PingOne authorization URL with pi.flow',
	},
	...
	{ title: 'Step 9: Security Features', subtitle: 'Demonstrate advanced security implementations' },
] as const;
...
type IntroSectionKey =
	...
	| 'flowTokensOverview'
	| 'flowTokensDetails' // Step 4 (Flow Tokens)
	| 'tokenExchangeOverview'
	| 'tokenExchangeDetails' // Step 5 (Standard Token Exchange)
	| 'userInfoOverview'
	| 'userInfoDetails' // Step 6
	| 'introspectionOverview'
	| 'introspectionDetails' // Step 7
	| 'completionOverview'
	| 'completionDetails'; // Step 8
...
const RedirectlessFlowV5Real: React.FC = () => {
	...
	const authUrlWithPiFlow = useMemo(() => {
		...
		return controller.authUrl.includes('?')
	...
	const handleGenerateAuthUrl = useCallback(async () => {
		...
		try {
			...
			v4ToastManager.showSuccess('Authorization URL generated (will add response_mode=pi.flow)!');
		} catch (error) {
			console.error('[RedirectlessFlowV5Real] Failed to generate authorization URL:', error);
			v4ToastManager.showError(
	...
	const handleAuthenticate = useCallback(async () => {
		if (!flowObject) {
			v4ToastManager.showError('No flow object available. Complete Step 2 first.');
			return;
		}
		...
		console.log('🔐 [RedirectlessFlowV5Real] Submitting credentials to:', authUrl);
		console.log('🔐 [RedirectlessFlowV5Real] Flow object:', flowObject);

		try {
			// Build the payload - PingOne DaVinci flows expect this format
			const payload = {
				username: authUsername,
				password: authPassword,
			};

			console.log('🔐 [RedirectlessFlowV5Real] Payload:', payload);
			...
			console.log('✅ [RedirectlessFlowV5Real] Authentication response:', result);

			// Check if we got tokens
			if (result.access_token || result._embedded?.tokens) {
				const tokens = result.access_token ? result : result._embedded.tokens;
				setFlowTokens(tokens);
				v4ToastManager.showSuccess('Authentication successful! Tokens received.');
				console.log('🎉 [RedirectlessFlowV5Real] Tokens received:', tokens);
			} else if (result.status === 'COMPLETED' && result._embedded?.tokens) {
				setFlowTokens(result._embedded.tokens);
				v4ToastManager.showSuccess('Flow completed! Tokens received.');
			} else {
	...
	}, [flowObject, authUsername, authPassword]);
	...
	const handleOpenAuthUrl = useCallback(async () => {
		...
		try {
			...
			if (flowData.code) {
				...
				controller.setAuthCodeManually(flowData.code);
				v4ToastManager.showSuccess('Authorization code received via API (no redirect)!');
			} else if (flowData.access_token) {
				// Got tokens directly
				console.log('🎉 [RedirectlessFlowV5Real] Received tokens directly!');
				setFlowTokens(flowData);
				v4ToastManager.showSuccess('Tokens received directly via API (no redirect)!');
			} else if (flowData._embedded?.tokens) {
				// Tokens embedded in flow response
				console.log('🎉 [RedirectlessFlowV5Real] Tokens found in flow object!');
				setFlowTokens(flowData._embedded.tokens);
				v4ToastManager.showSuccess('Tokens received from flow object!');
			} else if (flowData.id || flowData.flowId || flowData._embedded) {
				// Got a flow object - process authentication steps
				console.log('📋 [RedirectlessFlowV5Real] Received flow object:', {
					flowId: flowData.id || flowData.flowId,
				...
				console.log('📋 [RedirectlessFlowV5Real] Full flow object structure:', {
					...
					hasEmbedded: !!flowData._embedded,
					embeddedKeys: flowData._embedded ? Object.keys(flowData._embedded) : [],
				...
				} else if (flowData._links) {
					// Flow object has links but no runner - might need to follow links
					console.log('🔗 [RedirectlessFlowV5Real] Flow has links:', flowData._links);
					v4ToastManager.showSuccess(
						`Flow object received with links! Check console to see next steps.`
					);
				} else {
					v4ToastManager.showWarning(
						`Flow object received (Status: ${currentStep}). Check console for full structure.`
					);
				}
			} else {
			...
			console.log('💡 [RedirectlessFlowV5Real] To use true pi.flow:', {
				...
				requirement2: 'Application must support response_mode=pi.flow',
				requirement3: 'May require custom authentication handling',
				suggestion: 'Use the "Redirectless Flow (Educational)" version to learn about pi.flow',
			});
		}
	}, [controller, authUrlWithPiFlow]);
	...
	const isStepValid = useCallback(
		(stepIndex: number): boolean => {
			switch (stepIndex) {
				...
				case 3: // Step 3: Flow Object Response
					return !!(controller.authCode || localAuthCode || flowObject);
				case 4: // Step 4: Tokens Received (from flow)
					return !!flowTokens;
				case 5: // Step 5: Token Exchange (Standard OAuth)
	...
	const getStepRequirements = useCallback((stepIndex: number): string[] => {
		switch (stepIndex) {
			...
			case 3: // Step 3: Authorization Response
				return ['Receive authorization code or flow object from PingOne'];
									...
									<div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
										<div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
											<HighlightedActionButton onClick={handleOpenAuthUrl} $priority="success">
												<FiZap /> Call PingOne API (pi.flow)
											...
											<HighlightedActionButton
												...
												$priority="primary"
											>
												<FiExternalLink /> Use Standard Redirect
											</HighlightedActionButton>
										</div>
										<InfoBox $variant="info">
											<FiInfo size={16} />
...
if (flowData._links?.['password.check']) {
  // Password authentication required
  const authUrl = flowData._links['password.check'].href;
  
  // Display username/password form
  showAuthenticationForm(authUrl, flowId);
}

// Function to submit credentials
async function submitCredentials(authUrl, username, password) {
  const response = await fetch(authUrl, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
  ...
  if (result.status === 'COMPLETED') {
    // Flow completed - extract tokens
    const tokens = result._embedded?.tokens;
    return tokens;
  } else {
    ...
    return result;
												...
												>
													...
													>
														...
														<button
															type="submit"
															disabled={isAuthenticating || !authUsername || !authPassword}
															style={{
																width: '100%',
																padding: '0.75rem',
															...
															{(() => {
																...
																if (authUrl) {
																	try {
																		...
																		return `Will submit to: ${authUrl}`;
																	}
																}
																return 'No authentication URL found in flow object';
															})()}
														</p>
													</form>
			...
			case 4:
				// Step 4: Tokens Received from Flow
				return (
					<>
						{flowTokens ? (
							<>
								<CollapsibleSection>
									<CollapsibleHeaderButton
										onClick={() => toggleSection('flowTokensOverview')}
										aria-expanded={!collapsedSections.flowTokensOverview}
									>
										<CollapsibleTitle>
											<FiCheckCircle /> Tokens Received from Flow
										</CollapsibleTitle>
										<CollapsibleToggleIcon $collapsed={collapsedSections.flowTokensOverview}>
											<FiChevronDown />
										</CollapsibleToggleIcon>
									</CollapsibleHeaderButton>
									{!collapsedSections.flowTokensOverview && (
										<CollapsibleContent>
											<InfoBox $variant="success">
												<FiCheckCircle size={20} />
												<div>
								...
								<CollapsibleSection>
									<CollapsibleHeaderButton
										onClick={() => toggleSection('flowTokensDetails')}
										aria-expanded={!collapsedSections.flowTokensDetails}
									>
										<CollapsibleTitle>
											<FiKey /> Token Details
										</CollapsibleTitle>
										<CollapsibleToggleIcon $collapsed={collapsedSections.flowTokensDetails}>
											<FiChevronDown />
										</CollapsibleToggleIcon>
									</CollapsibleHeaderButton>
									{!collapsedSections.flowTokensDetails && (
										<CollapsibleContent>
											<ResultsSection>
												<ResultsHeading>
							...
							<>
								<InfoBox $variant="info">
									...
									<div>
										...
										<ol style={{ marginTop: '0.75rem', marginBottom: 0, paddingLeft: '1.5rem' }}>
											<li>Parse the flow object (see Step 3)</li>
											<li>Display authentication UI (username/password form)</li>
											<li>Submit credentials to the flow URL</li>
											<li>Receive tokens in the response</li>
										</ol>
									</div>
								</InfoBox>
								...
								>
									...
									>
										...
										<li>Eventually: You receive tokens and store them</li>
									</ol>
								</div>
							</>
						) : (
			...
			case 5:
				// Step 5: Token Exchange (Standard OAuth)
				return (
					<>
						<CollapsibleSection>
							<CollapsibleHeaderButton
							...
							>
								<CollapsibleTitle>
									<FiKey /> Token Exchange Overview (Standard OAuth)
								</CollapsibleTitle>
		...
		<Container>
			<ContentWrapper>
				...
				<MainCard>
					<StepHeader>
						<StepHeaderLeft>
							<VersionBadge>Redirectless Flow · V5 API</VersionBadge>
