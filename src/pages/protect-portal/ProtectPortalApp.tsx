...
const RiskBadge = styled.div<{ $level: 'LOW' | 'MEDIUM' | 'HIGH' }>`
	...
	font-weight: 700;
	background: ${(p) =>
		p.$level === 'LOW' ? FX.successLight : p.$level === 'MEDIUM' ? '#FFF8E1' : FX.errorLight};
	color: ${(p) => (p.$level === 'LOW' ? FX.success : p.$level === 'MEDIUM' ? '#B45309' : FX.error)};
	border: 1px solid
		${(p) => (p.$level === 'LOW' ? '#6EBF8B' : p.$level === 'MEDIUM' ? '#FCD34D' : '#F5A8A3')};
`;
...
// ─── Types ────────────────────────────────────────────────────────────────────
type ModalStep = null | 'login' | 'evaluating' | 'mfa' | 'success' | 'blocked' | 'error';
		...
		<Page>
			...
			<Hero>
				...
				<HeroSub>
					...
					logistics — protected by risk-based authentication.
				</HeroSub>
