...
const Login = () => {
			...
			try {
				...
				if (sharedCreds.clientId && finalEnvId) {
					...
					setCredentials((prev) => ({
						...
						tokenAuthMethod: (sharedCreds.clientAuthMethod as string) || 'client_secret_basic',
					}));
			...
			try {
				...
				if (sharedCredsSync.clientId && finalEnvId) {
					...
					setCredentials((prev) => ({
						...prev,
						environmentId: finalEnvId,
						clientId: sharedCredsSync.clientId || '',
						clientSecret: sharedCredsSync.clientSecret || '',
						tokenAuthMethod: (sharedCredsSync.clientAuthMethod as string) || 'client_secret_basic',
					}));
					setHasExistingCredentials(true);
					return;
				}
	...
	// Load saved credentials on component mount (backup check)
	useEffect(() => {
		...
		if (sharedCreds.clientId && finalEnvId && !credentials.clientId) {
			console.log(' [Login] Found credentials from V8 storage (backup check):', {
				...
				clientIdPrefix: `${sharedCreds.clientId?.substring(0, 8)}...`,
			});
	...
	const handleCredentialSave = async () => {
		...
		try {
			...
			await SharedCredentialsServiceV8.saveSharedCredentials({
				...
				clientAuthMethod: (credentials.tokenAuthMethod || 'client_secret_basic') as
					| 'client_secret_basic'
					| 'client_secret_post'
					| 'client_secret_jwt'
					| 'private_key_jwt',
			...
			setSaveStatus({
				...
				message:
					'Your login credentials have been saved successfully using V8 storage service. These credentials are now available across all OAuth flows.',
			});
	...
	return (
		...
		>
			...
			>
				...
				>
					Configure your PingOne environment to start using the OAuth Playground
				</p>
				<div
					style={{
						fontSize: '13px',
