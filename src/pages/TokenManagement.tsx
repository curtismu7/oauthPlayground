...
const SecurityScoreCard = styled.div<{ $score: number }>`
  ...
  background: ${({ $score }) => {
		...
		if ($score >= 60)
			return "linear-gradient(135deg, #fefce8 0%, #fef3c7 100%)";
		if ($score >= 40)
...
const TokenManagement = () => {
	...
	const isOAuthFlow =
		flowSourceState.includes("oauth") && !flowSourceState.includes("oidc");
	const isOIDCFlow =
	...
	const [tokenSource, setTokenSource] = useState<Record<
		string,
		unknown
	> | null>(null);
	const [tokenHistory, setTokenHistory] = useState<TokenHistoryEntry[]>([]);
	const [activeTab, setActiveTab] = useState<"analysis" | "diagnosis">(
		"analysis",
	);
	...
	const [historyEntryToRemove, setHistoryEntryToRemove] = useState<
		string | null
	>(null);
		...
		const loadCurrentToken = () => {
			try {
				...
				if (tokenToLoad) {
					...
					setTokenSource({
						source:
							currentTokens === tokens ? "Current Session" : "Stored Tokens",
	...
	const handleDecodeClick = () => {
		...
		if (!tokenString || tokenString.trim() === "") {
			...
			setMessage({
				...
				message:
					"No token available to decode. Please load a token first or enter one manually.",
			});
			return;
		}
	...
	const handleIntrospectToken = async () => {
		...
		try {
			...
			if (!allCredentials.environmentId) {
				throw new Error(
					"Environment ID not available for introspection. Please configure your credentials first.",
				);
			}
			...
			if (!allCredentials.clientId) {
				throw new Error(
					"Client ID not available for introspection. Please configure your credentials first.",
				);
			}
	...
	const confirmRemoveHistoryEntry = () => {
		if (historyEntryToRemove) {
			...
			setTokenHistory((prev) =>
				prev.filter((entry) => entry.id !== historyEntryToRemove),
			);
			setMessage({
				type: "success",
				title: "Entry Removed",
		...
		if (tokens?.access_token) {
			...
			if (!tokenAlreadyInHistory) {
				...
				const currentFlowSource =
					sessionStorage.getItem("flow_source") || "unknown";
	...
	const renderIssueKey = useCallback(
		(prefix: string, index: number, text: string) =>
			`${prefix}-${index}-${text.length}`,
		[],
	);
			...
			<TokenSection>
				...
				<CardBody>
					<TokenSurface
						...
						jsonContent={
							tokenString?.trim().startsWith("{") ? tokenString : undefined
						}
					...
					{isOAuthFlow && (
						...
						>
							...
							<div style={{ fontSize: "0.875rem", marginTop: "0.5rem" }}>
								ID Tokens are not available in pure OAuth 2.0 flows. ID Tokens
								are an OpenID Connect (OIDC) feature. Only Access and Refresh
								tokens are available for analysis.
							</div>
						</div>
					)}
								...
								onClick={() => {
									...
									if (storedTokens?.access_token) {
										...
										setMessage({
											...
											message:
												"No tokens found in secure storage. Complete an OAuth flow first.",
										});
									}
								}}
								style={{
								...
								onClick={() => {
									...
									if (tokens?.access_token) {
										...
										if (storedTokens?.access_token) {
											...
											setTimeout(
												() => decodeJWT(storedTokens.access_token),
												100,
											);
											...
											setMessage({
												...
												title: "No Token Available",
												message:
													"No token available from Dashboard Login. Please complete the OAuth flow first.",
											});
										}
									}
								}}
							...
							<ActionButton
								...
								onClick={() => {
									...
									setTokenSource({
										...
										description:
											"Intentionally flawed token for security demonstration",
										timestamp: new Date().toLocaleString(),
									});
									console.log(
					...
					<CardBody>
						...
						>
							...
							>
								<div
									style={{
										background: introspectionResults.active
											? "#16a34a"
											: "#dc2626",
										borderRadius: "50%",
										width: "2.5rem",
							...
							>
								{renderIntrospectionItem(
									"Client ID",
									introspectionResults.client_id || introspectionResults.sub,
								)}
			...
			<TokenSection>
						...
						<div style={{ display: "flex", gap: "0.5rem" }}>
							<ActionButton
								...
								onClick={() => {
									...
									showGlobalSuccess(
										`Token history refreshed! Found ${count} entries.`,
									);
								}}
				...
				<CardBody>
					...
					>
						...
						>
							<div>
								<strong>Token History Status:</strong> {tokenHistory.length}{" "}
								entries stored
							</div>
							<div>
				...
				<CardBody>
							...
							<div>
								...
								<div style={{ marginBottom: "1.5rem" }}>
									<RefreshStatus $status={getTokenExpirationStatus().status}>
										{getTokenExpirationStatus().status === "valid" && (
											<FiCheckCircle />
										)}
										{getTokenExpirationStatus().status === "expiring" && (
											<FiClock />
										)}
										{getTokenExpirationStatus().status === "expired" && (
											<FiXCircle />
										)}
										{getTokenExpirationStatus().status === "unknown" && (
											<FiInfo />
										)}
										{getTokenExpirationStatus().status === "valid" &&
											"Token is valid"}
										...
										{getTokenExpirationStatus().timeRemaining > 0 && (
											...
											>
												(
												{Math.floor(
													getTokenExpirationStatus().timeRemaining / 1000 / 60,
												)}{" "}
												minutes remaining)
											</span>
										)}
					...
					<TabContent $active={activeTab === "diagnosis"}>
						<div>
							...
							>
								Paste an error message below to get automated diagnosis and
								suggested fixes.
							</p>
