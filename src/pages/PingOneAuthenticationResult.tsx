...
const PingOneAuthenticationResult: React.FC = () => {
	...
	useEffect(() => {
		try {
			...
			if (savedFlowLog) {
				try {
					setFlowLog(JSON.parse(savedFlowLog));
				} catch {
					setFlowLog(null);
				}
			}
	...
	if (!result) {
		return (
			<Page>
				...
				<Subtitle>
					The portal is quiet. Run a redirect or redirectless login to populate this space with
			...
			<StepContainer>
				<StepHeader>
					<StepHeaderLeft>
						...
						<StepHeaderSubtitle>
							Latest {result.mode === 'redirectless' ? 'redirectless' : 'redirect'} run summary and
							tokens
						</StepHeaderSubtitle>
					</StepHeaderLeft>
					<StepHeaderRight>
					...
					<Layout>
						<TokenCard>
							...
							{Object.keys(result.tokens).length === 0 ? (
								<EmptyTokenNotice>
									No tokens were generated for the selected response type. Try choosing a response
									type that includes
