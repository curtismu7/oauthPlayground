// src/components/password-reset/tabs/AdminSetTab.tsx
...
interface AdminSetTabProps {
	environmentId: string;
	workerToken: string;
	onError?: (error: PasswordResetErrorInfo) => void;
}

export const AdminSetTab: React.FC<AdminSetTabProps> = ({
	environmentId,
	workerToken,
	onError,
}) => {
	...
	const handleSetPassword = async () => {
		if (!user || !user.id || !password || !workerToken || !environmentId) {
			raiseError({
				...
				message: 'Select a user and enter a new password before setting it.',
				suggestion:
					'Use the lookup form to select a user, then enter the password you want to apply.',
			});
			return;
		}
		...
		try {
			...
			const result = await setPasswordAdmin(environmentId, user.id, workerToken, password, {
				forceChange,
			...
			raiseError({
				...
				message: result.errorDescription || 'PingOne rejected the password update request.',
				suggestion: bypassPolicy
					? 'Verify that the worker token has permission to bypass password policy requirements.'
					: 'Ensure the new password meets your password policy or consider enabling “Bypass policy”.',
			});
		} catch (error) {
			raiseError({
				title: 'Password Update Failed',
				message: error instanceof Error ? error.message : 'An unexpected error occurred.',
				suggestion: 'Check your network connection and confirm the user still exists.',
			});
		} finally {
			setLoading(false);
		}
		...
		<Card>
			...
			{success && beforeState && afterState && (
				<UserComparisonDisplay
					...
					operationName="admin set password"
				/>
			)}
			...
			{user?.id && (
				<HighlightedSection highlightDuration={6000}>
					<Alert $type="info" style={{ marginTop: '0', marginBottom: '1rem', border: 'none' }}>
						<FiAlertCircle />
						<div>
							<strong>Ready to Set Password</strong>
							<p style={{ margin: '0.5rem 0 0 0', fontSize: '0.875rem' }}>
								Enter a new password for <strong>{user.username || user.email}</strong> below. You
								can optionally force them to change it on next sign-in or bypass password policy
								requirements.
							</p>
						</div>
					</Alert>
					...
					<Button
						...
						title={!password ? 'Please enter a password' : ''}
					>
