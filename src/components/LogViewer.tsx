...
import { FiDownload, FiFilter, FiRefreshCw, FiTrash2, FiSearch } from 'react-icons/fi';
...
const LogEntry = styled.div<{ level: LogLevel }>`
	...
	background: ${props => {
		switch (props.level) {
			case LogLevel.ERROR: return 'rgba(255, 68, 68, 0.1)';
			case LogLevel.WARN: return 'rgba(255, 170, 0, 0.1)';
			case LogLevel.INFO: return 'rgba(68, 68, 255, 0.1)';
			case LogLevel.DEBUG: return 'rgba(136, 136, 136, 0.1)';
			...
			<div>
				{filteredLogs.map((log, index) => (
					<LogEntry key={index} level={log.level}>
						...
						{log.data && (
							<Data>{JSON.stringify(log.data, null, 2)}</Data>
						)}
					</LogEntry>
				))}
			</div>
