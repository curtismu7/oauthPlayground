...
import { FiShield, FiAlertTriangle, FiCheckCircle, FiXCircle, FiTrendingUp, FiTrendingDown, FiEye, FiDownload, FiRefreshCw, FiSettings } from 'react-icons/fi';
...
const MetricIcon = styled.div<{ $metric: string; $status?: 'good' | 'warning' | 'danger' }>`
  color: ${({ $metric, $status, theme }) => {
    ...
    switch ($metric) {
      case 'security': return theme.colors.danger;
      case 'compliance': return theme.colors.success;
      case 'threats': return theme.colors.warning;
      case 'alerts': return theme.colors.info;
...
const ComplianceCard = styled.div<{ $status: 'compliant' | 'partial' | 'non_compliant' }>`
  ...
  border: 2px solid ${({ $status, theme }) => {
    switch ($status) {
      ...
      case 'non_compliant': return theme.colors.danger;
      default: return theme.colors.gray200;
    }
  }};
  padding: 1.5rem;
...
export const SecurityAnalyticsDashboard: React.FC = () => {
  ...
  return (
    <DashboardContainer role="main" aria-label="Security analytics dashboard">
      ...
      <MetricsGrid>
        <MetricCard $metric="security" $status={averageRiskScore > 70 ? 'danger' : averageRiskScore > 40 ? 'warning' : 'good'}>
          <MetricHeader>
            <MetricTitle>Security Risk Score</MetricTitle>
            <MetricIcon $metric="security" $status={averageRiskScore > 70 ? 'danger' : averageRiskScore > 40 ? 'warning' : 'good'}>
              <FiShield />
            </MetricIcon>
          </MetricHeader>
        ...
        <MetricCard $metric="alerts" $status={criticalAlerts.length > 0 ? 'danger' : highSeverityAlerts.length > 0 ? 'warning' : 'good'}>
          <MetricHeader>
            <MetricTitle>Critical Alerts</MetricTitle>
            <MetricIcon $metric="alerts" $status={criticalAlerts.length > 0 ? 'danger' : highSeverityAlerts.length > 0 ? 'warning' : 'good'}>
              <FiAlertTriangle />
            </MetricIcon>
          </MetricHeader>
        ...
        <MetricCard $metric="threats" $status={threatIntelligence.filter(t => t.severity === 'critical').length > 0 ? 'danger' : 'good'}>
          <MetricHeader>
            <MetricTitle>Active Threats</MetricTitle>
            <MetricIcon $metric="threats" $status={threatIntelligence.filter(t => t.severity === 'critical').length > 0 ? 'danger' : 'good'}>
              <FiEye />
            </MetricIcon>
          </MetricHeader>
          <MetricValue>{threatIntelligence.filter(t => t.active).length}</MetricValue>
          <MetricChange $positive={threatIntelligence.filter(t => t.severity === 'critical').length === 0}>
            {threatIntelligence.filter(t => t.severity === 'critical').length === 0 ? <FiCheckCircle /> : <FiAlertTriangle />}
            {threatIntelligence.filter(t => t.severity === 'critical').length === 0 ? 'No Critical Threats' : 'Critical Threats Detected'}
          </MetricChange>
        </MetricCard>

        <MetricCard $metric="compliance" $status={complianceReports.some(r => r.score < 80) ? 'warning' : 'good'}>
          <MetricHeader>
            <MetricTitle>Compliance Score</MetricTitle>
            <MetricIcon $metric="compliance" $status={complianceReports.some(r => r.score < 80) ? 'warning' : 'good'}>
              <FiCheckCircle />
            </MetricIcon>
          </MetricHeader>
          <MetricValue>
            {complianceReports.length > 0 
              ? (complianceReports.reduce((sum, r) => sum + r.score, 0) / complianceReports.length).toFixed(1)
              : '100'
            }
          </MetricValue>
          <MetricChange $positive={complianceReports.every(r => r.score >= 95)}>
            {complianceReports.every(r => r.score >= 95) ? <FiCheckCircle /> : <FiAlertTriangle />}
            {complianceReports.every(r => r.score >= 95) ? 'Fully Compliant' : 'Compliance Issues'}
          </MetricChange>
        </MetricCard>
      </MetricsGrid>
            ...
            <ComplianceCard key={standard} $status={status}>
              ...
              <ComplianceStatus>
                <ComplianceIcon $status={status}>
                  {status === 'compliant' ? <FiCheckCircle /> : 
                   status === 'partial' ? <FiAlertTriangle /> : <FiXCircle />}
                </ComplianceIcon>
                <StatusBadge $status={status === 'compliant' ? 'good' : status === 'partial' ? 'warning' : 'danger'}>
                  {status.replace('_', ' ')}
                </StatusBadge>
              </ComplianceStatus>
              ...
              <ComplianceDetails>
                ...
                {score < 95 && (
                  <div className="violations">
                    {complianceReports.find(r => r.standard === standard)?.violations.length || 0} violations
                  </div>
                )}
              </ComplianceDetails>
            </ComplianceCard>
