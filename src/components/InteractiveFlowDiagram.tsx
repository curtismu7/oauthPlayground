...
const ActorCard = styled.div<{ $actor: string }>`
  ...
  background: ${({ $actor }) => {
    switch ($actor) {
      case 'user': return 'linear-gradient(135deg, #dbeafe, #bfdbfe)';
      case 'client': return 'linear-gradient(135deg, #d1fae5, #a7f3d0)';
      case 'auth-server': return 'linear-gradient(135deg, #fef3c7, #fde68a)';
      case 'server': return 'linear-gradient(135deg, #e9d5ff, #ddd6fe)';
  ...
  .actor-icon {
    ...
    color: ${({ $actor }) => {
      switch ($actor) {
        ...
        case 'client': return '#065f46';
        case 'auth-server': return '#92400e';
        case 'server': return '#5b21b6';
...
const StepCard = styled.div<{ $status: string; $delay: number }>`
  ...
  background: ${({ $status }) => {
    switch ($status) {
      case 'active': return 'linear-gradient(135deg, #dbeafe, #bfdbfe)';
      case 'completed': return 'linear-gradient(135deg, #d1fae5, #a7f3d0)';
    ...
    steps: [
      {
        ...
        explanation: 'The user initiates the login process. The client application redirects the user to PingOne with the necessary parameters including client_id, redirect_uri, and scope.',
        actor: 'user',
      ...
      },
      {
        ...
        explanation: 'After successful authentication, PingOne generates a short-lived authorization code and redirects the user back to the client application with this code.',
        actor: 'auth-server',
      ...
      },
      {
        ...
        explanation: 'The client application makes a secure server-to-server request to PingOne, exchanging the authorization code for an access token. This happens behind the scenes.',
        actor: 'client',
      ...
      },
      {
        ...
        explanation: 'PingOne validates the authorization code and client credentials, then returns an access token (and optionally a refresh token) to the client application.',
        actor: 'auth-server',
      ...
      },
      {
        ...
        explanation: 'The client application can now use the access token to make authenticated requests to protected APIs. The token is included in the Authorization header.',
        actor: 'client',
    ...
    steps: [
      {
        ...
        title: 'Authorization Request',
        description: 'User clicks login, client redirects to auth server',
        explanation: 'The user initiates login. The client redirects to PingOne with response_type=token (instead of code) to request the access token directly.',
        actor: 'user',
        action: 'Clicks login button',
      ...
      },
      {
        ...
        explanation: 'PingOne redirects back to the client with the access token directly in the URL fragment (after #). This is less secure than the Authorization Code flow.',
        actor: 'auth-server',
      ...
      },
      {
        ...
        title: 'API Access',
        description: 'Client uses access token to call protected API',
        explanation: 'The client extracts the token from the URL fragment and uses it to make authenticated API calls. The token is visible in the browser URL.',
        actor: 'client',
        action: 'GET /api/user with token',
        data: 'GET /api/user\nAuthorization: Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9...',
        duration: 1000,
    ...
    steps: [
      {
        ...
        explanation: 'The client application makes a direct request to PingOne using its client credentials (client_id and client_secret). No user interaction is required.',
        actor: 'client',
      ...
      },
      {
        ...
        title: 'Access Token',
        description: 'Auth server returns access token',
        explanation: 'PingOne issues an access token with the requested scope. This token can be used to access APIs on behalf of the application (not a user).',
        actor: 'auth-server',
      ...
      },
      {
        ...
        explanation: 'The client uses the access token to make API calls. This is typically used for machine-to-machine communication or accessing application-specific resources.',
        actor: 'client',
    ...
    steps: [
      {
        ...
        explanation: 'The device (like a smart TV or IoT device) makes a request to PingOne to initiate the device authorization flow. No user credentials are needed at this stage.',
        actor: 'client',
      ...
      },
      {
        ...
        explanation: 'PingOne generates a device_code (for the device) and a user_code (for the user). The device displays the user_code and verification URL to the user.',
        actor: 'auth-server',
      ...
      },
      {
        ...
        explanation: 'The user goes to the verification URL on their phone/computer and enters the user_code. They then authenticate with their PingOne credentials.',
        actor: 'user',
      ...
      },
      {
        ...
        explanation: 'After authentication, the user sees a consent screen asking them to authorize the device. They click "Allow" to grant access to the device.',
        actor: 'auth-server',
      ...
      },
      {
        ...
        explanation: 'The device repeatedly polls PingOne using the device_code to check if the user has completed authorization. This continues until the user authorizes or the code expires.',
        actor: 'client',
      ...
      },
      {
        ...
        title: 'Access Token',
        description: 'Auth server returns access token',
        explanation: 'Once the user has authorized the device, PingOne returns an access token to the device. The device can now access protected resources on behalf of the user.',
        actor: 'auth-server',
        action: 'Returns access token',
        data: '{\n  "access_token": "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9...",\n  "token_type": "Bearer",\n  "expires_in": 3600,\n  "scope": "read write"\n}',
        duration: 1000,
      ...
      },
      {
        ...
        explanation: 'The device can now make authenticated API calls using the access token. This enables the device to access user-specific resources like playlists or preferences.',
        actor: 'client',
        action: 'GET /api/data with token',
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        explanation: 'Upon successful validation, the authorization server returns an access token directly to the client.',
        actor: 'auth-server',
        action: 'Returns access token',
        data: '{\n  "access_token": "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9...",\n  "token_type": "Bearer",\n  "expires_in": 3600,\n  "scope": "read write"\n}',
        duration: 1000,
      ...
      },
      {
        ...
        title: 'API Access',
        description: 'Client uses token to access protected resources',
        explanation: 'The client can now use the access token to make authenticated requests to protected APIs.',
        actor: 'client',
        action: 'GET /api/data with token',
    ...
    steps: [
      {
        ...
        explanation: 'The client application makes a secure request to PingOne\'s PAR endpoint, pushing all authorization parameters including client credentials, scopes, and redirect URI.',
        actor: 'client',
      ...
      },
      {
        ...
        explanation: 'PingOne validates the parameters and returns a short-lived request URI that contains a reference to the pushed parameters. This URI is much shorter and safer than the original parameters.',
        actor: 'auth-server',
      ...
      },
      {
        ...
        explanation: 'The user initiates the login process. Instead of passing all parameters in the URL, the client only needs to pass the short request URI to PingOne.',
        actor: 'user',
        action: 'Clicks login button',
      ...
      },
      {
        ...
        title: 'Access Token',
        description: 'Auth server returns access token',
        explanation: 'PingOne validates the authorization code and returns an access token (and optionally a refresh token) to the client application.',
        actor: 'auth-server',
        action: 'Returns access token',
...
const PingOneIcon = () => (
  ...
  const getActorIcon = (actor: string) => {
    switch (actor) {
      ...
      case 'auth-server': return <PingOneIcon />;
  ...
  const getFlowIcon = (flowId: string) => {
    switch (flowId) {
      case 'authorization-code': return <FiKey />;
      case 'implicit': return <FiArrowRight />;
      case 'client-credentials': return <FiCode />;
      case 'device-code': return <FiServer />;
      case 'resource-owner-password': return <FiShield />;
      case 'par': return <FiInfo />;
  ...
  const getActorTitle = (actor: string) => {
    switch (actor) {
      ...
      case 'auth-server': return 'PingOne';
  ...
  const getActorDescription = (actor: string) => {
    switch (actor) {
      ...
      case 'auth-server': return 'PingOne Identity Platform';
