...
import { FiX, FiShield, FiKey, FiSmartphone, FiMonitor, FiCheckCircle, FiAlertCircle } from 'react-icons/fi';
import { FIDO2Service, FIDO2Config } from '../services/fido2Service';
...
const FIDO2RegistrationModal: React.FC<FIDO2RegistrationModalProps> = ({
  ...
  const [selectedAuthenticatorType, setSelectedAuthenticatorType] = useState<'platform' | 'cross-platform' | 'any'>('any');
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (!caps.webAuthnSupported) {
        setError('WebAuthn is not supported in this browser. Please use a modern browser that supports passkeys.');
      } else {
        setError(null);
      }
    }
    ...
    try {
      ...
      const config: FIDO2Config = {
        ...
        authenticatorSelection: {
          authenticatorAttachment: selectedAuthenticatorType === 'any' ? undefined : selectedAuthenticatorType,
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                ...
                <div style={{
                  ...
                  {capabilities.webAuthnSupported ? (
                    <>
                      ...
                      {capabilities.platformAuthenticator && <div>✅ Built-in authenticator available</div>}
                      {capabilities.crossPlatformAuthenticator && <div>✅ External security keys supported</div>}
                    </>
                  ) : (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  ...
                  {capabilities.platformAuthenticator && (
                    <div
                      ...
                      style={{
                        padding: '1rem',
                        border: selectedAuthenticatorType === 'platform' ? '2px solid #3b82f6' : '1px solid #e5e7eb',
                        borderRadius: '0.5rem',
                        cursor: 'pointer',
                        backgroundColor: selectedAuthenticatorType === 'platform' ? '#eff6ff' : '#ffffff',
                  ...
                  {capabilities.crossPlatformAuthenticator && (
                    <div
                      ...
                      style={{
                        padding: '1rem',
                        border: selectedAuthenticatorType === 'cross-platform' ? '2px solid #3b82f6' : '1px solid #e5e7eb',
                        borderRadius: '0.5rem',
                        cursor: 'pointer',
                        backgroundColor: selectedAuthenticatorType === 'cross-platform' ? '#eff6ff' : '#ffffff',
                        transition: 'all 0.2s ease'
                      }}
                    >
                      <div style={{
                  ...
                  <div
                    ...
                    style={{
                      padding: '1rem',
                      border: selectedAuthenticatorType === 'any' ? '2px solid #3b82f6' : '1px solid #e5e7eb',
                      borderRadius: '0.5rem',
                      cursor: 'pointer',
            ...
            {selectedAuthenticatorType !== 'any' && (
              <div style={{
                ...
                <ol style={{
                  ...
                  {FIDO2Service.getSetupInstructions(selectedAuthenticatorType).map((instruction, index) => (
            ...
            <div style={{
              ...
              <div style={{
                ...
                <div><strong>Relying Party:</strong> {rpName}</div>
                <div><strong>Domain:</strong> {rpId}</div>
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              border: 'none',
              borderRadius: '0.375rem',
              backgroundColor: isRegistering || !capabilities?.webAuthnSupported || !!error ? '#9ca3af' : '#10b981',
              color: 'white',
              fontSize: '0.875rem',
              fontWeight: '600',
              cursor: isRegistering || !capabilities?.webAuthnSupported || !!error ? 'not-allowed' : 'pointer',
              transition: 'all 0.2s ease',
              display: 'flex',
              alignItems: 'center',
              gap: '0.5rem'
