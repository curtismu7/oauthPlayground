...
const getRecommendations = (answers: Record<string, string>): FlowRecommendation[] => {
  ...
  if (answers['no-user-interaction'] === 'no-user-interaction') {
    recommendations.push({
      ...
      reason: 'Since your application doesn\'t require user interaction, Client Credentials is the most appropriate flow for machine-to-machine authentication.'
    });
    ...
    recommendations.push({
      ...
      route: '/flows/pkce',
      reason: 'PKCE provides the best security for mobile apps and applications without a secure backend.'
    });
    ...
    recommendations.push({
      ...
      reason: 'Authorization Code flow is the gold standard for web applications with a secure backend.'
    });
  } else {
    recommendations.push({
      id: 'authorization-code',
      ...
      implementationTime: '2-4 hours',
      route: '/flows/authorization-code',
      reason: 'Authorization Code flow is the most versatile and secure option for most applications.'
    });
  }
  ...
  if (answers['app-type'] === 'spa' && answers['no-backend'] === 'no-backend') {
    recommendations.push({
      ...
      reason: 'While deprecated, this flow is still used in some legacy SPAs. Consider migrating to PKCE.'
    });
  }
