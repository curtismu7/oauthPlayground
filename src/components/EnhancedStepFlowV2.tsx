...
const StepConnector = styled.div<{ $completed: boolean; $active: boolean }>`
	...
	background: ${(props) => {
		...
		if (props.$active)
			return 'linear-gradient(to right, COLORS.PRIMARY.GREEN 50%, COLORS.TEXT.GRAY_LIGHTER 50%)';
		return '#e5e7eb';
	}};
	position: relative;
	z-index: 1;
...
const Button = styled.button<{
	...
	${(props) => {
        ...
        `;
          ...
          &:hover:not(:disabled) {
            background: #f9fafb;
            border-color: COLORS.TEXT.GRAY_MEDIUM;
          }
        `;
			case 'success':
				return `
				...
				<StepContent>
					...
					<ActionButtons>
						<Button
							...
							title={
								clampedStepIndex === 0 ? 'No previous step' : isExecuting ? 'Please wait...' : ''
							}
						>
							{isExecuting ? (
								<>
						...
						<Button
							...
							disabled={
								clampedStepIndex === steps.length - 1 || !isCurrentStepCompleted || isExecuting
							}
							$loading={isExecuting}
							style={{
								opacity:
									(!isCurrentStepCompleted && clampedStepIndex < steps.length - 1) || isExecuting
										? 0.5
										: 1,
								cursor:
									(!isCurrentStepCompleted && clampedStepIndex < steps.length - 1) || isExecuting
							...
							title={
								!isCurrentStepCompleted && clampedStepIndex < steps.length - 1
