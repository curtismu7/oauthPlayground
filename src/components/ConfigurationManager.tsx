...
const ConfigurationManager: React.FC<ConfigurationManagerProps> = ({
  ...
  const loadConfiguration = useCallback(async () => {
    setIsLoading(true);
    try {
      const loadedConfig = EnhancedConfigurationService.getFlowConfig(selectedFlowType, selectedEnvironment);
      ...
      // Get suggestions
      const configSuggestions = EnhancedConfigurationService.getConfigurationSuggestions(loadedConfig);
  ...
  const handleConfigChange = useCallback((field: keyof BaseFlowConfig, value: any) => {
    if (!config) return;
    ...
    // Re-suggest
    const configSuggestions = EnhancedConfigurationService.getConfigurationSuggestions(updatedConfig);
    setSuggestions(configSuggestions);
  }, [config]);
  ...
  const handleExportConfiguration = useCallback(() => {
    ...
    const exportData = EnhancedConfigurationService.exportConfiguration(selectedFlowType, selectedEnvironment);
    ...
    reader.onload = (e) => {
      try {
        ...
        if (imported.flowType === selectedFlowType) {
          ...
          // Re-validate
          const validationResult = EnhancedConfigurationService.validateConfiguration(imported.config);
          setValidation(validationResult);
        } else {
          alert(`Configuration is for ${imported.flowType}, but you're editing ${selectedFlowType}`);
        }
      } catch (error) {
  ...
  const renderConfigValue = (field: keyof BaseFlowConfig, value: any) => {
    if (typeof value === 'boolean') {
      ...
      return (
        <ConfigInput
          type="number"
          value={value}
          onChange={(e) => handleConfigChange(field, parseInt(e.target.value, 10))}
        />
      );
    } else {
      return (
  ...
  return (
    <Container>
      <Header>
        ...
        <Subtitle>
          Manage OAuth/OIDC flow configurations with validation, suggestions, and environment-specific settings
        </Subtitle>
      </Header>
      ...
      {validation && (
        <ValidationSection>
          <SectionTitle>
            {getValidationIcon(validation.isValid ? 'valid' : validation.errors.length > 0 ? 'invalid' : 'warning')}
            Configuration Validation
          </SectionTitle>

          <ValidationStatus status={validation.isValid ? 'valid' : validation.errors.length > 0 ? 'invalid' : 'warning'}>
            {validation.isValid ? (
              <>
                <FiCheckCircle />
                <div>
        ...
        <SuggestionsSection>
          ...
          {suggestions.map((suggestion, index) => (
            <SuggestionCard key={index}>
              ...
              <SuggestionText>
                <strong>Current:</strong> {JSON.stringify(suggestion.currentValue)}<br/>
                <strong>Suggested:</strong> {JSON.stringify(suggestion.suggestedValue)}<br/>
