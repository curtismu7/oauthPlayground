...
import { FiRefreshCw, FiCopy, FiCheckCircle, FiAlertCircle, FiSettings, FiKey } from 'react-icons/fi';
import QRCodeService, { type TOTPConfig, type QRCodeResult } from '../services/qrCodeService';
...
export const QRCodeServiceDemo: React.FC = () => {
  ...
  const handleGenerateSecret = () => {
    try {
      ...
      setStatus({ type: 'error', message: `Failed to generate secret: ${error instanceof Error ? error.message : 'Unknown error'}` });
    }
  };
  ...
  const handleGenerateQRCode = async () => {
    try {
      ...
      setStatus({ type: 'error', message: `Failed to generate QR code: ${error instanceof Error ? error.message : 'Unknown error'}` });
    }
  };
  ...
  const handleValidateCode = () => {
    try {
      ...
      if (result.valid) {
        setStatus({ type: 'success', message: `TOTP code is valid! (Time window: ${result.timeWindow || 0})` });
      } else {
        setStatus({ type: 'error', message: `TOTP code is invalid: ${result.error}` });
      }
    } catch (error) {
      setStatus({ type: 'error', message: `Validation error: ${error instanceof Error ? error.message : 'Unknown error'}` });
    }
  };
      ...
      <Grid>
        <div>
          <Section>
            ...
            <FormGroup>
              ...
              <Select
                value={config.digits}
                onChange={(e) => setConfig(prev => ({ ...prev, digits: parseInt(e.target.value) as any }))}
              >
            ...
            <FormGroup>
              ...
              <Input
                ...
                max="300"
                value={config.period}
                onChange={(e) => setConfig(prev => ({ ...prev, period: parseInt(e.target.value) }))}
              />
            </FormGroup>
        ...
        <div>
          ...
          <Section>
            ...
            <div>
              <p><strong>QR Code Generation:</strong> {capabilities.qrCodeGeneration ? '✅ Supported' : '❌ Not Supported'}</p>
              <p><strong>Manual Entry:</strong> {capabilities.manualEntry ? '✅ Supported' : '❌ Not Supported'}</p>
              <p><strong>TOTP Validation:</strong> {capabilities.totpValidation ? '✅ Supported' : '❌ Not Supported'}</p>
              <p><strong>Backup Codes:</strong> {capabilities.backupCodes ? '✅ Supported' : '❌ Not Supported'}</p>
            </div>
          </Section>
        </div>
      </Grid>
