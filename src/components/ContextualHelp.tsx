...
const helpContentMap: Record<string, HelpContent> = {
  'authorization-code': {
    whenToUse: 'Use Authorization Code flow when you have a secure backend that can store client secrets. This is the most secure OAuth flow and is recommended for web applications, mobile apps with backends, and any application that can securely store credentials.',
    prerequisites: [
    ...
    commonIssues: [
      {
        ...
        solution: 'Ensure the redirect URI in your authorization request exactly matches the one registered in your OAuth provider.'
      },
      {
        issue: 'Client secret exposed in frontend',
        solution: 'Move the token exchange to your backend server. Never include client secrets in client-side code.'
      }
    ],
    bestPractices: [
  ...
  'pkce': {
    whenToUse: 'Use PKCE (Proof Key for Code Exchange) when you cannot securely store a client secret, such as in mobile apps, SPAs, or any public client application. PKCE adds an extra layer of security to the Authorization Code flow.',
    prerequisites: [
    ...
    commonIssues: [
      {
        ...
        solution: 'Ensure the code verifier used in token exchange matches the one used to generate the code challenge.'
      },
      {
        issue: 'PKCE not supported by provider',
        solution: 'Check if your OAuth provider supports PKCE. Some older implementations may not support it.'
      }
    ],
    bestPractices: [
  ...
  'client-credentials': {
    whenToUse: 'Use Client Credentials flow for machine-to-machine authentication where no user interaction is required. This is perfect for server-to-server communication, background processes, and API services.',
    prerequisites: [
    ...
    commonIssues: [
      {
        ...
        solution: 'Verify your client ID and client secret are correct and properly encoded in the Authorization header.'
      },
      {
        issue: 'Insufficient scope permissions',
        solution: 'Check that your application has the necessary scopes configured in your OAuth provider.'
      }
    ],
    bestPractices: [
  ...
  'implicit': {
    whenToUse: 'The Implicit Grant flow is deprecated and should only be used for legacy applications or when migrating from older implementations. Consider using PKCE instead for new applications.',
    prerequisites: [
    ...
    commonIssues: [
      ...
      },
      {
        ...
        solution: 'Implement automatic re-authentication or migrate to a flow that supports refresh tokens.'
      }
    ],
    bestPractices: [
