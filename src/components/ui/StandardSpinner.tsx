...
export const StandardModalSpinner: React.FC<StandardModalSpinnerProps> = ({
	...
	return <LoadingSpinnerModalV8U show={show} message={message} theme={theme} />;
};
...
export const useStandardSpinner = (estimatedDuration?: number) => {
	...
	const spinnerType = estimatedDuration ? getSpinnerType(estimatedDuration) : 'button'; // Default to button for unknown duration
	...
	const executeWithSpinner = React.useCallback(
		async <T,>(
			asyncFn: () => Promise<T>,
			options?: {
...
export const AutoSpinner: React.FC<AutoSpinnerProps> = ({
	...
	const spinnerType = estimatedDuration ? getSpinnerType(estimatedDuration) : 'button';
	...
	if (spinnerType === 'modal') {
		return <StandardModalSpinner show={true} message={message} theme={theme} {...modalProps} />;
	}

	// For button spinner, we need a button element
	if (buttonProps) {
		return (
			<StandardButtonSpinner loading={true} {...buttonProps}>
				{children}
	...
	return (
		<StandardButtonSpinner loading={true} onClick={() => {}} disabled={true}>
			{children || 'Loading...'}
		</StandardButtonSpinner>
	);
};
