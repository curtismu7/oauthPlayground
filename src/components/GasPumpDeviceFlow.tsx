...
const GasPumpContainer = styled.div`
	background: linear-gradient(
		135deg,
		COLORS.PRIMARY.RED_DARK 0%,
		COLORS.PRIMARY.RED 50%,
		COLORS.PRIMARY.RED_DARK 100%
	);
	...
	box-shadow:
		...
		0 0 0 2px COLORS.TEXT.WHITE,
		inset 0 1px 0 rgba(255, 255, 255, 0.3);
	position: relative;
	overflow: hidden;
	border: 3px solid COLORS.TEXT.WHITE;
...
const CenterPanel = styled.div`
	...
	border: 3px solid COLORS.PRIMARY.YELLOW_LIGHT;
...
const FuelButton = styled.button<{
	$type: 'regular' | 'special' | 'super' | 'diesel' | 'rec90';
	$active?: boolean;
}>`
	background: ${(props) => {
		switch (props.$type) {
...
const StatusDisplay = styled.div<{ $status: string }>`
	background: ${(props) => {
		switch (props.$status) {
			case 'pending':
				return 'linear-gradient(135deg, COLORS.BG.WARNING 0%, COLORS.PRIMARY.YELLOW_LIGHT 100%)';
			case 'authorized':
				return 'linear-gradient(135deg, COLORS.BG.SUCCESS 0%, COLORS.PRIMARY.GREEN 100%)';
			case 'denied':
				return 'linear-gradient(135deg, COLORS.BG.ERROR_BORDER 0%, COLORS.PRIMARY.RED 100%)';
			case 'expired':
				return 'linear-gradient(135deg, #f3f4f6 0%, COLORS.TEXT.GRAY_MEDIUM 100%)';
			default:
				return 'linear-gradient(135deg, #f3f4f6 0%, COLORS.TEXT.GRAY_MEDIUM 100%)';
		}
	}};
...
const _ActionButton = styled.button<{ $variant: 'primary' | 'secondary' | 'success' | 'danger' }>`
	...
	${({ $variant }) => {
        ...
        `;
          ...
          color: white;
          &:hover { background: COLORS.PRIMARY.RED_DARK; transform: translateY(-1px); }
        `;
		}
	}}
`;
