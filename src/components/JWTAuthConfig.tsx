...
import { FiKey, FiShield, FiEye, FiEyeOff, FiCopy, FiCheckCircle, FiAlertCircle } from 'react-icons/fi';
...
const JWTAuthConfig: React.FC<JWTAuthConfigProps> = ({ onConfigChange, initialConfig }) => {
  const [authMethod, setAuthMethod] = useState<'CLIENT_SECRET_JWT' | 'PRIVATE_KEY_JWT'>('CLIENT_SECRET_JWT');
  ...
  const handleTestAuth = async () => {
    ...
    try {
      ...
      if (authMethod === 'CLIENT_SECRET_JWT') {
        result = await jwtAuthService.exchangeClientSecretJWT(config, 'client_credentials', 'pingone:read');
      } else {
        result = await jwtAuthService.exchangePrivateKeyJWT(config, 'client_credentials', 'pingone:read');
      }
    ...
    <Container>
      ...
      <FormGroup>
        ...
        <div className="help-text">
          The issuer URL for your PingOne environment
        </div>
        {errors.issuer && (
          <div className="error-text">
            <FiAlertCircle size={14} />
      ...
      <FormGroup>
        ...
        />
        <div className="help-text">
          The token endpoint URL for your PingOne environment
        </div>
      ...
      {authMethod === 'CLIENT_SECRET_JWT' && (
        <FormGroup>
          ...
          <div className="help-text">
            The client secret for your PingOne application
          </div>
          {errors.clientSecret && (
            <div className="error-text">
              <FiAlertCircle size={14} />
      ...
      <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
        ...
        {testResult && (
          <Button
            variant="secondary"
            onClick={() => handleCopyToClipboard(testResult)}
          >
            <FiCopy />
            Copy Result
          </Button>
