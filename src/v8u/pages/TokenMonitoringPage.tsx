...
export const TokenMonitoringPage: React.FC = () => {
	...
	// Listen for worker token updates (like other pages)
	useEffect(() => {
		const handleTokenUpdate = async () => {
			console.log('[TokenMonitoringPage] Worker token updated, syncing...');
			const service = TokenMonitoringService.getInstance();
			service.manualSyncWorkerToken();
		};

		window.addEventListener('workerTokenUpdated', handleTokenUpdate);
		window.addEventListener('unified-worker-token-updated', handleTokenUpdate);

		return () => {
			window.removeEventListener('workerTokenUpdated', handleTokenUpdate);
			window.removeEventListener('unified-worker-token-updated', handleTokenUpdate);
		};
	}, []);
