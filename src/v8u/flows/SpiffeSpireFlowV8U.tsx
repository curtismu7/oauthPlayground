...
export const SpiffeSpireFlowV8U: React.FC = () => {
	...
	const [showWhySpiffe, setShowWhySpiffe] = useState(true);
	const [showCoreConcepts, setShowCoreConcepts] = useState(true);
	const [showSpireArchitecture, setShowSpireArchitecture] = useState(true);
	const [showPingOneIntegration, setShowPingOneIntegration] = useState(true);
			...
			try {
				...
				if (stored) {
					...
					if (data.credentials?.environmentId) {
						...
						logger.info(
							`${MODULE_TAG} Auto-populated environment ID from worker token`,
							'Logger info'
						);
					}
				}
			...
			try {
				...
				if (stored) {
					...
					if (data.credentials?.environmentId && !environmentId) {
						setEnvironmentId(data.credentials.environmentId);
						logger.info(
							`${MODULE_TAG} Auto-populated environment ID from worker token update`,
							'Logger info'
						);
					}
				}
