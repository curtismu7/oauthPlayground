...
export class TokenMonitoringService {
  ...
  private loadApiCallsFromStorage(): void {
    try {
      const stored = typeof window !== 'undefined' ? window.localStorage.getItem(this.API_CALLS_KEY) : null;
      if (stored) {
    ...
    try {
      apiCallTrackerService.subscribe((trackedCalls) => {
        ...
        trackedCalls.forEach((trackedCall) => {
          ...
          if (trackedCall.url?.includes('/token') || 
            ...
            const apiCall: ApiCall = {
              ...
              timestamp: trackedCall.timestamp ? new Date(trackedCall.timestamp).getTime() : Date.now(),
              ...
              response: {
                ...
                body: trackedCall.response?.data ? JSON.stringify(trackedCall.response.data) : undefined,
              },
              duration: trackedCall.duration || 0,
              type: this.getApiCallType(trackedCall.url),
              success: (trackedCall.response?.status || 0) >= 200 && (trackedCall.response?.status || 0) < 300,
            };
  ...
  private syncTokensFromFlowContext(): void {
    try {
      const flowContext = typeof window !== 'undefined' ? window.sessionStorage.getItem('tokenManagementFlowContext') : null;
      if (flowContext) {
  ...
  private syncTokensFromOAuthFlow(oauthTokens: Record<string, string>, flowSource: string): void {
    console.log('[TokenMonitoring] Syncing tokens from OAuth flow:', { flowSource, tokenTypes: Object.keys(oauthTokens) });
    ...
    // Add access token
    if (oauthTokens.access_token) {
      const expiresIn = oauthTokens.expires_in ? parseInt(oauthTokens.expires_in) * 1000 : 60 * 60 * 1000; // Default 1 hour
  ...
  private async syncWorkerToken(): Promise<void> {
    try {
      ...
      console.log('[TokenMonitoring] Got worker token string:', workerTokenString ? 'SUCCESS' : 'FAILED');
      ...
      if (workerTokenString) {
        ...
        this.addToken({
          ...
          expiresAt: storedToken?.expiresAt || (status.tokenExpiresIn ? Date.now() + (status.tokenExpiresIn * 1000) : null),
  ...
  public addToken(token: Omit<TokenInfo, 'id' | 'status' | 'isVisible' | 'introspectionData'>): string {
    ...
    if (existingTokensOfSameType.length > 0) {
      console.log(`[TokenMonitoring] Found ${existingTokensOfSameType.length} existing tokens of type ${token.type}, removing them first`);
    ...
    try {
      ...
      if (flowContext) {
        ...
        if (credentials?.authHost && credentials?.environmentId && credentials?.clientId) {
          ...
          console.warn('[TokenMonitoring] No credentials available for real introspection, using mock');
        }
      } else {
  ...
  private async revokeTokenSessionDelete(token: TokenInfo, options: RevocationOptions): Promise<void> {
    ...
    try {
      ...
      if (flowContext) {
        ...
        if (credentials?.authHost && credentials?.environmentId) {
          ...
          console.log(`[TokenMonitoring] Successfully deleted session ${options.sessionId} for user ${options.userId}`);
        } else {
