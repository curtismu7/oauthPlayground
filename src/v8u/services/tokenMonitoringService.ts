...
export class TokenMonitoringService {
	...
	public addOAuthTokens(
		oauthTokens: OAuthTokenPayload,
		flowSource: string = 'unified-oauth'
	): void {
	...
	private loadApiCallsFromStorage(): void {
		try {
			...
			if (stored) {
				...
				logger.debug(
					`[TokenMonitoring] Loaded ${this.apiCalls.length} API calls from storage`,
					'Logger debug'
				);
			} else {
				this.apiCalls = [];
	...
	private getTokenStatus(unifiedToken: { expiresAt?: number }): TokenInfo['status'] {
		...
		if (unifiedToken.expiresAt <= now + 5 * 60 * 1000) {
			// 5 minutes
			return 'expiring';
		}
		return 'active';
	...
	private syncTokensFromLocalStorage(): void {
		...
		try {
			...
			if (accessToken && tokenType) {
				...
				logger.debug(
					'[TokenMonitoring] Token synced from localStorage successfully',
					'Logger debug'
				);
			}
		} catch (error) {
	...
	private clearTokensByType(tokenType: string): void {
		...
		logger.debug(
			`[TokenMonitoring] Cleared ${tokensToRemove.length} tokens of type: ${tokenType}`,
			'Logger debug'
		);
	}
			...
			if (workerTokenString) {
				...
				this.addToken({
					...
					expiresAt:
						...
						(workerTokenStatus.tokenExpiresIn
		...
		try {
			...
			if (flowContext) {
				...
				if (credentials?.authHost && credentials?.environmentId && credentials?.clientId) {
					...
					logger.debug(
						`[TokenMonitoring] Successfully introspected ${token.type} via API`,
						'Logger debug'
					);
					success = true;
			...
			logger.error(
				'[TokenMonitoring] Token introspection failed:',
	...
	private async revokeTokenOAuth(token: TokenInfo): Promise<void> {
		...
		if (flowContext) {
			...
			if (credentials?.authHost && credentials?.environmentId && credentials?.clientId) {
				...
				logger.debug(
					`[TokenMonitoring] Successfully revoked ${token.type} via OAuth API`,
					'Logger debug'
				);
			} else {
				logger.warn(
					'[TokenMonitoring] No credentials available for OAuth revocation',
					'Logger warning'
				);
			}
		} else {
			logger.warn(
				'[TokenMonitoring] No flow context available for OAuth revocation',
				'Logger warning'
			);
		}
	}
	...
	private async revokeTokenSSOSignoff(
		...
		if (flowContext) {
			...
			if (credentials?.authHost && credentials?.environmentId) {
				...
				logger.warn(
					'[TokenMonitoring] No credentials available for SSO sign-off',
					'Logger warning'
				);
			}
		} else {
		...
		try {
			...
			if (flowContext) {
				...
				if (credentials?.authHost && credentials?.environmentId) {
					...
					logger.warn(
						'[TokenMonitoring] No credentials available for session deletion',
						'Logger warning'
					);
				}
			} else {
				logger.warn(
					'[TokenMonitoring] No flow context available for session deletion',
					'Logger warning'
				);
			}
		} catch (error) {
		...
		try {
			...
			(importData as Array<Record<string, unknown>>).forEach((tokenData) => {
				...
				this.addToken({
					...
					expiresAt: tokenData.expiresAt ? new Date(String(tokenData.expiresAt)).getTime() : null,
