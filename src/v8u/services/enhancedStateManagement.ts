...
const EnhancedStateProvider = ({ children }: { children: ReactNode }) => {
	...
	// Listen for worker token updates for immediate refresh
	useEffect(() => {
		const handleWorkerTokenUpdate = async () => {
			console.log('[EnhancedState] Worker token updated, refreshing metrics...');
			try {
				const metrics = await getRealMetrics();
				dispatch({ type: 'UPDATE_REAL_METRICS', payload: metrics });
			} catch (error) {
				console.warn('[EnhancedState] Failed to update metrics on worker token change:', error);
			}
		};

		// Listen for both legacy and unified worker token events
		window.addEventListener('workerTokenUpdated', handleWorkerTokenUpdate);
		window.addEventListener('unified-worker-token-updated', handleWorkerTokenUpdate);

		return () => {
			window.removeEventListener('workerTokenUpdated', handleWorkerTokenUpdate);
			window.removeEventListener('unified-worker-token-updated', handleWorkerTokenUpdate);
		};
	}, []);
