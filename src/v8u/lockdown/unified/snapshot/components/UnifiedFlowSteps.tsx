...
export const UnifiedFlowSteps: React.FC<UnifiedFlowStepsProps> = ({
	...
	const fetchUserInfoWithDiscovery = useCallback(
		async (accessToken: string, environmentId: string): Promise<Record<string, unknown> | null> => {
			...
			try {
				...
				logger.warn(`${MODULE_TAG} Failed to fetch UserInfo`, err);
				return null;
			} finally {
	...
	const handleRestartFlow = useCallback(() => {
		...
		PKCEStorageServiceV8U.clearPKCECodes(flowKey).catch((err) => {
			logger.error(`${MODULE_TAG} Failed to clear PKCE codes from all storage`, err);
		});
	...
	useEffect(() => {
		if (prevFlowTypeRef.current !== flowType) {
			...
			PKCEStorageServiceV8U.clearPKCECodes(prevFlowKey).catch((err) => {
				logger.error(`${MODULE_TAG} Failed to clear PKCE codes for previous flow type`, err);
			});
	...
	useEffect(() => {
		if (currentStep === 4 && (flowType === 'oauth-authz' || flowType === 'hybrid')) {
			...
			if (errors.length > 0) {
				logger.warn(`${MODULE_TAG} Missing required fields on token exchange step`, {
					usePKCE: isPKCERequired,
					pkceEnforcement: credentials.pkceEnforcement,
					hasAuthorizationCode: !!flowState.authorizationCode,
		...
		if (flowState.tokens?.accessToken) {
			...
			try {
				...
				try {
					...
					logger.warn(`${MODULE_TAG} Failed to update enhanced state management`, enhancedErr);
				}
			} catch (err) {
				logger.error(`${MODULE_TAG} Failed to save tokens to sessionStorage`, err);
			}
		}
	...
	useEffect(() => {
		if (flowType === 'device-code' && flowState.deviceCode) {
			...
			try {
				...
				logger.error(`${MODULE_TAG} Failed to save device code data to sessionStorage`, err);
			}
		}
	}, [
	...
	useEffect(() => {
		if (flowState.authorizationCode && (flowType === 'oauth-authz' || flowType === 'hybrid')) {
			try {
				...
				logger.error(`${MODULE_TAG} Failed to save authorization code to sessionStorage`, err);
			}
		}
		...
		if (currentStep === 3 && (flowType === 'oauth-authz' || flowType === 'hybrid')) {
			...
			if (flowType === 'hybrid') {
				...
				if (
					...
					try {
						...
						logger.error(`${MODULE_TAG} Failed to parse callback for hybrid flow`, err);
					}
				}
			}
			...
			if (callbackDataStr) {
				try {
					...
					logger.error(`${MODULE_TAG} Failed to parse callback data from sessionStorage`, err);
				}
			}
			...
			if (flowType === 'hybrid') {
				...
				if (detectedCode && !flowState.authorizationCode) {
					...
					try {
						...
						logger.error(`${MODULE_TAG} Failed to parse authorization code for hybrid flow`, err);
				...
				if (fragmentTokens && !flowState.tokens?.accessToken) {
					...
					try {
						...
						logger.error(`${MODULE_TAG} Failed to save hybrid tokens to sessionStorage`, err);
					}
				}
				...
				if (hasUpdates) {
					...
					if (callbackUrl) {
						try {
							...
							logger.warn(`${MODULE_TAG} Failed to parse callback URL for allParams`, err);
						}
					}
					...
					logger.warn(`${MODULE_TAG} Hybrid flow: No updates applied`, {
			...
			if (
				...
				if (!hasCode || !hasState) {
					logger.warn(`${MODULE_TAG} Callback URL does not contain required OAuth parameters`, {
						callbackUrl,
						hasCode,
						hasState,
						allParams: Object.fromEntries(url.searchParams),
				...
				try {
					...
					logger.error(`${MODULE_TAG} Failed to auto-parse callback URL`, err);
			...
			if (fragment && (fragment.includes('access_token') || fragment.includes('id_token'))) {
				...
				(async () => {
					try {
						...
						logger.error(`${MODULE_TAG} ❌ Failed to auto-parse fragment`, err);
				...
				if (storedTokens) {
					try {
						...
						logger.error(`${MODULE_TAG} Failed to restore tokens from sessionStorage`, err);
					}
				}
			}
		}
															...
															<button
																...
																onClick={async () => {
																	try {
																		...
																		logger.error(`${MODULE_TAG} Error fixing errors:`, error);
																		modernMessaging.showBanner({
																			type: 'error',
																			title: 'Error',
	...
	const renderStep1PKCE = () => {
		...
		const handlePKCEChange = (codes: PKCECodes) => {
			...
			if (codes.codeVerifier && codes.codeChallenge) {
				...
				logger.warn(`${MODULE_TAG} ⚠️ handlePKCEChange called but codes are incomplete`, "Logger warning");
			}
		};
		...
		async (flowId: string, stateValue: string, resumeUrl?: string) => {
			...
			try {
				...
				if (!authCode) {
					...
					logger.error(`${MODULE_TAG} 🔌 Authorization code not found in resume response`, {
	...
	const handleSubmitRedirectlessCredentials = useCallback(
		async (username: string, password: string) => {
			...
			try {
				...
				if (status === 'MUST_CHANGE_PASSWORD') {
					...
					if (userId) {
						...
						try {
							...
							logger.error(`${MODULE_TAG} 🔌 Failed to lookup user:`, lookupError);
							const errorMsg =
				...
				logger.error(`${MODULE_TAG} 🔌 Failed to submit credentials:`, errorMessage);
	...
	const handlePasswordChange = useCallback(
		async (_oldPassword: string, newPassword: string) => {
			...
			try {
				...
				logger.error(`${MODULE_TAG} 🔌 Password change failed:`, error);
	...
	const handleProceedWithPingOneRequest = useCallback(async () => {
		...
		try {
			...
			logger.error(`${MODULE_TAG} ❌ No flowId in redirectless response:`, flowData);
			...
			try {
				...
				if (!workerToken) {
					try {
						...
						logger.warn(`${MODULE_TAG} Silent worker token retrieval failed:`, error);
				...
				logger.error(`${MODULE_TAG} ⚠️ Pre-flight validation error:`, validationError);
				const errorMessage =
										...
										<button
											...
											onClick={async () => {
												try {
													...
													logger.error(`${MODULE_TAG} Error retrieving worker token:`, error);
													modernMessaging.showBanner({
														type: 'error',
														title: 'Error',
	...
	const renderStep2Callback = () => {
				...
				if (flowType === 'hybrid') {
					...
					if (hasFragment && !flowState.tokens?.accessToken) {
						try {
							...
							logger.warn(`${MODULE_TAG} Failed to parse fragment for hybrid flow`, fragmentErr);
						}
					}
		...
		if (callbackDataStr) {
			try {
				...
				logger.error(`${MODULE_TAG} Failed to parse callback data:`, err);
			}
		}
			...
			const performPoll = async (): Promise<TokenResponse | null> => {
				...
				try {
					...
					if (!currentDeviceCode || currentDeviceCode === '') {
						logger.error(`${MODULE_TAG} Device code is missing or empty`, {
					...
					try {
						...
						if (!response.ok) {
							logger.error(`${MODULE_TAG} ❌ Polling response error (attempt ${pollCount}):`, {
								status: response.status,
								statusText: response.statusText,
						...
						logger.error(`${MODULE_TAG} ❌ Fetch error during polling (attempt ${pollCount}):`, {
							error: fetchError,
							tokenEndpoint,
					...
					try {
						...
						logger.error(`${MODULE_TAG} Failed to parse error response:`, responseText);
					...
					if (errorData.error === 'access_denied') {
						logger.error(`${MODULE_TAG} User denied authorization`, {
							error: errorData.error,
					...
					if (errorData.error === 'expired_token' || errorData.error === 'invalid_grant') {
						logger.error(`${MODULE_TAG} Device code expired or invalid`, {
							error: errorData.error,
							description: errorData.error_description,
						});
						throw new Error(
					...
					logger.error(`${MODULE_TAG} Token polling error`, {
						error: errorData.error,
						description: errorData.error_description,
					});
					throw new Error(
					...
					logger.warn(`${MODULE_TAG} Polling error (attempt ${pollCount}/${maxAttempts})`, error);
								...
								return (
									<DynamicDeviceFlow
										...
										onError={(error: string) => {
											logger.error(`${MODULE_TAG} Device authorization error`, error);
											setError(error);
										}}
									/>
								);
							} catch (error) {
								logger.error(`${MODULE_TAG} Error rendering DynamicDeviceFlow:`, error);
								return (
									<div
										style={{
											padding: '20px',
	...
	const renderStep3ExchangeTokens = () => {
		const handleExchangeTokens = async () => {
			...
			if (!flowState.authorizationCode) {
				logger.error(`${MODULE_TAG} ❌ VALIDATION FAILED: Missing authorization code`, "Logger error");
			...
			if (!credentials.clientId || !credentials.clientId.trim()) {
				logger.error(`${MODULE_TAG} ❌ VALIDATION FAILED: Missing client ID`, "Logger error");
				const errorMsg =
					'Client ID is required for token exchange. Credentials may have been lost. Please check your configuration.';
				setError(errorMsg);
				setValidationErrors([errorMsg]);
				logger.error(`${MODULE_TAG} Missing clientId in credentials:`, {
					credentials,
					hasClientId: !!credentials.clientId,
			...
			if (!credentials.environmentId || !credentials.environmentId.trim()) {
				logger.error(`${MODULE_TAG} ❌ VALIDATION FAILED: Missing environment ID`, "Logger error");
				const errorMsg =
					'Environment ID is required for token exchange. Please check your configuration.';
				setError(errorMsg);
				setValidationErrors([errorMsg]);
				logger.error(`${MODULE_TAG} Missing environmentId in credentials:`, {
					credentials,
					hasEnvironmentId: !!credentials.environmentId,
				});
				return;
			...
			if (!isPKCERequired && (!credentials.redirectUri || !credentials.redirectUri.trim())) {
				...
				setError(errorMsg);
				setValidationErrors([errorMsg]);
				logger.error(`${MODULE_TAG} Missing redirectUri in credentials (PKCE not required):`, {
					credentials,
					usePKCE: isPKCERequired,
					pkceEnforcement: credentials.pkceEnforcement,
					hasRedirectUri: !!credentials.redirectUri,
			...
			try {
				...
				if (credentials.enableRefreshToken && !tokens.refresh_token) {
					logger.warn(`${MODULE_TAG} ⚠️ Refresh token expected but not received`, {
						enableRefreshToken: credentials.enableRefreshToken,
						scopesRequested: credentials.scopes,
				...
				if (
					...
					try {
						...
						logger.error(`${MODULE_TAG} Failed to clear old PKCE codes:`, clearError);
					}
	...
	const handleFetchUserInfo = useCallback(async () => {
		...
		if (!canCallUserInfo) {
			...
			logger.warn(`${MODULE_TAG} ${errorMsg}`, "Logger warning");
			modernMessaging.showBanner({
				type: 'error',
				title: 'Error',
				message: errorMsg,
	...
	const handleIntrospectToken = useCallback(
		async (tokenType: 'access' | 'refresh' | 'id' = selectedTokenType) => {
			...
			if (
				...
				logger.error(`${MODULE_TAG} ${errorMsg}`, {
					environmentId: credentials.environmentId,
					clientId: credentials.clientId,
			...
			try {
				...
				if (!response.ok) {
					...
					logger.error(`${MODULE_TAG} Introspection error response:`, errorText);
					throw new Error(
				...
				logger.error(`${MODULE_TAG} Introspection error:`, err);
	...
	const handleRefreshToken = useCallback(async () => {
		...
		try {
			...
			logger.error(`${MODULE_TAG} ❌ Token refresh failed`, { error: message });
