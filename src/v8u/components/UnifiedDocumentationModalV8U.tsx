...
const SPEC_VERSIONS: SpecVersion[] = ['oauth2', 'oidc', 'oidc2.1'];
...
SPEC_VERSIONS.forEach((specVersion) => {
	FLOW_TYPES.forEach((flowType) => {
		...
		if (specVersion === 'oauth2' && (flowType === 'hybrid' || flowType === 'implicit')) {
			return; // Hybrid and Implicit are OIDC-only
		}
		if (specVersion === 'oidc2.1' && flowType === 'implicit') {
			return; // Implicit is deprecated in OIDC 2.1
		}
		USE_CASES.push({
	...
	if (flowType !== 'client-credentials') {
		baseCalls.push({
			...
			requestBody: {
				...
				scope: specVersion === 'oauth2' ? 'openid' : 'openid profile email',
				state: '{{state}}',
			},
			responseBody: {
...
export const UnifiedDocumentationModalV8U: React.FC<UnifiedDocumentationModalV8UProps> = ({
	...
	const [selectedCategory, setSelectedCategory] = useState<
		'all' | 'oauth2' | 'oidc' | 'oidc2.1' | 'specific'
	>('all');
	...
	const handleCategoryChange = (category: typeof selectedCategory) => {
		...
		} else if (category === 'oauth2' || category === 'oidc' || category === 'oidc2.1') {
			setSelectedUseCases(
				...
				<div style={{ marginBottom: '24px' }}>
					...
					<div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
						{[
							...
							{ value: 'oauth2', label: 'OAuth 2.0 Only' },
							{ value: 'oidc', label: 'OpenID Connect Only' },
							{ value: 'oidc2.1', label: 'OIDC 2.1 Only' },
