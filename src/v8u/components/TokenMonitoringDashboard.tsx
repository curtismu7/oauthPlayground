...
import { FiClock, FiRefreshCw, FiEye, FiEyeOff, FiActivity, FiAlertTriangle, FiCheck, FiX, FiInfo, FiTrash2, FiDownload } from 'react-icons/fi';
    ...
    <DashboardContainer>
      ...
      <TokenGrid>
        {tokenStates.map((token) => (
          <TokenCard key={token.id} $status={token.status}>
            ...
            <TokenDetails>
              <div><strong>Scope:</strong> {token.scope.join(', ') || 'None'}</div>
              {token.issuedAt && (
                <div><strong>Issued:</strong> {new Date(token.issuedAt).toLocaleString()}</div>
              )}
              {token.expiresAt && (
                <div><strong>Expires:</strong> {new Date(token.expiresAt).toLocaleString()}</div>
              )}
      ...
      {tokenStates.length === 0 && (
        <div style={{
          ...
          <FiClock style={{ fontSize: '2rem', marginBottom: '1rem', display: 'block', margin: '0 auto 1rem' }} />
      ...
      <IntrospectionModal $isOpen={showIntrospection}>
        <ModalContent>
          <ModalHeader>
            ...
            <ModalClose onClick={() => setShowIntrospection(false)}>
              ×
            </ModalClose>
          </ModalHeader>
          ...
          {selectedToken && (
            <div>
              <div style={{ marginBottom: '1rem', fontSize: '0.875rem', color: '#64748b' }}>
                <strong>Type:</strong> {selectedToken.type.replace('_', ' ').toUpperCase()}<br />
