		...
		const pkceItems = [
			{
				name: 'PKCE Required',
				status:
					specVersion === 'oauth2.1' && (flowType === 'oauth-authz' || flowType === 'hybrid')
						? ('pass' as const)
			...
			},
			{
				name: 'PKCE Method',
				status:
					flowType === 'implicit' || flowType === 'client-credentials' || flowType === 'device-code'
						? ('pass' as const)
		...
		const flowSecurityItems = [
			{
				...
				status:
					flowType === 'implicit' && specVersion === 'oauth2.1'
						? ('fail' as const)
						: flowType === 'implicit'
							? ('warning' as const)
							: ('pass' as const),
			...
			},
			{
				...
				description:
					specVersion === 'oauth2.1' ? 'OAuth 2.1 requires HTTPS' : 'HTTPS is strongly recommended',
			},
		];
						...
						<SecurityCategories>
							{securityChecks.map((check, index) => (
								<SecurityCategory
									key={index}
									$status={
										check.items.some((item) => item.status === 'fail')
											? 'fail'
											: check.items.some((item) => item.status === 'warning')
												? 'warning'
												: 'pass'
									}
								>
									...
									<CategoryItems>
										{check.items.map((item, itemIndex) => {
											...
											const shouldShowPKCEButton =
												item.name === 'PKCE Method' &&
												(flowType === 'oauth-authz' || flowType === 'hybrid') &&
												onTogglePKCE;

											const shouldShowRefreshTokenButton =
												item.name === 'Refresh Token Usage' && onToggleRefreshToken;
												...
												return (
													<SecurityItemWithAction key={itemIndex}>
														...
														<ActionButton
															...
															onClick={() =>
																onToggleRefreshToken?.(!credentials?.enableRefreshToken)
															}
														>
