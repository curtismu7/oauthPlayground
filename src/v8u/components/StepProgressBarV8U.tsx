...
export const StepProgressBarV8U: React.FC<StepProgressBarProps> = ({
	...
	return (
		<div className={`step-progress-bar-v8 ${className}`}>
			...
			<style>
				` .step-progress-bar-v8 display: flex; flex-direction: column; gap: 12px; padding: 16px;
				background: #f5f5f5; border-radius: 8px; margin-bottom: 24px; .progress-text display: flex;
				gap: 8px; font-size: 14px; font-weight: 500; color: #333; .progress-percentage font-weight:
				600; color: #2196f3; .progress-steps color: #666; .progress-bar-container width: 100%;
				height: 8px; background: #e0e0e0; border-radius: 4px; overflow: hidden; .progress-bar-fill
				height: 100%; background: linear-gradient( 90deg, #2196f3 0%, #1976d2 100% ); border-radius:
				4px; .step-indicators display: flex; justify-content: space-between; gap: 8px;
				.step-indicator flex: 1; display: flex; align-items: center; justify-content: center; width:
				32px; height: 32px; border-radius: 50%; background: #e0e0e0; color: #666; font-size: 12px;
				font-weight: 600; transition: all 0.3s ease; cursor: default; .step-indicator.completed
