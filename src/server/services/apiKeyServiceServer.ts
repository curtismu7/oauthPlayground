...
export class ApiKeyServiceServer {
	private static instance: ApiKeyServiceServer;

	private constructor() {}

	public static getInstance(): ApiKeyServiceServer {
	...
	public async hasApiKey(service: string): Promise<boolean> {
		try {
			...
			if (result.success && result.token) {
				const metadata = result.token.metadata as any;
