...
describe('CSRFProtection', () => {
  ...
  describe('getToken', () => {
    ...
    it('should generate new token if current is invalid', () => {
      ...
      const tokenData = (protection as unknown).tokens.get(token);
      if (tokenData) {
  ...
  describe('token lifecycle', () => {
    it('should clean up expired tokens', () => {
      ...
      const tokenData1 = (protection as unknown).tokens.get(token1);
      if (tokenData1) {
        tokenData1.expiresAt = Date.now() - 1000;
      }

      // Clean up expired tokens
      (protection as unknown).cleanupExpiredTokens();
...
describe('Middleware', () => {
  ...
  describe('csrfValidationMiddleware', () => {
    it('should validate CSRF request', () => {
      ...
      const isValid = csrfValidationMiddleware(request as unknown);
      expect(isValid).toBe(true);
    });
