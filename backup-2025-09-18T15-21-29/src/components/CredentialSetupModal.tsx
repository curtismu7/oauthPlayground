...
const CredentialSetupModal: React.FC<CredentialSetupModalProps> = ({ isOpen, onComplete }) => {
  ...
  const loadFromEnvironmentVariables = async () => {
    try {
      ...
      console.error('❌ [CredentialSetupModal] Failed to load from environment variables:', _error);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      try {
        ...
        console.error('❌ [CredentialSetupModal] Error loading existing credentials:', _error);
      }
    }
  }, [isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Failed to save configuration:', _error);
      setSaveStatus({
        type: 'danger',
