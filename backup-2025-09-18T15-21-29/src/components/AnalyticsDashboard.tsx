...
const MetricIcon = styled.div<{ $metric: string }>`
  color: ${({ $metric, theme }) => {
    switch ($metric) {
      case 'users': return theme.colors.primary;
      case 'performance': return theme.colors.success;
      case 'security': return theme.colors.warning;
      case 'errors': return theme.colors.danger;
...
const mockEvents = [
  { id: '1', type: 'page_view', timestamp: Date.now() - 300000, user: 'user1', details: 'OAuth Flows page' },
  { id: '2', type: 'flow_start', timestamp: Date.now() - 240000, user: 'user2', details: 'Authorization Code flow' },
  { id: '3', type: 'flow_complete', timestamp: Date.now() - 180000, user: 'user1', details: 'Implicit Grant flow' },
  { id: '4', type: 'security_event', timestamp: Date.now() - 120000, user: 'user3', details: 'Suspicious activity detected' },
  { id: '5', type: 'error_event', timestamp: Date.now() - 60000, user: 'user2', details: 'Token validation failed' }
];
...
export const AnalyticsDashboard: React.FC = () => {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      console.error('Failed to refresh analytics:', _error);
      setIsRefreshing(false);
  ...
  const getEventTypeColor = (type: string) => {
    switch (type) {
      ...
      case 'flow_start': return 'info';
      case 'flow_complete': return 'success';
      case 'security_event': return 'warning';
