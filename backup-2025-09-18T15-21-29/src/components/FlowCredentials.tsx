...
const FlowCredentials: React.FC<FlowCredentialsProps> = ({
  ...
  useEffect(() => {
    const loadCredentials = () => {
      try {
        ...
        logger.error('FlowCredentials', 'Failed to load credentials', error instanceof Error ? error.message : String(_error), error instanceof Error ? error : undefined);
      }
    };
  ...
  const hasConfigChanges = useMemo(() => {
    ...
    if (globalConfig) {
      try {
        ...
        console.error('Failed to parse global config:', _error);
      }
    }
  ...
  const handleCopyToClipboard = async (text: string, fieldName: string) => {
    ...
    try {
      ...
      logger.error('FlowCredentials', 'Failed to copy to clipboard', error instanceof Error ? error.message : String(_error), error instanceof Error ? error : undefined);
    }
  };
  ...
  const handleSaveCredentials = () => {
    try {
      ...
      logger.error('FlowCredentials', 'Failed to save credentials', error instanceof Error ? error.message : String(_error), error instanceof Error ? error : undefined);
  ...
  const handleResetCredentials = () => {
    try {
      ...
      logger.error('FlowCredentials', 'Failed to reset credentials', error instanceof Error ? error.message : String(_error), error instanceof Error ? error : undefined);
    }
  };
