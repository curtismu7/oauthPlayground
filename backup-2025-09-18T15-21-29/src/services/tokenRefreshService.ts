...
export class TokenRefreshService {
  ...
  async initialize(config: TokenRefreshConfig): Promise<void> {
    ...
    if (!this.isValidRefreshToken(config.refreshToken)) {
      logger.warn('TokenRefreshService', 'Invalid refresh token provided, skipping auto-refresh initialization', {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 400 || response.status === 401) {
          logger.error('TokenRefreshService', 'Invalid refresh token, stopping auto-refresh and clearing stored tokens', {
            status: response.status,
            refreshTokenLength: this.config?.refreshToken?.length || 0,
            refreshTokenPreview: this.config?.refreshToken ? this.config.refreshToken.substring(0, 10) + '...' : 'none',
          ...
          try {
            ...
            logger.error('TokenRefreshService', 'Failed to clear invalid tokens from storage', error);
          }
        }
  ...
  private async scheduleRefresh(): Promise<void> {
    ...
    try {
      ...
      if (!this.config?.refreshToken || !this.isValidRefreshToken(this.config.refreshToken)) {
        logger.warn('TokenRefreshService', 'No valid refresh token available for scheduling refresh');
        return;
      }
  ...
  getStatus(): {
    ...
    return {
      ...
      nextRefreshAt: this.refreshTimer ? new Date(Date.now() + (this.refreshTimer as unknown)._idleTimeout) : undefined
    };
  }
