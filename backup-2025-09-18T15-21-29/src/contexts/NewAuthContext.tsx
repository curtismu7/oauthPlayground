...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = useCallback(async (redirectAfterLogin = '/', callbackType: 'dashboard' | 'oauth' = 'oauth'): Promise<LoginResult> => {
    ...
    try {
      ...
      console.error('❌ [NewAuthContext] Login error:', _error);
    ...
    try {
      ...
      if (flowContext) {
        try {
          ...
          console.warn('Failed to parse flow context for redirect URI:', _error);
        }
      } else {
      ...
      if (!clientId || !environmentId) {
        ...
        try {
          ...
          console.error('❌ [NewAuthContext] Failed to load fallback credentials:', _error);
        }
      }
      ...
      if (flowContext) {
        try {
          ...
          console.warn('Failed to parse flow context:', _error);
  ...
  const contextValue = useMemo(() => {
    ...
    const pingoneConfig = (config?.pingone as unknown) || (config as unknown) || {};
