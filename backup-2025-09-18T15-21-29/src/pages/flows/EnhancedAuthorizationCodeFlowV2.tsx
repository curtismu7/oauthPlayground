...
const EnhancedAuthorizationCodeFlowV2: React.FC = () => {
  ...
  const handleResetFlow = useCallback(async () => {
    ...
    try {
      ...
      console.error('❌ [EnhancedAuthorizationCodeFlowV2] Error resetting flow:', _error);
  ...
  const handleClearCredentials = useCallback(async () => {
    ...
    try {
      ...
      console.error('❌ [EnhancedAuthorizationCodeFlowV2] Error clearing credentials:', _error);
    } finally {
  ...
  useEffect(() => {
    const loadCredentials = async () => {
      try {
        ...
        console.error('❌ [EnhancedAuthorizationCodeFlowV2] Failed to load credentials:', _error);
        logger.error('EnhancedAuthorizationCodeFlowV2', 'Failed to load credentials', String(_error));
      }
    };
          ...
          if (isFreshCode) {
            setTimeout(async () => {
              try {
                ...
                console.error('❌ [EnhancedAuthorizationCodeFlowV2] Auto token exchange failed:', _error);
              }
    ...
    const handleCredentialChange = () => {
      ...
      timeoutId = setTimeout(() => {
        try {
          ...
          console.error('❌ [EnhancedAuthorizationCodeFlowV2] Failed to reload credentials:', _error);
          logger.error('EnhancedAuthorizationCodeFlowV2', 'Failed to reload credentials', String(_error));
        }
  ...
  const saveCredentials = useCallback(async () => {
    ...
    try {
      ...
      console.error('❌ [EnhancedAuthorizationCodeFlowV2] Failed to save credentials:', _error);
      logger.error('EnhancedAuthorizationCodeFlowV2', 'Failed to save credentials', String(_error));
  ...
  const generatePKCECodes = useCallback(async () => {
    ...
    setPkceCodes({ codeVerifier: verifier, codeChallenge: challenge });
      ...
      console.error('❌ [EnhancedAuthorizationCodeFlowV2] Failed to generate PKCE codes:', _error);
      logger.error('Failed to generate PKCE codes', String(_error));
      throw error;
    }
  }, []);
      ...
      if (popup) {
        ...
        const messageHandler = (event: MessageEvent) => {
          ...
          if (event.data.type === 'oauth-callback') {
            ...
            if (_error) {
              logger.error('Authorization error received', _error);
              setAuthError(_error);
  ...
  const exchangeCodeForTokens = useCallback(async () => {
    ...
    try {
      ...
      logger.error('EnhancedAuthorizationCodeFlowV2', 'Token exchange failed', String(_error));
      setIsExchangingTokens(false);
      
      // Show error message
      updateStepMessage('exchange-tokens', `❌ Token exchange failed: ${error instanceof Error ? error.message : String(_error)}`);
      
      // Provide more specific error messages
      if (String(_error).includes('Invalid Grant')) {
      ...
      } else if (String(_error).includes('Client ID is required')) {
        throw new Error('OAuth credentials are missing. Please go back to Step 1 and save your credentials first.');
      } else if (String(_error).includes('Code verifier is required')) {
  ...
  const getUserInfo = useCallback(async () => {
    ...
    try {
      ...
      logger.error('EnhancedAuthorizationCodeFlowV2', 'UserInfo request failed', String(_error));
      throw error;
    } finally {
  ...
  const copyToClipboard = useCallback(async (text: string) => {
    try {
      ...
      logger.error('EnhancedAuthorizationCodeFlowV2', 'Failed to copy to clipboard', String(_error));
    }
  }, []);
              ...
              <ModalButton 
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    console.error('❌ [EnhancedAuthorizationCodeFlowV2] Error in modal button click:', _error);
