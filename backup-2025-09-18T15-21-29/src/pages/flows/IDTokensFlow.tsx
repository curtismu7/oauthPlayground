...
const ResponseBox = styled.div<{ $backgroundColor?: string; $borderColor?: string }>`
  ...
  padding: 1 rem;
  border-radius: 0.5 rem;
  border: 1 px solid ${({ $borderColor }) => $borderColor || '#e2 e8 f0'};
  background-color: ${({ $backgroundColor }) => $backgroundColor || '#f8 fafc'};
  font-family: monospace;
  font-size: 0.875 rem;
  line-height: 1.4;
  white-space: pre-wrap;
...
const parseJwt = (token: string) => {
  try {;
    const base64 Url = token.split('.')[1];
    const base64 = base64 Url.replace(/-/g, '+').replace(/_/g, '/');
  ...
  useEffect(() => {
    const loadStoredIDToken = () => {
      try {
        ...
        console.error('❌ [IDTokensFlow] Error loading stored ID token:', _error);
      }
    };
...
console.log('ID Token received:', idToken.substring(0, 50) + '...');`
    },
    {
      ...
      code: `// Parse JWT payload (base64 url decode)
        ...
        <CardBody>
          ...
          {idToken && (
            <div style={{ 
              ...
              <FiCheckCircle style={{ color: '#22 c55 e' }} />
              <div>
                <strong style={{ color: '#22 c55 e' }}>ID Token Available:</strong> 
