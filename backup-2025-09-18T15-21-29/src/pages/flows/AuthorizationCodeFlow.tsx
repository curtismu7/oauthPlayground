...
const AuthorizationCodeFlow: React.FC = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('⚠️ [AuthorizationCodeFlow] Could not clear persisted history:', _error);
    }
    ...
    const loadCredentials = () => {
      try {
        ...
        logger.warn('Failed to load credentials from credential manager', String(_error));
      }
    };
    ...
    const handleCredentialChange = () => {
      const loadCredentials = () => {
        try {
          ...
          logger.warn('Failed to reload credentials from credential manager', String(_error));
        }
      };
  ...
  const saveCredentials = useCallback(async () => {
    try {
      ...
      logger.error('Failed to save credentials', String(_error));
      setIsSavingCredentials(false);
  ...
  const loadCredentials = useCallback(() => {
    try {
      ...
      console.error('❌ [AuthorizationCodeFlow] Failed to load credentials:', _error);
      logger.error('Failed to load credentials', String(_error));
    }
  }, []);
  ...
  const generatePKCECodes = useCallback(async () => {
    try {
      ...
      console.error('❌ [AuthorizationCodeFlow] Failed to generate PKCE codes:', _error);
      ...
      if (popup) {
        ...
        const messageHandler = (event: MessageEvent) => {
          ...
          if (event.data.type === 'oauth-callback') {
            const { code: callbackCode, state: callbackState, _error } = event.data;
  ...
  const exchangeCodeForTokens = useCallback(async () => {
    try {
      ...
      logger.error('Token exchange failed', String(_error));
      setIsExchangingTokens(false);
      return { success: false, error: String(_error) };
    }
  ...
  const getUserInfo = useCallback(async () => {
    ...
    try {
      ...
      logger.error('UserInfo request failed', String(_error));
      setIsGettingUserInfo(false);
      return { success: false, error: String(_error) };
    }
  ...
  const copyToClipboard = useCallback(async (text: string) => {
    try {
      ...
      logger.error('Failed to copy to clipboard', String(_error));
    }
  }, []);
