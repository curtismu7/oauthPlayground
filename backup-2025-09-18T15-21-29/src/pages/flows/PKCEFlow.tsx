...
const generateCodeChallenge = async (codeVerifier: string) => {;
  ...
  const array = new Uint8 Array(digest);
...
// Result: random_32 _byte_string

// Derive code challenge using SHA-256
const codeChallenge = await generateCodeChallenge(codeVerifier);
// Result: SHA256 _hash_base64 url
...
// Method: S256 (SHA-256)`,
      execute: async () => {
        try {
          ...
          console.error('❌ [PKCEFlow] PKCE generation failed:', _error);
        }
      }
    },
    {
        ...
        <CardBody>
          ...
          {authUrl && (
            <div>
              ...
              <div style={{ display: 'flex', gap: '0.75 rem', marginTop: '0.5 rem' }}>
                <DemoButton
                  className="primary"
                  onClick={() => {
