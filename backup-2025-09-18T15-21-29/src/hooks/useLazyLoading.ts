...
export interface UseLazyLoadingConfig {
  ...
  retryDelay?: number;
  onLoadStart?: () => void;
  onLoadComplete?: (component: React.ComponentType<unknown>) => void;
...
export const useLazyLoading = (config: UseLazyLoadingConfig) => {
  ...
  const loadComponent = useCallback(async () => {
    ...
    try {
      ...
      logger.error(`[useLazyLoading] Failed to load ${flowType}:`, _error);
    }
  ...
  const preload = useCallback(async () => {
    ...
    try {
      ...
      logger.warn(`[useLazyLoading] Preload failed for ${flowType}:`, _error);
    }
  }, [config]);
...
export const usePreloadFlows = (flowTypes: string[]) => {
  ...
  const preloadFlows = useCallback(async () => {
    ...
    try {
      ...
      logger.error('[usePreloadFlows] Preload failed:', _error);
    } finally {
