...
export const useFlowAnalysis = (config: UseFlowAnalysisConfig = {}) => {
  ...
  const analyzeFlows = useCallback(async (flowTypes: string[]) => {
  ...
  const getRecommendations = useCallback(async (requirements: {
    ...
    try {
      ...
      logger.info('[useFlowAnalysis] Recommendations generated', { requirements, recommendations });
    } catch (_error) {
      const err = error as Error;
      setState(prev => ({
        ...prev,
  ...
  const getFlowDetails = useCallback((flowType: string) => {
    try {
      ...
      logger.error('[useFlowAnalysis] Failed to get flow details:', _error);
      return null;
    }
  }, []);
  ...
  const shareAnalysis = useCallback(async () => {
    ...
    if (navigator.share) {
      try {
        ...
        logger.error('[useFlowAnalysis] Failed to share analysis:', _error);
      }
    } else {
...
export const useFlowRecommendations = (requirements: {
  ...
  const generateRecommendations = useCallback(async () => {
    ...
    try {
      ...
      setError(_error);
      logger.error('[useFlowRecommendations] Failed to generate recommendations:', _error);
    } finally {
      setIsLoading(false);
    }
  }, [requirements]);
