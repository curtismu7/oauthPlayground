...
export const useErrorDiagnosis = (config: UseErrorDiagnosisConfig = {}) => {
  ...
  // Diagnose an error
  const diagnoseErrorValue = useCallback(async (
    errorMessage: string,
    errorCode?: string,
  ...
  // Resolve an error
  const resolveErrorValue = useCallback(async (
    errorId: string,
    ...
    try {
      ...
      if (debug) {
        logger.error('[useErrorDiagnosis] Error resolution failed:', _error);
      }
      throw error;
    }
  }, [enabled, debug]);
  ...
  const getErrorPatternsByCategory = useCallback((category: string) => {
    return state.errorPatterns.filter(pattern => pattern.category === category);
  }, [state.errorPatterns]);

  // Get error patterns by severity
  const getErrorPatternsBySeverity = useCallback((severity: string) => {
  ...
  const getHighSeverityErrors = useCallback(() => {
    return state.errorHistory.errors.filter(error => 
