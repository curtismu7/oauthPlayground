...
export const useUserBehaviorTracking = (config: UseUserBehaviorTrackingConfig = {}) => {
  ...
  // Track page view
  const handlePageView = useCallback((page: string, referrer?: string) => {
  ...
  const handleUserAction = useCallback((action: string, element?: string, properties?: Record<string, unknown>) => {
    if (enabled && trackUserActions) {
      ...
      if (debug) {
        logger.info('[useUserBehaviorTracking] User action tracked:', { action, element, properties });
      }
    }
  }, [enabled, trackUserActions, debug]);

  // Start flow tracking
  const startFlowTracking = useCallback((flowType: string, properties?: Record<string, unknown>) => {
  ...
  // Track flow step
  const trackFlowStepTracking = useCallback((flowType: string, stepName: string, properties?: Record<string, unknown>) => {
    if (!enabled || !trackFlows) return '';
  ...
  // Complete flow step
  const completeFlowStepTracking = useCallback((flowType: string, stepName: string, success: boolean, errorMessage?: string) => {
    if (!enabled || !trackFlows) return;
    ...
    if (stepId) {
      ...
      if (debug) {
        logger.info('[useUserBehaviorTracking] Flow step completed:', { flowType, stepName, success });
      }
    }
  }, [enabled, trackFlows, debug]);

  // Complete flow
  const completeFlowTracking = useCallback((flowType: string, success: boolean, errorMessage?: string) => {
    if (!enabled || !trackFlows) return;
  ...
  const getFlowCompletionRate = useCallback(() => {
    ...
    const completedFlows = state.currentSession?.flowInteractions.filter(f => f.success).length || 0;
  ...
  const getAverageStepDuration = useCallback((flowType?: string) => {
  ...
  // Start new session
  const startNewSession = useCallback((newUserId?: string) => {
    if (enabled) {
  ...
  const updateConfig = useCallback((newConfig: Partial<UseUserBehaviorTrackingConfig>) => {
    userBehaviorTracker.updateConfig({
...
export const useUserActionTracking = (enabled: boolean = true) => {
  ...
  const trackFocus = useCallback((element: string, properties?: Record<string, unknown>) => {
    trackUserAction('focus', element, properties);
  }, [trackUserAction]);

  const trackBlur = useCallback((element: string, properties?: Record<string, unknown>) => {
    trackUserAction('blur', element, properties);
  }, [trackUserAction]);

  const trackHover = useCallback((element: string, properties?: Record<string, unknown>) => {
    trackUserAction('hover', element, properties);
  }, [trackUserAction]);

  const trackScroll = useCallback((scrollDepth: number, properties?: Record<string, unknown>) => {
