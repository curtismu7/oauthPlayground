...
export const handleOAuthFlowError = (error: unknown, flowType: string, context: string): OAuthFlowError => {
  logger.error(`[${flowType}] ${context}:`, _error);
...
export const storeOAuthTokensSafely = async (
  ...
  try {
    ...
    logger.error(`[${flowType}] Error storing tokens for ${flowName}:`, _error);
    return false;
  }
};
...
export const logOAuthFlowEvent = (
  event: string,
  flowType: string,
  details?: unknown
): void => {
...
export const getOAuthFlowDescription = (flowType: string): string => {
  const descriptions: Record<string, string> = {
    'authorization-code': 'The most secure OAuth 2.0 flow, suitable for server-side applications with the ability to securely store client credentials.',
    'implicit': 'A simplified flow for client-side applications, but less secure due to tokens being exposed in the URL.',
    'client-credentials': 'Used for server-to-server authentication where the client acts on its own behalf.',
    'device-code': 'Designed for devices with limited input capabilities, such as smart TVs or IoT devices.',
    'pkce': 'An extension to the authorization code flow that adds security for public clients.',
    'hybrid': 'A combination of authorization code and implicit flows, providing both access tokens and ID tokens.',
