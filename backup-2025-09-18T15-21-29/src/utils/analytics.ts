...
export class AnalyticsManager {
  ...
  private initialize(): void {
    ...
    try {
      ...
      logger.error('[AnalyticsManager] Failed to initialize analytics:', _error);
    }
  }
  ...
  public track(eventType: AnalyticsEventType, properties: Record<string, unknown> = {}): void {
    ...
    try {
      ...
      logger.error('[AnalyticsManager] Failed to track event:', _error);
    }
  }
  ...
  // Track flow complete
  public trackFlowComplete(flowType: string, success: boolean, properties: Record<string, unknown> = {}): void {
  ...
  // Track flow error
  public trackFlowError(flowType: string, error: string, properties: Record<string, unknown> = {}): void {
  ...
  public trackPerformanceMetric(metric: string, value: number, properties: Record<string, unknown> = {}): void {
  ...
  public trackSecurityEvent(event: string, severity: 'low' | 'medium' | 'high' | 'critical', properties: Record<string, unknown> = {}): void {
  ...
  public async flush(): Promise<void> {
    ...
    try {
      ...
      logger.error('[AnalyticsManager] Failed to flush events:', _error);
  ...
  private setupPerformanceMonitoring(): void {
    ...
    window.addEventListener('load', () => {
      ...
      this.trackPerformanceMetric('dom_content_loaded', navigation.domContentLoadedEventEnd - navigation.domContentLoadedEventStart);
      this.trackPerformanceMetric('first_paint', performance.getEntriesByType('paint')[0]?.startTime || 0);
    });
...
export const trackFlowComplete = (flowType: string, success: boolean, properties?: Record<string, unknown>) => {
  analyticsManager.trackFlowComplete(flowType, success, properties);
};

export const trackFlowError = (flowType: string, error: string, properties?: Record<string, unknown>) => {
...
export const trackPerformanceMetric = (metric: string, value: number, properties?: Record<string, unknown>) => {
  analyticsManager.trackPerformanceMetric(metric, value, properties);
};

export const trackSecurityEvent = (event: string, severity: 'low' | 'medium' | 'high' | 'critical', properties?: Record<string, unknown>) => {
...
export const trackError = (error: Error, context?: string, properties?: Record<string, unknown>) => {
  analyticsManager.trackError(_error, context, properties);
};
