...
export interface TokenSource {
  source: 'login' | 'auth-code-flow' | 'implicit-flow' | 'pkce-flow' | 'client-credentials' | 'device-code' | 'userinfo' | 'manual';
  timestamp: number;
  description: string;
  tokens: {
    access_token?: string;
...
class TokenSourceTracker {
  ...
  storeTokenSource(source: Omit<TokenSource, 'timestamp'>): void {
    try {
      ...
      console.error('❌ [TokenSourceTracker] Error storing token source:', _error);
    }
  }
  ...
  getTokenSources(): TokenSource[] {
    try {
      ...
      console.error('❌ [TokenSourceTracker] Error getting token sources:', _error);
      return [];
    }
  }
  ...
  clearTokenSources(): void {
    try {
      ...
      console.error('❌ [TokenSourceTracker] Error clearing token sources:', _error);
    }
  }
