...
import { jwtVerify,
  createRemoteJWKSet,
  JWTVerifyOptions,
  type JWTHeaderParameters } from 'jose';
...
export const createSignedRequestObject = async (
  ...
  try {
    ...
    console.error('❌ [OAuth] Error creating signed request object:', _error);
...
export const pushAuthorizationRequest = async ({
  ...
  try {
    ...
    console.error('❌ [OAuth] Error in PAR request:', _error);
    const errorMessage = error instanceof Error ? error.message : String(_error);
  ...
  try {
    ...
    if (accessToken && payload.at_hash) {
      try {
        ...
        console.error('❌ [OIDC] at_hash validation error:', _error);
        throw new Error(`at_hash validation failed: ${error instanceof Error ? error.message : String(_error)}`);
      }
    ...
    console.error('❌ [OAuth] Error validating ID token:', _error);
    const errorMessage = error instanceof Error ? error.message : String(_error);
