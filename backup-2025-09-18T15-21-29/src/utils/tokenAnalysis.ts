...
export interface SecurityIssue {
  type: 'weak_algorithm' | 'missing_claims' | 'expired_token' | 'invalid_signature' | 'weak_key' | 'suspicious_claims' | 'token_reuse' | 'insecure_transport';
...
export class TokenAnalyzer {
  ...
  public analyzeToken(token: string): TokenAnalysisResult {
    try {
      ...
      logger.error('[TokenAnalyzer] Token analysis failed:', _error);
      return {
        isValid: false,
        tokenType: 'unknown',
        format: 'unknown',
  ...
  public getTokenRefreshInfo(token: string, refreshToken?: string): TokenRefreshInfo {
    ...
    // In a real implementation, this would check the token's refresh capabilities
    if (refreshToken) {
      refreshInfo.refreshEndpoint = '/oauth/token'; // Default endpoint
    }
  ...
  public performSecurityAnalysis(token: string): TokenSecurityAnalysis {
    ...
    const securityAnalysis: TokenSecurityAnalysis = {
      overallScore: 100 - analysis.riskScore,
      algorithmSecurity: this.analyzeAlgorithmSecurity(analysis),
      keySecurity: this.analyzeKeySecurity(analysis),
      claimSecurity: this.analyzeClaimSecurity(analysis),
      transportSecurity: this.analyzeTransportSecurity(analysis),
      lifecycleSecurity: this.analyzeLifecycleSecurity(analysis),
  ...
  private analyzeAlgorithmSecurity(analysis: TokenAnalysisResult): number {
    const algorithmIssue = analysis.securityIssues.find(issue => issue.type === 'weak_algorithm');
    if (algorithmIssue) {
      return algorithmIssue.severity === 'critical' ? 0 : algorithmIssue.severity === 'high' ? 25 : 50;
    }
    return 100;
  }
  ...
  private analyzeKeySecurity(analysis: TokenAnalysisResult): number {
    const keyIssues = analysis.securityIssues.filter(issue => 
      issue.type === 'weak_key' || issue.type === 'invalid_signature'
    );
  ...
  private analyzeClaimSecurity(analysis: TokenAnalysisResult): number {
    const claimIssues = analysis.securityIssues.filter(issue => 
      issue.type === 'missing_claims' || issue.type === 'suspicious_claims'
    );
  ...
  private analyzeLifecycleSecurity(analysis: TokenAnalysisResult): number {
    ...
    const lifecycleIssues = analysis.securityIssues.filter(issue => 
      issue.type === 'expired_token' || issue.type === 'token_reuse'
    );
