...
class PersistentCredentialsManager {
  ...
  private isGlobalConfigEnabled(): boolean {
    try {
      ...
      console.log('🔧 [PersistentCredentials] Could not check global config setting:', _error);
    }
    return false;
  }
  ...
  saveGlobalCredentials(credentials: FlowCredentials): boolean {
    try {
      ...
      const configToSave = {
        ...
        scopes: Array.isArray(credentials.scopes) ? credentials.scopes.join(' ') : credentials.scopes,
...
export const loadFlowState = (flowType: string) => 
  persistentCredentials.loadFlowState(flowType);

export const clearAllCredentials = () => 
  persistentCredentials.clearAll();
