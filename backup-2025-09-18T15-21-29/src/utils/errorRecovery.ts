...
export class EnhancedErrorRecovery {
  ...
  async handleError(
    ...
    const errorInfo = this.categorizeError(_error);
  ...
  private categorizeError(error: unknown): {
    ...
    const errorStr = String(_error);
    const errorMessage = error instanceof Error ? error.message : errorStr;

    // Network errors
    if (errorStr.includes('fetch') || errorStr.includes('NetworkError') || errorStr.includes('ERR_NETWORK')) {
      return {
        type: 'network',
  ...
  getErrorStats(): {
    ...
    errorHistory.forEach((error: unknown) => {
