...
export const VALIDATION_MESSAGES = {
  ...
  CODE_VERIFIER: 'Code verifier must be 43-128 characters and contain only A-Z, a-z, 0-9, -, ., _, ~',
  CODE_CHALLENGE: 'Code challenge must be 43-128 characters and contain only A-Z, a-z, 0-9, -, ., _, ~'
};
  ...
  validator.addRules([
    {
      ...
      rules: [
        { type: 'required' },
        { type: 'pattern', value: VALIDATION_PATTERNS.CLIENT_ID, message: VALIDATION_MESSAGES.CLIENT_ID }
      ]
    },
    {
      field: 'redirectUri',
      rules: [
        { type: 'required' },
        { type: 'url', message: VALIDATION_MESSAGES.URL }
      ]
    },
    {
      field: 'scope',
    ...
    },
    {
      ...
      rules: [
        { type: 'required' },
        { type: 'pattern', value: VALIDATION_PATTERNS.ENVIRONMENT_ID, message: VALIDATION_MESSAGES.ENVIRONMENT_ID }
      ]
    }
  ]);
  ...
  validator.addRules([
    {
      ...
      rules: [
        ...
        { type: 'custom', validator: (value) => 
          ['authorization_code', 'client_credentials', 'refresh_token', 'device_code'].includes(value)
        }
      ]
    },
    {
...
export const validateAuthorizationRequest = (request: Record<string, unknown>): ValidationResult => {
  ...
  validator.addRules([
    ...
    },
    {
      ...
      rules: [
        { type: 'required' },
        { type: 'custom', validator: (value) => 
          ['code', 'token', 'id_token', 'code token', 'code id_token', 'token id_token', 'code token id_token'].includes(value)
        }
      ]
    },
    {
  ...
  if (request.code_challenge) {
    validator.addRule({
      ...
      rules: [
        { type: 'pattern', value: VALIDATION_PATTERNS.CODE_CHALLENGE, message: VALIDATION_MESSAGES.CODE_CHALLENGE }
      ]
    });
  }
...
export const sanitizeUrl = (url: string): string => {
  ...
  try {
    ...
    errorHandler.handleError(_error, 'URL sanitization');
    return '';
  }
};
...
export const validateWithErrorHandling = <T>(
  ...
  try {
    ...
    errorHandler.handleError(_error, `Validation error in ${context}`);
    return {
      isValid: false,
      errors: [{
        field: 'validation',
