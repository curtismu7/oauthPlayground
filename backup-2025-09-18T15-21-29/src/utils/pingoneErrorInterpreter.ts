...
export interface InterpretedError {
  ...
  category: 'configuration' | 'authentication' | 'authorization' | 'network' | 'validation' | 'unknown';
}
...
export class PingOneErrorInterpreter {
  private static readonly ERROR_PATTERNS = {
    ...
    'access_denied': {
      ...
      suggestion: 'The user chose not to authorize your application. They can try again and accept the permissions.',
    ...
    'server_error': {
      ...
      suggestion: 'Please try again in a few moments. If the problem persists, contact PingOne support.',
    ...
    'invalid_code_verifier': {
      ...
      suggestion: 'Ensure the code verifier matches the code challenge used in the authorization request.',
      category: 'validation' as const,
      severity: 'error' as const
    },
  ...
  static interpret(error: PingOneError | unknown): InterpretedError {
    ...
    if (description.includes('pkce') || description.includes('code challenge') || description.includes('code verifier')) {
      return {
    ...
    if (description.includes('client_id') || description.includes('client id') || description.includes('client secret')) {
      return {
    ...
    if (description.includes('timeout') || description.includes('connection') || description.includes('network')) {
      return {
        title: 'Network Error',
        message: 'Unable to connect to PingOne servers.',
        suggestion: 'Check your internet connection and try again. If the problem persists, PingOne may be experiencing issues.',
        technicalDetails: `Error: ${errorCode} - ${errorDescription}`,
        severity: 'error',
        category: 'network'
      };
    ...
    const pattern = this.ERROR_PATTERNS[errorCode as keyof typeof this.ERROR_PATTERNS];
    if (pattern) {
      return {
        ...pattern,
        technicalDetails: `Error: ${errorCode} - ${errorDescription}`,
        suggestion: pattern.suggestion.replace('{redirect_uri}', errorDetails.redirect_uri || 'your redirect URI')
      };
    }
...
export const interpretPingOneError = (error: string | PingOneError): InterpretedError => {
  return PingOneErrorInterpreter.interpret(_error);
};
