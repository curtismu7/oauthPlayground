...
export class CSRFProtection {
  ...
  validateRequest(request: Request | Record<string, unknown>): boolean {
    ...
    if (this.config.enableHeader && request.headers) {
      token = request.headers[this.config.headerName.toLowerCase()] || 
...
export const validateCSRFRequest = (request: Request | Record<string, unknown>): boolean => {
...
export const csrfValidationMiddleware = (request: Request): boolean => {
  try {
    ...
    errorHandler.handleError(_error, 'CSRF validation');
    return false;
  }
};
