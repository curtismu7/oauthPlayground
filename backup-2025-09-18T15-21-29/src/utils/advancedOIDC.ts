...
export const fetchOIDCDiscovery = async (issuer: string): Promise<OIDCDiscoveryDocument> => {
  ...
  try {
    ...
    console.error('❌ [AdvancedOIDC] Discovery failed:', _error);
    throw new Error(`Failed to fetch OIDC discovery document: ${error instanceof Error ? error.message : String(_error)}`);
  }
};
...
export const createRequestObject = async (
  ...
  try {
    if (algorithm === 'HS256') {
      ...
      return await new SignJWT(requestObject as unknown)
    ...
    throw new Error(`Failed to create Request Object: ${error instanceof Error ? error.message : String(_error)}`);
  }
};
...
export const checkSessionStatus = async (
  ...
  try {
    ...
    console.error('❌ [AdvancedOIDC] Session check failed:', _error);
    return 'error';
  }
};
