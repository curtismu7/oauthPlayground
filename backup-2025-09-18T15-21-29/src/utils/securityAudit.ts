...
class SecurityAuditor {
      ...
      if (tokens) {
        ...
        if (typeof window !== 'undefined' && window.localStorage) {
          ...
          const tokenKeys = localStorageKeys.filter(key => 
            key.includes('token') || key.includes('auth') || key.includes('oauth')
          );
          ...
          if (tokenKeys.length > 0) {
            vulnerabilities.push({
              ...
              description: 'OAuth tokens are stored in browser localStorage, which can be accessed by any script on the domain',
              recommendation: 'Consider using httpOnly cookies or secure session storage for sensitive tokens',
        ...
        if (tokens.timestamp && tokens.expires_in) {
          ...
          if (timeUntilExpiry < 0) {
            ...
            vulnerabilities.push({
              ...
              description: 'Tokens will expire within 5 minutes, which may cause authentication failures',
              recommendation: 'Implement token refresh mechanism or warn users about impending expiration',
              affectedTokens: [tokens.access_token?.substring(0, 8) + '...']
            });
          }
        }
  ...
  private auditTokenTransmission(): SecurityVulnerability[] {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (!isHttps) {
        vulnerabilities.push({
          ...
          description: 'Application is running over HTTP, making token transmission vulnerable to interception',
    ...
    if (typeof document !== 'undefined') {
      ...
      const authCookies = cookies.filter(cookie => 
        cookie.includes('token') || cookie.includes('auth') || cookie.includes('oauth')
      );
  ...
  private auditConfigurationSecurity(): SecurityVulnerability[] {
    ...
    if (typeof window !== 'undefined' && window.localStorage) {
      const localStorageKeys = Object.keys(window.localStorage);
      const secretKeys = localStorageKeys.filter(key => 
  ...
  private auditTokenLifecycle(): SecurityVulnerability[] {
    ...
    try {
      ...
      if (analytics.averageTokenLifetime > 1440) { // More than 24 hours
        vulnerabilities.push({
  ...
  private storeAuditReport(report: SecurityAuditReport): void {
    try {
      ...
      console.error('Failed to store audit report:', _error);
    }
  }
  ...
  getAuditHistory(): SecurityAuditReport[] {
    try {
      ...
      console.error('Failed to get audit history:', _error);
      return [];
    }
  }
}
