...
const TokenStatus = styled.div`
  ...
  gap: 0.5rem;
  padding: 0.5rem 1rem;
  background: ${({ $hasTokens }: { $hasTokens: boolean }) => ($hasTokens ? '#f0fdf4' : '#fef2f2')};
  ...
  svg {
    color: ${({ $hasTokens }: { $hasTokens: boolean }) => ($hasTokens ? '#16a34a' : '#dc2626')};
  }
  
  span {
    font-size: 0.875rem;
    font-weight: 500;
    color: ${({ $hasTokens }: { $hasTokens: boolean }) => ($hasTokens ? '#166534' : '#991b1b')};
  }
`;
	...
	const features = [
		{
			...
			description:
				'Interactive demonstrations of Authorization Code, PKCE, Client Credentials, Device Code, and more.',
			link: '/flows',
		},
		{
			icon: <FiShield />,
			...
			description:
				'Comprehensive token operations including exchange, refresh, introspection, and revocation.',
					...
					return (
						...
						>
							...
							>
								...
								>
									{errorInfo.severity === 'error' ? (
										<FiSettings style={{ fontSize: '1.25rem', color: '#dc2626' }} />
									) : errorInfo.severity === 'warning' ? (
										<FiInfo style={{ fontSize: '1.25rem', color: '#d97706' }} />
									) : (
				...
				<CardBody>
					...
					>
						...
						<div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
							...
							<IconButton onClick={handleActivity} style={{ color: '#3b82f6' }}>
						...
						>
							...
							<div
								style={{
									marginTop: '0.75rem',
									padding: '0.75rem',
									backgroundColor: refreshStatus.isInitialized ? '#f0f9ff' : '#fef3c7',
									borderRadius: '0.375rem',
									border: `1px solid ${refreshStatus.isInitialized ? '#bfdbfe' : '#fbbf24'}`,
								}}
							>
								<h4
									style={{
										...
										fontSize: '0.875rem',
										fontWeight: '600',
										color: refreshStatus.isInitialized ? '#1e40af' : '#92400e',
										display: 'flex',
										alignItems: 'center',
										gap: '0.5rem',
									}}
								...
								{refreshStatus.isInitialized ? (
											...
											<div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
												<div
													style={{
														...
														height: '6px',
														borderRadius: '50%',
														backgroundColor: refreshStatus.autoRefreshEnabled
															? '#10b981'
															: '#6b7280',
													}}
												/>
												<span>
													Auto Refresh: {refreshStatus.autoRefreshEnabled ? 'Enabled' : 'Disabled'}
												</span>
											</div>
											{isTokenRefreshing && (
												<div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
												...
												onClick={async () => {
													try {
														...
														if (result.success) {
															...
															v4ToastManager.showError(`Failed to clear tokens: ${result.error}`);
														}
													} catch (error) {
														console.error(' [Dashboard] Failed to clear tokens:', error);
														v4ToastManager.showError(`Failed to clear tokens: ${error}`);
													}
												}}
												style={{
													padding: '0.25rem 0.5rem',
									...
									<div style={{ fontSize: '0.75rem', color: '#92400e' }}>
										...
										<p style={{ margin: '0', fontSize: '0.7rem', color: '#a16207' }}>
											Token refresh requires a valid refresh token. Complete an OAuth flow that
											provides refresh tokens to enable automatic token refresh.
										</p>
									</div>
								)}
							</div>
				...
				<CardBody>
					...
					<ol style={{ listStyle: 'decimal', paddingLeft: '1.25rem', lineHeight: '1.75' }}>
						<li style={{ marginBottom: '0.5rem' }}>
							Configure your PingOne environment in the Settings page
						</li>
						<li style={{ marginBottom: '0.5rem' }}>
							Use Quick Actions above to start common OAuth flows
						</li>
						<li style={{ marginBottom: '0.5rem' }}>
							Follow the interactive guide to understand each step
						</li>
							...
							{recentActivity.length > 0 ? (
								recentActivity.map((item, index) => (
									<div key={index} className="activity-item">
										...
										<div className="activity-content">
											<div className="activity-type">{item.type || 'Unknown Activity'}</div>
