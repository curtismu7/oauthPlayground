...
async function retryWithBackoff(fn, maxRetries = 3) {
  for (let i = 0; i < maxRetries; i++) {
	...
	private calculateConfidenceAndSeverity(diagnosis: ErrorDiagnosis): void {
		...
		diagnosis.severity = Object.keys(severityOrder)[maxSeverity - 1] as any;
	...
	private calculateStatistics(): ErrorStatistics {
		...
		const successRate =
			resolvedErrors.length > 0 ? (successfulResolutions.length / resolvedErrors.length) * 100 : 0;
		...
		const previousErrors = this.errorHistory.filter(
			(e) => e.timestamp >= twoWeeksAgo && e.timestamp < lastWeek
		).length;
	...
	private calculateTrends(): ErrorTrend[] {
		...
		for (let i = 29; i >= 0; i--) {
			...
			const dayErrors = this.errorHistory.filter(
