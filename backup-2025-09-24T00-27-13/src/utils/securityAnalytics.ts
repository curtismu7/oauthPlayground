...
export class SecurityAnalyticsManager {
  ...
  private calculateRiskScore(type: SecurityEventType, severity: SecuritySeverity, details: Record<string, any>): number {
    const baseScores = {
      low: 25,
      medium: 50,
      high: 75,
...
export const trackAuthenticationFailure = (userId: string, reason: string, details?: Record<string, any>) => {
  return securityAnalyticsManager.trackAuthenticationFailure(userId, reason, details);
};

export const trackAuthorizationFailure = (userId: string, resource: string, action: string, details?: Record<string, any>) => {
  return securityAnalyticsManager.trackAuthorizationFailure(userId, resource, action, details);
};

export const trackTokenValidationFailure = (tokenType: string, reason: string, details?: Record<string, any>) => {
  return securityAnalyticsManager.trackTokenValidationFailure(tokenType, reason, details);
};

export const trackSuspiciousActivity = (activity: string, severity: SecuritySeverity, details?: Record<string, any>) => {
  return securityAnalyticsManager.trackSuspiciousActivity(activity, severity, details);
};

export const trackComplianceViolation = (standard: ComplianceStandard, requirement: string, violation: string, details?: Record<string, any>) => {
