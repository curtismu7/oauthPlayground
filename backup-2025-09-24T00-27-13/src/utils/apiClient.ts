...
export async function makeApiRequest<T = any>(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`API request failed: ${response.status} ${response.statusText}. ${errorData.detail || errorData.message || 'Unknown error'}`);
    }
...
export async function discoverWorkerApp(
  ...
  try {
    ...
    const workerApp = applications.find(app => 
