...
describe('OIDCHybridFlowV3', () => {
  ...
  it('renders the hybrid flow component', () => {
    ...
    expect(screen.getByText(/OIDC 1.0 Hybrid Flow implementation/)).toBeInTheDocument();
  });
  ...
  it('has default values for required fields', () => {
    ...
    const responseTypeSelect = screen.getByLabelText(/Response Type/i) as HTMLSelectElement;
  ...
  it('displays educational content when expanded', () => {
    ...
    const educationalToggle = screen.getByText('OIDC Hybrid Flow Educational Content');
  ...
  it('shows response type options', () => {
    ...
    expect(responseTypeSelect.querySelector('option[value="code id_token"]')).toBeInTheDocument();
    expect(responseTypeSelect.querySelector('option[value="code token"]')).toBeInTheDocument();
    expect(responseTypeSelect.querySelector('option[value="code id_token token"]')).toBeInTheDocument();
  });
});
...
describe('OIDCHybridFlowV3 Integration', () => {
  it('handles form submission with valid data', async () => {
    ...
    const authEndpointInput = screen.getByLabelText(/Authorization Endpoint/i) as HTMLInputElement;
    const clientIdInput = screen.getByLabelText(/Client ID/i) as HTMLInputElement;
    
    fireEvent.change(authEndpointInput, { target: { value: 'https://auth.pingone.com/test/as/authorize' } });
    fireEvent.change(clientIdInput, { target: { value: 'test-client-id' } });
