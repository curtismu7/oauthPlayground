...
function resolveEnvironmentId(value?: string): string {
  ...
  if (!envId) {
    throw new MfaApiError('PingOne environment ID is not configured.', {
      ...
      description: 'Set PINGONE_ENVIRONMENT_ID (or VITE_PINGONE_ENVIRONMENT_ID) in the environment.',
    });
  }
  return envId;
}
...
export async function listDevices(credentials: MfaCredentials): Promise<MfaDevice[]> {
  try {
    ...
    const response = await axios.get(
      `${baseUrl}/users/${credentials.userId}/devices`,
      {
...
export async function registerDevice({ credentials, type, phoneNumber, emailAddress, nickname, deviceName }: RegisterDeviceRequest): Promise<RegisterDeviceResult> {
  try {
    const environmentId = resolveEnvironmentId(credentials.environmentId);
    const baseUrl = buildApiBaseUrl(environmentId);
...
export async function activateDevice({ credentials, deviceId, otp }: ActivateDeviceRequest): Promise<void> {
  try {
    const environmentId = resolveEnvironmentId(credentials.environmentId);
    const baseUrl = buildApiBaseUrl(environmentId);
...
export async function sendChallenge({ credentials, deviceId, method }: ChallengeRequest): Promise<MfaChallengeResponse> {
  try {
    const environmentId = resolveEnvironmentId(credentials.environmentId);
    const baseUrl = buildApiBaseUrl(environmentId);
...
export async function validateChallenge({ credentials, challengeId, code }: ValidateChallengeRequest): Promise<MfaChallengeResponse> {
  try {
    const environmentId = resolveEnvironmentId(credentials.environmentId);
    const baseUrl = buildApiBaseUrl(environmentId);
