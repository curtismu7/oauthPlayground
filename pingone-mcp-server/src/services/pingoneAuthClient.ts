...
function resolveEnvironmentId(value?: string): string {
  const envId = value ?? process.env.PINGONE_ENVIRONMENT_ID ?? process.env.VITE_PINGONE_ENVIRONMENT_ID;
  if (!envId) {
...
function resolveClientSecret(value?: string): string {
  const secret = value ?? process.env.PINGONE_CLIENT_SECRET ?? process.env.VITE_PINGONE_CLIENT_SECRET;
  if (!secret) {
...
function createApiError(error: unknown, fallbackMessage: string): PingOneApiError {
  ...
  if ((axios as { isAxiosError?: (value: unknown) => value is AxiosError }).isAxiosError?.(error)) {
    ...
    const description = (data?.error_description as string | undefined) ?? (data?.message as string | undefined);
...
export async function getUserInfo(environmentId: string, accessToken: string): Promise<Record<string, any>> {
  try {
...
export function toPingOneErrorPayload(error: unknown): PingOneErrorPayload {
  ...
  if ((axios as { isAxiosError?: (value: unknown) => value is AxiosError }).isAxiosError?.(error)) {
    ...
    return {
      ...
      message: (data?.error_description as string | undefined) ?? axiosError.message ?? 'PingOne request failed',
      description: (data?.error_description as string | undefined) ?? (data?.message as string | undefined),
      details: data,
    };
  }
