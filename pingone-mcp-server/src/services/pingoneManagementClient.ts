...
export async function issueWorkerToken(
  ...
  try {
    ...
    const scope = request.scope && request.scope.trim().length > 0 ? request.scope : DEFAULT_WORKER_SCOPE;
  ...
  try {
    ...
    if (!token) {
      if (!request.clientId || !request.clientSecret) {
        throw new PingOneApiError('Worker token or client credentials are required to fetch applications.', {
