...
function buildErrorResult<T extends z.ZodTypeAny>(
  component: string,
  error: unknown,
  validator: T
) {
...
export function registerMfaTools(server: McpServer, logger: Logger) {
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      try {
        ...
        return buildErrorResult('PingOne MFA device registration', error, registerDeviceOutputSchema);
      }
    }
  );
  ...
  server.registerTool(
    ...
    async (args) => {
      logger.info('Activating PingOne MFA device', { userId: args.userId, deviceId: args.deviceId });
  ...
  server.registerTool(
    ...
    async (args) => {
      logger.info('Sending PingOne MFA challenge', { userId: args.userId, deviceId: args.deviceId });
  ...
  server.registerTool(
    ...
    async (args) => {
      logger.info('Validating PingOne MFA challenge', { userId: args.userId, challengeId: args.challengeId });
      ...
      try {
        ...
        return buildErrorResult('PingOne MFA challenge validation', error, validateChallengeOutputSchema);
      }
    }
  );
