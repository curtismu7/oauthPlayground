...
function buildErrorResult(component: string, error: unknown) {
...
export function registerRedirectlessTools(server: McpServer, logger: Logger) {
	server.registerTool(
		...
		async (args) => {
			logger.info('Starting PingOne redirectless authentication', {
			...
			try {
				...
				return {
					content: [
						{
							type: 'text' as const,
							text: `Redirectless authentication completed with status ${structured.status ?? 'COMPLETED'}.`,
						},
			...
			try {
				...
				return {
					content: [
						{
							type: 'text' as const,
							text: 'PingOne redirectless authentication completed successfully.',
						},
						{ type: 'text' as const, text: JSON.stringify(structured.authContext ?? {}, null, 2) },
					],
					structuredContent: structured,
