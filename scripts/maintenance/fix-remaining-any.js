...
const specificFixes = [
	{
		...
		fixes: [
			{
				from: '.setProtectedHeader(header).sign(secretKey);',
				to: '.setProtectedHeader(header).sign(secretKey);',
			},
	...
	},
	{
		...
		fixes: [
			{
				from: 'export interface ApiResponse<T = any>',
				to: 'export interface ApiResponse<T = unknown>',
			},
