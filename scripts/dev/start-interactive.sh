...
kill_process_on_port() {
    ...
    if [ ! -z "$pid" ]; then
        echo -e "${YELLOW}⚠️  Port $port is in use by process $pid. Killing process...${NC}"
...
main() {
    ...
    # Step 1: Run tests (skip configuration check)
    echo ""
    ...
    echo ""
    echo -e "${CYAN}📋 Step 2: Starting Application${NC}"

    # Ensure port 3000 is available by killing any conflicting processes
    if ! kill_process_on_port 3000; then
        echo ""
        echo -e "${RED}❌ Failed to free up port 3000. Please manually kill the process and try again.${NC}"
        log "ERROR" "Failed to free up port 3000"
        exit 1
    fi
