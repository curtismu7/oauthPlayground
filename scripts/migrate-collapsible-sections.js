...
const ALWAYS_OPEN_SECTIONS = ['credentials', 'overview', 'configuration', 'intro', 'introduction'];
...
function migrateFile(filePath) {
	...
	content = content.replace(sectionPattern, (match, sectionContent) => {
		...
		const contentMatch = sectionContent.match(
			/<CollapsibleContent>([\s\S]*?)<\/CollapsibleContent>/
		);
		...
		if (!title) {
			console.log(
				`⚠️  Could not extract title for section ${sectionsTransformed} - needs manual review`
			);
