...
echo "🚀 Starting Protect Portal API Tests"
...
# Test 2: Embedded Login Initialize
if test_endpoint \
    "Embedded Login Initialize" \
    "POST" \
    "$BASE_URL/api/pingone/redirectless/authorize" \
...
# Test 3: MFA User Devices
if test_endpoint \
    "MFA User Devices" \
    "GET" \
    "$BASE_URL/api/pingone/user/$USER_ID/devices?environmentId=$ENV_ID&accessToken=$WORKER_TOKEN" \
    "-H 'Accept: application/json' -H 'Authorization: Bearer $WORKER_TOKEN'" \
    "" \
    "200"; then
    ((passed++))
else
...
if test_endpoint \
    "User Profile" \
    "GET" \
    "$BASE_URL/api/pingone/user/$USER_ID?environmentId=$ENV_ID&accessToken=$WORKER_TOKEN" \
