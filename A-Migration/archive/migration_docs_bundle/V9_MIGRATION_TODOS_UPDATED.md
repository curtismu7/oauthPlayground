...
> ³ RARFlowV9 imports both `Modern Messaging` ✅ and `legacy toast/messaging (`v4ToastManager`)` ❌ — v4 import needs removal  
...
#### `RARFlowV9.tsx`
- [ ] 🟠 Remove `legacy toast/messaging (`v4ToastManager`)` import — `Modern Messaging` is already imported and used; remove old import and any remaining v4 call sites
...
```bash
# Replace toast calls in a V9 file. Run once per file, then manually add Modern Messaging import.
FILE="src/pages/flows/v9/ClientCredentialsFlowV9.tsx"  # change per file

sed -i '' "s/legacy toast/messaging (`v4ToastManager`)\.showSuccess(/Modern Messaging.success(/g" "$FILE"
sed -i '' "s/legacy toast/messaging (`v4ToastManager`)\.showError(/Modern Messaging.error(/g" "$FILE"
sed -i '' "s/legacy toast/messaging (`v4ToastManager`)\.showWarning(/Modern Messaging.warning(/g" "$FILE"
sed -i '' "s/legacy toast/messaging (`v4ToastManager`)\.showInfo(/Modern Messaging.info(/g" "$FILE"
# Then: remove legacy toast/messaging (`v4ToastManager`) import line, add: import { Modern Messaging } from '@/v8/utils/toastNotificationsV8';
```
...
- [ ] **V9 toast system migration** — 8 of 9 V9 flows still using `legacy toast/messaging (`v4ToastManager`)`; replace with `Modern Messaging`
  - [ ] `OIDCHybridFlowV9.tsx`
  - [ ] `DeviceAuthorizationFlowV9.tsx`
  - [ ] `ImplicitFlowV9.tsx`
  - [ ] `SAMLBearerAssertionFlowV9.tsx`
