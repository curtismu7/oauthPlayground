...
> ⚠️ **Important:** The existing V9 flows in `src/pages/flows/v9/` currently use `legacy toast/messaging (`v4ToastManager`)` (the old V7 toast system). New flows **must** use `Modern Messaging`. See the Toast section below and the Programming Patterns audit in `migrate_vscode.md`.
...
## Messaging (MANDATORY replacement)

V9 flows use `Modern Messaging` from V8 utilities. **Never use `legacy toast/messaging (`v4ToastManager`)` in V9 flows:**

```tsx
import { Modern Messaging } from '@/v8/utils/toastNotificationsV8';

// Usage:
Modern Messaging.success('Token obtained successfully');
Modern Messaging.error('Authentication failed');
Modern Messaging.info('Redirecting to authorization server...');
```
...
- [ ] `Modern Messaging` used (not `legacy toast/messaging (`v4ToastManager`)`)
