...
find src -name "*.tsx" -o -name "*.ts" | xargs sed -i '' 's|utils/Legacy Toast (`v4ToastManager`)|utils/v4ToastMessages|g'
```
...
sed -i '' "s/Legacy Toast (`v4ToastManager`).showSuccess(/messaging.success(/g" "$FLOW"
sed -i '' "s/Legacy Toast (`v4ToastManager`).showError(/messaging.error(/g" "$FLOW"
sed -i '' "s/Legacy Toast (`v4ToastManager`).showWarning(/messaging.warning(/g" "$FLOW"
sed -i '' "s/Legacy Toast (`v4ToastManager`).showInfo(/messaging.info(/g" "$FLOW"
...
// ❌ BAD — cast bypasses type checking
  ...
  messaging.error(tokenError.message);  // could be undefined if err is a plain Error
}
...
// ✅ GOOD — type guard checks before casting
  ...
  if (err instanceof TokenExchangeError) {
    setError(err);
    messaging.error(err.message);
  } else {
    const message = err instanceof Error ? err.message : 'Unexpected error';
    messaging.error(`Token exchange failed: ${message}`);
  }
}
```
...
- [UI & Icon Migration Guide](./migrate_cursor.md) - Bootstrap/Nano/MDI icon migration (VS Code/Windsurf edition)
