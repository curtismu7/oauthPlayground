...
These rules are the compatibility guardrails for *all* changes that touch `src/services/**` (or any service-like shared module). The goal is **no breaking changes** during V7/V8 → V9 migration.

### 1) No breaking changes by default
- **Do not break exported signatures** (types, function params/returns, props, exports) for anything used by active flows.
- Prefer **additive evolution**: add optional fields, add new methods, add overloads, widen unions cautiously.

### 2) If you must change: adapter + deprecation path
- Keep the old API working (compat layer).
- Route old implementation to the new API under the hood.
- Mark old API `@deprecated` with a removal target and replacement guidance.

### 3) Contract-first development
Before adding non-trivial protocol/business logic to flows/pages:
- Confirm whether a service contract already covers the need.
- If a contract is missing or unclear, update this document **before** (or in the same PR as) adding code.

### 4) Enforceable checks (recommended)
- Disallow introducing `console.error(...)` / `console.warn(...)` for runtime failures (route to user messaging + structured logging).
- Avoid `any` in public service contracts.
- Optional: add an “API surface” test (e.g., `tsd`) to catch accidental signature breaks.

---

## 🧭 What counts as a Service vs UI Component vs Hook

> **Search-first:** before adding new service logic, search existing services in `src/services/` (local path example: `/Users/cmuir/P1Import-apps/oauth-playground/src/services`).

To keep “business logic in services” enforceable, entries in this doc are categorized as:

- **Service**: pure logic, network calls, validation, storage, transforms. Typically `*.ts`.
- **UI Component**: React component (props, rendering). Typically `*.tsx`.
- **Hook**: React hook (`useX`) coordinating UI + services.

> **Rule of thumb:** protocol/business logic should live in **services**. UI Components should remain thin and delegate to services.

---

## 🧩 Shared V9 Error Contract (recommended)

To support Modern Messaging consistently, prefer a normalized error shape from services.

```ts
export type V9ErrorSeverity = "info" | "warning" | "error" | "critical";

export interface V9ServiceError {
  code: string;                 // stable identifier (e.g., "network_timeout", "invalid_client")
  message: string;              // short technical summary (sanitized)
  severity: V9ErrorSeverity;    // drives banner vs critical error block
  retryable?: boolean;
  hint?: string;                // user guidance ("Check issuer URL and try again")
  requestId?: string;           // correlation id when available
  details?: Record<string, any>; // sanitized metadata (never secrets)
}

// Preferred helpers:
export function normalizeServiceError(err: unknown): V9ServiceError;
export function toUserGuidance(e: V9ServiceError): { title: string; message: string; nextSteps?: string[] };
```

> **Do not** surface secrets in `details` (tokens, client secrets, auth codes, PKCE verifiers).
...
### **V9 Services (Target)**
> **Naming convention (V9):** prefer `V9XService.ts` (PascalCase, `*.ts`). Avoid mixing styles (e.g., `credentialsServiceV9.ts`) unless the whole V9 set uses that convention consistently.
...
**Category**: UI Component (React `*.tsx`)
...
#### 7. **v4ToastManager**
**Status**: ⚠️ Deprecated for new work (legacy V7 compatibility only). Do not introduce new usages; migrate to Modern Messaging.
