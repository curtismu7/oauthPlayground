...
class KeyStorageService {
	 ...
	 */
	async storeKeyPairWithPublicKey(
		privateKey: string,
		publicKey: string,
		kid?: string
	): Promise<string> {
		try {
			...
			throw new Error(
				`Failed to store key pair: ${error instanceof Error ? error.message : String(error)}`
			);
		}
	}
	...
	private async extractPublicKey(privateKey: string): Promise<string> {
		try {
			...
			throw new Error(
				`Failed to extract public key: ${error instanceof Error ? error.message : String(error)}`
			);
		}
	}
	...
	getKeyStatistics(): {
		...
		return {
			...
			newestKey:
				sortedByDate.length > 0 ? sortedByDate[sortedByDate.length - 1].createdAt : undefined,
			algorithms,
		};
	}
}
